		...
		try {
				...
				if(reporttype.equalsIgnoreCase("detail")){
					...
					sql = "select a.date, a.desc1, a.aa, a.postacno, a.account, a.accountname, a.acno, a.paccount, a.paccountname, "+casestatement+"a.branch, a.description,a.transtype, a.posteddate, a.dramount, a.postamount, a.pendamount from ("
	    					+ "select m.date,m.desc1,m.tranid aa,d.postacno,he.account,he.description accountname,he.doc_no acno,pe.account paccount,pe.description paccountname,jv.doc_no transNo,b.branchname branch,jv.description,"  
	    					...
	    					+ "my_jvtran jv on m.tranid=jv.tranid inner join my_head he on m.acno=he.doc_no inner join my_head pe on d.postacno=pe.doc_no inner join MY_BRCH b on m.brhid=b.doc_no where jv.status=3 and jv.date<='"+sqlUpToDate+"'"+sql2+""+sql+" ) a"+joins+"";
	    		} else {
				...
				//System.out.println("prepaid grid===== "+sql);
