...
public class ClsAgeingStatementAction extends ActionSupport{
	...
	public void printAction() throws ParseException, SQLException{
		...
		try {
	        ...
	        param = new HashMap();
	        String sqld="",sqld1="",select="",joins="",joinss="",casestatement="",sqlUnapplyResult="0";
...
//			System.out.println("===========reportFileName======="+reportFileName);
        	...
        	int agmtconfig=0;
        	ResultSet rsagmtconfig=stmtAgeingStatement.executeQuery(stragmtconfig);
        	...
        	if(agmtconfig==1) {
        		...
        		agmtselect=",CONCAT(COALESCE(a.rtype,''),'-',convert(CASE WHEN a.rtype='RAG' THEN COALESCE(rag.voc_no,'') WHEN a.rtype='LAG' THEN COALESCE(lag.voc_no,'') ELSE '' END,char(15)),'-',CASE WHEN a.rtype='RAG' THEN COALESCE(rag.pono,'') WHEN a.rtype='LAG' THEN COALESCE(lag.po,'') ELSE '' END,'-',CONCAT(COALESCE(auth.AUTHNAME,''),' ',COALESCE(veh.reg_no,''),' ',COALESCE(plt.CODE_NAME,''))) concatreg,a.rtype agmttype,convert(CASE WHEN a.rtype='RAG' THEN rag.voc_no WHEN a.rtype='LAG' THEN lag.voc_no ELSE '' END,char(15)) agmtvocno,\r\n" + 
    		...
    		String sqlUnapplycarfare = "select "+casestatement+"a.date,a.transType,a.ref_detail,a.branchname,a.description,round(a.netamount,2) netamount,round(a.applied,2) applied,round(a.balance,2) balance,a.duedys,a.brhid "+agmtselect+" from ("
					+ "select j.doc_no transNo,coalesce(j.ref_detail,'') ref_detail,j.description,DATE_FORMAT((j.date),'%d-%m-%Y') date,j.dtype transType,round(sum(j.dramount)*j.id,2) netamount,round(coalesce(o.amount,0),2) applied," 
    				+ "round((sum(dramount) - coalesce(o.amount,0)*id)*j.id,2) balance,b.branchname,TIMESTAMPDIFF(Day,cast(j.date as datetime),cast('"+sqlUpToDate+"' as datetime)) duedys,j.brhid,j.rdocno,j.rtype,j.tr_no from my_jvtran j inner join my_head h "
    				+ "on j.acno=h.doc_no left join (select ap_trid,o.tranid,sum(coalesce(amount,0)) amount from my_outd o inner join my_jvtran j on j.tranid=o.AP_trid where j.date<='"+sqlUpToDate+"' group by tranid) o on "
    				+ "j.tranid=o.tranid inner join my_brch b on j.brhId=b.doc_no where j.status=3 and h.atype='"+atype+"' and j.acno="+acno+" and j.date<='"+sqlUpToDate+"'"+sqld+" "+sqlbrch+" group by j.tranid having balance<>0 order by j.date,j.doc_no) a "+joinss+""+joins+"";
		
           ResultSet resultSetUnapplycar = stmtAgeingStatement.executeQuery(sqlUnapplycarfare);
    		...
    		if(sqlUnapplyResult.equalsIgnoreCase("0")){
    			sqlUnapplycarfare="select null date,null transno,null transType,null ref_detail,null branchname,null description,null netamount,null applied,null balance,null duedys";
    		}
    		
    		String sqlOutStandingcarfare = "select @running_total:=@running_total + balance as runtot,a.* from "
    				+ "(select "+casestatement+" coalesce(invd.regno,opn.regno) regno ,coalesce(invd.claimno,opn.claimno) claimno,a.date,a.transType,a.ref_detail,a.branchname,a.description,round(a.netamount,2) netamount,round(a.applied,2) applied,round(a.balance,2) balance,a.duedys,a.brhid,coalesce(invd.vtype,opn.flname)  vehtype,a.rtype "+agmtselect+" from ("
...
//    		System.out.println("sqlOutStanding==="+sqlOutStanding);  
   		...
   		//System.out.println("===sqllaaa=="+unclearedreceiptssql2);
       	    ...
       	    param.put("ageingstatementoutstandingsqlcarfare", sqlOutStandingcarfare);
            ...
            if(reportFileName.contains("ageingStatementAlfahimPrint.jrxml")){
            	...
            	param.put("account", ageingStatementBean.getLblaccountno()+" - "+ageingStatementBean.getLblaccountname()+" - "+ageingStatementBean.getLblaccountaddress()+" - "+ageingStatementBean.getLblaccountmobileno());            	
            }
	        ...
	        param.put("ageingstatementunapplysqlcarfare", sqlUnapplycarfare);
	        ...
	        param.put("compemail", ageingStatementBean.getLblcompemail());
	        ...
	        if(branch.equalsIgnoreCase("a") || branch.equalsIgnoreCase("NA")){    
       		...
       		String strsql2="select coalesce(imgpath,'')imgpath,address from my_brch where doc_no='"+brhiid+"'";       
//       	    System.out.println("strsql2--->>>"+strsql2);
   ...
   System.out.println("==imgpal==reportFileName=="+imgpal);      
	 ...
	 private void generateReportPDF (HttpServletResponse resp, Map parameters, JasperReport jasperReport, Connection conn, String email, String print, String acno)throws JRException, NamingException, SQLException, IOException, AddressException, MessagingException {
		...
		HttpSession session=request.getSession();
         ...
         if(print.equalsIgnoreCase("1")) {
        	 ...
        	 String fileName="",path="", formcode="BAGS",filepath=""; 
			...
			//Added CC to Client Salesman for Carfare on 14-02-2024
     		...
     		DateFormat dateFormat = new SimpleDateFormat("dd_MM_yyyy_HH_mm_ss");
   		    ...
   		    System.out.println(ageingStatementBean.getLblaccountname());
     		fileName = "AgeingStatement["+ageingStatementBean.getLblaccountname().replace("/", "")+"]"+currdate+".pdf";
