	...
	public JSONArray finance(String branch,String fromdate,String todate,String accType,String accdocno,String dtype,String docRangeFrom,String docRangeTo,String amtRangeFrom,String amtRangeTo,String chk) throws SQLException {
		...
		try {
		        ...
		        if(dtype.equalsIgnoreCase("CPV") || dtype.equalsIgnoreCase("CRV") || dtype.equalsIgnoreCase("ICPV") || dtype.equalsIgnoreCase("ICRV") || dtype.equalsIgnoreCase("PC")){
		        	...
		        	sql = "select @i:=@i+1 slno, a.* from(SELECT m.doc_no,m.date,m.desc1 description,if(d.amount<0,(d.amount*-1),d.amount) amount,if(d.lamount<0,(d.lamount*-1),d.lamount) localamount,t.description accountname,c.code currency,"
						...
						+ "my_cashbd d on (m.tr_no=d.tr_no and d.sr_no=0) left join my_cashbd d1 on (m.tr_no=d1.tr_no and d1.sr_no=1) left join my_head t on d.acno=t.doc_no left join my_curr c on t.curid=c.doc_no where m.status=3 and m.dtype='"+dtype+"'"+sql+")a,(select @i:=0)c";
		        }
		        
		        if(dtype.equalsIgnoreCase("BPV") || dtype.equalsIgnoreCase("BRV") || dtype.equalsIgnoreCase("IBP") || dtype.equalsIgnoreCase("IBR")){

		        	sql = "select @i:=@i+1 slno, a.* from(SELECT m.doc_no,m.date,m.chqdt dates,m.chqno,concat(t.description,' [ ',m.desc1,' ] ') as remarks,if(t.description is null,t1.description,t.description) description,if(d.amount<0,(d.amount*-1),d.amount) amount,if(d.lamount<0,(d.lamount*-1),d.lamount) localamount,c.code currency,"
		        			...
		        			+ "left join my_chqbd d on (m.tr_no=d.tr_no and d.sr_no=0) left join my_chqbd d1 on (m.tr_no=d1.tr_no and d1.sr_no=1) left join my_head t on d.acno=t.doc_no  left join my_head t1 on d1.acno=t1.doc_no left join my_curr c on t.curid=c.doc_no where m.status=3 and m.dtype='"+dtype+"'"+sql+")a,(select @i:=0)c";
		        }
		        
		        if(dtype.equalsIgnoreCase("CNO") || dtype.equalsIgnoreCase("DNO") || dtype.equalsIgnoreCase("TCN") || dtype.equalsIgnoreCase("TDN")){

		        	sql = "select @i:=@i+1 slno, a.* from(SELECT h1.description customername,m.doc_no,m.refno,m.date,mm.voc_no invno,mm.date invdate,m.description,if(m.amount<0,(m.amount*-1),m.amount) amount,if((m.amount*d.rate)<0,(m.amount*d.rate*-1),(m.amount*d.rate)) localamount,t.description accountname,c.code currency,"
		        			...
		        			+ "(m.tr_no=d.tr_no and d.sr_no=1) left join gl_invm mm on (mm.doc_no=m.invno and m.invno!=0) left join my_head t on d.acno=t.doc_no left join my_curr c on t.curid=c.doc_no left join my_head h1 on h1.doc_no=m.acno where m.status=3 and m.dtype='"+dtype+"'"+sql+")a,(select @i:=0)c";      
		       ...
		       //System.out.println("sql"+sql);
		        ...
		        if(dtype.equalsIgnoreCase("JVT") || dtype.equalsIgnoreCase("IJV")){   
		        	...
		        	sql = "select @i:=@i+1 slno, a.* from(SELECT m.doc_no,m.date,m.description,if(d.dramount<0,(d.dramount*-1),d.dramount) amount,if(d.ldramount<0,(d.ldramount*-1),d.ldramount) localamount,t.description accountname,c.code currency,"
		        			...
		        			+ "(m.tr_no=d.tr_no  and d.ref_row=1) left join my_head t on d.acno=t.doc_no left join my_curr c on t.curid=c.doc_no where m.status=3 and m.dtype='"+dtype+"'"+sql+")a,(select @i:=0)c";
		        }
		        
		        if(dtype.equalsIgnoreCase("COT")){

		        	sql = "select @i:=@i+1 slno, a.* from(select m.doc_no,m.date,m.description,case when m.chqno='' then '' when m.chqno='0' then '' when m.chqno is null then '' else concat('No. : ',m.chqno,' Dated ',DATE_FORMAT(m.chqdt,'%m-%b-%Y')) END as 'cheque',"
		        		...
		        		+ "left join my_head t on m.acno=t.doc_no left join my_head t1 on m.acno_to=t1.doc_no left join my_curr c on t.curid=c.doc_no where m.status=3"+sql+")a,(select @i:=0)c";
		        }
		        
		        if(dtype.equalsIgnoreCase("SEC")){

		        	sql = "select @i:=@i+1 slno, a.* from(select m.date,m.doc_no,m.chqno,m.chqdt,m.amount,m.remarks,m.acno_from,m.acno_to,h.description bank,h1.description from my_secchq m left join my_head h on m.acno_from=h.doc_no left join my_head h1 on m.acno_to=h1.doc_no "
		        		+ "where m.status=3"+sql+")a,(select @i:=0)c";
		        }
		        
		        if(dtype.equalsIgnoreCase("FCR")){

		        	sql = "select @i:=@i+1 slno, a.* from(SELECT m.doc_no,m.date,m.desc1 description,if(d.amount<0,(d.amount*-1),d.amount) amount,if(d.lamount<0,(d.lamount*-1),d.lamount) localamount,t.description accountname,c.code currency FROM my_fuelcardbm m left join "  
		        		+ "my_fuelcardbd d on (m.tr_no=d.tr_no and d.sr_no=0) left join my_head t on d.acno=t.doc_no left join my_curr c on t.curid=c.doc_no where m.status=3 and m.dtype='FCR'"+sql+")a,(select @i:=0)c";
		        }
		        
		        if(dtype.equalsIgnoreCase("UCP")){

		        	sql = "select @i:=@i+1 slno, a.* from(select m.doc_no,m.date,m.desc1 remarks,m.chqdt dates,m.chqno,if(d.amount<0,(d.amount*-1),d.amount) amount,if(d.lamount<0,(d.lamount*-1),d.lamount) localamount,t.description accountname,c.code currency,"
		        		...
		        		+ "on (m.doc_no=d.rdocno and d.sr_no=0) left join my_unclrchqbd d1 on (m.doc_no=d1.rdocno and d1.sr_no=1) left join my_head t on d.acno=t.doc_no left join my_curr c on t.curid=c.doc_no where m.status=3 and m.dtype='UCP'"+sql+")a,(select @i:=0)c";
		        }
		        ...
		        if(dtype.equalsIgnoreCase("UCR")){
		        	...
		        	sql = "select @i:=@i+1 slno, a.* from(select m.doc_no,m.date,m.desc1 remarks,m.chqdt dates,m.chqno,if(d.amount<0,(d.amount*-1),d.amount) amount,if(d.lamount<0,(d.lamount*-1),d.lamount) localamount,t.description accountname,c.code currency,"
		        			...
		        			+ "on (m.doc_no=d.rdocno and d.sr_no=0) left join my_unclrchqreceiptd d1 on (m.doc_no=d1.rdocno and d1.sr_no=1) left join my_head t on d.acno=t.doc_no left join my_curr c on t.curid=c.doc_no where m.status=3 and m.dtype='UCR'"+sql+")a,(select @i:=0)c";
		        }
		        
		        if(dtype.equalsIgnoreCase("PRIV")){

		        	sql = "select @i:=@i+1 slno, a.* from(select t.description accountname,m.doc_no,m.date,concat(t.description,' [ ',m.desc1,' ] ') as remarks,t.description as reference,sum(coalesce(d.tax,0)) as taxtotal,sum(coalesce(d.nettotal,0))amount,sum(coalesce(d.nettaxamount,0))nettotal  from rl_prinvm m "
		        			+ "left join rl_prinvd d on m.doc_no=d.rdocno left join my_head t on m.acno=t.doc_no where m.status=3 "+sql+" group by m.doc_no)a,(select @i:=0)c";
		        }
		        
		        if(dtype.equalsIgnoreCase("MCP")){  

		        	sql = "select @i:=@i+1 slno, a.* from(SELECT m.doc_no,m.date,m.desc1 description,if(m.totalamount<0,(m.totalamount*-1),m.totalamount) amount,t.description accountname,c.code currency,"
		        			...
		        			+ "left join my_head t on m.acno=t.doc_no left join my_curr c on t.curid=c.doc_no where m.status=3 and m.dtype='"+dtype+"' "+sql+")a,(select @i:=0)c";   
		        }
