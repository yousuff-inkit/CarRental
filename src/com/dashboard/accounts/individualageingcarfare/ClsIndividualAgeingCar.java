package com.dashboard.accounts.individualageingcarfare;
...
public class ClsIndividualAgeingCar  { 
	...
	public JSONArray individualAgeing(String branch,String uptodate,String atype,String accdocno, String salesperson,String category,String check) throws SQLException {
		...
		try {
				...
				if(atype.equalsIgnoreCase("AR")){
					...
					/*
					 * sql = "select  @tot:=@tot+balance runbal,"
					 * +agmtconfig+" agmtconfig,a.name,a.contact,coalesce(sm.sal_name,'') salname,a.pmob,CONVERT(if(if(balance>0,balance,0)+if(balance<0,round((balance),2),0)<0,round((if(balance>0,balance,0)+if(balance<0,round((balance),2),0)*-1),2),''),CHAR(50)) 'ADVANCE',"
					 * +
					 ...
					 * + "a.date,a.transtype,"+casestatement+"a.acno,a.brhid,a.age,a.description"+
					 * agmtselect+" from (" +
					 ...
					 * +
					 * "coalesce(o.amount,0) balance, j.tranid, j.brhid,TIMESTAMPDIFF(Day,cast(j.date as datetime),cast('"
					 * +sqlUpToDate+"' as datetime)) age,j.rdocno,j.rtype,j.tr_no from my_jvtran j inner join my_brch b on j.brhId=b.doc_no "
					 * +
					 * "inner join my_curr bc on b.curId=bc.doc_no inner join my_head h on j.acno=h.doc_no inner join my_curr c on j.curId=c.doc_no left join my_acbook bk on h.cldocno=bk.cldocno "
					 * +condition+" " +
					 * "left join (select ap_trid,o.tranid,sum(coalesce(amount,0)) amount from my_outd o inner join my_jvtran j on j.tranid=o.tranid where j.date<='"
					 * +sqlUpToDate+"' group by ap_trid ) o on j.tranid=o.ap_trid " +
					 * "where j.status=3 and h.atype='"+atype+"' and j.date<='"+sqlUpToDate+"' "
					 * +sql+" and j.id>0 group by j.tranid having balance<>0 union all " +
					 ...
					 * +
					 * "balance, j.tranid, j.brhid,TIMESTAMPDIFF(Day,cast(j.date as datetime),cast('"
					 * +sqlUpToDate+"' as datetime)) age,j.rdocno,j.rtype,j.tr_no from my_jvtran j inner join my_brch b on j.brhId=b.doc_no inner join my_curr bc on b.curId=bc.doc_no "
					 * +
					 * "inner join my_head h on j.acno=h.doc_no inner join my_curr c on j.curId=c.doc_no left join my_acbook bk on h.cldocno=bk.cldocno "
					 * +condition+" left join (select ap_trid,o.tranid,sum(coalesce(amount,0)) " +
					 * "amount from my_outd o inner join my_jvtran j on j.tranid=o.ap_trid where j.date<='"
					 * +sqlUpToDate+"' group by tranid) o on j.tranid=o.tranid where j.status=3 and h.atype='"
					 * +atype+"' and j.date<='"+sqlUpToDate+"' "+sql+" and j.id<0 " +
					 * "group by j.tranid having balance<>0) a left join my_salm sm on sm.doc_no=a.sal_id "
					 * +joins+"  , (select @tot:=0) b order by acno,date";
					 * 
					 */
					
					sql = "select  @tot:=@tot+balance runbal,"+agmtconfig+" agmtconfig,a.name,a.contact,coalesce(sm.sal_name,'') salname,a.pmob,CONVERT(if(if(balance>0,balance,0)+if(balance<0,round((balance),2),0)<0,round((if(balance>0,balance,0)+if(balance<0,round((balance),2),0)*-1),2),''),CHAR(50)) 'ADVANCE',"
							+ "CONVERT(if(if(balance>0,balance,0)+if(balance<0,round((balance),2),0)>0,round((if(balance>0,balance,0)+if(balance<0,round((balance),2),0)),2),''),CHAR(50)) 'BALANCE',CONVERT(if(if(balance<0,"
							+ "round((balance),2),0)<0,round((if(balance<0,round((balance),2),0)*-1),2),''),CHAR(50)) 'UNAPPLIED',CONVERT(if(if(balance>0,balance,0)>0,round((if(balance>0,balance,0)),2),''),CHAR(50)) 'TOTAL',"
							...
							+ "group by j.tranid having balance<>0 order by acno,date) a left join my_salm sm on sm.doc_no=a.sal_id "+joins+"  , (select @tot:=0.0) b order by acno,date";

				} else if(atype.equalsIgnoreCase("AP")){
					condition=" and bk.dtype='VND'";
					
					sql = "select  @tot:=@tot+balance runbal,"+agmtconfig+" agmtconfig,a.name,a.contact,coalesce(sm.sal_name,'') salname,a.pmob,CONVERT(if(if(balance>0,balance,0)+if(balance<0,round((balance),2),0)>0,round((if(balance>0,balance,0)+if(balance<0,round((balance),2),0)),2),''),CHAR(50)) 'ADVANCE',"  
	...
	public ClsIndividualAgeingCarBean getPrint(String branch,String uptodate,String atype,String accdocno, String check) throws SQLException {
		 //System.out.println("Inside DAo 1");
		ClsIndividualAgeingCarBean bean = new ClsIndividualAgeingCarBean();
		  Connection conn = null;
		  java.sql.Date sqlUpToDate = null;
		...
		try {
            	...
            	if(agmtconfig==1) {
					joins+="LEFT JOIN gl_ragmt rag ON (a.rtype='RAG' AND a.rdocno=rag.doc_no and rag.status=3)\r\n" + 
                			...
                			"LEFT JOIN gl_vehplate plt ON veh.pltid=plt.doc_no\r\n" + 
                			"LEFT JOIN gl_vehauth auth ON veh.authid=auth.doc_no";
            		agmtselect=",CONCAT(a.rtype,'-',convert(CASE WHEN a.rtype='RAG' THEN rag.voc_no WHEN a.rtype='LAG' THEN lag.voc_no ELSE '' END,char(15)),'-',CASE WHEN a.rtype='RAG' THEN COALESCE(rag.pono,'') WHEN a.rtype='LAG' THEN COALESCE(lag.po,'') ELSE '' END) concatreg,CONCAT(COALESCE(auth.AUTHNAME,''),' ',COALESCE(veh.reg_no,''),' ',COALESCE(plt.CODE_NAME,''),veh.flname) regno,a.rtype agmttype,convert(CASE WHEN a.rtype='RAG' THEN rag.voc_no WHEN a.rtype='LAG' THEN lag.voc_no ELSE '' END,char(15)) agmtvocno,\r\n" + 
            				"CASE WHEN a.rtype='RAG' THEN COALESCE(rag.pono,'') WHEN a.rtype='LAG' THEN COALESCE(lag.po,'') ELSE '' END lpono";
            	}
				...
				if(atype.equalsIgnoreCase("AR")){
					...
					printqry = "SELECT base.*,@i:=@i+1 srno FROM (SELECT  @tot:=@tot+balance runbal,"+agmtconfig+" agmtconfig,a.name,a.contact,coalesce(sm.sal_name,'') salname,a.pmob,CONVERT(if(if(balance>0,balance,0)+if(balance<0,round((balance),2),0)<0,round((if(balance>0,balance,0)+if(balance<0,round((balance),2),0)*-1),2),''),CHAR(50)) 'ADVANCE',"
							+ "CONVERT(if(if(balance>0,balance,0)+if(balance<0,round((balance),2),0)>0,round((if(balance>0,balance,0)+if(balance<0,round((balance),2),0)),2),''),CHAR(50)) 'BALANCE',CONVERT(if(if(balance<0,"
							+ "round((balance),2),0)<0,round((if(balance<0,round((balance),2),0)*-1),2),''),CHAR(50)) 'UNAPPLIED',CONVERT(if(if(balance>0,balance,0)>0,round((if(balance>0,balance,0)),2),''),CHAR(50)) 'TOTAL',"
							+ "a.date,a.transtype,"+casestatement+"a.acno,a.brhid,a.age,a.description"+agmtselect+" from ("
							+ "select bk.sal_id,j.date,j.dtype transtype,j.doc_no transno,j.acno,h.description name,bk.per_mob pmob,bk.contactPerson contact,if(j.description='0','',j.description) description,sum(dramount) amount,CONVERT(coalesce(o.amount,''),CHAR(50)) applied,sum(dramount) - "
							...
							+ "inner join my_head h on j.acno=h.doc_no inner join my_curr c on j.curId=c.doc_no left join my_acbook bk on h.cldocno=bk.cldocno "+condition+" left join (select ap_trid,o.tranid,sum(coalesce(amount,0)) "
							+ "amount from my_outd o inner join my_jvtran j on j.tranid=o.ap_trid where j.date<='"+sqlUpToDate+"' group by tranid) o on j.tranid=o.tranid where j.status=3 and h.atype='"+atype+"' and j.date<='"+sqlUpToDate+"' "+sql+" and j.id<0 "
							+ "group by j.tranid having balance<>0 order by acno,date) a left join my_salm sm on sm.doc_no=a.sal_id "+joins+" ,(SELECT @tot:=0.0) b ORDER BY acno,DATE) base ,(SELECT @i:=0) sr";
					//System.out.println("iab 2---> "+printqry);
					Statement stmt=conn.createStatement();
					ResultSet rsqry=stmt.executeQuery(printqry);
					while(rsqry.next()){
						bean.setLblaccountname(rsqry.getString("name"));
						bean.setRunbal(rsqry.getDouble("runbal"));
					}
				}
