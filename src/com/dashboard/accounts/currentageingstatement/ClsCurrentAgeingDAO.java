	...
	public JSONArray currentAgeing(String branch,String uptodate,String atype,String accdocno, String salesperson,String category,String level1from, String level1to, String level2from, String level2to,
		...
		try {
				...
				if(atype.equalsIgnoreCase("AR")){
					...
					sql = "select rostatus+lostatus activecont,coalesce(cat.cat_name,'') catname,bk.period2 creditperiod,bk.credit creditlimit,a.*,bk.contactPerson 'contact',if(bk.per_mob is null,bk.com_mob,if(bk.per_mob='NA',bk.com_mob,if(bk.per_mob=' ',bk.com_mob,bk.per_mob))) pmob,s.sal_name from (select ag.account,ag.name,ag.acno,ag.brhid,CONVERT(if(sum(t7+u6)<0,round((sum(t7+u6)*-1),2),''),CHAR(50)) advance,\r\n" + 
							...
							"left join my_acbook bk on a.acno=bk.acno and bk.status=3 left join my_salm s on bk.sal_id=s.doc_no left join my_clcatm cat on bk.catid=cat.doc_no where 1=1"+sql1+"";
					...
					sql = "select coalesce(cat.cat_name,'') catname,bk.period2 creditperiod,bk.credit creditlimit,a.*,bk.contactPerson 'contact',if(bk.per_mob is null,bk.com_mob,if(bk.per_mob='NA',bk.com_mob,if(bk.per_mob=' ',bk.com_mob,bk.per_mob))) pmob,s.sal_name from (select ag.account,ag.name,ag.acno,ag.brhid,CONVERT(if(sum(t7+u6)>0,round((sum(t7+u6)),2),''),CHAR(50)) advance,\r\n" + 
							"CONVERT(if(sum(u6>0),round((sum(u6)),2),''),CHAR(50)) unapplied,\r\n" + 
							"CONVERT(if(sum(t7+u6)<0,round((sum(t7+u6))*-1,2),''),CHAR(50)) balance,\r\n" + 
							"CONVERT(if(sum(t7)<0,round((sum(t7)*-1),2),''),CHAR(50)) netamount,\r\n" + 
