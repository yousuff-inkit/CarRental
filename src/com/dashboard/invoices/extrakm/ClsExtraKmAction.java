...
//					System.out.println("Kmval: "+kmval);
					if(kmval>0){
						ResultSet rsinvdoc2=stmt.executeQuery("select voc_no from gl_invm where doc_no="+kmval+"");
						if(rsinvdoc2.next()){
							invoicedoc.add(rsinvdoc2.getString("voc_no"));
						}
...
//					System.out.println("FuelVal: "+fuelval);
					if(fuelval>0){
						ResultSet rsinvdoc3=stmt.executeQuery("select voc_no from gl_invm where doc_no="+fuelval+"");
						if(rsinvdoc3.next()){
							invoicedoc.add(rsinvdoc3.getString("voc_no"));
						}
