...
public class ClsLeavePayrollPostingAction {
	ClsCommon commonDAO = new ClsCommon();
	ClsLeavePayrollPostingDAO clsLeavePayrollPostingDAO = new ClsLeavePayrollPostingDAO();
	...
	java.sql.Date dbLeavePayrollPostingDate=null;
	...
	public String saveAction() throws ParseException, SQLException{
		...
		dbLeavePayrollPostingDate = commonDAO.changeStringtoSqlDate(getPostingDate());
		...
		if(mode.equalsIgnoreCase("A")){
			...
			System.out.println("textemp----"+getTxtselectedemployees());
			
			int val=clsLeavePayrollPostingDAO.insert(getCmbbranch(),dbLeavePayrollPostingDate,getTxtdrtotal(),getTxtremarks(),getTxtselectedemployees(),dbUpToDate,payrollpostingarray,session,request,mode);
			if(val>0.0){
				setDetail("Human Resource");
				setDetailname("Monthly Payroll Posting");
				setHiduptodate(dbUpToDate.toString());
				setHidpostingDate(dbLeavePayrollPostingDate.toString());
				setTxtselectedemployees("");
