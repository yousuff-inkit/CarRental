		...
		try {
	  	...
	  	if(type.equalsIgnoreCase("summary"))
	  		...
	  		String sql="select dt.dealno ,hh.description financiername ,hh.account account,DATE_FORMAT(dt.stdate,'%d.%m.%Y') stdate,  "
	  				...
	  				+ " from gl_termloandetd det inner join gl_termloanm tm on det.rdocno=tm.doc_no inner join my_unclrchqbm um on det.bpvno=um.doc_no and um.dtype='UCP' and um.status<>7 and um.brhid=tm.brhid group by rdocno) "
	  				+ "det on det.rdocno=vo.doc_no   left join (select rdocno,sum(pramt) pramt,sum(intstamt) intstamt,sum(totamt) totamt "
	  				+ "from gl_termloandetd det inner join gl_termloanm tm on det.rdocno=tm.doc_no  inner join my_unclrchqbm um on det.bpvno=um.doc_no and um.dtype='UCP' and um.bpvno<>0 and um.status<>7  and um.brhid=tm.brhid "
	          	...
	          	if(type.equalsIgnoreCase("detail"))
			    	{
	          		...
	          		String sql="select um.bpvno,if(um.bpvno>0,'Paid','Not paid') paid,det.date chqdate,dt.chqname,dt.sectychqno,vo.doc_no,dt.stdate,  "
	          				...
	          				+ "	convert(coalesce((select amount from my_unclrchqbd  where rdocno=um.doc_no and sr_no=2 and um.status<>7 and brhid=um.brhid),''),char(50)) interest,  "
	          				+ " convert(coalesce((select amount from my_unclrchqbd  where rdocno=um.doc_no and sr_no=3  and um.status<>7 and brhid=um.brhid),''),char(50)) principle,  "
	          				+ " convert(coalesce((select amount from my_unclrchqbd  where rdocno=um.doc_no and sr_no=2  and um.status<>7 and brhid=um.brhid)+(select amount from my_unclrchqbd  where rdocno=um.doc_no and sr_no=3  and um.status<>7 and brhid=um.brhid),''),char(50)) pytvalue,  "
	          				+ " convert(coalesce((select amount from my_unclrchqbd  where rdocno=um.doc_no and sr_no=2 and  um.bpvno<>0  and um.status<>7 and brhid=um.brhid),''),char(50)) interest1,  "
	          				+ " convert(coalesce((select amount from my_unclrchqbd  where rdocno=um.doc_no and sr_no=3 and  um.bpvno<>0 and um.status<>7 and brhid=um.brhid),''),char(50)) principle1,  "
	          				+ " convert(coalesce((select amount from my_unclrchqbd  where rdocno=um.doc_no and sr_no=2 and  um.bpvno<>0 and um.status<>7 and brhid=um.brhid)+(select amount from my_unclrchqbd  where rdocno=um.doc_no and sr_no=3 and  um.bpvno<>0 and um.status<>7 and brhid=um.brhid),''),char(50)) pytvalue1,  "
	          		        + " convert(coalesce((select amount from my_unclrchqbd  where rdocno=um.doc_no and sr_no=2 and  um.bpvno=0 and um.status<>7 and brhid=um.brhid),''),char(50)) interest2,  "
	          		        + " convert(coalesce((select amount from my_unclrchqbd  where rdocno=um.doc_no and sr_no=3 and  um.bpvno=0 and um.status<>7 and brhid=um.brhid),''),char(50)) principle2,  "
	          		        + " convert(coalesce((select amount from my_unclrchqbd  where rdocno=um.doc_no and sr_no=2 and  um.bpvno=0 and um.status<>7 and brhid=um.brhid)+(select amount from my_unclrchqbd  where rdocno=um.doc_no and sr_no=3 and  um.bpvno=0 and um.status<>7 and brhid=um.brhid),''),char(50)) pytvalue2  "
	          		        ...
	          		        + "    left join my_head hh on hh.doc_no=dt.rpaccno left join my_unclrchqbm um on det.bpvno=um.doc_no and um.dtype='UCP'  and um.status<>7 and um.brhid=vo.brhid "
