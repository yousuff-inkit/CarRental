...
*/
			String strSql="select minmov.oreason outremark,maxmov.ireason inremark,nrm.doc_no,nrm.fleet_no,veh.flname,veh.reg_no,nrm.date,if(nrm.drid<>0,drv.sal_name,stf.sal_name) employee,"+
			" grg.name garagename,case when nrm.clstatus=0 then 'Open' when nrm.clstatus=1 then 'Closed' end status,st.st_desc movtype,"+
			" minmov.dout outdate, minmov.tout outtime,minmov.kmout outkm,case when minmov.fout = 0.000 then 'Level 0/8' when minmov.fout = 0.125 then 'Level 1/8' when minmov.fout = 0.250 then 'Level 2/8' when minmov.fout = 0.375 then 'Level 3/8' when minmov.fout = 0.500 then 'Level 4/8' when minmov.fout = 0.625 then 'Level 5/8' when minmov.fout = 0.750 then 'Level 6/8' when minmov.fout = 0.875 then 'Level 7/8' when minmov.fout = 1.000 then 'Level 8/8' end"+
			" outfuel,CONVERT(if(nrm.delivery = 1,minmov.din,''),char(50)) deldate, if(nrm.delivery = 1,minmov.tin,'') deltime,"+
			" CONVERT(if(nrm.delivery = 1,minmov.kmin,''),char(50)) delkm,if(nrm.delivery=1,case when minmov.fin = 0.000 then 'Level 0/8' when minmov.fin = 0.125 then 'Level 1/8' when minmov.fin = 0.250 then 'Level 2/8' when minmov.fin = 0.375 then 'Level 3/8' when minmov.fin = 0.500 then 'Level 4/8' when minmov.fin = 0.625 then 'Level 5/8' when minmov.fin = 0.750 then 'Level 6/8' when minmov.fin = 0.875 then 'Level 7/8' when minmov.fin = 1.000 then 'Level 8/8' end,'') delfuel,"+
			" CONVERT(if(nrm.collection=1,maxmov.dout,''),char(50))  coldate,if(nrm.collection=1,maxmov.tout,'') coltime,"+
			" CONVERT(if(nrm.collection=1,maxmov.kmout,''),char(50)) colkm,if(nrm.collection=1,case when maxmov.fout = 0.000 then 'Level 0/8' when maxmov.fout = 0.125 then 'Level 1/8' when maxmov.fout = 0.250 then 'Level 2/8' when maxmov.fout = 0.375 then 'Level 3/8' when maxmov.fout = 0.500 then 'Level 4/8' when maxmov.fout = 0.625 then 'Level 5/8' when maxmov.fout = 0.750 then 'Level 6/8' when maxmov.fout = 0.875 then 'Level 7/8' when maxmov.fout = 1.000 then 'Level 8/8' end,'') colfuel,"+
			" maxmov.din indate,maxmov.tin intime,maxmov.kmin inkm, case when maxmov.fin = 0.000 then 'Level 0/8' when maxmov.fin = 0.125 then 'Level 1/8' when maxmov.fin = 0.250 then 'Level 2/8' when maxmov.fin = 0.375 then 'Level 3/8' when maxmov.fin = 0.500 then 'Level 4/8' when maxmov.fin = 0.625 then 'Level 5/8' when maxmov.fin = 0.750 then 'Level 6/8' when maxmov.fin = 0.875 then 'Level 7/8' when maxmov.fin = 1.000 then 'Level 8/8' end infuel,"+
			" (timestampdiff(MINUTE,cast(concat(minmov.dout,' ',minmov.tout) as datetime),cast(concat(maxmov.din,' ',maxmov.tin) as datetime)) / 1440) "+
			" totaldays,maxmov.kmin-minmov.kmout conkm,mu.user_name ouser,mcl.user_name cuser,nrm.entrytime from gl_nrm nrm "+
			" left join (select min(doc_no) minmovdocno,rdocno from gl_vmove where rdtype='MOV' group by rdocno) minmovqry on nrm.doc_no=minmovqry.rdocno "+
			" left join gl_vmove minmov on minmov.doc_no=minmovqry.minmovdocno "+
			" left join (select max(doc_no) maxmovdocno,rdocno from gl_vmove where rdtype='MOV' group by rdocno) maxmovqry on nrm.doc_no=maxmovqry.rdocno "+
			" left join gl_vmove maxmov on maxmov.doc_no=maxmovqry.maxmovdocno"+
			" left join gl_status st on (nrm.movtype=st.status and st.mov=1)"+
			" left join my_salesman drv on  (nrm.drid=drv.doc_no and drv.sal_type='DRV')"+
			" left join my_salesman stf on (nrm.staffid=stf.doc_no and stf.sal_type='STF')"+
			" left join gl_vehmaster veh on (nrm.fleet_no=veh.fleet_no)"+
			" left join gl_garrage grg on nrm.garageid=grg.doc_no"+
			" left join my_user mu on mu.doc_no=nrm.userid"+
			" left join my_user mcl on mcl.doc_no=nrm.closeuserid"+
			" where nrm.status<>7"+sqltest;
