...
public class ClsVehSaleInvoiceListDAO {
	...
	public JSONArray detail(String branch,String fromdate,String todate,String check) throws SQLException {
		...
		try {
		        ...
		        String detsql="select veh.ch_no chno,model.vtype model,sale.date,yom.yom,saled.sr_no,sale.voc_no doc_no,ac.refname client,saled.fleetno fleet_no ,saled.salesprice,ROUND(CASE WHEN saled.salesprice>0 AND ac.tax>0 THEN saled.salesprice*(tx.vat_per/100) ELSE 0.0 END,2) vat,saled.salesprice+ROUND(CASE WHEN saled.salesprice>0 AND ac.tax>0 THEN saled.salesprice*(tx.vat_per/100) ELSE 0.0 END,2) taxtotal,saled.dep_posted,saled.pvalue pur_value,saled.acdep acc_dep,"
		        		+" br.brand_name brand,saled.curdep cur_dep,saled.netbook,saled.netval net_pl,veh.flname,concat(veh.reg_no,'-',coalesce(vehp.code_name,'')) reg_no "+
	        			...
	        			" left join my_acbook ac on (sale.cldocno=ac.cldocno and ac.dtype='CRM') left join gl_vehplate vehp on veh.pltid=vehp.doc_no left join gl_yom yom on veh.yom=yom.doc_no left join gl_vehmodel model on model.doc_no=veh.vmodid "+
	        			"  LEFT JOIN ( SELECT tax.vat_per,br.doc_no brhid,tax.fromdate,tax.todate FROM gl_taxdetail tax LEFT JOIN gl_invmode inv ON tax.acidno=inv.idno \r\n" + 
	        			" LEFT JOIN my_brch br ON (tax.province=br.province) WHERE tax.status<>7 GROUP BY tax.fromdate,tax.todate,br.doc_no) tx ON \r\n" + 
	        			" (tx.brhid=sale.brhid AND sale.date BETWEEN tx.fromdate AND tx.todate)"+
	        			" where sale.status<>7 "+sql+" order by sale.date,sale.doc_no";
	...
	public JSONArray summary(String branch,String fromdate,String todate,String check) throws SQLException {
		...
		try {
				...
				sql = "select sale.voc_no doc_no,sale.date,if(sale.type='S','Sale','Total Loss') type,sale.description 'desc',ac.refname 'client',round(sum(coalesce(saled.salesprice,0)),2) total,"+
				" ROUND(CASE WHEN round(sum(coalesce(saled.salesprice,0)),2)>0 AND ac.tax>0 THEN round(sum(coalesce(saled.salesprice,0)),2)*(tx.vat_per/100) ELSE 0.0 END,2) vat,round(sum(coalesce(saled.salesprice,0)),2)+ROUND(CASE WHEN round(sum(coalesce(saled.salesprice,0)),2)>0 AND ac.tax>0 THEN round(sum(coalesce(saled.salesprice,0)),2)*(tx.vat_per/100) ELSE 0.0 END,2) taxtotal from gl_vsalem sale"
						+ " left join my_acbook ac on (sale.cldocno=ac.cldocno and ac.dtype='CRM') left join gl_vsaled saled on saled.rdocno=sale.doc_no "+
						" LEFT JOIN ( SELECT tax.vat_per,br.doc_no brhid,tax.fromdate,tax.todate FROM gl_taxdetail tax LEFT JOIN gl_invmode inv ON tax.acidno=inv.idno \r\n" + 
						" LEFT JOIN my_brch br ON (tax.province=br.province) WHERE tax.status<>7 GROUP BY tax.fromdate,tax.todate,br.doc_no) tx ON \r\n" + 
						" (tx.brhid=sale.brhid AND sale.date BETWEEN tx.fromdate AND tx.todate)"+
						" where sale.status<>7 "+sql+" group by sale.doc_no order by sale.date,sale.doc_no";
