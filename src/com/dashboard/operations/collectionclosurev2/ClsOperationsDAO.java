package com.dashboard.operations.collectionclosurev2;
...
public class ClsOperationsDAO  {
	...
	public JSONArray collectionClosureGridLoading(String branch,String fromdate,String todate,String payas,String status,String cldocno) throws SQLException {
        ...
        if(!(todate.equalsIgnoreCase("undefined")) && !(todate.equalsIgnoreCase("")) && !(todate.equalsIgnoreCase("0"))){
		  ...
		  try {
			    ...
			    if(val==1)    
			    {
			    	sql="select  cc.refno cardnum,cc.category clientcategory,date_format(cc.refdate,'%d.%m.%Y') cardexpiry,cc.cardtype cardtype,cc.paytype paytype,cc.account clientcode,cc.jvno pdocno,cc.posttype ptype,cc.salname salmn,cc.agmtopndate,cc.branch,cc.date,cc.rdocno,cc.dtype,cc.srno,cc.refname,concat(cc.payas,' ',cc.cardtype) payas,CONVERT(coalesce(cc.no,''),CHAR(100)) as rano,COALESCE(cc.cashtotal,0) cashtotal,COALESCE(cc.cardtotal,0) cardtotal,COALESCE(cc.chequetotal,0) chequetotal from "
		    		        + "(select  r.refno,cm.category,r.refdate,h.account,s.sal_name salname,r.jvno,r.posttype,COALESCE(ra.date,la.date) agmtopndate,if(r.ibbrchid=0,b.branchname,br.branchname) branch,r.date,r.rdocno,r.dtype,r.srno,a.refname,if(r.payas=1,'On Account',if(r.payas=2,'Advance','Security')) payas, "
		    				+ "if(r.cardtype=0,' ',if(r.cardtype=2,'Master',if(r.cardtype=1,'Visa','Amex'))) cardtype,if(r.paytype=1,'Cash',if(r.paytype=2,'Card','Cheque/Online')) paytype,CONCAT(IF(ra.voc_no IS NULL,'LAG','RAG') , ' - ' ,COALESCE(ra.voc_no,la.voc_no)) NO, "
		    				...
		    				+ "left join my_jvtran j on r.tr_no=j.tr_no AND H.DOC_NO=J.ACNO left join my_outd o on j.tranid=o.tranid left join my_jvtran jv on jv.tranid=o.ap_trid left join my_acbook a on r.cldocno=a.cldocno and a.dtype='CRM' left join my_clcatm cm on a.catid=cm.doc_no and a.dtype='CRM' left join my_salm s on s.doc_no=a.sal_id left join gl_ragmt ra on ((r.aggno=ra.doc_no and r.rtype='RAG' ) OR  (JV.RDOCNO=ra.doc_no and JV.rtype='RAG' )) "
		    	 ...
		    	 sql = "select cc.refno cardnum,cc.category clientcategory,date_format(cc.refdate,'%d.%m.%Y') cardexpiry,cc.cardtype cardtype,cc.paytype paytype,cc.account clientcode,cc.jvno pdocno,cc.posttype ptype,cc.salname salmn,cc.agmtopndate,cc.branch,cc.date,cc.rdocno,cc.dtype,cc.srno,cc.refname,concat(cc.payas,' ',cc.cardtype) payas,CONVERT(coalesce(cc.rano,''),CHAR(100))as rano,COALESCE(cc.cashtotal,0) cashtotal,COALESCE(cc.cardtotal,0) cardtotal,COALESCE(cc.chequetotal,0) chequetotal from "
							+ "(select r.refno,cm.category,r.refdate,h.account,s.sal_name salname,r.jvno,r.posttype,if(r.rtype='RAG',ra.date,la.date) agmtopndate,if(r.ibbrchid=0,b.branchname,br.branchname) branch,r.date,r.rdocno,r.dtype,r.srno,a.refname,if(r.payas=1,'On Account',if(r.payas=2,'Advance','Security')) payas,"
							+ "if(r.cardtype=0,' ',if(r.cardtype=2,'Master',if(r.cardtype=1,'Visa','Amex'))) cardtype,if(r.paytype=1,'Cash',if(r.paytype=2,'Card','Cheque/Online')) paytype,if(r.payas=1,concat(jv.rtype,' - ',CONVERT(jv.rdocno,CHAR(100))),concat(r.rtype,' - ',CONVERT(if(r.rtype='0','',if(r.rtype='RAG',ra.voc_no,la.voc_no)),CHAR(100)))) rano,"
							...
							+ "left join my_jvtran j on r.tr_no=j.tr_no left join my_outd o on j.tranid=o.tranid left join my_jvtran jv on jv.tranid=o.ap_trid left join my_acbook a on r.cldocno=a.cldocno and a.dtype='CRM' left join my_clcatm cm on a.catid=cm.doc_no and a.dtype='CRM' left join my_salm s on s.doc_no=a.sal_id left join gl_ragmt ra on "
	...
	public JSONArray collectionClosureGridExcelExport(String branch,String fromdate,String todate,String payas,String status,String cldocno) throws SQLException {
	 ...
	 public JSONArray clentdetails() throws SQLException {
		...
		/*
		 * public ClsOperationsBean getPrint(HttpServletRequest request,String
		 * branch,String fromdate,String todate) throws SQLException { ClsOperationsBean
		 * bean = new ClsOperationsBean();
		 * 
		 * Connection conn = null;
		 * 
		 * try {
		 * 
		 * conn = ClsConnection.getMyConnection(); Statement stmtCollectionClosure =
		 * conn.createStatement(); java.sql.Date sqlFromDate = null; java.sql.Date
		 * sqlToDate = null;
		 * 
		 * String sqld="",sql="",sql1 = "",sql2 = "";
		 * 
		 * if(!(fromdate.equalsIgnoreCase("undefined")) &&
		 * !(fromdate.equalsIgnoreCase("")) && !(fromdate.equalsIgnoreCase("0"))){
		 * sqlFromDate = ClsCommon.changeStringtoSqlDate(fromdate); }
		 * if(!(todate.equalsIgnoreCase("undefined")) && !(todate.equalsIgnoreCase(""))
		 * && !(todate.equalsIgnoreCase("0"))){ sqlToDate =
		 * ClsCommon.changeStringtoSqlDate(todate); }
		 * 
		 * if(!((branch.equalsIgnoreCase("a")) || (branch.equalsIgnoreCase("NA")))){
		 * sqld+=" and r.brhId="+branch+""; }
		 * 
		 * sql="select 'Collection Closure' vouchername,CONCAT('From ',DATE_FORMAT('"
		 * +sqlFromDate+"','%D %M  %Y '),'  To  ',DATE_FORMAT('"
		 * +sqlToDate+"','%D %M  %Y ')) vouchername1," +
		 ...
		 * 
		 * while(resultSet.next()){ bean.setLblcompname(resultSet.getString("company"));
		 ...
		 * 
		 * sql1 =
		 * "select cc.branch,cc.date,cc.rdocno,cc.dtype,cc.srno,cc.refname,concat(cc.payas,' ',cc.cardtype) payas,coalesce(cc.rano,'') as rano,cc.cashtotal,cc.cardtotal,cc.chequetotal from "
		 * +
		 ...
		 * +
		 * "where r.date>='"+sqlFromDate+"' and r.date<='"+sqlToDate+"' and r.status=3 "
		 * +sqld+" group by r.srno)cc";
		 * 
		 ...
		 * 
		 * String temp="";
		 * temp=resultSet1.getString("branch")+"::"+resultSet1.getString("rdocno")+"::"+
		 * resultSet1.getString("date")+"::"+resultSet1.getString("dtype")+"::"+
		 * resultSet1.getString("srno")+"::"+resultSet1.getString("refname")+"::"+
		 * resultSet1.getString("payas")+"::"+resultSet1.getString("rano")+"::"+
		 * resultSet1.getString("cashtotal")+"::"+resultSet1.getString("cardtotal")+"::"
		 * +resultSet1.getString("chequetotal"); printarray.add(temp); }
		 * request.setAttribute("printingarray", printarray);
		 * 
		 * sql2 =
		 * "select round((cashtotal+cardtotal+chequetotal),2) nettotal,round(cashtotal,2) cashtotal,round(cardtotal,2) cardtotal,\r\n"
		 * +
		 ...
		 * +
		 * "r.rtype='LAG') left join my_brch b on r.brhid=b.branch left join my_brch br on r.ibbrchid=br.branch where r.date>='"
		 * +sqlFromDate+"' and\r\n" + "r.date<='"+sqlToDate+"' and r.status=3 "
		 * +sqld+"  group by r.srno)cc ) as bb ";
		 * 
		 * ResultSet resultSet2 = stmtCollectionClosure.executeQuery(sql2);
		 * 
		 * String
		 * netamount="0.00",cashamount="0.00",cardamount="0.00",chequeamount="0.00";
		 * while(resultSet2.next()){ cashamount=resultSet2.getString("cashtotal");
		 ...
		 * 
		 * bean.setLblcashtotal(cashamount); bean.setLblcardtotal(cardamount);
		 * bean.setLblchequetotal(chequeamount); bean.setLblnetbalance(netamount);
		 * 
		 * stmtCollectionClosure.close(); conn.close(); } catch(Exception e){
		 * e.printStackTrace(); conn.close(); } finally{ conn.close(); } return bean; }
		 */
}