package com.dashboard.rentalagreement.driverremovelog;
     	...
     	String sqltest="";
			...
			try {
					...
					Statement stmt = conn.createStatement (); 
	            		...
	            		String sql ="select agmt.doc_no,agmt.voc_no,agmt.fleet_no fleet_no,r.brhid,v.flname,r.cldocno,r.srno,\r\n"
	            				+ "a.refname,dr.name drname, DATE_FORMAT(bl.edate, '%d.%m.%Y')edate,usr.user_name username from gl_biblog bl\r\n"
	            				+ "inner join  gl_rdriver r on bl.doc_no=r.srno\r\n"
	            				+ "inner join gl_ragmt agmt on r.rdocno=agmt.doc_no\r\n"
	            				+ "inner join my_acbook a on agmt.cldocno=a.cldocno and a.dtype='CRM'\r\n"
	            				+ "inner join gl_drdetails dr on dr.dr_id=r.drid\r\n"
	            				+ "left join gl_vehmaster v on  v.fleet_no=agmt.fleet_no\r\n"
	            				+ "left join my_user usr on bl.userid=usr.doc_no\r\n"
	            				+ "where bl.dtype='BDRR' and date(edate)>= '"+sqlfromdate+"' and date(edate)<= '"+sqltodate+"' "+sqltest +" group by r.srno order by bl.edate";
