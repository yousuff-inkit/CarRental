   ...
   //	System.out.println("hjbf"+salesmandoc);
 	...
 	try {
				...
				Statement stmtVeh = conn.createStatement ();
         		...
         		String sql =" select  @s:=@s+1 SL,r.doc_no docno,r.voc_no 'ra_no',r.fleet_no fleet,v.FLNAME 'fleetname',"
         				+ "v.reg_no 'regno',r.ofleet_no 'contractfleet',v1.reg_no 'contractregno',"
         				+ "a.refname 'clientname',  a.contactperson 'contactperson', cdr.name 'driver',"
         				+ "a.per_mob 'mobno',t.rentaltype 'rentaltype',  r.ddate 'duedate',r.dtime 'duetime',"
         				+ "  convert(coalesce(r.refno,''),char(30)) 'refno',r.mrano 'manualra',"
         				+ "  ur.user_name 'openuser', ms.sal_name 'salesman' , tc.rate rent,tc.cdw cdw,"
         				+ "  coalesce((select invoiced from gl_rcalc where rdocno=r.doc_no and invno >0 and idno=1 and  rowno=(select max(rowno) from gl_rcalc where rdocno=r.doc_no and invno >0 and idno=1)),0) 'invrent',"
         				+ "  coalesce((select invoiced from gl_rcalc where rdocno=r.doc_no and invno >0 and idno=17 and  rowno=(select max(rowno) from gl_rcalc where rdocno=r.doc_no and invno >0 and idno=17)),0) 'invcdw'  from gl_ragmt r	left join gl_vehmaster v on r.fleet_no=v.fleet_no  left join my_acbook a on a.cldocno=r.cldocno   and a.dtype='CRM'  left join gl_vehgroup g on g.doc_no=v.vgrpid"
