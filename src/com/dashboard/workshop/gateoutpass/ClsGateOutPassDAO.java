...
public class ClsGateOutPassDAO {
	...
	public JSONArray getGateInPassExcelData(String fromdate,String todate,String id,String gatedocno,String branch)throws SQLException
	{
		...
		try{
			...
			" 'Reg No',veh.flname 'Fleet Name',case when m.agmtexist=1 and m.newdriver=0 then dr.name when m.agmtexist=0 and m.newdriver=0 then "+
	...
	public JSONArray getGateInPassSearchData(String id,String fromdate,String todate,String branch) throws SQLException{
		...
		try{
			...
			strsql="select br.branchname,m.doc_no,m.date,veh.fleet_no,veh.reg_no,m.drivername from gl_workgateinpassm m left join gl_vehmaster veh on (m.fleet_no=veh.fleet_no) left join my_brch br on (m.brhid=br.doc_no) where m.status=3 and m.outstatus<>1 and m.processstatus=2"+sqltest+" order by m.doc_no";
