package com.dashboard.trafficfine.salikdownloadlist;
...
public class ClsSalikDownloadListDAO  { 
	...
	public JSONArray satDailyReportGridLoading(String fromdate,String todate,String satcatg,String datefilter,String check) throws SQLException {
		...
		try {
			...
			if(satcatg.equalsIgnoreCase("salik")){
				...
				String strsql="Select  case when salikupdate=1 then 'Epic' when salikupdate=2 then 'NTV' else '' end company,Salik_User username,trans,DATE_FORMAT(salik_date,'%d.%m.%Y') salik_date,salik_time,DATE_FORMAT(sal_date,'%d.%m.%Y') sal_date,regno,source,tagno,location,direction,amount,DATE_FORMAT((coalesce(autodate,date)),'%d.%m.%Y %H:%m:%s') date from gl_salikcommon where 1=1"+sql+"";
	...
	public JSONArray satDailyReportCountGridLoading(String fromdate,String todate,String satcatg,String datefilter,String check) throws SQLException {
		...
		try {
			...
			java.sql.Date edate=ClsCommon.changeStringtoSqlDate(todate);
			 ...
			 if(satcatg.equalsIgnoreCase("salik")){
				 if(datefilter.equalsIgnoreCase("1")) {
					resultSet= stmt.executeQuery ("select DATE_FORMAT((coalesce(salik_date,date)),'%d-%m-%Y %H:%m:%s') date,count(*) as datecount,DATE_FORMAT((coalesce(salik_date,date)),'%d.%m.%Y') hiddate from gl_salikcommon where DATE_FORMAT((coalesce(salik_date,date)),'%Y-%m-%d')<='"+edate+"' and DATE_FORMAT((coalesce(salik_date,date)),'%Y-%m-%d')>='"+sdate+"' group by coalesce(date(salik_date),date)");
				 } else if(datefilter.equalsIgnoreCase("2")) {
					 resultSet= stmt.executeQuery ("select DATE_FORMAT(autodate,'%d-%m-%Y %H:%m:%s') date,count(*) as datecount,DATE_FORMAT(autodate,'%d.%m.%Y') hiddate from gl_salikcommon where DATE_FORMAT(autodate,'%Y-%m-%d')<='"+edate+"' and DATE_FORMAT(autodate,'%Y-%m-%d')>='"+sdate+"' group by DATE_FORMAT(autodate,'%Y-%m-%d')");
				 }
			}
