...
//System.out.println(salesman);
   ...
   //	System.out.println("hjbf"+salesmandoc);
 	...
 	try {
				...
				String sql ="select  @s:=@s+1 SL,r.doc_no docno,r.voc_no 'ra_no',if(r.perfleet=0,r.tmpfleet,r.perfleet) fleet,v.reg_no 'regno',v.FLNAME 'fleetname',a.refname 'clientname',  a.contactperson 'contactperson', cdr.name 'driver',a.per_mob 'mobno',convert(coalesce(r.refno,''),char(30)) 'refno',  ur.user_name 'openuser', ms.sal_name 'salesman' ,tc.rate rent,tc.cdw cdw,  coalesce((select invoiced from gl_lcalc where rdocno=r.doc_no and invno >0 and idno=1 and  rowno=(select max(rowno) from gl_lcalc where rdocno=r.doc_no and invno >0 and idno=1)),0) 'invrent',coalesce((select invoiced from gl_lcalc where rdocno=r.doc_no and invno >0 and idno=17 and  rowno=(select max(rowno) from gl_lcalc where rdocno=r.doc_no and invno >0 and idno=17)),0) 'invcdw'  from gl_lagmt r left join gl_vehmaster v on if(r.perfleet=0,r.tmpfleet,r.perfleet)=v.fleet_no left join my_acbook a on a.cldocno=r.cldocno   and a.dtype='CRM' left join  gl_ltarif t on t.rdocno=r.doc_no and t.rstatus=5  left join  gl_ltarif tc on tc.rdocno=r.doc_no and tc.rstatus=7 left join gl_ldriver dr on dr.rdocno=r.doc_no and dr.status=3  left join gl_drdetails cdr on cdr.dr_id=dr.drid left join my_user ur on ur.doc_no=r.userid left join my_salm ms on ms.doc_no=r.salid ,(SELECT @s:= 0) AS s  where r.status=3 " +sqltest+ " group by r.doc_no";
