...
public class ClsVehiclePickUpDAO {
		...
		try {
			...
			/**/
			 String strSql = "select veh.flname,pick.doc_no,ac.refname,ac.per_mob,pick.fleet_no,veh.reg_no,if(pick.agmttype='RAG',ragmt.odate,lagmt.outdate) startdate,"+
					 ...
					 " gl_lagmt lagmt on (pick.agmttype='LAG' and pick.agmtno=lagmt.doc_no) left join  gl_ragmtclosem rc on rc.agmtno=pick.agmtno and pick.agmttype='RAG' and rc.status=3 left join my_acbook ac on (pick.cldocno=ac.cldocno and"+
					 " ac.dtype='CRM') left join gl_vehmaster veh on pick.fleet_no=veh.fleet_no where pick.status=3 and rc.agmtno is null";
	...
	public JSONArray getAgmtno(String docno,String fleet,String regno,String client,String date,String mobile,String branch,String agmt,String chk) throws SQLException {
		...
		try {
			...
			if(!fleet.equalsIgnoreCase("")){
				if(!agmt.equalsIgnoreCase("")) {
					if(agmt.equalsIgnoreCase("LAG") || (agmt.equalsIgnoreCase("RAG"))) {
...
//			System.out.println("Default SqlTest:"+sqltest);
			...
			JSONArray jsonArray = new JSONArray();
			 String strSql ="";
			/* String strSql = "select m1.invtype,m1.odate,m1.otime,coalesce(m1.orgregcard,0) orgregcard,plate.code_name platecode,m1.voc_no,replace(coalesce(m1.desc1,''),'/',' ') desc1,coalesce(round(case when m1.delstatus=1 then m1.delchg else 0 end,0),0) deliverychg,m3.acno,m2.fueltype,m1.odate,"+
					 	" m1.doc_no,m1.date,m1.fleet_no, m1.otime,round(m1.okm,2)okm,br.branchname,m2.flname,m2.reg_no,c1.color,g1.gid,m3.cldocno,m3.refname,m3.address,"+
			 ...
			 */
			 ...
			 if(agmt.equalsIgnoreCase("RAG")) {
				  strSql = "select m1.invtype,m1.odate,m1.otime,coalesce(m1.orgregcard,0) orgregcard,plate.code_name platecode,m1.voc_no,replace(coalesce(m1.desc1,''),'/',' ') desc1,coalesce(round(case when m1.delstatus=1 then m1.delchg else 0 end,0),0) deliverychg,m3.acno,m2.fueltype,m1.odate,"+
						 	" m1.doc_no,m1.date,mov.fleet_no, m1.otime,cast(round(mov.kmout,2) AS decimal(15))okm,br.branchname,m2.flname,m2.reg_no,c1.color,g1.gid,m3.cldocno,m3.refname,m3.address,"+
						 	" m3.mail1,m3.per_mob,m1.delstatus from gl_ragmt m1 inner join gl_vmove mov on (m1.doc_no=mov.rdocno and mov.rdtype='RAG' and mov.status='OUT') left join gl_vehmaster m2 on mov.fleet_no=m2.fleet_no left join my_color c1"+
						 	" on m2.clrid=c1.doc_no left join gl_vehgroup g1 on m2.vgrpid=g1.doc_no left join gl_vehplate plate on m2.pltid=plate.doc_no left join my_acbook m3 on m1.cldocno=m3.cldocno and"+
						 	" m3.dtype='CRM' left join my_brch br on m1.brhid=br.doc_no where m1.clstatus=0 and m2.tran_code<>'CU' and m1.status=3  "+sqltest+" order by m1.doc_no";
			 }
			 if(agmt.equalsIgnoreCase("LAG")) {
				  strSql = "select m1.voc_no,replace(coalesce(m1.desc1,''),'/',' ') desc1,coalesce(round(case when m1.delstatus=1 then m1.delchg else 0 end,0),0) deliverychg,m3.acno,m2.fueltype,"+
						 	" m1.doc_no,m1.date,mov.fleet_no,cast(round(mov.kmout,2) AS decimal(15))okm,br.branchname,m2.flname,m2.reg_no,c1.color,g1.gid,m3.cldocno,m3.refname,m3.address,"+
						 	" m3.mail1,m3.per_mob,m1.delstatus from gl_lagmt m1 inner join gl_vmove mov on (m1.doc_no=mov.rdocno and mov.rdtype='LAG' and mov.status='OUT') left join gl_vehmaster m2 on mov.fleet_no=m2.fleet_no left join my_color c1"+
						 	" on m2.clrid=c1.doc_no left join gl_vehgroup g1 on m2.vgrpid=g1.doc_no left join gl_vehplate plate on m2.pltid=plate.doc_no left join my_acbook m3 on m1.cldocno=m3.cldocno and"+
						 	" m3.dtype='CRM' left join my_brch br on m1.brhid=br.doc_no where m1.clstatus=0 and m2.tran_code<>'CU' and m1.status=3  "+sqltest+" order by m1.doc_no";
			 }
