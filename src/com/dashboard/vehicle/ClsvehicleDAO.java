		...
		try {
				...
				if(tran.equalsIgnoreCase("IN")){
					sql= " and m.dtype='VEH' left  join   (select max(doc_no) maxd,fleet_no from gl_vmove group by fleet_no) a "
							+ " on vm.doc_no=a.maxd and vm.fleet_no=a.fleet_no  left join gl_vmove vmr on vmr.doc_no=a.maxd and a.fleet_no=vmr.fleet_no "
							+ "left join my_brch br on br.doc_no=m.a_br left join my_locm l on l.doc_no=m.A_LOC ";
					...
					sql= " inner join (select max(doc_no) maxd,fleet_no,rdocno,rdtype from gl_vmove group by fleet_no) a "
							+ " on vm.doc_no=a.maxd and vm.fleet_no=a.fleet_no left join gl_vmove vmr on vmr.doc_no=a.maxd and a.fleet_no=vmr.fleet_no  left join gl_ragmt r on r.doc_no=vm.rdocno and vm.rdtype='rag' left join my_brch br on br.doc_no=r.brhid left join my_locm l on l.doc_no=r.locid ";
				}
				else if (tran.equalsIgnoreCase("LA") ){
					sql=  " inner join (select max(doc_no) maxd,fleet_no,rdocno,rdtype from gl_vmove group by fleet_no) a "
							+ " on vm.doc_no=a.maxd and vm.fleet_no=a.fleet_no left join gl_vmove vmr on vmr.doc_no=a.maxd and a.fleet_no=vmr.fleet_no  left join gl_lagmt la on la.doc_no=vm.rdocno and vm.rdtype='lag'  left join my_brch br on br.doc_no=la.brhid LEFT JOIN (SELECT BRHID,MIN(DOC_NO) LOC FROM MY_LOCM GROUP BY BRHID) brh on brh.brhid=br.doc_no  left join my_locm l on l.doc_no=brh.loc ";
			...
			if(chkdatails.equalsIgnoreCase("search"))	// this used for detail search
			{
				...
				String sqldata=("select if(vmr.status='IN',vmr.ireason,vmr.oreason) remarks,concat(coalesce(auth.authid,''),m.reg_no,coalesce(plt.code_name,'')) vehiclenum,coalesce(auth.authname,'') authority,coalesce(plt.code_name,'') platecode,g1.name grname,convert((SELECT DATEDIFF(m.ins_exp,curdate())),char(20)) AS days, "
						...
						+ " l.LOC_NAME,g.gname,c.color,b.brand_name,m.ch_no from gl_vehmaster m  "
			...
			else
			{
				...
				String sqldata=("select if(vmr.status='IN',vmr.ireason,vmr.oreason) remarks,if(vm.rdtype='RAG' or vm.rdtype='LAG','',convert((SELECT DATEDIFF( curdate(),coalesce(vm.din,vm.dout) )),char(20))) AS idledays,concat(coalesce(auth.authid,''),m.reg_no,coalesce(plt.code_name,'')) vehiclenum,coalesce(auth.authname,'') authority,coalesce(plt.code_name,'') platecode,g1.name grname,convert((SELECT DATEDIFF(m.ins_exp,curdate())),char(20)) AS days,br.branchname,m.FLEET_NO,m.FLNAME,m.REG_NO,m.SRVC_KM+m.lst_srv SRVC_KM,y.YOM,m.renttype renttype,  "
						...
						" 'Level 6/8' WHEN coalesce(vm.fin,vm.fout)='0.875'   THEN 'Level 7/8' WHEN coalesce(vm.fin,vm.fout)='1.000' THEN 'Level 8/8'  END as 'C_FUEL',l.LOC_NAME,g.gname,c.color,b.brand_name,m.ch_no "
						+ " from gl_vehmaster m  left join gl_vehgroup g on g.doc_no=m.VGRPID "
	...
	public JSONArray searchAvivehexcel(String tran,String chkdatails,String brch) throws SQLException {
		...
		try {
			...
			if(chkdatails.equalsIgnoreCase("search"))	// this used for detail search
			{
				...
				String sqldata="	select br.branchname 'Avail.Br',l.LOC_NAME 'Location' ,m.FLEET_NO 'Fleet',m.FLNAME 'Fleet Name',vm.emp_type 'Type',"
						+ " CASE  WHEN vm.emp_type='CRM' THEN (select refname from my_acbook where cldocno=vm.emp_id and dtype='CRM')   \r\n" + 
						 " WHEN vm.emp_type='DRV' THEN (select sal_name from my_salesman where doc_no=vm.emp_id and sal_type='DRV') \r\n" + 
 		...
 		//	System.out.println("------1-------"+sqldata);
			...
			else
			{
				...
				String sqldata="	select br.branchname 'Avail.Br',l.LOC_NAME 'Location',m.FLEET_NO 'Fleet',m.FLNAME 'Fleet Name',"+sqlgrname+"\r\n" + 
						"	g.gname 'Group',c.Color,m.REG_NO,coalesce(auth.authname,'') 'Authority',coalesce(plt.code_name,'') 'Plate Code',y.YOM,\r\n" + 
						...
						"	 'Level 3/8' WHEN coalesce(vm.fin,vm.fout)='0.500' THEN 'Level 4/8'      WHEN coalesce(vm.fin,vm.fout)='0.625' THEN 'Level 5/8'  WHEN coalesce(vm.fin,vm.fout)='0.750' THEN\r\n" + 
						"	 'Level 6/8' WHEN coalesce(vm.fin,vm.fout)='0.875'   THEN 'Level 7/8' WHEN coalesce(vm.fin,vm.fout)='1.000' THEN 'Level 8/8'\r\n" + 
						"	 END as 'C_FUEL',m.renttype ,if(vm.rdtype='RAG' or vm.rdtype='LAG','',convert((SELECT DATEDIFF( curdate(),coalesce(vm.din,vm.dout) )),char(20))) AS idledays,if(vmr.status='IN',vmr.ireason,vmr.oreason) remarks from gl_vehmaster m left join my_locm l on l.doc_no=m.A_LOC\r\n" + 
	...
	public JSONArray alldetailsSearchs(String cmbbranch,String chkdatails) throws SQLException { 
		...
		try {
				...
				/*	String sqldata=" select  m.FLEET_NO 'Fleet NO ',a.AUTHname 'Authority', p.code_name 'Plate Code', m.REG_NO 'Reg_NO',m.FLNAME 'Fleet Name',\r\n" + 
					...
					String sqldata=" select brch.branchname as 'Avail Br.',l.LOC_NAME as 'Location', m.FLEET_NO 'Fleet NO ',a.AUTHname 'Authority', p.code_name 'Plate Code', m.REG_NO 'Reg_NO',m.FLNAME 'Fleet Name',\r\n" + 
							"g.gname 'Tariff Group',b.brand_name Brand,mo.vtype Model,s.st_desc Type,CASE WHEN movs.rdtype='RAG' THEN r.voc_no WHEN  movs.rdtype='LAG'\r\n" + 
							" THEN ll.voc_no else movs.rdocno  END as\r\n" + 
							"   'RDocno',CASE  WHEN movs.emp_type='CRM' THEN (select refname from my_acbook where cldocno=movs.emp_id and dtype='CRM')\r\n" + 
							"   WHEN movs.emp_type='DRV' THEN (select sal_name from my_salesman where doc_no=movs.emp_id and sal_type='DRV')\r\n" + 
							...
							"    y.YOM,c.Color ,convert(if(movs.status='OUT',round(movs.kmout),if(movs.status='IN',round(movs.kmin),m.cur_km)),char(40)) Currentkm,movs.dout dates,m.REG_DATE, m.INS_EXP,\r\n" + 
							"	m.PRCH_COST 'Purchase Cost',m.ADD1 'Additional Cost',m.tval 'Total cost' ,m.depr 'Depreciation %' , PERIOD_DIFF( EXTRACT( YEAR_MONTH FROM curdate()),\r\n" + 
							"	EXTRACT( YEAR_MONTH FROM movs.din)) 'Age(months)', m.salik_tag 'Salik Tag',if(movs.rdtype='RAG' or movs.rdtype='LAG','',convert((SELECT DATEDIFF( curdate(),coalesce(movs.din,movs.dout) )),char(20))) AS idledays,if(vmr.status='IN',vmr.ireason,vmr.oreason) remarks \r\n" + 
						 	"   from gl_vehmaster m left join gl_vehin i on(i.doc_no=m.INS_COMP) left join my_locm l on l.doc_no=m.a_loc left join\r\n" + 
							"   gl_yom y on y.doc_no=m.yom  left join my_brch brch on(brch.doc_no=m.a_br) left join gl_vehfin f on (m.finId=f.doc_no)\r\n" + 
							"   left join gl_vehauth a on (a.doc_no=m.authid)   left join gl_vehplate p on(p.doc_no=m.pltid) left join gl_vehgroup g\r\n" + 
							"   on g.doc_no=m.vgrpid left join gl_vehbrand b on b.doc_no=m.brdid left join gl_vehmodel mo on mo.doc_no=m.vmodid\r\n" + 
							...
							"   left join gl_vmove movs on movs.fleet_no=m.fleet_no "
							+ " inner  join   (select max(doc_no) maxd,fleet_no from gl_vmove group by fleet_no) a "
							+ " on movs.doc_no=a.maxd and movs.fleet_no=a.fleet_no left join gl_vmove vmr on vmr.doc_no=a.maxd and a.fleet_no=vmr.fleet_no    "
							+ "   left join gl_status s on m.tran_code=s.status\r\n" + 
							"   left join gl_ragmt r on  r.doc_no=movs.rdocno and movs.rdtype='RAG'\r\n" + 
							"   left join gl_lagmt ll on ll.doc_no=movs.rdocno and movs.rdtype='LAG'\r\n" + 
							"   where m.statu=3  and m.fstatus<>'Z'";
	...
	public JSONArray alldetailsSearch(String cmbbranch,String chkdatails) throws SQLException { 
		...
		try {
				...
				if(chkdatails.equalsIgnoreCase("chk"))
				{
					...
					String sqldata=" select if(movs.rdtype='RAG' or movs.rdtype='LAG','',convert((SELECT DATEDIFF( curdate(),coalesce(movs.din,movs.dout) )),char(20))) AS idledays,if(vmr.status='IN',vmr.ireason,vmr.oreason) remarks, brch.branchname,l.LOC_NAME, concat(coalesce(a.authid,''),m.reg_no,coalesce(p.code_name,'')) vehiclenum,p.code_name platecode,CASE  WHEN movs.emp_type='CRM' THEN (select refname from my_acbook where cldocno=movs.emp_id and dtype='CRM')\r\n" + 
							"   WHEN movs.emp_type='DRV' THEN (select sal_name from my_salesman where doc_no=movs.emp_id and sal_type='DRV')\r\n" + 
							...
							" movs.dout dates,m.salik_tag,PERIOD_DIFF( EXTRACT( YEAR_MONTH FROM curdate()),EXTRACT( YEAR_MONTH FROM movs.din)) age, PERIOD_DIFF( EXTRACT( YEAR_MONTH FROM curdate()),EXTRACT( YEAR_MONTH FROM m.rel_date)) vehage," + 
...
/*			String sqldata=("\r\n" + 
	...
	//System.out.println("--------sqldata74-----"+sqldata);
				ResultSet resultSet = stmtVeh1.executeQuery (sqldata);
				RESULTDATA=ClsCommon.convertToJSON(resultSet);
				}
				stmtVeh1.close();
	...
	public JSONArray searchReadyToRent(String brch) throws SQLException { 
		...
		try {
			...
			String sqldata=("select plt.code_name platecode,'Security Pass' securitypass,concat(au.authid,m.reg_no,p.code_name) vehiclenum,convert(coalesce(rr.ofleet_no,'0'),char(30)) ofleet_no,convert((SELECT DATEDIFF(m.ins_exp,curdate())),char(20)) AS days,m.doc_no,m.a_br branchid,m.VGRPID groupid,br.branchname,m.FLEET_NO,m.FLNAME,m.REG_NO,v.kmin,m.SRVC_KM+m.lst_srv SRVC_KM,y.YOM,m.renttype renttype,    "
					...
					+ " if(convert(coalesce(rr.ofleet_no,'0'),char(30))>0,'Permanent','') permanent,m.ch_no from gl_vehmaster m "
					...
					+ "	left join gl_yom y on y.doc_no=m.yom left join gl_vehplate p on m.pltid=p.doc_no left join gl_vehauth au on m.authid=au.doc_no left join gl_vehplate plt on m.pltid=plt.doc_no  where m.statu=3  "+sqlbrch+" "); 
			
             // System.out.println("sql 77: "+sqldata);
	...
	public JSONArray vehicleListSearch(String chk) throws SQLException { 
		...
		try {
            	...
            	{
			String sqldata=("select group_concat(spma.details) modelac, group_concat(spe.details) engine, group_concat(sps.details) segment, group_concat(spsp.details) specs, group_concat(sptt.details) transmittype ,s.st_desc,convert(m.cur_km,char(40)) currentkm, "
						...
						+ " left join gl_status s on m.tran_code=s.status left join gl_vdspec sd on sd.rdocno=m.doc_no and m.dtype='veh'" +      
						" left join gl_vehspec spma on spma.doc_no=sd.specid and spma.name='Model Ac' " +    
						" left join gl_vehspec spe on spe.doc_no=sd.specid and spe.name='Engine'"  + 
						" left join gl_vehspec sps on sps.doc_no=sd.specid and sps.name='Segment'"  + 
						" left join gl_vehspec spsp on spsp.doc_no=sd.specid and spsp.name='Specs'"  + 
						" left join gl_vehspec sptt on sptt.doc_no=sd.specid and sptt.name='Transmission Type' where m.statu=3  and m.fstatus<>'Z' group by m.doc_no,m.fleet_no ");
...
//System.out.println("----------sqldata---------"+sqldata);   
	 ...
	 * JSONArray RESULTDATA=new JSONArray();
	 * 
	 * Connection conn = null; try { conn = ClsConnection.getMyConnection();
	 * Statement stmtVeh1 = conn.createStatement ();
	 * 
	 * 
	 * if(chk.equalsIgnoreCase("load")) {
	 * 
	 * String sqldata=
	 * ("select m.FLEET_NO,m.REG_NO,m.SALIK_TAG,m.tcno,m.FLNAME 'FLEET NAME',a.AUTHname 'AUTHORITY',p.code_name 'PLATE CODE' ,"
	 * +
	 ...
	 * ResultSet resultSet = stmtVeh1.executeQuery(sqldata);
	 * RESULTDATA=ClsCommon.convertToEXCEL(resultSet); }
	 * //System.out.println("============"+RESULTDATA); stmtVeh1.close();
	...
	public JSONArray vehicleDetailsSearch(String check) throws SQLException {
		...
		try {
				...
				String sqldata="select @i:=@i+1 slno, a.* from(select coalesce(case when mov.status='IN' then  mov.kmin when mov.status='OUT' then mov.kmout else '' end,'' ) CUR_KM,coalesce(ac.refname,'') client,if(r.voc_no>0,convert(coalesce(r.voc_no,''),char(20)),convert(coalesce(l.voc_no,''),char(20))) agmtno,if(r.voc_no is not null or r.voc_no!='','RAG',if(l.voc_no is null or l.voc_no='','','LAG')) agmtype,m.DOC_NO,m.DATE,m.REG_NO,m.FLEET_NO,m.FLNAME,m.PRCH_INV,m.PRCH_DTE prch_dte,m.PRCH_COST,m.INST_AMT,m.INT_AMT,m.DN_PAY,i.inname INS_COMP "
						+ ",m.PNO,m.INS_AMT,m.PRMYM_PERC,m.PRMYM,m.INS_EXP,m.REL_DATE,m.REG_EXP,m.DEPR,m.ENG_NO,m.CH_NO,le.loc_name A_LOC,m.STATUS, "
						+ " m.SRVC_KM+m.lst_srv SRVC_KM,m.SRVC_DTE,m.LST_SRV,m.WAR,m.WAR_KM,m.ITYPE,m.ADD1,m.TVAL,m.VIN,m.C_FUEL,s.st_desc trancode,m.a_br branch,"
						+ " y.YOM,m.REG_DATE,m.LPO,m.SALIK_TAG,m.PUR_TYPE,m.NO_INST,m.FSTATUS, "
						+ "brch.branchname A_BR,m.WAR_PRD,m. CURR_VALUE,m.CATEGORY,m.renttype,m.dtype,m.comp_id,m.tcno,m.branded,f.fname finname,a.AUTHname, "
						+ "p.code_name PLTname ,g.gname VGRPname,b.brand_name BRDname,mo.vtype VMODname,v.name DLRname,brch1.branchname  BRHname,m.COSTID costname, "
						+ "c.color CLRname,m.WAREND,m.STATU,m.FUELTYPE,m.TCAP,m.cost,m.accdepr,coalesce(mort.fname,'') mortgage from gl_vehmaster m left join gl_status s on s.status=m.tran_code left join gl_ragmt r on r.fleet_no=m.fleet_no and r.clstatus=0 left join gl_lagmt l on if(l.perfleet=0,l.tmpfleet,l.perfleet)=m.fleet_no and l.clstatus=0 left join my_acbook ac on if(r.doc_no>0,ac.cldocno=r.cldocno,ac.cldocno=l.cldocno) and ac.dtype='crm' "
						+ "left join gl_vehin i on(i.doc_no=m.INS_COMP) left join my_locm le on le.doc_no=m.a_loc left join gl_yom y on y.doc_no=m.yom "
						+ " left join my_brch brch on(brch.doc_no=m.a_br) left join gl_vehfin f on (m.finId=f.doc_no) left join gl_vehauth a on (a.doc_no=m.authid)  "
						+ " left join gl_vehplate p on(p.doc_no=m.pltid) left join gl_vehgroup g on g.doc_no=m.vgrpid left join gl_vehbrand b on b.doc_no=m.brdid "
						+ "left join gl_vehmodel mo on mo.doc_no=m.vmodid left join my_vendorm v on v.doc_no=m.dlrid left join my_brch brch1 on(brch1.doc_no=m.brhid) "
						+ "left join my_color c on(c.doc_no=m.clrid) left join gl_vehfin mort on m.mortgageid=mort.doc_no left join (select max(doc_no) movdocno,fleet_no from gl_vmove group by fleet_no) maxmov on maxmov.fleet_no=m.fleet_no left join gl_vmove mov on mov.doc_no=maxmov.movdocno where m.statu=3)a,(select @i:=0)c ";
			//System.out.println("gridata143=============="+sqldata);
				ResultSet resultSet = stmtVeh1.executeQuery (sqldata);
				RESULTDATA=ClsCommon.convertToJSON(resultSet);
  ...
  // System.out.println("========    "+expdate);
       // java.sql.Date sqlStartDate = ClsCommon.changeStringtoSqlDate(expdate);
		...
		try {
			    ...
			    if(branch.equalsIgnoreCase("a"))
		    	{
		        	String sqlexp=("select ROUND(@i:=@i+1 ,0) slno,a.* from(select 'Attach' attachbtn,m.doc_no docno,m.brhid, m.fleet_no,m.flname,m.reg_no,if(m.tran_code='FS',null,m.reg_date) reg_date,if(m.tran_code='FS',null,m.ins_exp) ins_exp,m.ins_amt,g.gname,c.color,b.brand_name,mo.vtype model,'Edit' btnsave,concat(au.authname,' - ',plt.code_name) platecode"
		        			+ "	from gl_vehmaster m left join my_locm l on l.doc_no=m.A_LOC  left join gl_vehgroup g on g.doc_no=m.VGRPID "
		        			+ "	left join my_color c on(m.clrid=c.doc_no) 	left join gl_vehbrand b on(m.brdid=b.doc_no) "
		        			+ " left join gl_vehmodel mo on mo.doc_no=m.vmodid left join gl_vehin i on(i.doc_no=m.ins_comp) "
		        			+ " left join gl_vehplate plt on m.pltid=plt.doc_no left join gl_vehauth au on au.doc_no=m.authid where m.statu=3 and fstatus in ('L','N') and  m.ins_exp<='"+sqlStartDate+"')a,(select @i:=0)b" );
		        		//System.out.println("sqlexp  "+sqlexp);
					ResultSet resultSet = stmtVeh1.executeQuery (sqlexp);
					RESULTDATA=ClsCommon.convertToJSON(resultSet);
		    		...
		    		String sqlexp=("select ROUND(@i:=@i+1 ,0) slno,a.* from(select 'Attach' attachbtn,m.doc_no docno,m.brhid, m.fleet_no,m.flname,m.reg_no,if(m.tran_code='FS',null,m.reg_date) reg_date,if(m.tran_code='FS',null,m.ins_exp) ins_exp,m.ins_amt,g.gname,c.color,b.brand_name,mo.vtype model,'Edit' btnsave"
		        			+ "	from gl_vehmaster m left join my_locm l on l.doc_no=m.A_LOC  left join gl_vehgroup g on g.doc_no=m.VGRPID "
		        			+ "	left join my_color c on(m.clrid=c.doc_no) 	left join gl_vehbrand b on(m.brdid=b.doc_no) "
		        			+ " left join gl_vehmodel mo on mo.doc_no=m.vmodid left join gl_vehin i on(i.doc_no=m.ins_comp) where m.statu=3 and fstatus in ('L','N') and  m.brhid='"+branch+"' and m.ins_exp<='"+sqlStartDate+"')a,(select @i:=0)b");
		//	System.out.println("sqlexp  "+sqlexp);
					ResultSet resultSet = stmtVeh1.executeQuery (sqlexp);
					RESULTDATA=ClsCommon.convertToJSON(resultSet);
	...
	public JSONArray nonmovement(String branch,String expdate) throws SQLException {
		...
		try {
			    ...
			    if(branch.equalsIgnoreCase("a"))
		    	{
			    	String sqlexp=("select ROUND(@i:=@i+1 ,0) slno,a.* from(select concat(coalesce(auth.authid,''),veh.reg_no,coalesce(plt.code_name,'')) vehiclenum,plt.code_name platecode,veh.reg_no,n.fleet_no fleetno,n.doc_no,br.branchname,v.dout,v.tout,round(v.kmout) kmout,CASE WHEN v.fout='0.000' THEN 'Level 0/8' WHEN v.fout='0.125' "
		        			...
		        			+ "left join my_salesman st on st.doc_no=n.staffid and st.sal_type='STF' left join gl_vehmaster veh on veh.fleet_no=n.fleet_no left join gl_vehplate plt on veh.pltid=plt.doc_no left join gl_vehauth auth on veh.authid=auth.doc_no where n.clstatus=0 and n.status=3 and   n.date<='"+sqlStartDate+"')a,(select @i:=0)b " );
		    		...
		    		String sqlexp=("select ROUND(@i:=@i+1 ,0) slno,a.* from(select concat(coalesce(auth.authid,''),veh.reg_no,coalesce(plt.code_name,'')) vehiclenum,plt.code_name platecode,veh.reg_no,n.fleet_no fleetno,n.doc_no,br.branchname,v.dout,v.tout,round(v.kmout) kmout,CASE WHEN v.fout='0.000' THEN 'Level 0/8' WHEN v.fout='0.125' "
		        			...
		        			+ "left join gl_vmove v on v.rdocno=n.doc_no and v.rdtype='MOV' left join gl_status s   on(v.trancode=s.status)  "
		        			+ "left join my_salesman dr on dr.doc_no=n.drid and dr.sal_type='DRV' "
		        			+ "left join my_salesman st on st.doc_no=n.staffid and st.sal_type='STF'  left join gl_vehmaster veh on veh.fleet_no=n.fleet_no left join gl_vehplate plt on veh.pltid=plt.doc_no left join gl_vehauth auth on veh.authid=auth.doc_no "
		        			+ "where n.clstatus=0 and n.status=3 and n.brhid='"+branch+"' and   n.date<='"+sqlStartDate+"')a,(select @i:=0)b " );
