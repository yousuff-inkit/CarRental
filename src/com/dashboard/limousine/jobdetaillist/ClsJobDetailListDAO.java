...
public class ClsJobDetailListDAO {
	...
	public JSONArray getJobDetails(String id,String branch,String fromdate,String todate) throws SQLException{
		...
		try{
			...
			" tran.pickuplocid=pickup.doc_no left join gl_cordinates dropoff on tran.dropfflocid=dropoff.doc_no where book.status=3 and tran.doc_no IS NOT NULL "+sqltest+" union all"+
			...
			" hours.pickuplocid=pickup.doc_no where book.status=3 and hours.doc_no IS NOT NULL "+sqltest;
			ResultSet rs=stmt.executeQuery(strsql);
			System.out.println("JOB DETAIL GRID SQL = "+strsql);
