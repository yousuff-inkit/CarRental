...
public class ClsEmailLogDAO {  
	...
	public JSONArray EmailData(HttpSession session,int id,String fromdate,String todate) throws SQLException{
		...
		try {
			...
			sql="select e.doc_no,date_format(e.edate,'%d.%m.%Y %h:%m:%s') edate, e.dtype, e.brhid, mb.branchname branch, a.refname client, coalesce(u.user_id,'') mailuser, coalesce(e.email,'') email from emaillog e "
					+ "left join my_acbook a on a.cldocno=e.cldocno and a.dtype='crm' left join my_user u on u.doc_no=e.userid "
