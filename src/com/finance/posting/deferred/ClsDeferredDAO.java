package com.finance.posting.deferred;
...
public class ClsDeferredDAO {

	ClsCommon commonDAO = new ClsCommon();
	ClsConnection connDAO = new ClsConnection();
	ClsDeferredBean prePaymentBean = new ClsDeferredBean();
	...
	public int insert(String formdetailcode,String cmbtype,int txtaccountdocno, int txttrno, int txttranid, String txtdtype, String txtdistributiondocno, int txtcosttype, int txtcostno,double txtamount, Date startDate, String cmbfrequency,int txtdueafter, int txtinstnos, 
		...
		//System.out.println("distributionarray"+distributionarray);
			...
			if(cmbtype.equalsIgnoreCase("1")){	
				...
				if(masterDocno>0){
					...
					if(!distribution[2].equalsIgnoreCase("undefined") && !distribution[2].equalsIgnoreCase("NaN")){
						...
						stmtPREP1 = conn.prepareCall("insert into my_defd(tranId, sr_no, date, posted, postacno, jobid, amount) values(?,?,?,?,?,?,?)");
			...
			if(cmbtype.equalsIgnoreCase("2")){
				...
				/* String sqls=("select sum(posted) posted from my_defd where tranid="+txttranid);
				// System.out.println("sumsql=="+sqls);
				...
				// System.out.println("posted=="+posted);
				 if(posted==0 || posted==0.00 || posted==0.0){*/
				
					 String sql1=("update my_defm set postacno='"+txtdistributiondocno+"',costtype='"+txtcosttype+"',costcode='"+txtcostno+"' where tr_no="+txttrno+" and tranid="+txttranid);
		            //System.out.println("updprepm======="+sql1);
					 int data1 = stmtPREP.executeUpdate(sql1);
					 
					 
					 String sql21=("update my_defd set postacno='"+txtdistributiondocno+"'where tranid="+txttranid+" and posted=0");
			            //System.out.println("updprepd======="+sql21);
						 int data21 = stmtPREP.executeUpdate(sql21);
				...
				if(!distribution[2].equalsIgnoreCase("undefined") && !distribution[2].equalsIgnoreCase("NaN")){
				    ...
				    int rowno=distribution[4].trim().equalsIgnoreCase("undefined") || distribution[4].trim().equalsIgnoreCase("NaN") || distribution[4].trim().equalsIgnoreCase("") ||distribution[4].trim().isEmpty()?0:Integer.parseInt(distribution[4].trim().toString());
				    		
				    		if(rowno>0) {
				    			stmtPREP1 = conn.prepareCall("update my_defd set date=?,amount=? where rowno=?");
			...
			if(cmbtype.equalsIgnoreCase("3")){
				...
				for(int i=0;i<tranidarray.size();i++){ 
					String sql1 = "update my_defd d set d.posted="+tranidarray.get(i)+", d.jvtrno="+trno+" where d.rowno="+rownos[i];
					//System.out.println("prepdupdate======="+sql1);
		        ...
		        if(check.equalsIgnoreCase("1")){
		           ...
		           if(!(date.equalsIgnoreCase("undefined"))&&!(date.equalsIgnoreCase(""))&&!(date.equalsIgnoreCase("0")))
		           {
			        ...
			        sql="select t.doc_no,t.account,t.description,t.curid,c.code currency,cb.rate,c.type,t.gr_type from my_head t left join my_curr c on t.curid=c.doc_no "
			  	        	  + "left join my_curbook cb on t.curid=cb.curid inner join (select max(cr.doc_no) doc_no,cr.curid curid,cr.toDate,cr.frmDate from my_curbook cr "
			  	        	  + "where coalesce(toDate,curdate())>='"+sqlDate+"' and frmDate<='"+sqlDate+"' group by cr.curid) as bo on(cb.doc_no=bo.doc_no and cb.curid=bo.curid) "
			  	        	  + "where t.atype='GL' and t.gr_type='5' and t.m_s=0 "+sqltest;
			           
			        }
			    // System.out.println("searchtype==== "+sql);   
		...
		try {
				...
				if(cmbtype.equalsIgnoreCase("1"))
	    		{
					...
					xsql = "select "+casestatement+"a.account, a.accountname, a.description, a.date, a.trno, a.tranid, a.doc_no, a.dtype, a.brhId, a.branch, a.atype, a.acno, a.curId,"
							...
							+ "left join my_defd d on jv.tranid=d.tranid where he.den=616 and jv.status=3 and jv.brhid="+branch+" and jv.prep=0 and jv.dramount<0 and d.jvtrno is null"+xsql+") a "+joins+"";
	    		...
	    		else if(cmbtype.equalsIgnoreCase("2"))
	    		{
	    			...
	    			xsql = "select "+casestatement+"a.account, a.accountname, a.acno, a.atype, a.curid, a.c_rate, a.postacno, a.paccount, a.paccountname, a.freq, a.freqtype,a.noofins, a.instamt, "
	    					...
	    					+ "(select coalesce(sum(d.amount),0) from my_defd d where posted!=0 and d.tranid=aa) postamount ,abs(m.amount) dramount,(select m.amount-postamount) pendamount,jv.doc_no transno,jv.dtype transType "
	    					+ "from my_defm m inner join my_defd d on m.tranId=d.tranId inner join my_jvtran jv on m.tranid=jv.tranid inner join my_head he on m.acno=he.doc_no "
	    		...
	    		else if(cmbtype.equalsIgnoreCase("3"))
	    		{
	    			...
	    			/*xsql = "select he.account,he.description accountname,he.doc_no acno,he.atype,he.curid,c.c_rate,m.postacno,pe.account paccount,pe.description paccountname,m.doc_no,m.desc1,"
	    					...
	    					+ "from my_defd d where posted!=0 and d.tranid=aa) postamount ,coalesce(abs(m.amount)) totalamount,(select d.amount-postamount) pendamount from my_defm m inner join "
	    					+ "my_defd d on m.tranId=d.tranId  inner join my_jvtran jv on m.tranid=jv.tranid inner join my_head he on m.acno=he.doc_no inner join my_head pe on m.postacno=pe.doc_no "
	    			...
	    			/*xsql = "select "+casestatement+"a.account, a.accountname, a.acno, a.atype, a.curid, a.c_rate, a.postacno, a.paccount, a.paccountname, a.doc_no, a.desc1, a.aa, a.brhId, a.trno, a.branch, a.dramount, a.rowno, a.date, a.posted, a.description, a.dtype, a.tranid, a.costtype," 
	    					...
	    					+ "from my_defd d where posted!=0 and d.tranid=aa) postamount ,coalesce(abs(m.amount)) totalamount,(select d.amount-postamount) pendamount,m.doc_no transno,jv.dtype transType from my_defm m inner join "
	    					+ "my_defd d on m.tranId=d.tranId  inner join my_jvtran jv on m.tranid=jv.tranid inner join my_head he on m.acno=he.doc_no inner join my_head pe on m.postacno=pe.doc_no "
	    			...
	    			xsql = "select "+casestatement+"a.account, a.accountname, a.acno, a.atype, a.curid, a.c_rate, a.postacno, a.paccount, a.paccountname, a.doc_no, a.desc1, a.aa, a.brhId, a.trno, a.branch, a.dramount, a.rowno, a.date, a.posted, a.description, a.dtype, a.tranid, a.costtype," 
	    					...
	    					+ "(select d.amount-coalesce(b.postamount,0)) pendamount,jv.doc_no transno,jv.dtype transType from my_defm m inner join "
	    					+ "my_defd d on m.tranId=d.tranId  inner join my_jvtran jv on m.tranid=jv.tranid inner join my_head he on m.acno=he.doc_no inner join my_head pe on m.postacno=pe.doc_no "
	    					...
	    					+ "d.sr_no from my_defd d where posted!=0 group by d.tranid,d.sr_no) b on b.tranid=d.tranid and b.sr_no=d.sr_no where d.date>='"+sqlFromDate+"' and d.date<='"+sqlToDate+"' and  posted=0 and jv.brhid="+branch+""+sqls+") a "+joins+"";
	    			 			
	    			
	    		}
				//System.out.println("gridload=="+xsql);  
	...
	public JSONArray distributionGridReloading(HttpSession session,String tranId,String check) throws SQLException {
		...
		try {
				...
				String sqlnew=("select sr_no,date,amount,posted,rowno from my_defd where tranid="+tranId+"");
        ...
        try {
                ...
                */ 
            	 ...
            	 String sql="select costcde,acno doc_no,atype,account,accountname,costgroup,costtype,costcode,dramount debit,cramount credit,curid currencyid,c_rate rate,desc1 description, 0 as ref_row,rowno,0 as id from "
						...
						+ " d.posted,jv.dtype,jv.tranid,m.costtype,m.costcode,co.costgroup,(select coalesce(sum(d.amount),0) from my_defd d where posted!=0 "
						+ " and d.tranid=aa) postamount ,coalesce(abs(m.amount)) totalamount,(select d.amount-postamount) pendamount from my_defm m inner join"
						+ " my_defd d on m.tranId=d.tranId  inner join my_jvtran jv on m.tranid=jv.tranid inner join my_head he on m.acno=he.doc_no"
						...
						+ " d.posted,jv.dtype,jv.tranid,m.costtype,m.costcode,co.costgroup,(select coalesce(sum(d.amount),0) from my_defd d where posted!=0 "
						+ " and d.tranid=aa) postamount ,coalesce(abs(m.amount)) totalamount,(select d.amount-postamount) pendamount from my_defm m inner join "
						+ " my_defd d on m.tranId=d.tranId  inner join my_jvtran jv on m.tranid=jv.tranid  inner join"
				...
				//System.out.println("to be posted grid-----------"+sql);
				ResultSet resultSet = stmtPREP.executeQuery(sql);
		...
		try {
				...
				if(cmbtype.equalsIgnoreCase("1"))
	    		{
					...
					/*xsql = "select b.branchname 'Branch',jv.dtype 'Doc Type',jv.doc_no 'Doc No.',he.atype 'Type',he.account 'Account No.',he.description 'Account Name',0 'Post A/C No.',"
							...
							+ "jv.costtype=co.costtype left join my_defd d on jv.tr_no=d.jvtrno where he.den=616 and jv.brhid="+branch+" and jv.prep=0 and jv.dramount>0 and d.jvtrno is null" + xsql;*/
					...
					xsql = "select a.branch 'Branch',a.dtype 'Doc Type',"+casestatement+"a.atype 'Type', a.account 'Account No.', a.accountname 'Account Name',0 'Post A/C No.', 0 'Post A/C Name', "
							...
							+ "co.costgroup,abs(jv.dramount) dramount,abs(jv.dramount) pendamount,jv.doc_no transno,jv.dtype transType from my_jvtran jv inner join MY_BRCH b on "
							+ "jv.brhid=b.doc_no inner join my_head he on jv.acno = he.doc_no inner join my_curr c on jv.curId=c.doc_no left join my_costunit co on jv.costtype=co.costtype "
							+ "left join my_defd d on jv.tr_no=d.jvtrno where he.den=616 and jv.status=3 and jv.brhid="+branch+" and jv.prep=0 and jv.dramount<0 and d.jvtrno is null"+xsql+") a "+joins+"";
	    		...
	    		else if(cmbtype.equalsIgnoreCase("2"))
	    		{
	    			...
	    			/*xsql = "select b.branchname 'Branch',jv.dtype 'Doc Type',jv.doc_no 'Doc No.',he.atype 'Type',he.account 'Account No.',he.description 'Account Name',pe.account 'Post A/C No.',"
	    					+ "pe.description 'Post A/C Name',m.date 'Date',jv.description 'Description',abs(m.amount) 'Amount',(select coalesce(sum(d.amount),0) from my_defd d where posted!=0 and d.tranid=m.tranid) 'Posted',"
	    					+ "(select m.amount-posted) 'Balance' from my_defm m inner join my_defd d on m.tranId=d.tranId inner join my_jvtran jv on m.tranid=jv.tranid inner join my_head he on m.acno=he.doc_no "
	    		...
	    		else if(cmbtype.equalsIgnoreCase("3"))
	    		{
	    			...
	    			/*xsql = "select b.branchname 'Branch',jv.dtype 'Doc Type',m.doc_no 'Doc No.',he.atype 'Type',he.account 'Account No.',he.description 'Account Name',pe.account 'Post A/C No.',"
	    					+ "pe.description 'Post A/C Name',d.date 'Date',jv.description 'Description',d.amount 'Amount',(select coalesce(sum(d.amount),0) from my_defd d where posted!=0 and d.tranid=m.tranid) 'Posted',"
	    					+ "(select d.amount-posted) 'Balance' from my_defm m inner join my_defd d on m.tranId=d.tranId  inner join my_jvtran jv on m.tranid=jv.tranid inner join my_head he on m.acno=he.doc_no "
	    			...
	    			/*xsql = "select a.branch 'Branch',a.dtype 'Doc Type',"+casestatement+"a.atype 'Type',a.account 'Account No.', a.accountname 'Account Name', a.paccount 'Post A/C No.', a.paccountname 'Post A/C Name',"
	    					...
	    					+ "jv.description,jv.dtype,jv.tranid,m.costtype,m.costcode,co.costgroup,(select coalesce(sum(d.amount),0) from my_defd d where posted!=0 and d.tranid=aa) postamount ,coalesce(abs(m.amount)) totalamount,"
	    					+ "(select d.amount-postamount) pendamount,m.doc_no transno,jv.dtype transType from my_defm m inner join my_defd d on m.tranId=d.tranId  inner join my_jvtran jv on m.tranid=jv.tranid inner join my_head he "
	    			...
	    			xsql = "select a.branch 'Branch',a.dtype 'Doc Type',"+casestatement+"a.atype 'Type',a.account 'Account No.', a.accountname 'Account Name', a.paccount 'Post A/C No.', a.paccountname 'Post A/C Name',"
	    					...
	    					+ "(select d.amount-coalesce(b.postamount,0)) pendamount,jv.doc_no transno,jv.dtype transType from my_defm m inner join my_defd d on m.tranId=d.tranId  inner join my_jvtran jv on m.tranid=jv.tranid inner join my_head he "
	    					...
	    					+ "(select coalesce(sum(d.amount),0) postamount,d.tranid,d.sr_no from my_defd d where posted!=0 group by d.tranid,d.sr_no) b on b.tranid=d.tranid and b.sr_no=d.sr_no where d.date>='"+sqlFromDate+"' and "
   							+ "d.date<='"+sqlToDate+"' and  posted=0 and jv.brhid="+branch+""+sqls+") a "+joins+"";
	...
	public ClsDeferredBean getPrint(HttpServletRequest request,int tranid,int branch) throws SQLException {
		 ClsDeferredBean bean = new ClsDeferredBean();
		 Connection conn = null;
		...
		try {
			...
			String headersql="select 'Prepayment' vouchername,c.company,c.address,c.tel,c.fax,lc.loc_name location,b.branchname,b.pbno,b.stcno,b.cstno from my_defm m inner join "
			...
			String frequency="";
			String sqls = "select freqtype from my_defm where brhid="+branch+" and tranid="+tranid+"";
			ResultSet resultSets = stmtPREP.executeQuery(sqls);
			...
			String sql = "select m.tranid aa,CONCAT(he.account,' - ',he.description) accountname,CONCAT(jv.dtype,' - ',jv.doc_no) voucherno,jv.description,DATE_FORMAT(m.date ,'%d-%m-%Y') voucherdate,"
					+ "round(abs(m.amount),2) amount,(select round(coalesce(sum(d.amount),0),2) from my_defd d where posted!=0 and d.tranid=aa) postedamount ,(select round(m.amount-postedamount,2)) balanceamount," 
					...
					+ "m.desc1,coalesce(DATE_FORMAT(DATE_ADD(coalesce(m.stDate,null),INTERVAL if(m.noOfIns=0,0,(m.noOfIns-1)) "+frequency+"),'%d-%m-%Y'),' ') endDate,b.branchname branch from my_defm m inner join "
					+ "my_defd d on m.tranId=d.tranId inner join my_jvtran jv on m.tranid=jv.tranid inner join my_head he on m.acno=he.doc_no inner join my_head pe on m.postacno=pe.doc_no inner join MY_BRCH b on "
			...
			String sql1="select (DATE_FORMAT(date,'%D %M  %Y ')) date,round(amount,2) amount,if(posted>0,'YES','NO') posted from my_defd where tranid="+tranid+"";
	 ...
	 public int prepaymentMasterInsertion(Connection conn, String formdetailcode, Date prePaymentDate, int docno,int txtaccountdocno, int txttrno, int txttranid, int txtdistributiondocno, int txtcosttype,int txtcostno, double txtamount,
	    	...
	    	try{
		    	...
		    	stmtPREP = conn.prepareCall("{CALL deferredDML(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");
