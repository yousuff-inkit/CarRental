...
public class ClsPrePaymentDAO {
	...
	public int insert(String formdetailcode,String cmbtype,int txtaccountdocno, int txttrno, int txttranid, String txtdtype, String txtdistributiondocno, int txtcosttype, int txtcostno,double txtamount, Date startDate, String cmbfrequency,int txtdueafter, int txtinstnos, 
			double txtinstamt,String txtdescription,double txtdebittotal,String txtrowno, Date toDate, ArrayList<String> distributionarray,ArrayList<String> journalvouchersarray,HttpSession session,HttpServletRequest request,double txtdecamount,String hidmodes) throws SQLException {
		...
		//System.out.println("distributionarray"+distributionarray);
	        ...
	        /* MARK AS DISTRIBUTED BUTTON CLICK FUNCTION IN DISTRIBUTION*/
	        if(hidmodes.equalsIgnoreCase("DIS")){
	        	/*For Distribution*/
				if(cmbtype.equalsIgnoreCase("1")){	
					int masterDocno=prepaymentMasterInsertions(conn,formdetailcode,prePaymentDate,docno,txtaccountdocno,txttrno,txttranid,txtdistributiondocno,txtcosttype,txtcostno,txtamount,startDate,cmbfrequency,txtdueafter,txtinstnos,txtinstamt,txtdescription,session, request,txtdecamount);
					if(masterDocno>0){
						...
						if(!distribution[2].equalsIgnoreCase("undefined") && !distribution[2].equalsIgnoreCase("NaN")){
							...
							stmtPREP1 = conn.prepareCall("insert into my_prepd(tranId, sr_no, date, posted, postacno, jobid, amount,jvtrno) values(?,?,?,?,?,?,?,?)");
							stmtPREP1.setInt(1,txttranid);
							stmtPREP1.setInt(2,(i+1));
							stmtPREP1.setDate(3,distributionDate);
							stmtPREP1.setString(4,"999999999");
							stmtPREP1.setString(5,"0");
							...
							stmtPREP1.setString(8,"999999999");
			...
			} /* MARK AS DISTRIBUTED BUTTON CLICK FUNCTION ENDS IN DISTRIBUTION*/
	        ...
	        /* NORMAL SAVE  BUTTON CLICK FUNCTION ENDS IN DISTRIBUTION*/
				...
				// System.out.println("posted=="+posted);
				 if(posted==0 || posted==0.00 || posted==0.0){*/
		            ...
		            //System.out.println("updprepm======="+sql1);
					 ...
					 String sql21=("update my_prepd set postacno='"+txtdistributiondocno+"'where tranid="+txttranid+" and posted=0");
			...
			if(cmbtype.equalsIgnoreCase("3")){
				...
				for(int i=0;i<tranidarray.size();i++){ 
					String sql1 = "update my_prepd d set d.posted="+tranidarray.get(i)+", d.jvtrno="+trno+" where d.rowno="+rownos[i];
		        ...
		        if(check.equalsIgnoreCase("1")){
		           ...
		           if(!(date.equalsIgnoreCase("undefined"))&&!(date.equalsIgnoreCase(""))&&!(date.equalsIgnoreCase("0")))
		           {
			        ...
			        sql="select t.doc_no,t.account,t.description,t.curid,c.code currency,cb.rate,c.type,t.gr_type from my_head t left join my_curr c on t.curid=c.doc_no "
			  	        	  + "left join my_curbook cb on t.curid=cb.curid inner join (select max(cr.doc_no) doc_no,cr.curid curid,cr.toDate,cr.frmDate from my_curbook cr "
			  	        	  + "where coalesce(toDate,curdate())>='"+sqlDate+"' and frmDate<='"+sqlDate+"' group by cr.curid) as bo on(cb.doc_no=bo.doc_no and cb.curid=bo.curid) "
			  	        	  + "where t.atype='GL'  and t.gr_type='4'   and t.m_s=0 "+sqltest;
	...
	public JSONArray prePaymentGridLoading(HttpSession session,String cmbtype,String txtaccid,String fromDate,String toDate,String check) throws SQLException {
		...
		try {
	    		...
	    		{
	    			/*For Distribution*/
					xsql = "select ''settleamt,"+casestatement+"a.account, a.accountname, a.description, a.date, a.trno, a.tranid, a.doc_no, a.dtype, a.brhId, a.branch, a.atype, a.acno, a.curId,"
							...
							+ "left join my_prepd d on jv.tranid=d.tranid where he.den=350 and jv.status=3 and jv.brhid="+branch+" and jv.prep=0 and jv.dramount>0 and d.jvtrno is null"+xsql+") a "+joins+"";
	    		...
	    		{
	    			/*Summary*/
	    			xsql = "select a.settleamt,"+casestatement+"a.account, a.accountname, a.acno, a.atype, a.curid, a.c_rate, a.postacno, a.paccount, a.paccountname, a.freq, a.freqtype,a.noofins, a.instamt, "
	    					...
	    					+ "(select coalesce(sum(d.amount),0) from my_prepd d where posted!=0 and d.tranid=aa) postamount ,abs(m.amount) dramount,(select m.amount-postamount) pendamount,jv.doc_no transno,jv.dtype transType,m.settleamt "
	...
	public JSONArray distributionGridReloading(HttpSession session,String tranId,String check) throws SQLException {
		...
		try {
              ...
              //  System.out.println("disturbuted grid------"+"select sr_no,date,amount,posted,rowno from my_prepd where tranid="+tranId+" ");
				RESULTDATA=commonDAO.convertToJSON(resultSet);
	...
	/* NORMAL SAVE BUTTON INSERT INTO MY_PREPM DML*/
	 public int prepaymentMasterInsertion(Connection conn, String formdetailcode, Date prePaymentDate, int docno,int txtaccountdocno, int txttrno, int txttranid, String txtdistributiondocno, int txtcosttype,int txtcostno, double txtamount,
			    Date startDate,String cmbfrequency, int txtdueafter, int txtinstnos,double txtinstamt,String txtdescription, HttpSession session,HttpServletRequest request,double txtdecamount) throws SQLException{
	    	try{
				...
				stmtPREP.setString(3,txtdistributiondocno); //Post acno
...
/* MARK AS DISTRIBUTED BUTTON INSERT INTO MY_PREPM DML*/
	 public int prepaymentMasterInsertions(Connection conn, String formdetailcode, Date prePaymentDate, int docno,int txtaccountdocno, int txttrno, int txttranid, String txtdistributiondocno, int txtcosttype,int txtcostno, double txtamount,
			    Date startDate,String cmbfrequency, int txtdueafter, int txtinstnos,double txtinstamt,String txtdescription, HttpSession session,HttpServletRequest request,double txtdecamount) throws SQLException{
	    	try{
				CallableStatement stmtPREP;
				...
				stmtPREP.setDate(1,prePaymentDate); //date
				stmtPREP.setInt(2,txtaccountdocno); //acno
				stmtPREP.setInt(3,0); //Post acno
				stmtPREP.setDate(4,startDate); //start date
				stmtPREP.setInt(5, txtdueafter); //due after
				stmtPREP.setInt(6, 0); //cost type
				stmtPREP.setInt(7, 0); //cost code
				stmtPREP.setString(8, "0"); //frequency type
				stmtPREP.setInt(9, txtinstnos); //no of installments
				stmtPREP.setDouble(10, txtamount); //amount
