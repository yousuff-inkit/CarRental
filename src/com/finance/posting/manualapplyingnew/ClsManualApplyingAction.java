		...
		if(mode.equalsIgnoreCase("A")){
			...
			/*Apply-Invoice Grid Saving Ends*/
						...
						if(val>0){
						    ...
						    setChkstatus("3");
						    setData();
						    String retvocno=request.getAttribute("transno").toString();
						   // System.out.println("retvocnoaction====="+retvocno);
						    String retdtype=request.getAttribute("transType").toString();
						    //System.out.println("retdtypeaction====="+retdtype);
	                        setMsg("Please Recheck Applied Amount: "+retdtype+" - "+retvocno);   
                            return "fail";
						}
						else{
							setData();
