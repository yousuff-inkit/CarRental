...
public class ClsRentalAgreementDAO {
	...
	public   JSONArray getActualclientSearch(HttpSession session,String clname,String mob,String lcno,String passno,String nation,String dob,String id,String catdocno) throws SQLException {
	    	...
	    	if(!(nation.equalsIgnoreCase(""))){
	    	 ...
	    	 if(catdocno!=null && !catdocno.equalsIgnoreCase("undefined") && !catdocno.equalsIgnoreCase("")) {
	    		 sqltest+=" and cat.doc_no="+catdocno;
	    	 }
			...
			try {
					...
					ResultSet rslpoconfig=stmtVeh8.executeQuery(strlpoconfig);
					...
					String sqlselect="";
					if(lpoconfig==1) {
						sqlselect+=", coalesce(cat.lpovalidate,0) lpostatus,coalesce(cat.racratestatus,0) racratestatus";
					}
					
					int actualclientconfig=0;
					String stractualclient="select method from gl_config where field_nme='RAGActualClient'";
					ResultSet rsactualclient=stmtVeh8.executeQuery(stractualclient);
					while(rsactualclient.next()) {
						actualclientconfig=rsactualclient.getInt("method");
					}
					
					if(actualclientconfig==1) {
						sqlselect+=", coalesce(cat.actualclient,0) actualclientstatus";
					}
			...
			/*		String clsql= ("select distinct a.pcase,d.dob,d.dlno,d.nation,d.name drname,a.contactPerson,a.mail1,a.cldocno,trim(a.RefName) RefName,a.per_mob,concat(coalesce(a.address,''),'  ',coalesce(a.address2,'')) as address ,a.per_tel,a.codeno,a.acno,m.doc_no, "
					...
					String clsql= ("select distinct "+method+" method,"+lpoconfig+" lpoconfig,cat.doc_no catdocno "+sqlselect+",if(cat.retailstatus=1,1,0) retailstatus,c.cldocno,c.advance,c.invc_method,c.pcase,d.dob,d.dlno,d.nation,"+
					...
					" c.per_tel,c.codeno,c.acno,m.doc_no,trim(m.sal_name) sal_name from my_acbook c left join my_salm m on c.sal_id=m.doc_no and m.status<>7"+
	...
	int weekend,String hidchkorgregcard,String hidchkigst,String hidrentalproject,ClsRentalAgreementAction masteraction) throws SQLException {
		String 	barchid="0";
	//	System.out.println("TASYSTEM: "+tasystem);
		...
		try{
			...
			//Checking Duplicate Agreement exists
			
			String strdup="select count(*) itemcount from gl_ragmt where status<>7 and fleet_no="+fleetNO+" and clstatus=0";
			ResultSet rsdup=conn.createStatement().executeQuery(strdup);
			int dupcount=0;
			while(rsdup.next()) {
				dupcount=rsdup.getInt("itemcount");
			}
			if(dupcount>0) {
				System.out.println("Duplicate Agreement exists");
				conn.close();
				return -1;
			}
...
CallableStatement stmtrentalagmt = conn.prepareCall("{CALL rentalAgmtDML(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");
   ...
   //  System.out.println("-----------"+stmtrentalagmt);
	...
	if(docno>0){
		...
		String chkinsurcomp=masteraction.getHidchkinsurcomp().trim().equalsIgnoreCase("") || masteraction.getHidchkinsurcomp().trim().equalsIgnoreCase("undefined") || masteraction.getHidchkinsurcomp()==null ?"0":masteraction.getHidchkinsurcomp().trim();
		String insurcldocno=masteraction.getCmbinsurcomp().trim().equalsIgnoreCase("") || masteraction.getCmbinsurcomp().trim().equalsIgnoreCase("undefined") || masteraction.getCmbinsurcomp()==null ?"0":masteraction.getCmbinsurcomp().trim();
		String insurcompdays=masteraction.getInsurcompdays().trim().equalsIgnoreCase("") || masteraction.getInsurcompdays().trim().equalsIgnoreCase("undefined") || masteraction.getInsurcompdays()==null ?"0":masteraction.getInsurcompdays().trim();
		String cmbenqtype=masteraction.getCmbenqtype().trim().equalsIgnoreCase("") || masteraction.getCmbenqtype().trim().equalsIgnoreCase("undefined") || masteraction.getCmbenqtype()==null ?"0":masteraction.getCmbenqtype().trim();
		
		String racrate=masteraction.getRacrate().trim().equalsIgnoreCase("") || masteraction.getRacrate().trim().equalsIgnoreCase("undefined") || masteraction.getRacrate()==null ?"0":masteraction.getRacrate().trim();
		
		String actualcldocno=masteraction.getActualcldocno().trim().equalsIgnoreCase("") || masteraction.getActualcldocno().trim().equalsIgnoreCase("undefined") || masteraction.getActualcldocno()==null ?"0":masteraction.getActualcldocno().trim();
		
		int lpoconfig=0;
		String strlpoconfig="select method from gl_config where field_nme='LPOValidate'";
		ResultSet rslpoconfig=stmtorgreg.executeQuery(strlpoconfig);
		while(rslpoconfig.next()) {
			lpoconfig=rslpoconfig.getInt("method");
		}
		...
		ResultSet rsinsurcompconfig=stmtorgreg.executeQuery(strsql);
		...
		int actualclientconfig=0;
		String stractualclient="select method from gl_config where field_nme='RAGActualClient'";
		ResultSet rsactualclient=stmtorgreg.executeQuery(stractualclient);
		while(rsactualclient.next()) {
			actualclientconfig=rsactualclient.getInt("method");
		}
		
		int clcount = 0;
		String strcount = "SELECT coalesce(rostatus,0)+1 clcount FROM my_acbook WHERE cldocno='"+actualcldocno+"' AND dtype='CRM'";
		ResultSet rscount = stmtorgreg.executeQuery(strcount);
		while(rscount.next()) {
			clcount = rscount.getInt("clcount");
		}  
		
		String strorg="UPDATE  my_acbook SET rostatus='"+clcount+"' WHERE cldocno='"+actualcldocno+"' AND dtype='CRM'";
	    stmtorgreg.executeUpdate(strorg);
		
		String sqlupdate="";
		if(lpoconfig==1) {
			sqlupdate+=" racrate="+racrate+",";
		}
		if(insurcompconfig==1) {
			sqlupdate+=" chkinsurcomp="+chkinsurcomp+",insurcldocno="+insurcldocno+",insurcompdays="+insurcompdays+",";
		}
		
		if(actualclientconfig==1) {
			sqlupdate+=" actualcldocno="+actualcldocno+",";
		}
		String strorgreg="update gl_ragmt set "+sqlupdate+"orgregcard="+hidchkorgregcard+",igststatus="+hidchkigst+",rentalproject="+hidrentalproject+",enqtype="+cmbenqtype+" where doc_no="+docno;
...
/*   					String sqltari=("select m.doc_no,m.validto,concat(m.tariftype,' - ',m.notes) tariftype,m.clientid, coalesce(insurexcess,0) insurexcess, coalesce(cdwexcess,0) cdwexcess,  "
				...
				String sqltari=("select m.doc_no,m.validto,concat(m.tariftype,' - ',m.notes) tariftype,m.clientid, coalesce(insurexcess,0) insurexcess, coalesce(cdwexcess,0) cdwexcess,  "
   				...
   				System.out.println("=========tariff========="+sqltari);
 ...
 public JSONArray clientDriver(String clientid) throws SQLException {
	...
	public  String getcard() throws SQLException {
	   ...
	   //  System.out.println("-----sqls----"+sqls);
	    ...
	    public   JSONArray mainSrearch(HttpSession session,String sclname,String smob,String rno,String flno,String sregno,String smra,String lponumber,String branch_chk) throws SQLException {
	    	...
	    	if(!(lponumber.equalsIgnoreCase(""))){
	    		sqltest=sqltest+" and r.pono like '%"+lponumber+"%'";
	    	}
			...
			try{
				...
				ResultSet rsinsurcompconfig=stmt.executeQuery(strinsurcompconfig);
				while(rsinsurcompconfig.next()){
					insurcompconfig=rsinsurcompconfig.getInt("method");
				}
				String sqljoins="",sqlselect="";
				if(insurcompconfig==1) {
					sqljoins=" left join my_acbook insurac on (r.insurcldocno=insurac.cldocno and insurac.dtype='CRM')";
					sqlselect=" coalesce(insurac.refname,'') insurcompname,";
					if(!(sclname.equalsIgnoreCase(""))){
			    		sqltest=sqltest+" and (a.RefName like '%"+sclname+"%' or insurac.refname like '%"+sclname+"%')";
			    	}
				}
				else {
					if(!(sclname.equalsIgnoreCase(""))){
			    	 ...
			    	 if(branch_chk.equalsIgnoreCase("1"))  // WIB with in branch, WOB with out branch
			    	 {
			    		 ...
			    		 String str1Sql=("select "+sqlselect+" 'WOB' checks,r.pono,r.doc_no,r.voc_no,r.fleet_no,r.acno,r.cldocno,r.date,r.salid,r.delivery,r.chif,r.drid,r.addr,r.said, "
									...
									+ "left join my_salm m on a.sal_id=m.doc_no left join gl_rtarif t on t.rdocno=r.doc_no "+sqljoins+" where r.status=3  "
			        ...
			        String brnchid=session.getAttribute("BRANCHID").toString();
						...
						String str1Sql=("select "+sqlselect+" 'WIB' checks,r.pono,r.doc_no,r.voc_no,r.fleet_no,r.acno,r.cldocno,r.date,r.salid,r.delivery,r.chif,r.drid,r.addr,r.said, "
								+ " r.raid,round(r.okm) okm,r.odate,r.otime,r.ofuel,r.ochkid,r.ddate,r.dtime,r.tselect,r.tdocno,r.insex,r.invtype, "
								+ " r.addrchg,r.mrano,r.mrano mrnumber,r.pono,r.ofleet_no,v.reg_no,a.RefName,a.per_mob,t.gid from gl_ragmt r  "
								+ " left join gl_vehmaster v on v.fleet_no=r.fleet_no left join my_acbook a on a.cldocno= r.cldocno and a.dtype='CRM'  "
								+ "left join my_salm m on a.sal_id=m.doc_no left join gl_rtarif t on t.rdocno=r.doc_no "+sqljoins+" where r.status=3 and "
	    ...
	    public   JSONArray clientSearch(HttpSession session,String clname,String mob,String lcno,String passno,String nation,String dob,String id) throws SQLException {
	   	 JSONArray RESULTDATA=new JSONArray();
	   	 if(!id.equalsIgnoreCase("1")) {
	   		 return RESULTDATA;
	   	 }
...
/*
	      ...
	      String brid=session.getAttribute("BRANCHID").toString();
			...
			/*		String clsql= ("select distinct a.pcase,d.dob,d.dlno,d.nation,d.name drname,a.contactPerson,a.mail1,a.cldocno,trim(a.RefName) RefName,a.per_mob,concat(coalesce(a.address,''),'  ',coalesce(a.address2,'')) as address ,a.per_tel,a.codeno,a.acno,m.doc_no, "
					...
					String clsql= ("select distinct "+method+" method,"+lpoconfig+" lpoconfig,cat.doc_no catdocno "+sqlselect+",coalesce(round(a.balance,2),0) outstanding,if(cat.retailstatus=1,1,0) retailstatus,c.cldocno,c.advance,c.invc_method,c.pcase,d.dob,d.dlno,d.nation,"+
					" d.name drname,c.contactPerson,c.mail1,trim(c.RefName) RefName,c.per_mob,concat(coalesce(c.address,''),'  ',coalesce(c.address2,'')) as address ,"+
	    ...
	    public   JSONArray vehSearch(HttpSession session,String fleetno,String regno,String flname,String color,String group,String aa) throws SQLException {
				...
				try {
					...
					if(aa.equalsIgnoreCase("yes"))
					{
						...
						//Getting contract vehicle restrict config
						int restrictContractVeh=0;
						String strconfig="select method from gl_config where field_nme='restrictContractVeh'";
						ResultSet rsconfig=stmtVeh8.executeQuery(strconfig);
						while(rsconfig.next()){
							restrictContractVeh=rsconfig.getInt("method");
						}
						String sqljoin="",sqlfilter="";
				...
				System.out.println("------vehcile---------"+vehsql);
	    ...
	    public   ArrayList<String> getAppInspectionPrintPics(String docno, String url,String agmtno,String agmtype) throws SQLException {
			// TODO Auto-generated method stub
			Connection conn=null;
			...
			try {
				...
				strnew="SELECT path FROM my_fileattach WHERE doc_no="+docno+" AND dtype='VIP' AND sr_no<>"+signsrno+"";
		...
		public   ArrayList<String> getInspectionPrintPicsLeft(String docno, String url,String agmtno,String agmtype) throws SQLException {
			// TODO Auto-generated method stub
			Connection conn=null;
			ArrayList<String> picarray=new ArrayList<String>();
			try {
		...
		public   ArrayList<String> getInspectionPrintPicsRight(String docno, String url,String agmtno,String agmtype) throws SQLException {
			// TODO Auto-generated method stub
			Connection conn=null;
			ArrayList<String> picarray=new ArrayList<String>();
			try {
	    ...
	    public  ClsRentalAgreementBean getViewDetails(HttpSession session,int docNo) throws SQLException {
			...
			int lpoconfig=0;
			String strlpoconfig="select method from gl_config where field_nme='LPOValidate'";
			ResultSet rslpoconfig=stmtCPV0.executeQuery(strlpoconfig);
			while(rslpoconfig.next()){
				lpoconfig=rslpoconfig.getInt("method");
			}
			int insurcompconfig=0;
			String strsql="select method from gl_config where field_nme='RAGInsurComp'";
			ResultSet rsinsurcompconfig=stmtCPV0.executeQuery(strsql);
			while(rsinsurcompconfig.next()){
				insurcompconfig=rsinsurcompconfig.getInt("method");
			}
			String sqlselect="";
			if(lpoconfig==1) {
				sqlselect+=" round(coalesce(r.racrate,0),2) racrate,";
			}
			if(insurcompconfig==1) {
				sqlselect+=" r.chkinsurcomp,r.insurcldocno,r.insurcompdays,";
			}
			
			int actualclientconfig=0;
			String stractualclient="select method from gl_config where field_nme='RAGActualClient'";
			ResultSet rsactualclient=stmtCPV0.executeQuery(stractualclient);
			while(rsactualclient.next()) {
				actualclientconfig=rsactualclient.getInt("method");
			}
			String sqljoin="";
			if(actualclientconfig==1) {
				sqlselect+=" r.actualcldocno,concat(coalesce(acc.refname,''),',','CONTACT PERSON : ',coalesce(acc.contactperson,''), ' , ', ' MOB : ',coalesce(acc.per_mob,''), ' , ' ,'EMAIL : '  ,coalesce(acc.mail1,'') , ' , ' ,'ADDRESS : ',coalesce(acc.address,''), ' , ' ,'TEL NO : ',coalesce(acc.per_tel,'')) actualclientname,";
				sqljoin+=" left join my_acbook acc on acc.cldocno=r.actualcldocno   and  acc.dtype='CRM'";
			}
			System.out.println("inside");
			String strSql=("select "+sqlselect+"r.rentalproject hidrentalproject,r.enqtype,coalesce(proj.project_name,'') rentalproject,coalesce(r.igststatus,0) igststatus,coalesce(r.orgregcard,0) orgregcard,concat(r.ofleet_no,' (',vehcontract.reg_no,' - ',contractplate.code_name,')') contractvehicle,r.weekend,r.voc_no,if(r.clstatus=0,'Open',if(r.clstatus=1,CONCAT(coalesce(DATE_FORMAT(racl.date,'%d-%m-%Y'),''),' Close'),'Cancel')) clstatus,r.doc_no,r.fleet_no,r.desc1, "
					...
					+ "  v.a_loc,c.color,g.gid,a.cldocno,a.RefName , CONCAT('CONTACT PERSON : ',coalesce(a.contactperson,''), ' , ', ' MOB : ',coalesce(a.per_mob,''), ' , "
					...
					+ "	 left join gl_vehgroup g on g.doc_no=v.vgrpid left join gl_vehplate plate on v.pltid=plate.doc_no left join my_acbook a on a.cldocno=r.cldocno   and  a.dtype='CRM'  "+sqljoin+""
		...
		System.out.println("sql7==================="+strSql);
			...
			while (resultSet.next()) {
				...
				if(actualclientconfig==1) {
					RentalAgreementBean.setActualcldocno(resultSet.getString("actualcldocno"));
					RentalAgreementBean.setActualclientname(resultSet.getString("actualclientname"));
				}
				if(insurcompconfig==1) {
					RentalAgreementBean.setHidchkinsurcomp(resultSet.getString("chkinsurcomp"));
					RentalAgreementBean.setHidcmbinsurcomp(resultSet.getString("insurcldocno"));
					RentalAgreementBean.setInsurcompdays(resultSet.getString("insurcompdays"));
				}
				
				if(lpoconfig==1) {
					RentalAgreementBean.setRacrate(resultSet.getString("racrate"));
				}
				RentalAgreementBean.setHidcmbenqtype(resultSet.getString("enqtype"));
			   ...
			   RentalAgreementBean.setJqxDateOut(resultSet.getString("odate").toString());
				...
				// System.out.println("Excessinsur---"+RentalAgreementBean.getExcessinsur());
				//payment
...
/*	      strSql=("select r.voc_no,round(r.insex,2) insex,if(r.clstatus=0,'Open','Closed') clstatus, r.doc_no,round(r.addrchg,2) addrchg,r.mrano,round(r.okm) okm,"
		   ...
		   String strindigo="select coalesce(br.email,'') indigoemail,coalesce(br.address,'') indigoaddress,coalesce(ra.sal_code,'') rentalagent,coalesce(dr.nation,'') nationality,coalesce(br.tel,'') indigobranchtel,coalesce(br.fax,'') indigobranchmobile,agmt.cldocno from gl_ragmt agmt left join my_brch br on "+
...
//		   System.out.println("@@@@@@@@@============="+strindigo+"====nation===="+rsindigo.getString("nationality"));
			  ...
			  bean.setLblindigoemail(rsindigo.getString("indigoemail"));
			  bean.setLblindigoaddress(rsindigo.getString("indigoaddress"));
		...
		//   System.out.println("----suss---"+strrentserdue);
		  ...
		  // System.out.println("---ly---------:"+strrate);
	       ...
	       String str="select (select round(exkmrte,2) exkmrte from gl_rtarif where rdocno="+docno+" and rstatus=7) exkmrte,agmt.clstatus,comp.company,comp.address,comp.tel,comp.fax,loc.loc_name location,br.branchname,coalesce(agmt.voc_no,'') kmagmtno,coalesce(ac.refname,'') kmclient,coalesce(agmt.refno,'') kmmrano,concat(coalesce(veh.fleet_no,''),' - ',coalesce(veh.reg_no,''),' - ',coalesce(veh.flname,'')) kmvehicle,"+
					" coalesce(date_format(agmt.date,'%d/%m/%Y'),'') kmdate,coalesce(drv.name,'') kmdriver,coalesce(drv.mobno,'') kmmobno,coalesce(drv.dlno,'') kmdlno,coalesce(drv.dob,'') kmdob from gl_ragmt agmt left join my_acbook ac on (agmt.cldocno=ac.cldocno and ac.dtype='CRM') "+
					...
					ResultSet rs=stmtinvoice.executeQuery(str);
					
					while(rs.next()){
						
						bean.setKmdriver(rs.getString("kmdriver"));
						bean.setKmmobno(rs.getNString("kmmobno"));
						bean.setKmdlno(rs.getString("kmdlno"));
						bean.setKmdob(rs.getString("kmdob"));
						
					}
		   		   //	System.out.println("kmdriver--"+bean.getKmdriver());	   
					 strSql=("select p.code_name pltid,CASE WHEN rc.colfuel='0.000' THEN ''WHEN rc.colfuel='0.125' THEN 'Level 1/8' WHEN rc.colfuel='0.250'   THEN 'Level 2/8' WHEN rc.colfuel='0.375'    THEN 'Level 3/8'    WHEN rc.colfuel='0.500' THEN 'Level 4/8' WHEN rc.colfuel='0.625' THEN 'Level 5/8'    WHEN rc.colfuel='0.750' THEN 'Level 6/8' WHEN rc.colfuel='0.875' THEN 'Level 7/8' WHEN rc.colfuel='1.000'    THEN 'Level 8/8' WHEN rc.colfuel is null THEN ''   END as 'colfuel',coalesce(round(rc.colkm),'') colkm,coalesce(DATE_FORMAT(rc.coldate,'%d-%m-%Y'),'') coldate,coalesce(rc.coltime,'')coltime, "
				                + " v.srvc_km,u.authname ,coalesce(ra.authority,'')authority,coalesce(DATE_FORMAT(r.date,'%d-%m-%Y'),'') createdate,coalesce(r.desc1,'') agmtdesc,coalesce(ch.sal_name,'') chkoutby,usr.user_id,a.trnnumber,a.cldocno,v.ch_no,coalesce(r.desc1,'') description,coalesce(DATE_FORMAT(r.ddate,'%d-%m-%Y'),'') ddate, dtime, ra.sal_name ragnt,sa.sal_name sagnt, sm.sal_name ,   round(coalesce(r.delchg,0),2) delchg,a.per_tel,if(a.ser_default=1,if(g1.method=1,round(g1.value,2),0),round(per_salikrate,2)) salikcharge, \r\n" 
				    		    + "    if(a.ser_default=1,round(g2.value,2),round(per_trafficharge,2)) trafficcharge, \r\n"  
				      		    + "     4+if(a.ser_default=1,if(g1.method=1,round(g1.value,2),0),round(per_salikrate,2)) saliktotal,concat(u.authid,' ',p.code_name,' ',v.reg_no) reg_noval,r.voc_no,round(r.insex,2) insex,if(r.clstatus=0,'Open','Closed') clstatus,a.per_tel,a.fax1, r.doc_no,round(coalesce(r.addrchg,0),2) addrchg,r.mrano,coalesce(round(r.okm),'') okm,"
					       		...
					       		+ "a.address,a.address2,a.per_mob,a.mail1,v.fueltype,br.branchname,concat(v.reg_no,' ',u.authid,' ',p.code_name) authregno,concat(v.reg_no,' - ',p.code_name) reg_no,v.flname,g.gname,mo.vtype,p.code_name,"
					       		+ "(select round(rate,2) rate  from gl_rtarif where  rdocno="+docno+" and rstatus=7) as rate, (select  round(cdw,2)+round(cdw1,2) "
					       		+ "  from gl_rtarif where  rdocno="+docno+" and rstatus=7) as cdw , (select round(gps,2)+round(babyseater,2)+round(cooler,2) "
					       		+ "	 from gl_rtarif where  rdocno="+docno+" and rstatus=7) as accessories,(select round(gps,2)+round(babyseater,2) from gl_rtarif where  rdocno="+docno+" and rstatus=7) as raaccessorysum, "
					       		...
					       		+ "left join gl_vehplate p on p.doc_no=v.pltid left join gl_rtarif t on t.rdocno=r.doc_no  	   			"
					       		+ " left join gl_ragmtclosem rc on rc.agmtno=r.doc_no and rc.status=3 left join gl_yom yo on yo.doc_no=v.yom  left join my_user usr on r.userid=usr.doc_no	   		"
	 ...
	 // System.out.println("-----Print Query----"+strSql);
	       ...
	       while(resultSet.next()){
	    	  // bean.setLbmasterdate(resultSet.getString("date"));
	    	   bean.setSrvckm(resultSet.getString("srvc_km"));  
	    	   bean.setPltid(resultSet.getString("pltid"));  
	    	  ...
	    	  // getDuedate getSalagent getRaagent
	    	   //classic address2
				...
				bean.setLblduedate(resultSet.getString("ddate"));
		...
		//mm
		   bean.setSalname(resultSet.getString("sal_name"));
	    	...
	    	// in details
		    	   ...
		    	   bean.setColdates(resultSet.getString("coldate"));
		    	   bean.setColtimes(resultSet.getString("coltime"));
		    	   bean.setColkmins(resultSet.getString("colkm"));
		    	   bean.setColfuels(resultSet.getString("colfuel"));
	    	  ...
	    	  //  bean.setFaxno(resultSet.getString("fax1"));
	    	    ...
	    	    if(resultSet.getString("coldate").equalsIgnoreCase("") && resultSet.getString("coltime").equalsIgnoreCase("")  && resultSet.getString("colkm").equalsIgnoreCase("")  && resultSet.getString("colfuel").equalsIgnoreCase(""))
                {
            bean.setColdetails(""); 
        ...
        else
        {
	    	...
	    	bean.setIndetails  ("In");
	    	   ...
	    	   bean.setExcessinsur(resultSet.getString("insex"));
	    	   ...
	    	   bean.setRacardoo(resultSet.getString("scdw"));
	    	   bean.setRaaccessorysum(resultSet.getString("raaccessorysum"));
	...
	// driven
	       ...
	       String strcosmo="select (select round(rate,2) rate  from gl_rtarif where  rdocno="+docno+" and rstatus=7) as rate,"+
	    		  ...
	    		  " cosmocheckout,v.fleet_no cosmofleetno,brd.brand_name cosmobrand,d.issfrm cosmoissuedat,coalesce(round(r.insex,2),'') insurexcess from gl_ragmt r left join my_acbook a on a.cldocno=r.cldocno  and  a.dtype='CRM' left join gl_drdetails d on "
		       		...
		       		+ "left join gl_vehplate p on p.doc_no=v.pltid left join gl_rtarif t on t.rdocno=r.doc_no  	   			"
		       		+ " left join gl_ragmtclosem rc on rc.agmtno=r.doc_no and rc.status=3 left join gl_yom yo on yo.doc_no=v.yom 	   		"
      ...
      int i=0;
		...
		//    	    System.out.println("datesssssss"+resultmain.getString("issuedby"));
	    	   ...
	    	   if(i==2)
	    	   {
		    	  ...
		    	  bean.setNation2(resultmain.getString("nation"));
	    	   }
	 ...
	 ResultSet ress1 = stmtinvoice51.executeQuery(sql011); 
		 ...
		 //   System.out.println("----------"+companysql);
	         ...
	         ResultSet resultsetcompany = stmtinvoice10.executeQuery(companysql); 
		       ...
		       Statement confstmt = conn.createStatement ();
               
                /*String totalsql="select if(bb.inv-aa.total=0,'',bb.inv-aa.total) balance,if(aa.total=0,'',aa.total) total,if(bb.inv=0,'',bb.inv) inv from ((select coalesce(round(sum(dramount),2),0) total from my_jvtran where rtype='RAG' and rdocno="
               +docno+"   and dtype='CRV' and id=1)  aa, "
 +
 "(select coalesce(round(sum(dramount),2),0) inv from my_jvtran where rtype='RAG' and rdocno="
 +docno+"   and dtype='INV' and id=1) bb);  ";*/

		       
		       int config=0;
		       String  totalsql="";
		       String confsql="select method from gl_config where field_nme='epicPrintRentalClose'";
		       ResultSet configRs=confstmt.executeQuery(confsql);
		       if(configRs.next()) {
		           config=configRs.getInt("method");
		       }
		       if(config>0) {
		           totalsql="select sum(debit) inv,sum(credit) total,sum(if(debit-credit=0,'',debit-credit)) as balance from(select a.* "
	                       + " from (select j.doc_no transno,j.tranid,CONVERT(if(j.ldramount>0,round((j.ldramount*1),2),0),CHAR(100)) debit,"
	                       + " CONVERT(if(j.ldramount<0,round((j.ldramount*-1),2),0),   CHAR(100)) credit from my_jvtran j inner join gl_ragmt r"
	                       + " on (j.rdocno=r.doc_no and rtype='RAG') left join my_acbook c on (r.cldocno=c.cldocno and c.dtype='CRM' and "
	                       + "c.acno=j.acno) left join (select acno from my_account where codeno='rsecurity') h on j.acno=h.acno where j.status=3 "
	                       + "and j.rdocno=2803 and j.rtype='RAG' and (c.doc_no is not null or h.acno is not null) union all   "
	                       + "( select transno,tranid,if(sum(gr.debit)=0,0,round(sum(gr.debit),2)),if(sum(gr.credit)=0,0,round(sum(gr.credit),2)) "
	                       + "from  (select j1.acno,j1.doc_no transno,j1.tranid,CONVERT(if(j1.ldramount>0,round((d.amount),2),0),CHAR(100)) debit,"
	                       + "CONVERT(if(j1.ldramount<0,round((d.amount),2),0),CHAR(100)) credit,j.ldramount from my_jvtran j inner join"
	                       + " my_outd d on d.ap_trid=j.tranid      inner join my_jvtran j1 on d.tranid=j1.tranid  inner join gl_ragmt r on "
	                       + "j.rdocno=r.doc_no and j.rtype='RAG' inner join my_acbook c on c.doc_no=r.cldocno and c.dtype='CRM' and c.acno=j.acno"
	                       + " where  j.status=3 and j.rdocno='"+docno+"' and j.rtype='RAG' and j1.tr_no is not null group by j1.doc_no)gr group"
	                       + " by gr.tranid)) a group by a.tranid)a";
...
System.out.println("======="+totalsql);
			   ...
			   //    System.out.println("delsql-----------+++++----------"+delsql);
				     ...
				     /*  String  colsql="select DATE_FORMAT(v.dout,'%d-%m-%Y') din, v.tout tin, round(v.kmout) kmin, CASE WHEN v.fout='0.000' THEN 'Level 0/8' WHEN v.fout='0.125' THEN 'Level 1/8' WHEN v.fout='0.250' "
					       		...
					       		+ "and v.rdtype='RAG' and v.status='IN' and trancode='DL' inner join gl_ragmtclosem col on col.agmtno=r.doc_no and col.status=3 "
			...
			*/
				       String  colsql="select DATE_FORMAT(v.din,'%d-%m-%Y') din, v.tin tin, round(v.kmin) kmin, CASE WHEN v.fin='0.000' THEN 'Level 0/8' WHEN v.fin='0.125' THEN 'Level 1/8' WHEN v.fin='0.250' "
                               + "THEN 'Level 2/8' WHEN v.fin='0.375' THEN 'Level 3/8' WHEN v.fin='0.500' THEN 'Level 4/8' WHEN "
                               + "v.fin='0.625' THEN 'Level 5/8'  WHEN v.fin='0.750' THEN 'Level 6/8' WHEN v.fin='0.875' THEN "
                               + "'Level 7/8' WHEN v.fin='1.000' THEN 'Level 8/8'  END as 'fin' from gl_ragmt r inner join gl_vmove v on v.rdocno=r.doc_no "
                               + "and v.rdtype='RAG' and v.status='IN' and trancode='DL' inner join gl_ragmtclosem col on col.agmtno=r.doc_no and col.status=3 "
                               + " where col.colstatus=1 and r.doc_no="+docno+"  and v.doc_no =(select max(doc_no) from gl_vmove where rdocno="+docno+" "
                               + "and rdtype='RAG' and status='IN' and trancode='DL' and repno=0 and  custno=0 and exchng=0) ";
           
				       
				     //  System.out.println("colected=-"+colsql);
						...
						// System.out.println("1========"+rstmtsql2);
						      ...
						      //   setSecuritycardno(),setSecurityexpdate(),setSecuritypreauthno(),setSecuritypreauthamt(),
								...
								double tariftotal=0,cdwtotal=0,accsssorytotal=0,deltotal=0,ycdwtotal=0,ycdw1total=0;;
								...
								ycdwtotal=(Double.parseDouble(bean.getSelcarcdw())*totaldays)/divval; 	   
								ycdw1total=(Double.parseDouble(bean.getSelcarcdw1())*totaldays)/divval; 	   
								int totaldays1=0;
                                if (rentaltype.equalsIgnoreCase("Daily") ||rentaltype.equalsIgnoreCase("Weekly") || rentaltype.equalsIgnoreCase("Monthly")) {
                                    totaldays1=1;
                                    divval=1;
                                    deltotal=(Double.parseDouble(bean.getLaldelcharge())*totaldays1)/divval;
						       ...
						       bean.setSelcarcdwtotal(df.format(ycdwtotal)+"");
						       bean.setSelcarcdw1total(df.format(ycdw1total)+"");
	 ...
	 /*String clsprintqry="select tarif.rdocno,tarif.gps,tarif.babyseater,tarif.cooler,agmt.pono,agmt.doc_no,agmt.desc1,rpt.cardno,rpt.amount from gl_ragmt agmt left  join "
		...
		//Overriding for Carfare
			...
			//Overriding Client as Insurance Company-Carfare
			int insurcompconfig=0;
			String strinsurcompconfig="select method from gl_config where field_nme='RAGInsurComp'";
			ResultSet rsinsurcompconfig=stmt.executeQuery(strinsurcompconfig);
			while(rsinsurcompconfig.next()) {
			...
			if(insurcompconfig==1) {
				String strinsurcomp="SELECT chkinsurcomp,insurcldocno,COALESCE(ac.RefName,'') refname,COALESCE(ac.address,'') address,COALESCE(ac.per_mob,'') mobile,COALESCE(ac.mail1,'') email FROM gl_ragmt agmt \r\n" + 
						"LEFT JOIN my_acbook ac ON agmt.insurcldocno=ac.cldocno AND ac.dtype='CRM' where agmt.doc_no="+docno;
				ResultSet rsinsurcomp=stmt.executeQuery(strinsurcomp);
				int chkinsurcomp=0;
				while(rsinsurcomp.next()) {
					chkinsurcomp=rsinsurcomp.getInt("chkinsurcomp");
					if(chkinsurcomp==1) {
						bean.setLblindigocldocno(rsinsurcomp.getString("insurcldocno"));
						bean.setClname(rsinsurcomp.getString("refname"));
						bean.setCladdress(rsinsurcomp.getString("address"));
						bean.setClmobno(rsinsurcomp.getString("mobile"));
						bean.setClemail(rsinsurcomp.getString("email"));
					}
				}
			}
			
			//Overriding Client as Actual Client -carfare
			int actualclientconfig=0;
			String stractualclientconfig="select method from gl_config where field_nme='RAGActualClient'";
		//	System.out.println("Actual Client Config:"+stractualclientconfig);
			ResultSet rsactualclientconfig=stmt.executeQuery(stractualclientconfig);
			while(rsactualclientconfig.next()) {
				actualclientconfig=rsactualclientconfig.getInt("method");
			}
			
			if(actualclientconfig==1) {
				String stractualclient="SELECT actualcldocno,COALESCE(ac.RefName,'') refname,COALESCE(ac.address,'') address,COALESCE(ac.per_mob,'') mobile,COALESCE(ac.mail1,'') email,coalesce(cat_name,'') actualcategory FROM gl_ragmt agmt \r\n" + 
						"LEFT JOIN my_acbook ac ON agmt.actualcldocno=ac.cldocno AND ac.dtype='CRM' LEFT JOIN my_acbook ac1 ON agmt.cldocno=ac1.cldocno AND ac1.dtype='CRM'  left join my_clcatm clm on clm.doc_no=ac1.catid where agmt.doc_no="+docno+" and coalesce(agmt.actualcldocno,0)>0";
				//System.out.println("Actual Client:"+stractualclient);
				ResultSet rsactualclient=stmt.executeQuery(stractualclient);
				while(rsactualclient.next()) {
					
					bean.setLblindigocldocno(rsactualclient.getString("actualcldocno"));
					bean.setClname(rsactualclient.getString("refname"));
					bean.setCladdress(rsactualclient.getString("address"));
					bean.setClmobno(rsactualclient.getString("mobile"));
					bean.setClemail(rsactualclient.getString("email"));
				 
					bean.setActualcategory(rsactualclient.getString("actualcategory"));
					bean.setActualcldocno(rsactualclient.getString("actualcldocno"));;
				}
			}
	       conn.close();
	      }
...
/*						String sql="select a.*,if(a.transtype='INV',m.voc_no,a.transno) transno  from (select j.acno,j.doc_no transno,j.tr_no,j.tranid,j.dtype transtype,j.date trdate,j.rdocno rentaldoc,j.rtype rentaltype,  "
					...
					*/
						...
						/*#*/	+ " ( select brhid,acno,transno,tr_no,tranid,transtype,trdate,rentaldoc,rentaltype,   if(sum(gr.debit)=0,'',round(sum(gr.debit),2)),if(sum(gr.credit)=0,'',round(sum(gr.credit),2)) "
								...
								+ "    CONVERT(if(j1.ldramount>0,round(sum(d.amount),2),''),CHAR(100)) debit, 	CONVERT(if(j1.ldramount<0,round(sum(d.amount),2),''),CHAR(100)) credit,j.ldramount,if(j1.description='0','',j1.description)  "
								+ "  description  from my_jvtran j inner join  	my_outd d on d.ap_trid=j.tranid      inner join my_jvtran j1 on d.tranid=j1.tranid  inner join gl_ragmt r  "
								+ "   	on j.rdocno=r.doc_no and j.rtype='RAG' inner join my_acbook c on c.doc_no=r.cldocno and c.dtype='CRM' and c.acno=j.acno    "
								+ "    	where  j.status=3 and j.rdocno='"+rentaldocval+"' and j.rtype='RAG' and j1.tr_no is not null group by j1.doc_no) gr group by gr.tranid)) a "
			...
			try {
				...
				if(delstatus==1){
					String strmovsql="select mov.din,mov.tin,round(mov.kmin,2) kmin,mov.fin from gl_vmove mov left join gl_ragmt agmt "+
								" on (mov.rdocno=agmt.doc_no and mov.rdtype='RAG') where mov.rdtype='RAG' and mov.rdocno="+docNo+" and mov.trancode='DL' and mov.repno=0  and mov.exchng=0 and mov.custno=0 ";
...
String sql6="";
    ...
    */
	...
	public JSONArray getNotInvoicedSalik(String docno,String id) throws SQLException{
	    ...
	    public JSONArray getFuelDetails(String docno,String id) throws SQLException{
	    	JSONArray kmdata=new JSONArray();
	    	if(!id.equalsIgnoreCase("1")){
	    		return kmdata;
	    	}
	    	...
	    	try{
	    		...
	    		String sqlconfig="select if((select method from gl_config where field_nme='prate')=1,"
	    				+ "(select value from gl_config where field_nme='prate'),0.00) prate, "
	    				+ " if((select method from gl_config where field_nme='drate')=1,"
	    				+ "(select value from gl_config where field_nme='drate'),0.00)drate";
	    		ResultSet rs=stmt.executeQuery(sqlconfig);
	    		while(rs.next()){
	    			prate=rs.getDouble("prate");
					drate=rs.getDouble("drate");
					}
	    		if(delcal==0){
	    			sqltest=" and trancode<>'DL'";
	    		}
	    		String strsql="select mov.fleet_no,veh.reg_no,veh.flname,mov.kmout,mov.dout,mov.tout,mov.kmin,mov.din,coalesce(mov.tin,'') tin,mov.kmin-mov.kmout kmdiff, "
	    				+ "	mov.fin,mov.fout,(mov.fin-mov.fout) fueldiff,tcap,tcap*(mov.fin-mov.fout)*(if( veh.fueltype='P','"+prate+"','"+drate+"'))*-1 fuelamount from "
	    				+ "	gl_vmove mov left join gl_vehmaster veh on mov.fleet_no=veh.fleet_no where mov.rdocno="+docno+" and mov.rdtype='RAG' and mov.status='IN'"+sqltest;
	    		//System.out.println(strsql);
	    		ResultSet rs1=stmt.executeQuery(strsql);
	    		kmdata=commonDAO.convertToJSON(rs1);
	    		stmt.close();
	    	}
	    	catch(Exception e){
	    		e.printStackTrace();
		...
		public ClsRentalAgreementBean getFuelPrint(HttpServletRequest request,
				int docNo) throws SQLException {
			// TODO Auto-generated method stub
			ClsRentalAgreementBean bean=new ClsRentalAgreementBean();
			Connection conn=null;
			...
			//	System.out.println("str---"+str);
				...
				bean.setLbltotalusedkm("0");
				bean.setLbltotalexcesskmrate("0");
				bean.setLbltotalfuelused("0");
				bean.setLbltotalfuelamt("0");
				...
				if(bean.getLblclstatus().equalsIgnoreCase("1")){
		    		...
		    		String sqlconfig="select if((select method from gl_config where field_nme='prate')=1,"
		    				...
		    				+ " if((select method from gl_config where field_nme='drate')=1,"
		    				+ "(select value from gl_config where field_nme='drate'),0.00)drate";
		    		ResultSet rs2=stmt.executeQuery(sqlconfig);
		    		while(rs2.next()){
		    		...
		    		String strsql="select coalesce(round(sum(mov.fin-mov.fout),2),0.00) fuelsum,if(format(coalesce(round(sum(tcap*(mov.fin-mov.fout)*(if( veh.fueltype='P','"+prate+"','"+drate+"')))*-1,2),0.00),2)<0,0.00,format(coalesce(round(sum(tcap*(mov.fin-mov.fout)*(if( veh.fueltype='P','"+prate+"','"+drate+"')))*-1,2),0.00),2)) amountsum ,if( veh.fueltype='P','"+prate+"','"+drate+"') fuelrate from "
		    				+ "	gl_vmove mov left join gl_vehmaster veh on mov.fleet_no=veh.fleet_no where mov.rdocno="+docNo+" and mov.rdtype='RAG' and mov.status='IN'"+sqltest;
		    //		System.out.println("fuelrate=====>"+strsql);
		    		ResultSet rsclose=stmt.executeQuery(strsql);
		    		
					while(rsclose.next()){
						bean.setLbltotalfuelused(rsclose.getString("fuelsum"));
						bean.setLbltotalfuelamt(rsclose.getString("amountsum"));

					}
					
				}
				fuelarray=getPrintFuelArray(docNo,stmt);
				request.setAttribute("FUELPRINT", fuelarray);
			}
			catch(Exception e){
				e.printStackTrace();
			}
		...
		private ArrayList<String> getPrintFuelArray(int docNo, Statement stmt) {
			// TODO Auto-generated method stub
			ArrayList<String> fuelarray=new ArrayList<>();
			try{
				int delcal=0;
	    		...
	    		String sqlconfig="select if((select method from gl_config where field_nme='prate')=1,"
	    				...
	    				+ " if((select method from gl_config where field_nme='drate')=1,"
	    				+ "(select value from gl_config where field_nme='drate'),0.00)drate";
	    		ResultSet rs1=stmt.executeQuery(sqlconfig);
	    		while(rs1.next()){
	    			prate=rs1.getDouble("prate");
					drate=rs1.getDouble("drate");
					}
	    		
	    		
	    		String strsql="select mov.fleet_no,veh.reg_no,veh.flname,coalesce(round(mov.fout,2),0.00) fout,coalesce(date_format(mov.dout,'%d/%m/%Y'),'') dout,mov.tout,"+
	    		" coalesce(round(mov.fin,2),0.00) fin,coalesce(date_format(mov.din,'%d/%m/%Y'),'') din,coalesce(mov.tin,'') tin,coalesce(round(mov.fin,2)-round(mov.fout,2),0.00) fueldiff,tcap,"
	    		+ " format(coalesce(round(tcap*(mov.fin-mov.fout)*(if( veh.fueltype='P','"+prate+"','"+drate+"'))*-1,2),0.00),2) fuelamount from gl_vmove mov left join " +
	    		" gl_vehmaster veh on mov.fleet_no=veh.fleet_no where mov.rdocno="+docNo+" and mov.rdtype='RAG'"+sqltest;
	    		//System.out.println("printarray===>"+strsql);
	    		ResultSet rs=stmt.executeQuery(strsql);
	    		while(rs.next()){
	    			fuelarray.add(rs.getString("fleet_no")+" :: "+rs.getString("reg_no")+" :: "+rs.getString("flname")+" :: "+rs.getString("dout")+" :: "+rs.getString("tout")+" :: "+rs.getString("fout")+" :: "+rs.getString("din")+" :: "+rs.getString("tin")+" :: "+rs.getString("fin")+" :: "+rs.getString("fueldiff")+" :: "+rs.getString("tcap")+" :: "+rs.getDouble("fuelamount") );
		    	//	System.out.println("fuelarray===>"+fuelarray);
