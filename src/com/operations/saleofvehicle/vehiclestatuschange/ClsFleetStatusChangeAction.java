...
public class ClsFleetStatusChangeAction {
	...
	public void setData(java.sql.Date sqlStartDate) {
...
//							System.out.println(val);
			...
			int Status=fleetstatusDAO.delete(sqlStartDate,getFleetstatustime(),getFleetno(),getHidcurrentstatus(),getCmbchangestatus(),getReason(),session,getMode(),getDocno(),getFormdetailcode());
			
			if(Status>0){
				setData(sqlStartDate);
				setDeleted("DELETED");
				...
				setData(sqlStartDate);
				setMsg("Not Deleted");
				setExtramsg("Found other movements against vehicle");
				return "fail";
			}
			else{
				setData(sqlStartDate);
