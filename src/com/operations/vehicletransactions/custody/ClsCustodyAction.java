...
/*		getDeldriver() getDeliveryto() getDeldate() getDeltime() 
        getDelkm() getDelfuel()  getHiddeldate() getHiddeltime() getHiddelfuelval()  getDelchkval()  getDeldriverid()*/
			...
			int retu=CustodyDAO.delupdate(getDeldriverid(),getDeliveryto(),deldate,getDeltime(),getDelkm(),getDelfuel(),getDocno(),getMasterrefno(),getTxtfleetno(),session);  
			 ...
			 setHidedate(viewbean.getDate()); 
			  ...
			  java.sql.Date sqldate1=null,sqldate2=null,sqldate3=null,sqldate4=null,sqldate5=null,sqldate6=null,sqldate7=null;
			  if(getDate()!=null && !getDate().trim().equalsIgnoreCase("") && getDate().trim().equalsIgnoreCase("undefined")) {
				  sqldate1=commDAO.changeStringtoSqlDate(getDate());
			  }
			  if(getRefdate()!=null && !getRefdate().trim().equalsIgnoreCase("") && getRefdate().trim().equalsIgnoreCase("undefined")) {
				  sqldate2=commDAO.changeStringtoSqlDate(getRefdate());
			  }
			  if(getDateout()!=null && !getDateout().trim().equalsIgnoreCase("") && getDateout().trim().equalsIgnoreCase("undefined")) {
				  sqldate3=commDAO.changeStringtoSqlDate(getDateout());
			  }
			  if(getColleteddate()!=null && !getColleteddate().trim().equalsIgnoreCase("") && getColleteddate().trim().equalsIgnoreCase("undefined")) {
				  sqldate4=commDAO.changeStringtoSqlDate(getColleteddate());
			  }
			  if(getIndate()!=null && !getIndate().trim().equalsIgnoreCase("") && getIndate().trim().equalsIgnoreCase("undefined")) {
				  sqldate5=commDAO.changeStringtoSqlDate(getIndate());
			  }
			  if(getOutdate()!=null && !getOutdate().trim().equalsIgnoreCase("") && getOutdate().trim().equalsIgnoreCase("undefined")) {
				  sqldate1=commDAO.changeStringtoSqlDate(getOutdate());
			  }
			  if(getDate()!=null && !getDate().trim().equalsIgnoreCase("") && getDate().trim().equalsIgnoreCase("undefined")) {
				  sqldate1=commDAO.changeStringtoSqlDate(getDate());
			  }
			
			  int deletevalue=CustodyDAO.delete(getDocno(),mode,session,request);
			  System.out.println("Delete Value:"+deletevalue);
			  if(deletevalue>0) {
				  setFullData(getDocno(), sqldate1, sqldate2, sqldate3, sqldate4, sqldate5, sqldate6, sqldate7);
				  ...
				  setMsg("Not Deleted");
				  if(deletevalue==-1) {
					  setExtramsg("Other Movements Found Against Document");
				  }
				  else if(deletevalue==-2) {
					  setExtramsg("Please choose latest custody");
				  }
	...
	public void setFullData(int docno,java.sql.Date masterDate,java.sql.Date srenralrefdate,
			java.sql.Date rentdateout,java.sql.Date sqlcoldate,java.sql.Date sqlindate,java.sql.Date branchoutdate,java.sql.Date deldate) {
		setDocno(docno);
		if(masterDate!=null) {
			setHidedate(masterDate.toString()); 
		}
		if(srenralrefdate!=null) {
			setHiderefdate(srenralrefdate.toString()) ; 
		}
		if(rentdateout!=null) {
			setHideroutdate(rentdateout.toString()) ; 
