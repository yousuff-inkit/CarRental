...
private String drname,drvisano,drpassno,drpassexp,drdlno,drlexp,drdob;
...
public String getDrvisano() {
    return drvisano;
}
public void setDrvisano(String drvisano) {
    this.drvisano = drvisano;
}
public String getDrpassno() {
    return drpassno;
}
public void setDrpassno(String drpassno) {
    this.drpassno = drpassno;
}
public String getDrpassexp() {
    return drpassexp;
}
public void setDrpassexp(String drpassexp) {
    this.drpassexp = drpassexp;
}
public String getDrdlno() {
    return drdlno;
}
public void setDrdlno(String drdlno) {
    this.drdlno = drdlno;
}
public String getDrlexp() {
    return drlexp;
}
public void setDrlexp(String drlexp) {
    this.drlexp = drlexp;
}
public String getDrdob() {
    return drdob;
}
public void setDrdob(String drdob) {
    this.drdob = drdob;
}

//vehoutdetails

    private String vehcolor,vehmodel;
...
//		System.out.println("Date:"+date+"Date Out:"+dateout+"InCollectDate"+incollectdate);
					int val=replacementDAO.insert(date,getCmbrentaltype(),getRefno(),getTxtfleetno(),dateout,getTimeout(),getOutkm(),getCmbfuel(),getCmbtrreason(),getCmbreplacetype(),getMode(),
							...
							getFormdetailcode(),deliveryoutdate,getDeliveryouttime(),getDeliveryoutkm(),getCmbdeliveryoutfuel(),getInfleettrancode(),getHidcollectdriver(),getDescription(),getIndesc(),getReferenceno());
					if(val>0.0){
...
//						System.out.println("Else Val"+val);
		...
		int value=replacementDAO.update(getDocno(),getHidchkdelivery(),getHidcollectdriver(),oncollectdate,getOncollecttime(),getOncollectkm(),getCmboncollectfuel(),
				...
				dateout,refdate,getInfleettrancode(),getCmbrentaltype(),getIndesc(),getReferenceno());
			if(value>0){
				setIndesc(getIndesc());
				setReferenceno(getReferenceno());
				setHidchkcollection(getHidchkcollection());
...
//		System.out.println("view===="+getTxtfleetno());
		//ClsVehicleBean bean = new ClsVehicleBean(); 
				...
				setReferenceno(bean.getReferenceno());
...
public String printAction() throws ParseException, SQLException{
	 ...
	 setReferenceno(printbean.getReferenceno());
   ...
   //   System.out.println("imgg==="+printbean.getLblinimg());
	...
	// driver details
	      setDrname(printbean.getDrname());
	      setDrvisano(printbean.getDrvisano());
	      setDrpassno(printbean.getDrpassno());
	      setDrpassexp(printbean.getDrpassexp());
	      setDrdlno(printbean.getDrdlno());
	      setDrlexp(printbean.getDrlexp());
	      setDrdob(printbean.getDrdob());
	      
	      //vehoutdetails
	      setVehcolor(printbean.getVehcolor());
	      setVehmodel(printbean.getVehmodel());
