...
<%	
	...
	try{
		...
		/* String sqls = "select DATE_FORMAT(min(date),'%d-%m-%Y') deprdates from (select v.depr_date date from gl_vehmaster v left join gc_assettran g on " 
		        ...
		        String sqls = "select DATE_FORMAT(min(date),'%d-%m-%Y') deprdates from (select v.depr_date date,if(l.expirydate is not null, l.expirydate,if(l.per_name=1,DATE_ADD(l.outdate,INTERVAL l.per_value YEAR),DATE_ADD(l.outdate,INTERVAL l.per_value MONTH))) leasetodate from gl_vehmaster v " 
 ...
 */
			 ...
			 + "min(v.depr_date))) dateDiff,DAY(LAST_DAY('"+sqlDate+"')) noofdays,leasetodate from gl_vehmaster v inner join (select if(l.expirydate is not null, l.expirydate,if(l.per_name=1,DATE_ADD(l.outdate,INTERVAL l.per_value YEAR),DATE_ADD(l.outdate,INTERVAL l.per_value MONTH))) leasetodate,"
			+" perfleet from gl_lagmt l where  l.status=3 and l.clstatus=0 and l.brhid='"+branch+"' UNION ALL select s.todate  date,s.fleet_no  from gl_stockvehicles s  left join gl_vehmaster v on v.fleet_no=s.fleet_no where s.status=3 and v.stockrelease=1 and s.vehreturnstatus=0) l on v.fleet_no=l.perfleet where "
