...
ClsCommon ClsCommon=new ClsCommon();
  ...
  String blexid = request.getParameter("blexid");
...
int blexrstatus=4;
	...
	for(int k=0;k<psrnoarray.size();k++)
	{
		...
		String rate = ""+(pmgntarr[0].trim().equalsIgnoreCase("undefined") || pmgntarr[0].trim().equalsIgnoreCase("NaN")|| pmgntarr[0].trim().equalsIgnoreCase("")|| pmgntarr[0].isEmpty()?0:pmgntarr[0].trim())+"";
		  ...
		  String babyseater = ""+(pmgntarr[6].trim().equalsIgnoreCase("undefined") || pmgntarr[6].trim().equalsIgnoreCase("NaN")|| pmgntarr[6].trim().equalsIgnoreCase("")|| pmgntarr[6].isEmpty()?0:pmgntarr[6].trim())+"";
		  ...
		  String exkmrte = ""+(pmgntarr[9].trim().equalsIgnoreCase("undefined") || pmgntarr[9].trim().equalsIgnoreCase("NaN")|| pmgntarr[9].trim().equalsIgnoreCase("")|| pmgntarr[9].isEmpty()?0:pmgntarr[9].trim())+"";
		  
		  String oinschg = ""+(pmgntarr[10].trim().equalsIgnoreCase("undefined") || pmgntarr[10].trim().equalsIgnoreCase("NaN")|| pmgntarr[10].trim().equalsIgnoreCase("")|| pmgntarr[10].isEmpty()?0:pmgntarr[10].trim())+"";
		  ...
		  String chaufchg = ""+(pmgntarr[12].trim().equalsIgnoreCase("undefined") || pmgntarr[12].trim().equalsIgnoreCase("NaN")|| pmgntarr[12].trim().equalsIgnoreCase("")|| pmgntarr[12].isEmpty()?0:pmgntarr[12].trim())+"";
		 ...
		 String sqlappra="update  gl_ltarif set rate='"+rate+"', cdw='"+cdw+"', pai='"+pai+"', cdw1='"+cdw1+"' , pai1='"+pai1+"', gps='"+gps+"', babyseater='"+babyseater+"', cooler='"+cooler+"', kmrest='"+kmrest+"', exkmrte='"+exkmrte+"',chaufchg='"+chaufchg+"',chaufexchg='"+chaufexchg+"'  where rdocno='"+rdocno+"' ";
		System.out.println("sqlappra================"+sqlappra);
    ...
    System.out.println("--result--"+result);
     ...
     //  System.out.println("----invtodate2-"+invtodate);
	  ...
	  else if(invoicevalue.equalsIgnoreCase("2")) 
	  {
	   ...
	   else if(invoicevalue.equalsIgnoreCase("4")) 
		  {
	           String foursql="SELECT DATE_ADD('"+sqloutDate+"',INTERVAL 3 month) todate";
				    	   
				    	 //  System.out.println("--quarterlyfoursql--"+foursql);
			
				     	  ResultSet rss5=  stmt.executeQuery(foursql);
				     		 
				     		 if(rss5.next())
				     		 {
				     			 invtodate=rss5.getDate("todate"); 
				     		 }
				    	   
				    	   
		   }  else if(invoicevalue.equalsIgnoreCase("5")) 
			  {
	           String fivesql="SELECT DATE_ADD('"+sqloutDate+"',INTERVAL 6 month) todate";
				    	   
				    	   //System.out.println("--halfyearlyfivesql--"+fivesql);
			
				     	  ResultSet rss6=  stmt.executeQuery(fivesql);
				     		 
				     		 if(rss6.next())
				     		 {
				     			 invtodate=rss6.getDate("todate"); 
				     		 }
				    	   
				    	   
		   }  else if(invoicevalue.equalsIgnoreCase("6")) 
			  {
	           String sixsql="SELECT DATE_ADD('"+sqloutDate+"',INTERVAL 12 month) todate";
				    	   
				    	  // System.out.println("--yearlysixsql--"+sixsql);
			
				     	  ResultSet rss7=  stmt.executeQuery(sixsql);
				     		 
				     		 if(rss7.next())
				     		 {
				     			 invtodate=rss7.getDate("todate"); 
				     		 }
   ...
   upsql="update  gl_lagmt set invtodate='"+invtodate+"',adv_chk='"+advance+"',  inv_type='"+invoicevalue+"'   where doc_no='"+rdocno+"'  ";
		 
		  int val= stmt.executeUpdate(upsql);
		     
		 	String sqlsave="insert into gl_biblog (doc_no, brhId, dtype, edate, userId, ENTRY) values ("+rdocno+",'"+branchid+"','BLEX',now(),'"+session.getAttribute("USERID").toString()+"','A')";
		 	stmt.executeUpdate(sqlsave);  
	
			String sqlapprv="update gl_blex set approvestatus=1 where idgl_blex='"+blexid+"' and rano='"+rdocno+"' ";
...
{
	String sqlcan="update gl_blex set approvestatus=2 where idgl_blex='"+blexid+"' and rano='"+rdocno+"' ";
