...
String docno=request.getParameter("docno")==""?"":request.getParameter("docno");
String bankacno=request.getParameter("bankacno")==""?"":request.getParameter("bankacno");
String clientacno=request.getParameter("clientacno")==""?"":request.getParameter("clientacno");
String pytdate=request.getParameter("pytdate")==""?"":request.getParameter("pytdate");
String masterdocno=request.getParameter("masterdocno")==""?"0":request.getParameter("masterdocno").toString();
String pyttotalrent=request.getParameter("pyttotalrent")==""?"0":request.getParameter("pyttotalrent");
String pytadvance=request.getParameter("pytadvance")==""?"0":request.getParameter("pytadvance");
String pytbalance=request.getParameter("pytbalance")==""?"0":request.getParameter("pytbalance");
String pytstartdate=request.getParameter("pytstartdate")==""?"0":request.getParameter("pytstartdate");
String pytmonthsno=request.getParameter("pytmonthsno")==""?"0":request.getParameter("pytmonthsno");
String pytbankacno=request.getParameter("pytbankacno")=="" || request.getParameter("pytbankacno")==""?"0":request.getParameter("pytbankacno");
String brhid1=request.getParameter("brhid1")==""?"0":request.getParameter("brhid1");
String agmtvocno=request.getParameter("agmtvocno")==""?"0":request.getParameter("agmtvocno");
...
try{
	...
	if(!masterdocno.equalsIgnoreCase("0")){
		String strsql="update gl_leaseext set totalrent="+pyttotalrent+",startdt='"+sqlpytdate+"',noofmonth="+pytmonthsno+",bankacno="+pytbankacno+" where srno="+masterdocno;
	...
	session.setAttribute("BRANCHID",brhid1);
	...
	for(int i=0;i<mainarray.size();i++){
		...
		ucrarray.add(bankacno+"::"+curid+"::"+currate+"::false::"+amount+"::Cheque for LAG "+agmtvocno+"--"+docno+"::"+baseamt+"::0::0::0");
		//ucrarray.add("undefined"+"::"+curid+"::"+currate+"::false::"+amount+"::Cheque for LAG "+agmtvocno+"--"+docno+"::"+baseamt+"::0::0::0");
		ucrarray.add(clientacno+"::"+curid+"::"+currate+"::true::"+amount*-1+"::Cheque for LAG "+agmtvocno+"--"+docno+"::"+baseamt*-1+"::0::0::0");
		int val=ucrdao.insert(master_date, "UCR", "LAG - "+agmtvocno+"--"+docno, currate, sqldate, chequeno,clientname, 0,"Cheque for LAG "+agmtvocno+"--"+docno, amount,
		...
		if(val<=0){
			...
			String strupdate="update gl_leasepytcalcext set bpvno="+val+",chequeno='"+chequeno+"' where srno="+detaildocno;
	...
	if(status==1){
		String lagupdate="update gl_leaseext set bankacno="+bankacno+" where srno="+docno;
