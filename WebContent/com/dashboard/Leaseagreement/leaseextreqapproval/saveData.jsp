...
try{
	...
	String reqamt=request.getParameter("reqamt")==""?"0":request.getParameter("reqamt").trim().replace(",","").trim(); 
	String exttilldate=request.getParameter("exttilldate")==""?"":request.getParameter("exttilldate").trim();
	String newresval=request.getParameter("newresval")==""?"0":request.getParameter("newresval").trim().replace(",","").trim(); 
				...
				// System.out.println("Request=="+sql);
				 ...
				 String dates="",bpvno="",chequeno="",per_fleet="",rsdval="";
				  String lano="",outdate="";
					...
					ResultSet rsmaxdocno=stmt.executeQuery("select d.date,amount,bpvno,chequeno,l.perfleet,m.residualval,l.outdate from gl_leasepytcalcext d left join gl_leaseext m on m.srno=d.rdocno left join gl_lagmt l on l.doc_no=m.lano where rdocno='"+extdocno+"'");
					while (rsmaxdocno.next()){
						...
						outdate=rsmaxdocno.getString("outdate");
					...
					String newfromdt="";
					String strsel="select if(l.extno>=1,date_add(todt,INTERVAL 1 DAY),date_add(date_add(outdate, interval (case when per_name=1 then per_value*12 when per_name=2 then per_value else 0 end) month),INTERVAL 1 DAY)) as newfromdt from gl_leaseext e left join gl_lagmt l on l.doc_no=e.lano where  lano='"+docno+"'and e.extno=(if(l.extno>=1,l.extno,l.extno+1))";
					 System.out.println("leasetodt=="+strsel);
						...
						String strinsertagmt="update gl_lagmt set extno='"+extnno+"',expirydate='"+tosqlDate+"' where doc_no='"+docno+"' ";
