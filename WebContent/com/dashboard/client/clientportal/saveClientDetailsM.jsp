...
<%
String cldocnolist=request.getParameter("cldocnolist")==null?"":request.getParameter("cldocnolist").toString();
...
System.out.println(cldocnolist+"::"+clientusername+"::"+clientpassword+"::"+chksendmail+"::"+branchid);
System.out.println(cldocnolist+"cldocnolist");
String cldocno[]=cldocnolist.split("::");
int errorstatus=0;
...
try{
	...
	String strupdate="update my_acbook ac set clientusername='"+clientusername+"',clientpassword='"+clientpassword+"' where cldocno="+cldocno[i]+" and dtype='CRM' and status=3";		
	System.out.println(strupdate);
	int update=stmt.executeUpdate(strupdate);
	if(update<=0){
		errorstatus=1;
	}
	String strlog="insert into gl_biblog (doc_no, brhId, dtype, edate, userId, ENTRY) values ("+cldocno[i]+",'"+branchid+"','BICP',now(),'"+session.getAttribute("USERID").toString()+"','A')";
	...
	if(chksendmail.equalsIgnoreCase("1")){
		...
		String strSql2 = "select mail1 from my_acbook where cldocno="+cldocno[i]+" and dtype='CRM' and status=3";
		//System.out.println("sql=="+strSql2);
		...
		" <tr><td>Yours Sincerely<br>Team EuroStar<br></td></tr>') msg from my_acbook a left join my_brch br on a.brhid=br.doc_no left join my_comp comp on br.cmpid=comp.doc_no where a.dtype='CRM' and a.cldocno="+cldocno[i];
		...
		if(!email.trim().equalsIgnoreCase("")) {
			...
			*/ 
			try {
				String mailstatus=sendmail.SendTomailINT( saveFile,"CRM",email,cldocno[i]+"",session.getAttribute("BRANCHID").toString().trim(),session.getAttribute("USERID").toString().trim(),invno,sub,msg); 
