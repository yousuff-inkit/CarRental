<%@ page import="com.dashboard.rentalagreement.ClsrentalagreementDAO" %>
<% String contextPath=request.getContextPath();%>
 <%

   String barchval = request.getParameter("barchval")==null?"NA":request.getParameter("barchval");
	String fromdate = request.getParameter("fromdate")==null?"0":request.getParameter("fromdate").trim();
  	String todate = request.getParameter("todate")==null?"0":request.getParameter("todate").trim();
  	String cldocno = request.getParameter("cldocno")==null?"NA":request.getParameter("cldocno").trim();

  	
  	String status = request.getParameter("status")==null?"NA":request.getParameter("status").trim();

  	String fleet = request.getParameter("fleet")==null?"NA":request.getParameter("fleet").trim();

  	String group = request.getParameter("group")==null?"NA":request.getParameter("group").trim();
  	String brand = request.getParameter("brand")==null?"NA":request.getParameter("brand").trim();
  	String model = request.getParameter("model")==null?"NA":request.getParameter("model").trim();

  	String type = request.getParameter("type")==null?"NA":request.getParameter("type").trim();
  	String outchk = request.getParameter("outchk")==null?"NA":request.getParameter("outchk").trim();
  	String inchk = request.getParameter("inchk")==null?"NA":request.getParameter("inchk").trim();
  	
 	String catid = request.getParameter("catid")==null?"NA":request.getParameter("catid").trim(); 
 	String salesman = request.getParameter("salesman")==null?"NA":request.getParameter("salesman").trim();
	ClsrentalagreementDAO crad=new ClsrentalagreementDAO();
  	
 %> 
 <script type="text/javascript">
 
 var temp4='<%=barchval%>';
 var datasssss=[];
 var dataildata;
 var aa;
  if(temp4!='NA')
 { 

	<%--  dataildata='<%=crad.detailsgrids(barchval,fromdate,todate,cldocno,status,fleet,group,brand,model,type,outchk,inchk,catid,salesman)%>';
	 --%>   
	  datasssss='<%=crad.detailsgrid(barchval,fromdate,todate,cldocno,status,fleet,group,brand,model,type,outchk,inchk,catid,salesman)%>';
 aa=0;
 }
  
 
  else
	  {
	  aa=1;
	  }
         
        $(document).ready(function () { 
         
        	
             var num = 0; 
            var source = 
            {
                datatype: "json",
                datafields: [

                             
                 			{name : 'doc_no', type: 'String'  },   //this is the agreement
     						{name : 'refname', type: 'String'},
     					    {name : 'fleet_no', type: 'String'}, 
     						{name : 'vehdetails', type: 'String'}, 
     						{name : 'vehiclenum',type:'string'},
 	                        {name : 'cldocno', type: 'String'  },
     						{name : 'odate', type: 'date'  },
     						{name : 'otime', type: 'String'  },
     						{name : 'ddate', type: 'date'  },
     						{name : 'dtime', type: 'String'  },
     						{name : 'deldate', type: 'date'  },
     						{name : 'deltime', type: 'String'  },
     						{name : 'coldate', type: 'date'  },
     						{name : 'coltime', type: 'String'  },
     						{name : 'rentaltype', type: 'String'  },
     						{name : 'per_mob', type: 'String'  },
     						{name : 'contactperson', type: 'String'  },
     						{name : 'brhid', type: 'string'  },
     						{name : 'reg_no', type: 'string'  },
     						{name : 'drname', type: 'String'  },
     						{name : 'mrno', type: 'String'  },
     						{name : 'rent', type: 'number'  },
     						{name : 'cdw', type: 'number'  },
     						{name : 'refnos', type: 'string'  },
     						{name : 'indate', type: 'date'  },
     						{name : 'intime', type: 'String'  },
     						{name : 'invdate', type: 'date'  },
     						{name : 'invrental', type: 'number'  },
     						{name : 'invcdw', type: 'number'  },
     						{name : 'sal_name', type: 'string'  },
     						{name : 'ofleet_no', type: 'string'  },
     						{name : 'oreg_no', type: 'string'  },
     						{name : 'user_name', type: 'string'  },
     						{name : 'closeuser', type: 'string'  },
     						{name : 'outlocation', type: 'string'  },
     						{name : 'attach', type: 'string'  },
     						{name : 'docno', type: 'string'  },
     						{name : 'project_name',type:'string'},
							{name : 'used_days', type: 'number'  },
							{name : 'tdocno', type: 'number'  },
							{name : 'acno',type:'string'},
							{name : 'branchname',type:'string'},
							{name : 'agmtstatus',type:'string'},
							{name : 'insurcompconfig',type:'number'},
							{name : 'insurcompname',type:'string'},
							{name : 'insurcompdays',type:'number'},
							{name : 'racrate',type:'number'},
							{name : 'enqtype',type:'string'},
							{name : 'slno', type: 'number'    },
							  
							],
                          	localdata: datasssss,
                          	       
          
				
                
                pager: function (pagenum, pagesize, oldpagenum) {
                   
                }
            };
            
            $("#detailsgrid").on("bindingcomplete", function (event) {
            	// your code here.
            	var insurcompconfig=$("#detailsgrid").jqxGrid('getcellvalue',0,'insurcompconfig');
            	if(insurcompconfig=="1"){
            		$("#detailsgrid").jqxGrid('setcolumnproperty','insurcompname','hidden',false);
            		$("#detailsgrid").jqxGrid('setcolumnproperty','insurcompdays','hidden',false);
            		$("#detailsgrid").jqxGrid('setcolumnproperty','racrate','hidden',false);
            	}
            });                       
                

            var dataAdapter = new $.jqx.dataAdapter(source,
            		 {
                		loadError: function (xhr, status, error) {
	                    alert(error);    
	                    }
		            }
            );
            $("#detailsgrid").jqxGrid(
            { 
            	
            	
            	width: '100%',
                height: 500,
                source: dataAdapter,
                showaggregates:true,
                enableAnimations: true,
                filtermode:'excel',
                filterable: true,
                showfilterrow: true,
                sortable:true,
                selectionmode: 'singlerow',
                pagermode: 'default',
                editable:false,
                
     					
			         columns: [
                            { text: 'Sr.No', datafield: 'slno', width: '3%' }, 
							{ text: 'Branch', datafield: 'branchname', width: '8%' },   //voc no
							{ text: 'RA No', datafield: 'doc_no', width: '6%' },   //voc no
							
							{ text: 'Fleet', datafield: 'fleet_no', width: '4%' },
							{ text: 'Fleet Name', datafield: 'vehdetails', width: '14%' },
							{ text: 'Reg NO', datafield: 'reg_no', width: '5%' },
							{ text: 'Vehicle Number', datafield: 'vehiclenum', width: '8%'},
							{ text: 'Contract Fleet', datafield: 'ofleet_no', width: '7%' },
							{ text: 'Contract Reg No', datafield: 'oreg_no', width: '8%' },                                          
							{ text: 'Client Id', datafield: 'cldocno', width: '5%' },
							{ text: 'Client Name', datafield: 'refname', width: '18%' },
							{ text: 'Acc No', datafield: 'acno', width: '5%' }, 
							{ text: 'Agmt Status', datafield: 'agmtstatus', width: '6%' },
							{ text: 'Contact Person', datafield: 'contactperson', width: '8%'},
							{ text: 'Driver', datafield: 'drname', width: '10%'},
							{ text: 'Mob NO', datafield: 'per_mob', width: '8%'},
							{ text: 'Tariff Docno', datafield: 'tdocno', width: '8%'},
						 	{ text: 'Rental Type', datafield: 'rentaltype', width: '6%' },
							{ text: 'Out Date', datafield: 'odate', width: '6%',cellsformat:'dd.MM.yyyy'},
							{ text: 'Out Time', datafield: 'otime', width: '5%' }, 
							{ text: 'Out Location', datafield: 'outlocation', width: '10%' }, 
							{ text: 'Due Date', datafield: 'ddate', width: '6%',cellsformat:'dd.MM.yyyy'},
							{ text: 'DueTime', datafield: 'dtime', width: '5%' },
							{ text: 'Del. Date', datafield: 'deldate', width: '6%',cellsformat:'dd.MM.yyyy'},
							{ text: 'Del. Time', datafield: 'deltime', width: '5%' },
							{ text: 'Ref No', datafield: 'refnos', width: '5%'},
						    { text: 'Manual RA', datafield: 'mrno', width: '5%'},
							{ text: 'In Date', datafield: 'indate', width: '6%',cellsformat:'dd.MM.yyyy'},
							{ text: 'In Time', datafield: 'intime', width: '5%' },
							{ text: 'Col Date', datafield: 'coldate', width: '6%',cellsformat:'dd.MM.yyyy'},
							{ text: 'Col Time', datafield: 'coltime', width: '5%' },
							{ text: 'Rent', datafield: 'rent', width: '4%',cellsformat: 'd2', cellsalign: 'right', align:'right'},
							{ text: 'CDW', datafield: 'cdw', width: '4%',cellsformat: 'd2', cellsalign: 'right', align:'right'},
							{ text: 'Inv Date', datafield: 'invdate', width: '6%',cellsformat:'dd.MM.yyyy'},
							{ text: 'Inv Rent', datafield: 'invrental', width: '5%',cellsformat: 'd2', cellsalign: 'right', align:'right'},
							{ text: 'Inv CDW', datafield: 'invcdw', width: '5%',cellsformat: 'd2', cellsalign: 'right', align:'right'},
							{ text: 'Used Days', datafield: 'used_days', width: '6%',cellsformat: 'd2', cellsalign: 'right', align:'right'},
							{ text: 'Open User', datafield: 'user_name', width: '15%'},
							{ text: 'Close User', datafield: 'closeuser', width: '12%'},
							{ text: 'Salesman', datafield: 'sal_name', width: '15%'},
							{ text: 'Project', datafield: 'project_name', width: '10%'},
							{ text: 'branchid', datafield: 'brhid', width: '10%',hidden:true },
							{ text: 'docno', datafield: 'docno', width: '10%',hidden:true },
							{ text: 'Attachment',columntype:'button', datafield: 'attach', width: '8%'},
							{ text: 'Insur Comp Config', datafield: 'insurcompconfig', width: '8%',hidden:true},
							{ text: 'Insur.Company', datafield: 'insurcompname', width: '12%',hidden:true},
							{ text: 'Insur.Comp Days', datafield: 'insurcompdays', width: '8%',hidden:true},
							{ text: 'RAC Rate', datafield: 'racrate', width: '8%',hidden:true,cellsformat:'d2',align:'right',cellsalign:'right'},
							{ text: 'Enq.Type', datafield: 'enqtype', width: '10%'},
								
							// regno contract,fleetno contract
					
					]
            });
            $("#overlay, #PleaseWait").hide();
            $('#detailsgrid').on('cellclick', function (event) 
            		{ 
            	 	
            		var rowindex1=event.args.rowindex;
            			 if(event.args.datafield=="attach"){
            				var docno = $('#detailsgrid').jqxGrid('getcellvalue', rowindex1, "doc_no");
            				var brchid = $('#detailsgrid').jqxGrid('getcellvalue', rowindex1, "brhid");
            				
            				var frmdet="RAG";
		             		if (docno!="") {
		   				
		   				   var  myWindow= window.open("<%=contextPath%>/com/common/Attachmaster.jsp?formCode="+frmdet+"&docno="+docno+"&brchid="+brchid,"_blank","top=180,left=310,Width=800,Height=430,location=no,scrollbars=no,toolbar=no,resizable=no,meanubar=no,titlebar=no");
		 							  myWindow.focus();
		 				
		 				} else {
		 					$.messager.alert('Message','Select a Document....!','warning');
		 					return;
		 				    }
            			}
         		});
       
        });
        
        
				       
                       
    </script>
    <div id="detailsgrid"></div>