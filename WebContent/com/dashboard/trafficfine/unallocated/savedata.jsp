...
//System.out.println("sql7"+branchid);
...
ClsCommon ClsCommon=new ClsCommon();
	...
	if(hidchkdate.equalsIgnoreCase("1")) {
  		sqltest=sqltest+" and traffic_date>='"+fromsqldate+"' ";
  		sqltest1=sqltest1+" and t.traffic_date>='"+fromsqldate+"' ";
  	}
	...
	ArrayList<String> rowValues= new ArrayList<String>();
	 ...
	 try{
		 ...
		 conn = ClsConnection.getMyConnection();
			...
			+" where t.traffic_date<='"+uptosqldate+"' "+sqltest1+"  and t.isallocated=0 and trim(t.PCOLOR)<>''  and t.status in (0,3) "+sqlexcludefleets;
				    		 ...
				    		 String fleet_no=rs1.getString("fleet_no");
						    	 ...
						    	 if(rs1.getString("rdtype").equalsIgnoreCase("RAG") || rs1.getString("rdtype").equalsIgnoreCase("MOV") || rs1.getString("rdtype").equalsIgnoreCase("LAG")  || rs1.getString("rdtype").equalsIgnoreCase("VCU") )
						    	 {
						           ...
						           //	 System.out.println("-----ragsql--"+ragsql);  
						    		...
						    		String sqlup2="update GL_TRAFFIC set isallocated=1,reason='Allocated',ra_no='"+rdocno+"',rtype='"+trancode+"',emp_id='"+empid+"',emp_type='"+emptype+"',Fleetno='"+fleet_no+"' where  traffic_date<='"+uptosqldate+"' "+sqltest+"  and  TICKET_NO='"+subarray[1].trim()+"'";
						           //	 System.out.println("-----3--"+sqlup2);  
						    		stmt2.executeUpdate(sqlup2);
						    		 stmt2.close();
						    		 
						    		 String sqlup5="insert into gl_biblog (doc_no, brhId, dtype, edate, userId, ENTRY) values (0,'"+branchid+"','BSLKA',now(),'"+session.getAttribute("USERID").toString()+"','A')";
