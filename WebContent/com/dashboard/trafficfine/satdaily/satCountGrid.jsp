...
<script type="text/javascript">
	 ...
	 var bb='<%=satcateg%>';
	if(bb!='0'){
		satdata= '<%=DAO.satDailyReportCountGridLoading(fromdate,todate,satcateg,datefilter,check,userName,saliksource)%>';     
	}
    ...
    $('#jqxSatCount').on('rowdoubleclick', function (event) { 
  			...
  			if (document.getElementById('rdticketdate').checked) {
  			 ...
  			 if (document.getElementById('rdsalik').checked) {  
  				 ...
  				 $("#satSalikDiv").load("satSalikGrid.jsp?fromdate="+fromdate+"&todate="+todate+"&satcateg="+satcateg+"&datefilter="+datefilter+"&check="+check+"&username="+userName);
  			} else if (document.getElementById('rdtraffic').checked) {
  				satcateg="traffic";
  				 $("#overlay, #PleaseWait").show();
  				 $("#satTrafficDiv").load("satTrafficGrid.jsp?fromdate="+fromdate+"&todate="+todate+"&satcateg="+satcateg+"&datefilter="+datefilter+"&check="+check+"&username="+userName);
  			}
