...
ClsCommon ClsCommon=new ClsCommon();
	...
	try{
		...
		ArrayList<String> trafficarray= new ArrayList<String>();
			...
			trafficarray.add(expenseaccount+":: Ticket No: "+ticketno+" Fine Date & Time: "+tdate+" & "+ttime+" of Reg No: "+regno+"-"+platecode+" is Passed as Journal Voucher on "+sqlDate+":: "+currencyexp+"::"+rateexp+":: "+(amount+(value*amountcount))+":: "+(amount+(value*amountcount))*rateexp+":: "+srno+":: 1::0::0"); //Employee Account
			trafficarray.add(trafficaccount+":: Ticket No: "+ticketno+" Fine Date & Time: "+tdate+" & "+ttime+" of Reg No: "+regno+"-"+platecode+" is Passed as Journal Voucher on "+sqlDate+":: "+currency+":: "+rate+":: "+amount*-1+":: "+(amount*rate)*-1+"::"+srno+":: -1:: 0:: 0"); //Traffic Account
		
		if(!(value==0)){
			trafficarray.add(trafficsrvemp+":: Ticket No: "+ticketno+" Fine Date & Time: "+tdate+" & "+ttime+" of Reg No: "+regno+"-"+platecode+" is Passed as Journal Voucher on "+sqlDate+":: "+currencysrvemp+":: "+ratesrvemp+":: "+(value*amountcount)*-1+":: "+(value*amountcount*ratesrvemp)*-1+"::"+srno+":: -1:: 0:: 0"); //Traffic Service Employee Account
		}
//	System.out.println("==== "+trafficarray);
		ClsJournalVouchersDAO jvt = new ClsJournalVouchersDAO();
		
		docno=jvt.insert(sqlDate, formdetailcode.concat("-6"), status, "Ticket No: "+ticketno+" Fine Date & Time: "+tdate+" & "+ttime+" of Reg No: "+regno+"-"+platecode+" is Passed as Journal Voucher on "+sqlDate, (amount+(value*amountcount)), (amount+(value*amountcount)), trafficarray, session, request);
