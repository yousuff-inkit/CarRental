...
<%@page import="com.operations.clientrelations.client.ClsClientAction" %>
<%	
		 ...
		 if((bibpid.equalsIgnoreCase("1")))
		{
		 ...
		 if((bibpid.equalsIgnoreCase("4")))
			 ...
			 ResultSet rsstk = stmt.executeQuery(sqls);
				...
				String sqlpd="select pd.cperson,pd.mob,pd.email,pd.tel,pd.actvty_id,pd.extn from cm_prosclientd pd where pd.rtrno="+trdocno+"";
				 ResultSet rspd = stmt.executeQuery(sqlpd);
					...
					ClsClientAction ClsClientAction=new ClsClientAction();
			int dat=clientDAO.insert(sqlDate,"CRM",name,"",session.getAttribute("CURRENCYID").toString(),"1", catid,
					...
					"","","",sqlDate,"","",sqlDate,"","",blankarray,blankarray,blankarray,blankarray,blankarray,session,request,"0",ClsClientAction);
