...
$(document).ready(function () {
    ...
    $("#unRentableGrid").jqxGrid({
        ...
        columns: [
            ...
            // FIX 3: Replaced column type button with an explicit HTML button using cellsrenderer and onmousedown
            { text: 'Action', datafield: 'btnsave', width: '6%', editable: false,
                cellsrenderer: function (row, column, value) {
                    return '<div style="margin-top: 5px; text-align: center;"><button type="button" onmousedown="forceSaveGridRow(' + row + ')" style="cursor: pointer; padding: 3px 8px; border-radius: 4px; background: #2563eb; color: #fff; border: none;">Save</button></div>';
                }
            },
            ...
            { text: 'Plate Code', datafield: 'platecode', width: '6%',cellclassname: cellclassname  ,editable:false},
            { text: 'Vehicle No', datafield: 'vehiclenum', width: '6%',cellclassname: cellclassname    ,editable:false},
    ...
    // (Old cellclick listener with dataField=="btnsave" removed here to avoid double-firing bugs)
});

// FIX 3: Intercept the click on mousedown so it commits the edit immediately instead of waiting for blur
function forceSaveGridRow(rowBoundIndex) {
    // 1. Force the grid to end editing right now (commits what the user just typed)
    $("#unRentableGrid").jqxGrid('endcelledit', rowBoundIndex, "remarks", false);

    // 2. Use a tiny timeout to let the grid register the new value into its data array
    setTimeout(function() {
