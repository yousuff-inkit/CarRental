...
<%
	...
	try{
		...
		String agmtvocno="";
		String strgetagmtvocno="";
		if(ratype.equalsIgnoreCase("RAG")){
			strgetagmtvocno="select voc_no from gl_ragmt where doc_no="+rano;
		}
		else if(ratype.equalsIgnoreCase("LAG")){
			strgetagmtvocno="select voc_no from gl_lagmt where doc_no="+rano;
		}
		if(!strgetagmtvocno.equalsIgnoreCase("")){
			ResultSet rsgetagmtvocno=stmt.executeQuery(strgetagmtvocno);
			while(rsgetagmtvocno.next()){
				agmtvocno=rsgetagmtvocno.getString("voc_no");
			}
		}
		...
		if(process.equalsIgnoreCase("REFUNDED")){
		   ...
		   if(remarks.equalsIgnoreCase("")){
			   remarks="Refund of "+ratype+" #"+agmtvocno+" on "+refunddate+"";   
		   }
		   else{
			   remarks="Refund of "+ratype+" #"+agmtvocno+" on "+refunddate+" - "+remarks;
		   }
			
			docno=rrp.insert(sqlDate, paytype, Integer.parseInt(typeaccount), cardtype, chequeno, sqlChequeDate, remarks, Integer.parseInt(chckibbranch), ibbranch, Integer.parseInt(clientdocno), Integer.parseInt(clientaccount), ratype, rano, "1", securityacno, securityamount, deductionamount, additionalamount, netamount, onaccountamount, remarks, paidto, applysecurityarray, session, request);
				...
				//docno=rrp.insert(sqlDate, paytype, Integer.parseInt(typeaccount), cardtype, chequeno, sqlChequeDate, remarks, Integer.parseInt(chckibbranch), ibbranch, Integer.parseInt(clientdocno), Integer.parseInt(clientaccount), ratype, rano, "1", securityacno, securityamount, deductionamount, additionalamount, netamount, onaccountamount, process+" ON "+sqlDate, paidto, applysecurityarray, session, request);
				  ...
				  if(remarks.equalsIgnoreCase("")){
					   remarks="Release of "+ratype+" #"+agmtvocno+" on "+refunddate+"";   
				   }
				   else{
					   remarks="Release of "+ratype+" #"+agmtvocno+" on "+refunddate+" - "+remarks;
				   }
				  journalvouchersarray.add(securityacno+"::"+remarks+":: "+curid+"::"+rate+":: "+securityamount+":: "+(securityamount)*rate+":: "+mainbranch+":: 1::0::0"); //Security Account
				  ...
				  journalvouchersarray.add(clientsaccount+"::"+remarks+":: "+curid+"::"+rate+":: "+(securityamount)*-1+":: "+((securityamount)*rate)*-1+":: "+mainbranch+":: -1::0::0"); //Client Account
				  
				  docno=jvt.insert(sqlDate, "JVT".concat("-11"), "BSR", remarks, securityamount, securityamount, journalvouchersarray, session, request);
