...
var Jobcloseexcel;
if(id=="1"){	
    Jobcloseexcel='<%=closedao.exceljobcloseData(fromdate, todate, bookdocno, clientid,branch, id)%>';
}
$(document).ready(function () { 
		var urljobgrid = 'getgriddata.jsp?clientid=<%=clientid%>&branch=<%=branch%>&fromdate=<%=fromdate%>&todate=<%=todate%>&bookdocno=<%=bookdocno%>&ispickupbased=<%=ispickupbased%>&id=<%=id%>';
        var source =
           {
           datatype: "json",
           datafields: [
       ...
       /*      alert("here");
                ...
                columns: [
							...
							{ text: 'Status',datafield:'bstatus',width:'8%'},
							{ text: 'Ref No',datafield:'refno',width:'8%'},
							{ text: 'Booking No.',datafield:'bookingno',width:'8%'},
							{ text: 'Trip Type',datafield:'triptype',width:'8%'},
							...
							{ text: 'Job Name',datafield:'jobnametemps',width:'6.2%'},
							{ text: 'Job Name', hidden:true,datafield:'jobnametemp',width:'6.2%',cellsrenderer: function (row, columnfield, value, defaulthtml, columnproperties) {
							...
							{ text: 'Vehicle Group',datafield:'groupname',width:'10%'},
							{ text: 'V. No.',datafield:'vesselno',hidden:!isTariffHide,width:'10%'},
							{ text: 'V. Name',datafield:'vesselname',hidden:!isTariffHide,width:'10%'},
							...
							{ text: 'Start Km',datafield:'startkm',hidden:true}
         	              ]
            ...
            $('#jobGrid').on('rowselect', function (event) 
  		    ...
  		    if(status=="Assigned"){
  		    	...
  		    	//alert("assigned"+rowData.docname);
  		    	$('#calculationGrid').jqxGrid('setcellvalue',calcindex,'jobname',rowData.docname);
  		    	$('#calculationGrid').jqxGrid('setcellvalue',calcindex,'jobnametemp',rowData.bookdocno+" - "+rowData.docname);
  		    	...
  		    	$('#calculationGrid').jqxGrid('setcellvalue',calcindex,'tariff',rowData.tarif);
  		    	...
  		    	$('#calculationGrid').jqxGrid('setcellvalue',calcindex,'endtime',rowData.endtime);
  		    	$('#calculationGrid').jqxGrid('setcellvalue',calcindex,'enddate',rowData.enddate);
  		    	$('#calculationGrid').jqxGrid('setcellvalue',calcindex,'endkm',rowData.endkm);
  		    	...
  		    	$('#calculationGrid').jqxGrid('setcellvalue',calcindex,'startkm',rowData.startkm);
  		    	$('#calculationGrid').jqxGrid('setcellvalue',calcindex,'excesskmchg',rowData.exkmrate);
  		    	$('#calculationGrid').jqxGrid('setcellvalue',calcindex,'excesshrchg',rowData.extimerate);
  		    	$('#calculationGrid').jqxGrid('setcellvalue',calcindex,'nighttariff',rowData.nighttarif);
  		    	$('#calculationGrid').jqxGrid('setcellvalue',calcindex,'excessnighthrchg',rowData.nightexhrrate);
  		    	$('#calculationGrid').jqxGrid('setcellvalue',calcindex,'waitingchg',0.0);
