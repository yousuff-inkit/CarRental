...
try{
	...
	// to allow invoicing of external vendor assignment 
	
	String getAEVConfig = "select method from gl_config where field_nme='allowexternalvendorinvoice'";
	ResultSet rs = stmt.executeQuery(getAEVConfig);
	int isAEV = 0;
	if(rs.next()) {
		isAEV = rs.getInt("method");
	}
	
	if(isAEV==1){
		String  strLimoBookMUpdate="";
		if(jobtype.equalsIgnoreCase("Transfer")){
			strLimoBookMUpdate = "UPDATE gl_limobooktransfer SET masterstatus=3 WHERE bookdocno="+bookdocno+" AND docname='"+docname+"'";
		} else if(jobtype.equalsIgnoreCase("Limo")){
			strLimoBookMUpdate = "UPDATE gl_limobookhours SET masterstatus=3 WHERE bookdocno="+bookdocno+" AND docname='"+docname+"'";
		}
		int updateLimoBookMUpdate = stmt.executeUpdate(strLimoBookMUpdate);
		if(updateLimoBookMUpdate<=0) errorstatus=1;
		
		String strupdatelimomgmt = "update gl_limomanagement set bstatus=7 where docno=" + bookdocno +" and job='" + docname + "'";
		int updatelimomgmt = stmt.executeUpdate(strupdatelimomgmt);
		if(updatelimomgmt<=0) errorstatus=1;
	} else {
