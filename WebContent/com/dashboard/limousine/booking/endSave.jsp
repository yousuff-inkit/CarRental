...
double vat=0.0,taxamount=0.0,extrahrcharges=0.0,total=0.0,totalchrg=0.0;
 ...
 //System.out.println("======aa==== ");		
		...
		Statement stmt=conn.createStatement();
	  			...
	  			String sqlr = "SELECT startkm,date_format(startdate,'%d.%m.%Y') startdate,rowno FROM an_starttripdet WHERE jobno ='"+jobno+"'";
	  			ResultSet rsq = stmt.executeQuery(sqlr);
	  			while(rsq.next()){
	  				startkm=rsq.getString("startkm");
	  				startdate=rsq.getString("startdate");
	  				docno=rsq.getString("rowno");
	  			}
	  			System.out.println(startkm+" "+startdate+" "+docno);
				...
				String sqlg = "SELECT gid FROM (SELECT CONCAT(bookdocno,'-',docname) jobname, gid FROM gl_limobookhours UNION ALL SELECT CONCAT(bookdocno,'-',docname) jobname, gid FROM gl_limobooktransfer) a  WHERE a.jobname='"+jobno+"'";
				ResultSet rg = stmt.executeQuery(sqlg);
				while(rg.next()){
					grpid=rg.getString("gid");
				}
	  ...
	  int vals1=stmt.executeUpdate(sqlss1);
			...
			System.out.println("input=="+docs+"=="+jobno+"==="+jobname+"==="+total);	
				...
				if(maxdoc<=0){
					...
					msg="1";
					String systemnote="Ended Trip of " + bookdocno + " - "+jobname+" - "+ sqlenddate+" - "+endtime+" - "+endkm+" - "+city+" - by "+username;
					String sqllog = "INSERT INTO gl_limomgmtlog (bookdocno, jobname, brhid, userid, logdate, remarks, systemremarks) VALUES ('"+bookdocno+"','"+jobname+"','"+branch+"','"+userid+"', now(), '"+remark+"','"+systemnote+"')";
					stmt.executeUpdate(sqllog);
			/* 	}  */
			
			String sqlupdatemulti="update gl_multivehassign set bookingstatus=7 where bookingno="+bookdocno+" and jobname='"+jobname+"'";
			int resmulti=stmt.executeUpdate(sqlupdatemulti);
			if(resmulti<1){
				msg="0";
			}
