...
String ishidedriver=request.getParameter("ishidedriver")==null?"0":request.getParameter("ishidedriver"); 
int val=0;        
int sqlupdate=0;      
...
try{  
    ...
    for (int i = 0; i < rownoarray.length; i++) {
		...
		String sqljoidriver ="";
		String sqljoinstatus ="";
		String sqljoindriverinsert="";
		String sqljoindriverinsertvar="";
		System.out.println(detaildocno+"::"+bookdocno+"::"+type+"::"+rowno);
		if(!(detaildocno.equalsIgnoreCase(""))){
			
			System.out.println(ishidedriver);
			if(!ishidedriver.equalsIgnoreCase("true")){
				sqljoidriver=",assigneddriver="+driver;
				sqljoinstatus=",bstatus=4";
				sqljoindriverinsert=", driver";
				sqljoindriverinsertvar=",'"+driver+"'";
				
				String jobdocno="";
				String jobname="";
				
				
				String sqlupdatematable= "SELECT docno,job FROM gl_limomanagement WHERE tdocno='"+detaildocno+"'";	
				ResultSet rssqlupdatematable = stmt.executeQuery(sqlupdatematable);
				if(rssqlupdatematable.next()){
					jobdocno=rssqlupdatematable.getString("docno");
					jobname=rssqlupdatematable.getString("job");
				}
				
				String updatequery = "update gl_multivehassign set drvid="+driver+" where bookingno="+jobdocno+" and jobname='"+jobname+"'";
				int isdoneupate = stmt.executeUpdate(updatequery);
				if(isdoneupate<=0){
					errorstatus=1;
					System.out.println("Limo multiassignt table Update Error"); 	
				}
				
			}
			if(type.equalsIgnoreCase("Transfer")){      
				strsql="update gl_limobooktransfer set pickupdate='"+sqldate+"',pickuptime='"+time+"'"+sqljoidriver+" where doc_no='"+detaildocno+"'";	
				...
				if(sqlupdate<=0){
			 	...
			 	//bstatus=6
				strsql2="update gl_limomanagement set pickupdate='"+sqldate+"',pickuptime='"+time+"'"+sqljoinstatus+",remarks='"+remarks+"' where tdocno='"+detaildocno+"'";	
			...
			if(sqlupdate>0){        
				...
				strsql3="insert into gl_cgtime(type, detaildocno, bookdocno, pdate, ptime"+sqljoindriverinsert+", remarks) values('"+type+"','"+detaildocno+"','"+bookdocno+"','"+sqldate+"','"+time+"'"+sqljoindriverinsertvar+",'"+remarks+"')";	
