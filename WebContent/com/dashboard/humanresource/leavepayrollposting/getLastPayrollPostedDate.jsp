...
String brhid = request.getParameter("branch")==null?"0":request.getParameter("branch");
	try{
	    ...
	    /* Date to be Posted*/
	    String sql = "select LAST_DAY(DATE_ADD(min(d.date),INTERVAL 1 DAY)) tobepostdate,Coalesce(CONCAT('Done till ',DATE_FORMAT(min(d.date),'%b %Y')),'') message from hr_payrolld d left join hr_payroll m on m.doc_no=d.rdocno where d.leaveposted=0 and d.leave1>0 and m.status=3 "+sqltst+"";
