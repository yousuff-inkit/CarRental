<%@page import="com.dashboard.humanresource.employeeuserlink.ClsEmployeeUserLink" %>
<%
ClsEmployeeUserLink DAO=new ClsEmployeeUserLink();
%>
...
<script type="text/javascript">
      ...
      var temp='<%=check%>';
	  	...
	  	if(temp=='1'){ 
	  		  data='<%=DAO.empUserLinkGridLoading(rpttype,check)%>';  
	  	}
        ...
        $(document).ready(function () {
        	...
        	$("#empUserLinkGridID").on("bindingcomplete", function (event) {
        		if(document.getElementById("rdlinking").checked==true){
        			$('#empUserLinkGridID').jqxGrid('hidecolumn', 'user_name');
        		} else if(document.getElementById("rddelete").checked==true){
        			$("#empUserLinkGridID").jqxGrid('showcolumn', 'user_name');
       		 }
        	});
            ...
            $('#empUserLinkGridID').on('rowdoubleclick', function (event) {
                var rowindex1 = event.args.rowindex;
                if(document.getElementById("rdlinking").checked==true){
                	document.getElementById("txtempid").value = $('#empUserLinkGridID').jqxGrid('getcellvalue', rowindex1, "docno");
                	$('#btnlinking').attr("disabled",false);$('#btnremovelinking').attr("disabled",true);
                	var values= $('#empUserLinkGridID').jqxGrid('getcellvalue',rowindex1, "empinfo");
                    var values1 = values.toString().replace(/\*/g, '\n');
                    document.getElementById("txtempinfo").value=values1;
