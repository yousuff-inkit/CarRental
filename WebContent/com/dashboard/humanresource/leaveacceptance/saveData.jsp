...
<%	
	...
	try{
        ...
        String method="";
        String sqls="select method from gl_config where field_nme='workdetails'";

         ResultSet rs = stmt.executeQuery(sqls);
	        while (rs.next()) {
	        	method=rs.getString("method").trim();
	        }
	    
	    // check config to set brhid based on employee or not 
	    
	    String empconfigchksql = "SELECT COALESCE(method,0) method FROM gl_config WHERE field_nme='EmployeeBranchCheck'";
	    ResultSet rsc = stmt.executeQuery(empconfigchksql);
	    String brchmethod = ""; 
	    while(rsc.next()){
	    	brchmethod=rsc.getString("method");
	    }
	    
	    // end check config to set brhid based on employee or not
			    ...
			    for(int j=0;j<requestedarray.size();j++){
			        ...
			        if(method.equalsIgnoreCase("1"))
			        {
		                  String sqld="select db1,db2 from my_comp";
								...
								ResultSet rs4 = stmt3.executeQuery(sqld);
								
								String db1="";
								String db2="";
								while(rs4.next()) {
									db1=rs4.getString("db1");
									db2=rs4.getString("db2");
							  	}
				    	    	String costtype ,costcode,dbname;
		                     String sqlx="select x.* from(select m.dtype costtype,m.brhid,m.doc_no tr_no,cast(concat(m.doc_no, if(sd.doc_no is null,'',concat('',sd.doc_no))) as char(100)) code,"
				    	    		+ "cast(concat(if(sd.name is null,'' ,concat(sd.name,' ')),a.refname) as char(100)) name,'"+db1+"' dbname "
				    	    		...
				    	    		+ " sd.doc_no=s.rdocno where m.status=3 and a.status=3  and m.dtype='sjob' union all(select m.dtype costtype,m.brhid,m.doc_no tr_no,"
				    	    		+ "cast(concat(m.doc_no, if(sd.doc_no is null,'',concat('',sd.doc_no))) as char(100)) code,"
				    	    		+ "cast(concat(if(sd.name is null,'' ,concat(sd.name,' ')),a.refname) as char(100)) name,"
				    	    		...
				    	    		+ " left join "+db2+".cm_subdivision sd on sd.doc_no=s.rdocno where m.status=3 and a.status=3  and m.dtype='sjob' )"
		   	    				+ " UNION ALL select 'STC' costtype,m.brhid,m.doc_no tr_no,cast(concat(m.doc_no ) as char(100)) code, cast(concat(a.refname) as char(100)) name,'"+db1+"' dbname  from  "+db1+".cm_staffcontrm m left join  "+db1+".my_acbook a on m.cldocno=a.doc_no and a.dtype='CRM'  where m.status=3 and a.status=3 UNION ALL "
			    					+ "select 'COST' ,1,c1.costcode code,c1.doc_no doc_no,c1.description name, '"+db1+"' DBNAME from  "+db1+".HR_EMPM M LEFT JOIN  "+db1+".my_ccentre c1 ON C1.DOC_NO=M.COSTID left join  "+db1+".my_ccentre c2 on(c1.doc_no=c2.grpno) where c1.m_s=0 AND M.DOC_NO='"+empid+"'"
			    					+ " UNION ALL select 'STC' costtype,m.brhid,m.doc_no tr_no,cast(concat(m.doc_no ) as char(100)) code, cast(concat(a.refname) as char(100)) name,'"+db2+"' dbname  from  "+db2+".cm_staffcontrm m left join  "+db2+".my_acbook a on m.cldocno=a.doc_no and a.dtype='CRM'  where m.status=3 and a.status=3 UNION ALL "
			    					+ "select 'COST' ,1,c1.costcode code,c1.doc_no doc_no,c1.description name, '"+db2+"' DBNAME from  "+db2+".HR_EMPM M LEFT JOIN  "+db2+".my_ccentre c1 ON C1.DOC_NO=M.COSTID left join  "+db2+".my_ccentre c2 on(c1.doc_no=c2.grpno) where c1.m_s=0 AND M.DOC_NO='"+empid+"'" + ") x "
			    		    	    		+ " left join hr_workdetails d on (d.empid='"+empid+"' and d.costtype=x.costtype and d.costcode=x.code and d.dbname=x.dbname)"
			    		    	    		+ " and d.year="+year+" and d.month="+month+" where d.empId is null";     
								//System.out.println("sqljobwise+++++++++"+sql3);

		                     ResultSet rs3 = stmt2.executeQuery(sqlx);
				    	    ...
				    	    while(rs3.next()){
				    	    	...
				    	    	dbname=rs3.getString("dbname");
									...
									String sqly="INSERT INTO hr_workdetails(empId, year, month,  brhid,costcode,costtype,dbname) VALUES("+empid+", "+year+", "+month+", "+branchid+", "+costcode+", '"+costtype+"', '"+dbname+"')";
									int data4=stmt3.executeUpdate(sqly);
					    	    	
					    	    	//System.out.println("insert-------------"+sqly);
			        ...
			        if(alreadyAttendance==0) {
			    	    ...
			    	    // check the EmployeeBranchCheck config to set branchid and set brhid 
			    	    if(brchmethod.equalsIgnoreCase("1")){
			    	    	String getbidsql = "select coalesce(brhid,0) brhid from hr_empm where doc_no="+empid+" and active=1 and status=3 limit 1";
			    	    	ResultSet rs1 = stmt.executeQuery(getbidsql);
			    	    	if(rs1.next()){
			    	    		branchid = rs1.getString("brhid");
			    	    	}
			    	    }
			    	    // end check the EmployeeBranchCheck config to set branchid  and set brhid
			    	    
			    	    
					    String sql7="INSERT INTO hr_timesheet(empId, year, month, totdays, brhid) VALUES("+empid+", "+year+", "+month+", "+totaldays+","+branchid+")";
					    System.out.println("hr time sheet = "+sql7);
			        ...
			        int data2 = stmt.executeUpdate(sql10);
		                 ...
		                 String sqlq="UPDATE hr_workdetails SET date"+day+"='"+leave+"' where empId="+empid+" and year=Year('"+sqlRequestedDate+"') and month=MONTH('"+sqlRequestedDate+"')";
		              //  System.out.println("#######"+sqlq);
		                 int data3= stmt2.executeUpdate(sqlq);
					        }
