...
<script type="text/javascript">
      $(document).ready(function () {
 		 ...
 		 document.getElementById("formdet").innerText="Work Details Approval(WDA)";
 		document.getElementById("formdetail").value="Work Details Approval";
 		document.getElementById("formdetailcode").value="WDA";
 		window.parent.formCode.value="WDA";
 		window.parent.formName.value="Work Details Approval";
 		
    	
		$('#attendanceGridID').jqxGrid('clearfilters', true);  
		$('#attendanceGrid1ID').jqxGrid('clearfilters', true);  
		 ...
		 $('#txtcomments').attr('readonly', true);
  		 ...
  		 $("#attendanceGrid1ID").jqxGrid({ disabled: true});
    ...
    function saveComment(){
    	...
    	var dbname=$('#hiddbname').val();
    	var empid=$('#hidempid').val();
    	var ddate=$('#hiddate').val();

//alert(empid+","+dbname+","+costtype+","+costcode+","+ddate);
    	var x=new XMLHttpRequest();
		x.onreadystatechange=function(){
			if (x.readyState==4 && x.status==200)
			{
		...
		x.open("GET","saveComment.jsp?comment="+comment.replace(/ /g, "%20")+"&costcode="+costcode+"&costtype="+costtype+"&dbname="+dbname+"&empid="+empid+"&ddate="+ddate+"&year="+$('#cmbyear').val()+"&month="+$('#cmbmonth').val(),true);
		x.send();
    }
   
    function funclearupdate(docno)
    {
		...
		x.onreadystatechange=function(){
			if (x.readyState==4 && x.status==200)
			{
				var items=x.responseText.trim().split(",");
				if(items==0){
					$.messager.alert('Message', '  Cleared Successfully ', function(r){});
				}else if(items==1){
					$.messager.alert('Message', '   Not Cleared ', function(r){});
					}
	            $("#commentsDiv").load("commentsGrid.jsp?year="+year+"&month="+month+"");
		...
		x.open("GET","clearupdate.jsp?docno="+docno+"&clear="+clear,true);
		//alert("done");
		x.send();
    }
	...
	var i=0;var j=0;var k=0;var temp="",tempcnfm="",tempdbname="";		    	
    ...
    var temp1="",temp2="",temp3="";
		   ...
		   $.messager.confirm('Confirm', 'Do you want to save changes?', function(r){
            ...
            var date31=day31.sum;
			 ...
			 if(document.getElementById("errormsg").innerText!="")
				...
				if(selectedrows.length==0){
					...
					$.messager.alert('Warning','Select an Employee to Save.');
					return false;
				}
			    	 for(var m=0 ; m < rows.length ; m++){
	 					...
	 					if(selectedrows[j]==m){
							if(k==0){
								temp=rows[m].srno;
								tempcnfm=rows[m].confirmed;

								k=1;
							}
							else{
								temp=temp+","+rows[m].srno;
								tempcnfm=tempcnfm+","+rows[m].confirmed;
			...
			//	alert(temp2);
  	 			 ...
  	 			 if((selectedcnfm.includes("1")))
					 ...
					 return 0;
	            				...
	            				newTextBox = $(document.createElement("input"))
											...
											.attr("hidden", "true");
											length=length+1;
												newTextBox.val(rows[i].emp+"::"+year+":: "+month+":: "+$('#attendanceGridID').jqxGrid('getcelltext', i, "totdays")+":: "+$('#attendanceGridID').jqxGrid('getcelltext', i, "tot_leave1")+":: "+$('#attendanceGridID').jqxGrid('getcelltext', i, "code")+":: "+$('#attendanceGridID').jqxGrid('getcelltext', i, "costtype")+":: "+$('#attendanceGridID').jqxGrid('getcelltext', i, "dbname")+":: "+branch+":: "
								       				...
								       				+$('#attendanceGridID').jqxGrid('getcelltext', i, "date31")+":: "+rows[i].srno+"::"+confirmed+"::"+$('#attendanceGridID').jqxGrid('getcelltext', i, "clientaprvdhrs")+"::"+selectedsrno+"::"+rows[i].dbs+ " :: ");
			 ...
			 document.getElementById("frmWorkDetailsApproval").submit();
	 ...
	 function funViewAttendance(){
		 document.getElementById("errormsg").innerText="";
			...
			$("#attendanceGrid1ID").jqxGrid('addrow', null, {});
			$("#attendanceGrid1ID").jqxGrid({ disabled: false});
			...
			$("#commentsGrid").jqxGrid({ disabled: false});
		    	...
		    	$("#overlay, #PleaseWait").show();
		    	$("#attendanceDiv1").load("attendanceGrid1.jsp?totdays="+totdays+"&year="+year+"&month="+month+"&check="+check+"&branch="+branch+"");
	  	 		...
	  	 		if (r){
	  	 			...
	  	 			//  $("#overlay, #PleaseWait").show();
	  	 			 ...
	  	 			 if(!(selectedcnfm.includes("0")))
	  	 			  ...
	  	 			  saveGridData(selectedsrno,confirmed,selecteddb);
	  	 	  }
	  	  });  
			
		}
	 
		function saveGridData(selectedsrno,confirmed,selecteddb){
			var x=new XMLHttpRequest();
			x.onreadystatechange=function(){
			if (x.readyState==4 && x.status==200)
				{
					...
					if(parseInt(items)>0){
						...
						$.messager.alert('Message', '   Confirmed ', function(r){
					    });
						$('#attendanceGridID').jqxGrid('clearfilters', true);  
						$('#attendanceGrid1ID').jqxGrid('clearfilters', true);  
						...
						$.messager.alert('Message', '   Not Confirmed ', function(r){
					    });
		...
		x.open("GET","confirmData.jsp?selectedsrno="+selectedsrno+"&confirmed="+confirmed+"&selecteddb="+selecteddb,true);
		x.send();
		}
	...
	function  funClearInfo(){
			...
			$("#attendanceGridID").jqxGrid('addrow', null, {});
			$("#attendanceGridID").jqxGrid({ disabled: true});
			$("#attendanceGrid1ID").jqxGrid('clear');
			$("#attendanceGrid1ID").jqxGrid('addrow', null, {});
			$("#attendanceGrid1ID").jqxGrid({ disabled: true});
...
<form id="frmWorkDetailsApproval" action="saveWorkDetailsApproval" method="post" autocomplete="off">
...
<tr>
  ...
  <tr>
    ...
    <td  width="15%"><select id="cmbyear" name="cmbyear" style="width:50%;" onchange="getDay();" value='<s:property value="cmbyear"/>'>
      </select>
      <input type="hidden" id="hidcmbyear" name="hidcmbyear" value='<s:property value="hidcmbyear"/>'/></td>
  </tr>
  <tr>
    <td width="1%" align="left">Month</td>
    <td><select id="cmbmonth" name="cmbmonth" style="width:50%;" onchange="getDay();" value='<s:property value="cmbmonth"/>'>
  ...
  <tr hidden="true">
    <td ><input type="radio" id="rdtotalleaves" name="rdo" value="rdtotalleaves" ><label id="rdtotalleaves1" for="rdtotalleaves"></label></td>
    </tr>
	<tr>
    ...
    <tr>
        <td colspan="2" ><input type="button" class="myButtonses" name="btnView" id="btnView"  value="View"  onclick="funViewAttendance();"/>
   
   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" class="myButtonses" name="btnsave" id="btnsave"  value=" Save  " onclick="funNotify();">
    </td></tr>
    <tr>
    <td colspan="2" >    <input type="button" class="myButtonses" name="clear" id="clear"  value="Clear" onclick="funClearInfo();">
    
   
   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" class="myButtonses" id="btnConfirmed" name="btnConfirmed" value="Confirm" onclick="funConfirm();"></td>
    </tr>
    <tr>
    <td ></td>
    </tr><tr>
    ...
    <td width="70%" >
    	<table width="100%">
    <tr><td><div id="attendanceDiv1"><jsp:include page="attendanceGrid1.jsp"></jsp:include></div></td></tr>
