...
<script type="text/javascript">
	...
	function timeStringToMinutes(timeString) {
		  ...
		  var [time, period] = timeString.split(' '); // Split by space to handle AM/PM if needed
		  var [hours, minutes] = time.split(':').map(Number);

		  // Calculate the total minutes
		  let totalMinutes = hours * 60 + minutes;

		  // Adjust for AM/PM if needed (assuming 12-hour format)
		  if (period === 'PM' && hours !== 12) {
		    totalMinutes += 12 * 60; // Add 12 hours in minutes
		  } else if (period === 'AM' && hours === 12) {
		    totalMinutes -= 12 * 60; // Subtract 12 hours in minutes for midnight (12:00 AM)
		  }
...
var outtimecon = timeStringToMinutes(outtime);
var intimecon = timeStringToMinutes(intime);
var fillothrscon = timeStringToMinutes(fillothrs);
var fillhothrscon = timeStringToMinutes(fillhothrs);

var timediff=outtimecon-intimecon;
//alert(timediff)
            ...
            if(fillothrscon>timediff) {
            	$.messager.show({title:'Message',msg:'OT should be lesser than inout time',showType:'show',style:{left:27,right:'',top:document.body.scrollTop+document.documentElement.scrollTop,bottom:''}});
				return 0;

            }
            if(fillhothrscon>timediff) {
            	$.messager.show({title:'Message',msg:'HOT should be lesser than inout time',showType:'show',style:{left:27,right:'',top:document.body.scrollTop+document.documentElement.scrollTop,bottom:''}});
				return 0;

            }
  			
  			
  		}else if(document.getElementById("chcknormal").checked==true){
  			
  			 var normalval = new Date($('#timeSheetFillNormalHrs').jqxDateTimeInput('val', 'date'));
             var otval = new Date($('#timeSheetFillOTHrs').jqxDateTimeInput('val', 'date'));
               if(Date.parse(normalval) < Date.parse(otval)){
             	$.messager.show({title:'Message',msg:'OT should be lesser than normal',showType:'show',style:{left:27,right:'',top:document.body.scrollTop+document.documentElement.scrollTop,bottom:''}});
 				return 0;
             }
