<%@page import="com.dashboard.humanresource.timesheetv2.ClsTimeSheetDAO"%>
        ...
        $(document).ready(function () {  
            ...
            $("#timeSheetDetailsGridID").jqxGrid(
            {
                ...
                columns: [
							...
							{ text: 'Cost BRANCH', datafield: 'costbrhid', width: '7%',editable: false,hidden: true },
							{ text: 'Total Time', datafield: 'intime', cellsformat: 'H:mm',  width: '7%', cellsalign: 'left',columntype: 'datetimeinput',editable: true,
								createeditor: function(row,cellvalue,editor,celltext,cellwidth,cellheight) {
								    editor.jqxDateTimeInput({showCalendarButton : false});
								} 
							},
							{ text: 'Out Time', datafield: 'outtime', cellsformat: 'H:mm',  width: '7%', cellsalign: 'left',columntype: 'datetimeinput',editable: true,hidden:true,
								createeditor: function(row,cellvalue,editor,celltext,cellwidth,cellheight) {
								    editor.jqxDateTimeInput({showCalendarButton : false});
								} 
							},
							{ text: 'Normal Hours', datafield: 'hrs', cellsformat: 'H:mm',  width: '7%', cellsalign: 'left',columntype: 'datetimeinput',editable: false,hidden:true,
								createeditor: function(row,cellvalue,editor,celltext,cellwidth,cellheight) {
								    editor.jqxDateTimeInput({showCalendarButton : false});
								} 
							}, 
							{ text: 'OT Hours', datafield: 'othrs', cellsformat: 'H:mm',  width: '7%', cellsalign: 'left',columntype: 'datetimeinput',hidden:true,
								createeditor: function(row,cellvalue,editor,celltext,cellwidth,cellheight) {
								    editor.jqxDateTimeInput({showCalendarButton : false});
								} 
							}, 
							{ text: 'HOT Hours', datafield: 'hothrs', cellsformat: 'H:mm',  width: '7%', cellsalign: 'left',columntype: 'datetimeinput',hidden:true,
								createeditor: function(row,cellvalue,editor,celltext,cellwidth,cellheight) {
								    editor.jqxDateTimeInput({showCalendarButton : false});
								} 
							}, 
            ...
            $("#timeSheetDetailsGridID").on('cellvaluechanged', function (event) {
                ...
                if(datafield=="othrs" || datafield=="hothrs" || datafield=="normalhrs"){
                    var othrs=$("#timeSheetDetailsGridID").jqxGrid('getcelltext', rowindexestemp, "othrs");
                    var hothrs=$("#timeSheetDetailsGridID").jqxGrid('getcelltext', rowindexestemp, "hothrs");
	          		var normalhrs=$("#timeSheetDetailsGridID").jqxGrid('getcelltext', rowindexestemp, "hrs");
	          		if(othrs > normalhrs ){
		              // 	$.messager.alert('Message','Othrs should be lessthan Normalhrs','warning');
		               	$('#timeSheetDetailsGridID').jqxGrid('setcellvalue', rowindexestemp, "othrs" ,"");
							return 0;
		               }
		               if(hothrs > normalhrs ){
		               //	$.messager.alert('Message','Hothrs should be lessthan Normalhrs','warning');
	          	...
	          	if(datafield=="intime" || datafield=="outtime"){
	                ...
	                //alert(Date.parse(endDate)+"==="+Date.parse(startDate));
  	           ...
  	           if(event.args.columnindex == 0)
        		  {
        			...
        			$('#timeSheetDetailsGridID').jqxGrid('setcellvalue', rowindexestemp, "empid" ,'');	
	       			...
	       			$('#timeSheetDetailsGridID').jqxGrid('setcellvalue', rowindexestemp, "costcodename" ,'');
	       			$('#timeSheetDetailsGridID').jqxGrid('setcellvalue', rowindexestemp, "costcode" ,'');
	       			$('#timeSheetDetailsGridID').jqxGrid('setcellvalue', rowindexestemp, "costbrhid" ,'');
	       			...
	       			$('#timeSheetDetailsGridID').jqxGrid('setcellvalue', rowindexestemp, "description" ,'');
                  } 
               });
