...
<% ClsralaDriverUpdateDAO cord=new ClsralaDriverUpdateDAO();%>
...
<script type="text/javascript">
        ...
        $(document).ready(function () { 	
           ...
           var temp1='<%=docNo%>';
             ...
             if(temp1>0)
          	 {   
           	     data='<%=cord.driverGridReloading(docNo,cldocno,type,session)%>';        
          	 }
            ...
            $("#jqxDriver").jqxGrid(
            {
                ...
                columns: [		
							...
							{ text: 'Status', datafield: 'status', width: '9%',cellclassname:cellclassname,editable:false },
							...
							{ text: 'Date of Birth', datafield: 'dob', columntype: 'datetimeinput',  cellsformat: 'dd.MM.yyyy', width: '7%',
			                 ...
			                 { text: 'Licence#', datafield: 'dlno', width: '5%' ,
									validation:function(cell,value){
										if(value.length==0){
											return { result: false, message: "Required" };
										}
							...
							{ text: 'Lic Type', datafield: 'ltype', width: '5%',columntype:'dropdownlist',
							...
							{ text: 'Pass Exp.', datafield: 'pass_exp', columntype: 'datetimeinput', cellsformat: 'dd.MM.yyyy', width: '5%' ,
								validation: function (cell, value) {
			                          var year = value.getFullYear();
			                          var cdate=new Date();
			                          var cyear=cdate.getFullYear();
							...
							{ text: 'Visa#', datafield: 'visano', width: '5%',
								cellbeginedit: function (row) {
								       var tempchk=$('#jqxDriver').jqxGrid('getcellvalue', row, "dr_id");
								           if (parseInt(tempchk)>0)
								            {
							...
							{ text: 'rdocno', datafield: 'rdocno', editable: false, hidden: true,  width: '10%' },
							{ text: 'srno', datafield: 'srno', editable: false, hidden: true,  width: '10%' },
							{ text: 'type', datafield: 'type', editable: false, hidden: true,  width: '10%' },
       	...
       	$('#jqxDriver').on('rowdoubleclick', function (event) {
	    	var rowindex = event.args.rowindex;
	    	var status=	document.getElementById("hidstatus").value =$('#jqxDriver').jqxGrid('getcellvalue', rowindex, "status");
	    	document.getElementById("hiddrid").value =$('#jqxDriver').jqxGrid('getcellvalue', rowindex, "dr_id");
	        document.getElementById("hidtype").value = $('#jqxDriver').jqxGrid('getcellvalue', rowindex, "type");
	        document.getElementById("hidrdoc").value = $('#jqxDriver').jqxGrid('getcellvalue', rowindex, "rdocno");
	        if(status=="ACTIVE")
	     		{
	     		document.getElementById("btnstatus").value ="INACTIVATE"
	     		}
	     	if(status=="INACTIVE")
     		{
     		document.getElementById("btnstatus").value ="ACTIVATE"
     		}
