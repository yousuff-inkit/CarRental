...
function funRemoveReadOnly(){
	...
	if ($("#mode").val() == "A") {
		     ...
		     //$("#cvAnalyseGridID").load("cvAnalyseGrid.jsp");
	}
	...
	var rows = $("#cvAnalyseGridID").jqxGrid('getrows');
	 var cvlength=0;
		 for(var i=0 ; i < rows.length ; i++){
			...
			//alert(chk+"---------"+grade)
			if((typeof(chk) != "undefined" && typeof(chk) != "NaN" && chk!=0 && chk!="")){
			newTextBox = $(document.createElement("input"))
			    .attr("type", "dil")
			    .attr("id", "txtcvanalyse"+cvlength)
			    .attr("name", "txtcvanalyse"+cvlength)
			    .attr("hidden", "true");
			cvlength=cvlength+1;
			newTextBox.val(rows[i].questions+" :: "+rows[i].remarks+" :: "+rows[i].grade+" :: "+rows[i].srno);
	...
	function cvConfig(){
		...
		x.onreadystatechange = function() {
			if (x.readyState == 4 && x.status == 200) {
				...
				$('#hidcmbcvconfig').val(items.trim());
				chkcvconfig = items.trim();
			} else {
			}
		}
		x.open("GET", "cvconfig.jsp", true);
		x.send();
	}
...
$(function(){
    $('#frmCandidateMaster').validate({
            rules: {
            	...
            	txtrefno:"required",
            	txtmobileno: {maxlength:12,minlength:12},
             ...
             messages: {
            	 ...
            	 txtqualification:" *",
            	 txtrefno:" *",
            	 txtmobileno: {maxlength:" Maximum 12 Digits",minlength:" Please Enter 12 Digits"},
             }
    });});
</script>
</head>
<body onload="setValues();cvConfig();">
...
<div class='hidden-scrollbar'>	
	...
	<table width="100%" border="0">
	  ...
	  <tr>
	    ...
	    <td ><input type="text" id="txtmobileno" name="txtmobileno" placeholder="Mobile Number" style="width:100%;" value='<s:property value="txtmobileno"/>'/></td>
	    <td align="right">Email</td>
	    <td ><input type="email" id="txtemail" name="txtemail" placeholder="Email" style="width:100%;" value='<s:property value="txtemail"/>'/></td>
	  ...
	  <tr>
	    ...
	    <td ><input type="text" id="txtpostapplied" name="txtpostapplied" placeholder="Post Applied For" style="width:100%;" value='<s:property value="txtpostapplied"/>'/></td>
	    ...
	    <td colspan="3"><input type="text" id="txtremarks" name="txtremarks" placeholder="Remarks" style="width:100%;" value='<s:property value="txtremarks"/>'/></td>
	   </tr>
	</table>
	</fieldset>
	...
	<input type="hidden" id="hidcmbcvconfig" name="hidcmbcvconfig"  value='<s:property value="hidcmbcvconfig"/>'/>
