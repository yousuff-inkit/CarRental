...
/* Custom UI Buttons */
.modern-ui .myButton, .modern-ui .myButtonss {
    height: 24px !important;
...
.modern-ui .myButton:hover, .modern-ui .myButtonss:hover { 
...
/* Tab specific styling to match modern UI */
.modern-ui .content-container { padding: 15px; background: #fff; }
.modern-ui .small-image { border: none; padding: 3px; vertical-align: middle;}
.modern-ui .jqx-tabs-title { padding: 5px 15px; }
</style>
...
$(document).ready(function () {  
    ...
    $("#postDate").jqxDateTimeInput(dateWidgetSettings);
...
function funNotify(){
    ...
    if($("#mode").val() == "A" || $("#mode").val() == "E" ) {
        ...
        for(var i=0;i<rows.length;i++){
            if(rows[i].clears==true){ aa=1; break; }
        }  
...
function funPrintBtn(){
    if (($("#mode").val() == "view") && $("#masterdoc_no").val()!="") {
        ...
        var win= window.open(reurl[0]+"printmaintWork?docno="+document.getElementById("masterdoc_no").value+"&date="+$('#hidmaintainceDate').val()+"&dtype="+dtype,"_blank","top=250,left=310,Width=800,Height=800,location=no,scrollbars=no,toolbar=yes");
        win.focus();
    } else {
...
function secnext() {
    ...
    for(var i=0 ; i < rows.length-1 ; i++){
        array.push({"type":rows[i].type.replace("&","%26"),"description":rows[i].description.replace("&","%26"),"remarks":rows[i].remarks.replace("&","%26"),"lbrcost":rows[i].lbrcost,"partscost":rows[i].partscost,"total":rows[i].total});
    ...
    const jsobj={"workorderdata":array};
    const myarray = JSON.stringify(jsobj);
    ajaxcall(list,myarray);  
}

function ajaxcall(list,myarray) {
    ...
    x.open("GET","workordersavedata.jsp?myarray="+encodeURIComponent(myarray)+'&doc='+document.getElementById("masterdoc_no").value+'&garrageid='+document.getElementById("garrageid").value+'&maintype='+document.getElementById("maintype").value); 
    x.send();
}
...
function ajaxcall1(lists){
    ...
    x.open("GET","approvelsavedata.jsp?list="+encodeURIComponent(lists)+'&doc='+document.getElementById("masterdoc_no").value+'&currkm='+document.getElementById("currkm").value+'&nextserdue='+document.getElementById("nextserdue").value);   
    x.send();
}
...
function funremoveapproval(){
    var docval=document.getElementById("masterdoc_no").value;
     $.messager.confirm('Message', 'Do you want to remove approval?', function(r){
        if(r==false) { return false;  }
        else{ removeapproval(docval); }
     });
}

function removeapproval(docval){
    var x=new XMLHttpRequest();
    x.onreadystatechange=function(){
        if (x.readyState==4 && x.status==200) {
             var items= x.responseText.trim();
             if(parseInt(items)==1) {
                ...
                $.messager.alert('message','Please remove posting first');
             } else{
                $.messager.alert('message','Not Removed');
             }
        }
    }
  x.open("GET","removeapproval.jsp?docval="+docval, true); 
  x.send();
}       

function funremoveposting(){
    var docval=document.getElementById("masterdoc_no").value;
     $.messager.confirm('Message', 'Do you want to remove posting , it will affect the accounting entry ?', function(r){
        if(r==false) { return false;  }
        else{ removeposting(docval); }
     });
}
    
function removeposting(docval){
    var x=new XMLHttpRequest();
    x.onreadystatechange=function(){
        if (x.readyState==4 && x.status==200) {
             var items= x.responseText.trim();
  ...
  x.open("GET","removeposting.jsp?docval="+docval, true); 
  x.send();
}
</script>
</head>
...
<div class="modern-ui">
    ...
    <div class="middle-panel">
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr>
                ...
                <td width="25%">
                    <input type="text" id="mtflname" tabindex="-1" name="mtflname" value='<s:property value="mtflname"/>' readonly>
                </td>
            ...
            <tr>
                ...
                <td colspan="2" style="padding-left: 10px;">
                    <label id="mainwk" name="mainwk" style="color:#b22222; font-weight:bold; font-size: 12px;"></label>
                </td>
            </tr>
        </table>
    </div>
    ...
    <div class="middle-panel" style="padding-top:25px;">
        <span class="middle-panel-title">Workflow</span>
        
        <div id='jqxTabs'>
            <ul>
                <li>
                    <div style="height: 25px; display: flex; align-items: center; gap: 5px;">
                       <img src="<%=contextPath%>/icons/maintreq.png" alt="" class="small-image" width="18" height="18"/> 
                       <span>Maintenance Required</span>
                    </div>
                </li>
                <li>
                    <div id="worderdiv" style="height: 25px; display: flex; align-items: center; gap: 5px;">   
                        <img src="<%=contextPath%>/icons/maintworkorder.png" alt="" class="small-image" width="18" height="18"/> 
                        <span>Work Order</span>
                    </div>
                </li>
                <li>
                    <div id="jobappdiv" style="height: 25px; display: flex; align-items: center; gap: 5px;">
                        <img src="<%=contextPath%>/icons/maintapproval.png" alt="" class="small-image" width="18" height="18"/> 
                        <span>Job Approval</span>
                    </div>
                </li>
                <li>
                    <div id="postdiv" style="height: 25px; display: flex; align-items: center; gap: 5px;">
                        <img src="<%=contextPath%>/icons/maintposting.png" alt="" class="small-image" width="18" height="18"/>
                        <span>Posting</span>
                    </div>
                </li>
            </ul>
            ...
            <div class="content-container">
                ...
                <input type="hidden" id="first" onclick="firstnext();">
            </div>
            ...
            <div class="content-container">
                <table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-bottom: 10px;">
                    <tr>
                        ...
                        <td class="lbl-right" width="8%">Garage</td>
                        <td width="30%">
                            <div class="input-search-container">
                ...
                <div class="grid-container" style="height: 300px; margin-bottom: 15px;">
                    <div id="servgrid"><jsp:include page="servicemaingrid.jsp"></jsp:include></div>
                </div>
                
                <div style="text-align: right;">
                    <input type="button" id="secondedit" onclick="secondedits();" class="myButton" value="EDIT">
                    <input type="button" id="second" style="display:none;" onclick="secnext();" class="myButton" value="SAVE">
                </div>
            </div>
            ...
            <div class="content-container">  
                <table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-bottom: 10px;">
                    <tr>
                        <td class="lbl-right" width="10%">Curr. KM</td>
                        <td width="15%">
                            <input type="text" id="currkm" name="currkm" onkeypress="javascript:return isNumber (event)" value='<s:property value="currkm"/>'> 
                        </td>
                        
                        <td class="lbl-right" width="15%">Next Ser.Due KM</td>
                        <td width="15%">
                            <input type="text" id="nextserdue" name="nextserdue" onkeypress="javascript:return isNumber (event)" value='<s:property value="nextserdue"/>'> 
                        </td>
                        <td></td>
                    </tr>
                </table>  
                ...
                <div id="imgdiv" style="position: relative;">
                    <div style="display:none; position:absolute; z-index: 10; top:50px; left:50%; transform: translateX(-50%);" id="hidediv">
                        <img alt="Loading" src="<%=contextPath%>/icons/31load.gif"> 
                    </div>
                </div>     
                         
                <div class="grid-container" style="height: 150px; margin-bottom: 15px;">
                ...
                <div style="text-align: right;">
                    <input type="button" id="thirdedit" onclick="thirdedits();" class="myButton" value="EDIT">
                    <input type="button" style="display:none;" id="third" onclick="trdnext();" class="myButton" value="SAVE">
                    <input type="button" id="thirdremove" onclick="funremoveapproval();" class="myButton btn-danger" value="REMOVE" style="margin-left: 10px; background: linear-gradient(135deg, #dc3545 0%, #c82333 100%);">
                </div>
            </div>
            ...
            <div class="content-container">
                <table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-bottom: 10px;">
                    <tr>
                        ...
                        <td class="lbl-right" width="10%">Inv Date</td>
                        <td width="15%">
                        ...
                        <td class="lbl-right" width="10%">Posting Date</td>
                        <td width="15%">
                        ...
                        <td align="center">
                            <button type="button" id="btnpostdetails" class="myButton" onclick="getPostDetails();">Post Details</button>
                        </td>
                    </tr>
                </table>  

                <div class="grid-container" style="height: 300px; margin-bottom: 15px;">
                    <div id="postingss"><jsp:include page="posting.jsp"></jsp:include></div>
                </div>

                <div style="text-align: right;">
                    <input type="button" id="lastedit" onclick="lastedits();" class="myButton" value="EDIT">
                    <input type="button" id="last" style="display:none;" onclick="lastsave();" class="myButton" value="SAVE">
                    <input type="button" id="lastremove" onclick="funremoveposting();" class="myButton btn-danger" value="REMOVE" style="margin-left: 10px; background: linear-gradient(135deg, #dc3545 0%, #c82333 100%);">
                </div>
            </div>
