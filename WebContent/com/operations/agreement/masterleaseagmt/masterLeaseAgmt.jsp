...
$('#date,#startdate,#enddate').on('change', function (event) { if($(this).attr('id')=='date'){
var maindate = $('#date').jqxDateTimeInput('getDate'); if ($("#mode").val() == "A") { funDateInPeriod(maindate);
}
}
else if($(this).attr('id')=='startdate'){
var maindate = $('#startdate').jqxDateTimeInput('getDate'); if ($("#mode").val() == "A") {
funDateInPeriod(maindate);
}
}
else if($(this).attr('id')=='enddate'){
var maindate = $('#enddate').jqxDateTimeInput('getDate'); if ($("#mode").val() == "A") {
funDateInPeriod(maindate);
}
...
$('#date,#startdate,#enddate').jqxDateTimeInput({ disabled: false}); if($('#mode').val()=='A'){
...
function funFocus(){ document.getElementById("cldocno").focus();
}

function funNotify(){
var maindate = $('#date').jqxDateTimeInput('getDate'); var validdate=funDateInPeriod(maindate); if(validdate==0){
...
{
document.getElementById("errormsg").innerText=" Select Client"; document.getElementById("cldocno").focus();
return 0;
}
var startdate=$('#startdate').jqxDateTimeInput('getDate'); if(startdate==null || startdate==""){
document.getElementById("errormsg").innerText=""; document.getElementById("errormsg").innerText="Start Date is Mandatory"; return 0;
}
var enddate=$('#enddate').jqxDateTimeInput('getDate'); if(enddate==null || enddate==""){
document.getElementById("errormsg").innerText=""; document.getElementById("errormsg").innerText="End Date is Mandatory"; return 0;
}
startdate=new Date($('#startdate').jqxDateTimeInput('getDate')); startdate.setHours(0,0,0,0);
enddate=new Date($('#enddate').jqxDateTimeInput('getDate')); enddate.setHours(0,0,0,0);
if(enddate<startdate){ document.getElementById("errormsg").innerText="";
...
var rows=$('#masterLeaseGrid').jqxGrid('getrows'); var z=0;
...
newTextBox.val(rows[i].brandid+"::"+rows[i].modelid+"::"+rows[i].specid+"::"+rows[i].leaseduratio n+"::"+rows[i].qty+"::"+rows[i].rate+"::"+rows[i].cdw+"::"+rows[i].pai+"::"+rows[i].gps+"::"+rows[i].c hildseat+"::"+rows[i].kmrestrict+"::"+rows[i].excesskmrate);
newTextBox.appendTo('form'); z++;
}
}
if(z==0){ document.getElementById("errormsg").innerText="";
document.getElementById("errormsg").innerText="please type in valid vehicle details"; return 0;
}

$('#gridlength').val(z); return 1;
}

function setValues() { if($('#msg').val()!=""){
...
var nmaxs = validdesc.length; if(nmaxs>99)
{
document.getElementById("errormsg").innerText="Description Cannot Contain More Than
100 Characters"; document.getElementById("description").focus();
...
function funSearchLoad(){ changeContent('mainSearch.jsp', $('#window'));
}
...
{
document.getElementById("errormsg").innerText="Enter Numbers Only"; return false;
}
document.getElementById("errormsg").innerText=""; return true;
}

function funPrintBtn(){
if (($("#mode").val() != "A") && $("#docno").val()!="") { var url=document.URL;
var reurl=url.split("saveLeaseAgreement");
$("#docno").prop("disabled", false); win.focus();
}
else {
