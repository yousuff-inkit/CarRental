...
$(document).ready(function () {
  ...
  $('#collectdate').on('change', function (event) 
			   {  
		  if(document.getElementById("mode").value=="A"){
			...
			document.getElementById("lblinvoicedone").innerText="";
	    	 ...
	    	 if($('#collecttime').jqxDateTimeInput('getDate')!=null && $('#collectdate').jqxDateTimeInput('getDate')!=null){
				   var collecttime=new Date($('#collecttime').jqxDateTimeInput('getDate'));
				   var collectdate=new Date($('#collectdate').jqxDateTimeInput('getDate'));
				   var status=checkPriorTime(collecttime,collectdate);
				   if(status){
					   document.getElementById("errormsg").innerText="";
					   return true;
				   }
	   ...
	   $('#collecttime').on('change', function (event) 
			   {  
		   if(document.getElementById("mode").value=="A"){
			document.getElementById("useddays").value="";
			document.getElementById("usedhours").value="";
 ...
 function getPriorClose(){
	...
	x.onreadystatechange = function() {
		if (x.readyState == 4 && x.status == 200) {
			...
			document.getElementById("defaultdate").value=items[2];
			document.getElementById("agmtinvaudit").value=items[4];
			...
			if(document.getElementById("agmtinvaudit").value=="1"){
				$('.cellaudit').show();
				$('.celldesc').attr('colspan','3');
			}
			else{
				$('.cellaudit').hide();
				$('.celldesc').attr('colspan','5');
			}
			} else {
			}
		}
...
/* 		getCheckin();
	...
	function funNotify(){
		...
		if(document.getElementById("chkcollection").checked){
		var coltimeprior=new Date($('#collecttime').jqxDateTimeInput('getDate'));
		var coldateprior=new Date($('#collectdate').jqxDateTimeInput('getDate'));
		var coltimepriorstatus=checkPriorTime(coltimeprior,coldateprior);
		if(coltimepriorstatus){
			document.getElementById("errormsg").innerText="";
		}
		else if(coltimepriorstatus==false && document.getElementById("priormethod").value=="2"){
			$('#collecttime').jqxDateTimeInput('focus');
			return 0;
		}
		}
...
if(document.getElementById("rentalagent").value==""){
	...
	function setValues(){
		...
		if(document.getElementById("hidchkaudit").value=="0"){
			document.getElementById("chkaudit").checked=true;
		}
		else{
			document.getElementById("chkaudit").checked=false;
		}
}
	...
	function onAuditChange(){
		if(document.getElementById("chkaudit").checked==true){
			document.getElementById("hidchkaudit").value="0";
		}
		else{
			document.getElementById("hidchkaudit").value="1";
		}
	}	
	
	function disCheck(){
		var x = new XMLHttpRequest();
		var doc_no=$("#agreementno").val();
		var tempval = $('#totalgrid').jqxGrid('getcellvalue', 0, 'rentaltype');
     	x.onreadystatechange = function() {
     		if (x.readyState == 4 && x.status == 200) {
     			var items = x.responseText.trim();
     		 
     			$('#calcuvals').val(items);  
     			console.log($('#calcuvals').val()+"====items==="+items)
     			
     		} else {
     			}
     		}
     		x.open("GET", "getDiscuntchck.jsp?doc_no="+doc_no+'&rentaltype='+tempval, false);
     		x.send();
	}
	...
	function getConfigs(){
		...
		$.get('getConfig.jsp',function(data){
			...
			$('#discountconfig').val(data.actualclientconfig);
		});
	}
...
<body onload="setValues();disCheck();">
  		...
  		<tr>
    		<td colspan="2"><fieldset>
      			<table width="100%">
        			...
        			<tr>
        				<td colspan="2" align="center" class="cellaudit">
        					<div>
        						<input type="checkbox" name="chkaudit" id="chkaudit" onchange="onAuditChange();">
        						<input type="hidden" name="hidchkaudit" id="hidchkaudit" value='<s:property value="hidchkaudit"/>'>
        						<label for="chkaudit" style="position: absolute;margin-top: 5px;margin-left: 5px;">Audit Required</label>
        					</div>
        				</td>
        				<td align="right">Description</td>
          				<td colspan="3" align="left" class="celldesc">
          					<input type="text" name="description" id="description" value='<s:property value="description"/>' ><label id="lblchkorgregcard" for="chkorgregcardcollect" style="margin-left:25px;">Org Reg Card Collected</label>
						...
						<input type="hidden" name="hidchkorgregcard" id="hidchkorgregcard" value='<s:property value="hidchkorgregcard"/>'>
          				...
          				<td align="left">
          					<select name="cmbcloseloc" id="cmbcloseloc" value='<s:property value="cmbcloseloc"/>' style="width:100%;" >
    ...
    <td width="34%" rowspan="3">
    	<div style="text-align:center;">
    		<table style="width:100%;">
    			<tr>
    				<td align="right">Ins.Total</td>
    				<td><input type="text" name="insurcomptotal" id="insurcomptotal" value='<s:property value="insurcomptotal"/>' readonly/></td>
    				<td align="right">Client.Total</td>
    				<td><input type="text" name="clienttotal" id="clienttotal" value='<s:property value="clienttotal"/>' readonly/></td>
    			</tr>
    		</table>
    	</div>
    	<div style="text-align:center;"><label name="lblinvoicedone" id="lblinvoicedone" style="color:red;font-weight:bold;font-size:14;"></label></div>
    	<div style="text-align:center;"><input type="button" name="btncalculate" id="btncalculate" class="myButton" value="Calculate" onclick="funCalculate();"></div>
...
<input type="hidden" name="agmtinvaudit" id="agmtinvaudit">
<input type="hidden" name="calcuvals" id="calcuvals">
<input type="hidden" name="deleted" id="deleted">
<input type="hidden" name="discountconfig" id="discountconfig">
