...
try{
	...
	if(delstatus==1){
		String strmovsql="select mov.din,mov.tin,mov.kmin,mov.fin from gl_vmove mov left join gl_ragmt agmt "+
					" on (mov.rdocno=agmt.doc_no and mov.rdtype='RAG') where mov.rdtype='RAG' and mov.rdocno="+agmtno+" and mov.trancode='DL' and mov.repno=0  and mov.exchng=0 and mov.custno=0 ";
	...
	" an_acollection where agmtno="+agmtno+" and agmttype='RAG'";
	...
	while(rscollect.next()){
		if(rscollect.getDate("sqlcollectdate")!=null){
			...
			objcollect.put("collectsqldate",rscollect.getDate("sqlcollectdate").toString());
		...
		if(rscollect.getDate("sqlindate")!=null){
			...
			objcollect.put("insqldate",rscollect.getDate("sqlindate").toString());
			objcollect.put("intime",rscollect.getString("intime"));
		}
		
	}
	
	String strpickup="select agmtno,pdate sqldate,date_format(pdate,'%d.%m.%Y') pdate,ptime,pkm,pfuel from gl_vehpickup p where p.status=3 and agmttype='RAG' and agmtno="+agmtno;
