...
try{
	...
	*/
	...
	/** (delivery - collectoin) date or (out- in) date calculation STARTS 	*/
		...
		if(delcal==0){
			sqltest=" and trancode!='DL' and repno=0  and exchng=0 and custno=0 ";
		}
		...
		if(firstinvstatus==1){
  		...
  		// System.out.println("Checking:"+nocreditdatediff+"////"+startdays);
			if(nocreditdatediff<startdays && advchk==1 && advinvcount==1){
				...
				if(nocreditconfig>0){
				// datediff=0;   //uncomment this changed for wti 
				datediff=(date2.getTime()-invdate.getTime())<=0?0:(date2.getTime()-invdate.getTime())/(24*60*60*1000);  //comment if uncommented above live change done for wti
				closeinvdate=invdate;
				finaltime1=Double.parseDouble(finaltime1)>0?finaltime1:"0.0";
				}
				else{
					closeinvdate=invdate;
