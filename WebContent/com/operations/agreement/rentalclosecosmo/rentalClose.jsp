...
$(document).ready(function () {
  ...
  getAgmtLocation();
	...
	$('#unameWindow').jqxWindow({width: '30%', height: '40%',  maxHeight: '70%' ,maxWidth: '51%' , title: 'Search Salik User',position: { x: 300, y: 87 } , theme: 'energyblue', showCloseButton: true});
			...
			document.getElementById("lblinvoicedone").innerText="";
	    	 ...
	    	 $("#calculationgrid").jqxGrid("addrow", null, {});
		      ...
		      if($('#intime').jqxDateTimeInput('getDate')!=null){
			   ...
			   var strtime=combinedatetime(indate,intime);
			   var status=checkPriorTime(strtime);
			   if(status){
...
/* 		getCheckin();
	...
	function funNotify(){
		...
		var docdate=new Date($('#closedate').jqxDateTimeInput('getDate'));
	   docdate.setHours(0,0,0,0);
	   var currentdate=new Date();
	   currentdate.setHours(0,0,0,0);
	   //alert(docdate.getTime()+"//////"+currentdate.getTime());
	   if(docdate.getTime()!=currentdate.getTime()){
			document.getElementById("errormsg").innerText="Document date must be current date";
			return 0;
	   }
		...
		var strtime=combinedatetime(indateprior,intimeprior);
		var intimepriorstatus=checkPriorTime(strtime);
...
if(document.getElementById("rentalagent").value==""){
		...
		/*var totalgridemptyvalidation=$("#totalgrid").jqxGrid("getcellvalue",0,"rate");
		if(totalgridemptyvalidation=="" || totalgridemptyvalidation==null || totalgridemptyvalidation=="undefined" || typeof(totalgridemptyvalidation)=="undefined"){
			document.getElementById("errormsg").innerText="";
			document.getElementById("errormsg").innerText="Please Process";
			return 0;
		}
		var calcgridemptyvalidation=$("#calculationgrid").jqxGrid("getcellvalue",0,"description");
		if(calcgridemptyvalidation=="" || calcgridemptyvalidation==null || calcgridemptyvalidation=="undefined" || typeof(calcgridemptyvalidation)=="undefined"){
			document.getElementById("errormsg").innerText="";
		...
		changeContent('masterSearch.jsp', $('#window'));
			...
			function getDeliveryDriverConfig(){
	          ...
	          function funDownload(){
	          	...
	          	var salikuser=$('#txtusername').val();
	          	if(agmtno=="" || fleetno==""){
	          		$.messager.alert('Warning','Please select an agreement');
	          		return false;
	          	}
	          	$('#overlay,#PleaseWait').show();
	          	document.getElementById("errormsg").innerText="Downloading Salik,Please Wait";
	         	var x = new XMLHttpRequest();
	         	x.onreadystatechange = function() {
	         		if (x.readyState == 4 && x.status == 200) {
	         			var items = x.responseText.trim().split("::");
	         			if(items[0]=="success"){
	         				$("#loadsalikdata").load("SATloadDetails.jsp?xdocs="+items[1]+"&id=1");
	         			}
	         			else{
	         				$.messager.alert('Warning','No Saliks found');
	         		...
	         		x.open("GET", "downloadData.jsp?agmtno="+agmtno+"&fleetno="+fleetno+"&salikuser="+salikuser, true);
	         		x.send();
	          }
	          
	          
	          
	          function funAllocateSalik(){
          		var saveval="10";
          		var fleetno=document.getElementById("hidfleet").value;
          		ajaxcall(saveval,fleetno);
	          }
	          function ajaxcall(saveval,fleetno){
				var x=new XMLHttpRequest();
				x.onreadystatechange=function(){
					if (x.readyState==4 && x.status==200)
					{
				    	...
				    	{
				    	   	$.messager.alert('Message', ' Saliks Downloaded and Successfully Allocated ');
				    	    var val ="10";
				    	}
				    	else
				    	{
				...
				x.open("GET","savedata.jsp?saveval="+saveval+"&fleetno="+fleetno,true);
				x.send();
			}
...
<input type="hidden" name="hidchkorgregcard" id="hidchkorgregcard" value='<s:property value="hidchkorgregcard"/>'>
  ...
  <tr>
    ...
    <td><fieldset><table width="100%">
      <tr>
        <td><button type="button" name="btnallocatesalik" id="btnallocatesalik" onclick="funAllocateSalik();" hidden="true">ALLOCATE SALIK</button>
        <div id="loadsalikdata"><jsp:include page="SATloadDetails.jsp"></jsp:include></div></td>
        </tr>
      </table>
    </fieldset></td>
    </tr>
...
<input type="hidden" name="iscaptcha" id="iscaptcha">
