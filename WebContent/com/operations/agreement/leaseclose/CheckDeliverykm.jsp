...
stmt.close();
	...
	String strmovsql="select mov.din,mov.tin,mov.kmin,mov.fin from gl_vmove mov left join gl_lagmt agmt "+
			" on (mov.rdocno=agmt.doc_no and mov.rdtype='LAG') where mov.rdtype='LAG' and mov.rdocno="+agmtno+" and mov.trancode='DL' and mov.repno=0  and mov.exchng=0 and mov.custno=0 ";
...
//Date and time validation ends
	 ...
	 java.sql.Date invtmpdate=null,invtmptodate=null;
		String strinv="select invdate,invtodate,adv_chk,inv_type from gl_lagmt where doc_no="+agmtno+"";
	...
	while(rsinv.next()){
	 ...
	 int closecalflag=0;
		...
		//Disable Invoice Pending validation for complimentary agreements 
		if(invtype==3){
			invtostatus=0;
			datediff=0;
		}
