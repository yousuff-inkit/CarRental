...
<%String data=request.getParameter("data")==null?"0":request.getParameter("data").trim();%>
<%String issrs=request.getParameter("issrs")==null?"0":request.getParameter("issrs").trim();%>
<script type="text/javascript">
var descdatas;
var gridamtdec="d"+(window.parent.document.getElementById("amtdec").value);
var tempdocno='<%=docno%>';
var isSRS='<%=issrs%>';
if(tempdocno>0){
	descdatas='<%=viewDAO.reloadnipurchase(session,docno)%>';  
} else if (isSRS==1){
	descdatas='<%=viewDAO.getGridDataSRS(session,data,issrs)%>';
}
else
{   
     descdatas=[];
         ...
         var rendererstring=function (aggregates){
            ...
            $("#nidescdetailsGrid").jqxGrid(
            {
                            	 ...
                            	 document.getElementById("rowindex").value =cell3.rowindex;
            	                   ...
            	                   if (key == 114) {   
                                	   ...
                                	   //costcodeSearchContent('costcodegridsearch.jsp?costtype='+value);
                                	    costcodeSearchContent(<%=contextPath+"/"%>+"com/costcenter/costCodeSearchGrid.jsp?formname=nidescdetailsGrid&costtype="+value+"&brhid="+$("#brchName").val());            
            	                	   $('#nidescdetailsGrid').jqxGrid('render');
            	        	           }
            	                   }
            	               }
                ...
                columns: [
							...
							{ text: 'Unit Price', datafield: 'unitprice', width: '5%',cellsalign: 'right', align:'right',cellsformat:gridamtdec,editable: true },
							{ text: 'Total', datafield: 'total', width: '5%',cellsformat:gridamtdec,cellsalign: 'right', align:'right', editable:false},
							{ text: 'Discount', datafield: 'discount', width: '4%',cellsalign: 'right', align:'right',cellsformat:gridamtdec,editable: true ,aggregates: ['sum1'],aggregatesrenderer:rendererstring1},
							{ text: 'Net Total', datafield: 'nettotal', width: '5%' ,cellsformat:gridamtdec,cellsalign: 'right', align:'right',aggregates: ['sum'],aggregatesrenderer:rendererstring,editable:false},
							
							
							{ text: 'Tax %', datafield: 'taxper', width: '3%', cellsformat: gridamtdec, cellsalign: 'right', align: 'right'  ,editable: true},
							{ text: 'Tax Amount', datafield: 'taxperamt', width: '6%', cellsformat: gridamtdec  , cellsalign: 'right', align: 'right'  ,editable:false,aggregates: ['sum'],aggregatesrenderer:rendererstring},
							{ text: 'Net Total', datafield: 'taxamount', width: '6%', cellsformat: gridamtdec, cellsalign: 'right', align: 'right'  ,aggregates: ['sum'],aggregatesrenderer:rendererstring ,editable:false },
							...
							{ text: 'nuprice', datafield: 'nuprice', width: '9%',cellsformat:gridamtdec,hidden:true,editable: true},
							...
							{ text: 'Cost Code', datafield: 'costcodeid', width: '8%',hidden:true ,editable: true},  
          ...
          if(termsval>0)
        	  {
               ...
               if(value==4 || value==5){
                  ...
                  costcodeSearchContent(<%=contextPath+"/"%>+"com/costcenter/costCodeSearchGrid.jsp?formname=nidescdetailsGrid&costtype="+values+"&brhid="+$("#brchName").val());            
                           }   
                      } 
            ...
            $("#nidescdetailsGrid").on('cellvaluechanged', function (event) 
            {
    		    ...
    		    var taxperc = $("#taxperc").val()
		    		...
		    		if(datafield=="taxper")
		    		{
		   				...
		   				if(parseInt(taxperc)==0){
		   					tper=0;
		   					$('#nidescdetailsGrid').jqxGrid('setcellvalue', rowBoundIndex, "taxper",0);
		   				}
		           		if(parseFloat(tper)>0){
		            		  var taxempamount=parseFloat(netotal)*(parseFloat(tper)/100);
		            		  taxempamount=Math.round(taxempamount*100)/100;
		            		  
		            		  $('#nidescdetailsGrid').jqxGrid('setcellvalue', rowBoundIndex, "taxperamt",taxempamount.toFixed(2));
