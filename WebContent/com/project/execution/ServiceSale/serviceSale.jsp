	...
	$(document).ready(function () {
	    ...
	    $('#printWindow').jqxWindow({width: '30%', height: '30%',  maxHeight: '50%' ,maxWidth: '40%' , title: 'Print',position: { x: 300, y: 87 } , theme: 'energyblue', showCloseButton: true, keyboardCloseKey: 27});
		  	 ...
		  	 $('#nipuraccid').dblclick(function(){
				...
				function getTaxper(date){
				    ...
				    x.onreadystatechange = function(){
					    if (x.readyState == 4 && x.status == 200) {
						    var items = x.responseText.trim();
						    var nontax = $("#hidnontax").val()
						    if(parseInt(nontax)>0){
						    	 $('#taxperc').val(items);
			   				}else{
			   				 $('#taxperc').val(0);
			   				}
	...
	function funNotify(){	
		 		...
		 		if(mod1=='A'){
		 			 var summaryDataSRS = $("#nidescdetailsGrid").jqxGrid('getcolumnaggregateddata', 'nettotal', ['sum'], true);
			    	 $('#nettotal').val(summaryDataSRS.sum);
		 		}
					...
					document.getElementById("errormsg").innerText="";     
							...
							else
							{
							 ...
							 for(var i=0 ; i < rows.length ; i++){
								 ...
								 .attr("hidden", "true"); 
								 var aa=0;
								 newTextBox.val(rows[i].srno+"::"+rows[i].qty+" :: "+ (typeof(rows[i].description)=='undefined'?rows[i].description:rows[i].description.replace(/'/g,'')) +" :: "
 ...
 */			
			...
			function funSearchLoad(){
				...
				function funRemoveReadOnly(){ 
					...
					if (mod1 == "A") { 
						...
						getCurrencyIds($("#nipurchasedate").val());
					    $("#nidescdetailsGrid").jqxGrid('clear');
						$("#nipurdetails").load("descgridDetails.jsp?data="+encodeURIComponent(data)+"&issrs=1");
					}
					//alert("Ended fun remove");
						  ...
						  /*  if(document.getElementById("interstate").checked=true){
	                       ...
	                       var interstate=document.getElementById("hidinterstate").value;
							...
							if(interstate>0){
							 ...
							 if(mod1=='A'){
						    	 var summaryDataSRS= $("#nidescdetailsGrid").jqxGrid('getcolumnaggregateddata', 'nettotal', ['sum'],true);
		    	                 document.getElementById("nettotal").value=summaryDataSRS.sum;
							 }
   ...
   function PrintContent(url) {
	...
	<body onload="setValues();funinterstate();funbnkdetls();" onmouseover="funchkforedit();" >
	...
	<input type="hidden" id="ordermasterdoc_no" name="ordermasterdoc_no"  value='<s:property value="ordermasterdoc_no"/>'/>
	     ...
	     <input type="hidden" id="hidnontax" name="hidnontax"  value='<s:property value="hidnontax"/>'/>
	     ...
	     <input type="hidden" id="srsrip" name="srsrip"  value='<s:property value="srsrip"/>'/>  
	     <input type="hidden" id="srsmonth" name="srsmonth"  value='<s:property value="srsmonth"/>'/>  
	     <input type="hidden" id="srsyear" name="srsyear"  value='<s:property value="srsyear"/>'/>  
