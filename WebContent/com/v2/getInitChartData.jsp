...
<%
	...
	try{
		...
		" (SELECT COUNT(*) FROM gl_bookingm WHERE STATUS=3 AND rano=0) bookingcount";
		//System.out.println(strgetoccupancy);
		double totalcount=0,agmtcount=0,availtempcount=0,unrentablelcount=0,garagecount=0,bookingcount=0;
		...
		while(rsgetoccupancy.next()){
			...
			bookingcount=rsgetoccupancy.getDouble("bookingcount");
		}
		...
		objdata.put("occtotalvalue",agmtcount);
		objdata.put("occavailvalue",agmtcount);
		objdata.put("occgaragevalue",garagecount);
		objdata.put("occunrentablevalue",unrentablelcount);
		...
		String strfleetstatus="SELECT st.st_desc trancode,COUNT(*) itemcount FROM gl_vehmaster "+
		...
		" WHERE VEH.statu=3 AND VEH.fstatus<>'Z' GROUP BY veh.TRAN_CODE";
		ResultSet rsfleetstatus=stmt.executeQuery(strfleetstatus);
		JSONArray fleetstatusarray=new JSONArray();
		while(rsfleetstatus.next()){
			JSONObject objtemp=new JSONObject();
			objtemp.put("trancode", rsfleetstatus.getString("trancode"));
			objtemp.put("itemcount",rsfleetstatus.getString("itemcount"));
			fleetstatusarray.add(objtemp);
		}
		//Adding Booking Count to Fleet Status
		JSONObject objfsbooking=new JSONObject();
		objfsbooking.put("trancode", "BOOKING");
		objfsbooking.put("itemcount", bookingcount);
		fleetstatusarray.add(objfsbooking);
		
		//Traffic 
		
		
		
		objdata.put("fleetstatusdata", fleetstatusarray);
		...
		objdata.put("bookingcount",bookingcount);
		...
		objdata.put("portaluser",session.getAttribute("PORTALUSER")==null?"":session.getAttribute("PORTALUSER"));
	}
	catch(Exception e){
		e.printStackTrace();
		conn.close();
