...
<script type="text/javascript">
	$(document).ready(function() {
        ...
        /* FIXED: Formatted jqxDateTimeInput heights to match modern UI 24px */
        ...
        $("#inputjqxCashPaymentDate").css("margin-top", "0px");
	...
	function funRemoveReadOnly() {
		...
		if ($("#mode").val() == "E") {
			...
			$("#jqxCashPayment").jqxGrid('addrow', null, {
				"docno" : "", "type" : "", "accounts" : "", "accountname1" : "", "currency" : "",
				"currencyid" : "", "rate" : "", "costtype" : "", "costgroup" : "", "costcode" : "",
				"dr" : true, "amount1" : "", "baseamount1" : "", "description" : "", "grtype" : "",
				"currencytype" : "", "sr_no" : ""
			});
		} else {
			$("#btnvaluechange").hide();
	...
	function funNotify() {
		...
		for (var i = 0; i < rows.length; i++) {
			...
			if (typeof (chk) != "undefined" && typeof (chk) != "NaN" && chk != "") {
				newTextBox = $(document.createElement("input")).attr("type", "dil").attr("id", "test" + length).attr("name", "test" + length).attr("hidden", "true");
				length = length + 1;
				if (rate == "" || rate == "0" || rate == "0.00") {
					val = 1;
					break;
		...
		for (var i = 0; i < rows.length; i++) {
			...
			if (typeof (chks) != "undefined" && typeof (chks) != "NaN" && chks != "") {
				newTextBox = $(document.createElement("input")).attr("type", "dil").attr("id", "txtapply" + lengthapply).attr("name", "txtapply" + lengthapply).attr("hidden", "true");
		...
		for (var i = 0; i < rows.length; i++) {
			...
			if (typeof (chkd) != "undefined" && typeof (chkd) != "NaN" && chkd != "") {
				newTextBox = $(document.createElement("input")).attr("type", "dil").attr("id", "txtapplyupdate" + lengthupdate).attr("name", "txtapplyupdate" + lengthupdate).attr("hidden", "true");
	...
	function funPrintBtn() {
		if (($("#mode").val() == "view") && $("#docno").val() != "") {
			...
			$.messager.confirm('Confirm', 'Do you want to have header?', function(r) {
				var printUrl = reurl[0] + "printCashPayment?docno=" + document.getElementById("docno").value + "&branch=" + document.getElementById("brchName").value + "&header=" + (r ? 1 : 0);
				var win = window.open(printUrl, "_blank", "top=150,left=250,width=1020,height=500,scrollbars=yes");
