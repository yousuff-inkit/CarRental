	...
	$(document)
			.ready(
					function() {
						...
						$("#jqxCashPaymentDate").jqxDateTimeInput({
							width : '125px',
							height : '15px',
							formatString : "dd.MM.yyyy"
						});
						$("#maindate").jqxDateTimeInput({
							width : '125px',
							height : '15px',
							formatString : "dd.MM.yyyy"
						});

						$('#accountDetailsToWindow').jqxWindow({
							width : '51%',
							height : '58%',
							maxHeight : '70%',
							maxWidth : '51%',
						...
						$('#accountDetailsFromWindow').jqxWindow({
							width : '51%',
							height : '58%',
							maxHeight : '70%',
							maxWidth : '51%',
						...
						$('#cashPaymentGridWindow').jqxWindow({
							width : '51%',
							height : '58%',
							maxHeight : '70%',
							maxWidth : '51%',
						...
						$('#costTypeSearchGridWindow').jqxWindow({
							width : '25%',
							height : '58%',
							maxHeight : '70%',
							maxWidth : '25%',
						...
						$('#costCodeSearchWindow').jqxWindow({
							width : '25%',
							height : '58%',
							maxHeight : '70%',
							maxWidth : '25%',
								...
								.on(
										...
										function(event) {
											...
											if (parseInt(validdate) == 0) {
												document
														.getElementById("errormsg").innerText = "Transaction prior or after Account Period is not valid.";
												return 0;
											}
										});
						...
						$('#txtfromaccid')
								.dblclick(
										function() {
											var date = $('#jqxCashPaymentDate')
													.jqxDateTimeInput('getDate');
											$("#maindate").jqxDateTimeInput(
													'val', date);
											accountFromSearchContent(
<%=contextPath + "/"%>
	+ "com/finance/accountsDetailsSearch.jsp?date="
													+ date);
										});
						...
						$('#txttoaccid')
								.dblclick(
										function() {
											...
											$("#maindate").jqxDateTimeInput(
													'val', date);
											accountToSearchContent(
<%=contextPath + "/"%>
	+ "com/finance/clientAccountDetailsSearch.jsp?atype="
													+ $('#cmbtotype').val()
													+ "&date=" + date);
										});
					});
	...
	function funwarningopen() {
		$.messager
				.confirm(
						'Confirm',
						'Transaction will affect Links to the applied Bank Reconcilations & Prepayments.',
						function(r) {
							if (r) {
								$("#mode").val("EDIT");
								$('#txtfromaccid').attr('readonly', true);
								$('#txtfromaccname').attr('readonly', true);
								$('#txtfromamount').attr('readonly', false);
								$('#txtdescription').attr('readonly', false);
								$('#txttoaccid').attr('readonly', true);
								$('#txttoaccname').attr('readonly', true);
								$('#txttoamount').attr('readonly', false);
								$('#txtfromrate').attr('readonly', false);
								$('#txtfrombaseamount').attr('readonly', true);
								$('#txttorate').attr('readonly', false);
								$('#txttobaseamount').attr('readonly', true);
								$('#txtapplyinvoiceamt').attr('readonly', true);
								$('#txtapplyinvoiceapply').attr('readonly',
										true);
								$('#txtapplyinvoicebalance').attr('readonly',
										true);
								$('#txtdrtotal').attr('readonly', true);
								$('#txtcrtotal').attr('readonly', true);
								$('#frmCashPayment select').attr('disabled',
										false);
								$("#jqxApplyInvoicing").jqxGrid({
									disabled : false
								});
			...
			if (parseFloat(txttoamount.value) < parseFloat(txttoamount.oldvalue)) {
				...
				$.messager
						.alert(
								'Message',
								'Applied for '
										+ txttoamount.oldvalue
										+ '/-, Remove applied & then change the Payment Amount.',
								'warning');
				return;
			}
		}
	...
	function funRemoveReadOnly() {
		...
		if ($("#mode").val() == "E") {
			...
			$('#txtdescription').attr('readonly', false);
			$("#jqxCashPayment").jqxGrid('addrow', null, {
				"docno" : "",
				"type" : "",
				"accounts" : "",
				"accountname1" : "",
	...
	function funNotify() {
		...
		if (drtot == "" || crtot == "" || drtot == "NaN" || crtot == "NaN"
				|| drtot == 0 || crtot == 0 || drtot == 0.0 || crtot == 0.0
				|| drtot == 0.00 || crtot == 0.00) {
		...
		for (var i = 0; i < rows.length; i++) {
			...
			if (typeof (chk) != "undefined" && typeof (chk) != "NaN"
					&& chk != "") {
				newTextBox = $(document.createElement("input")).attr("type",
						"dil").attr("id", "test" + length).attr("name",
						"test" + length).attr("hidden", "true");
				length = length + 1;
				if (rate == "" || rate == "0" || rate == "0.00") {
					val = 1;
					break;
				...
				newTextBox.val(rows[i].docno + "::" + rows[i].currencyid + "::"
						+ rows[i].rate + "::" + rows[i].dr + "::" + amount
						+ "::" + rows[i].description + "::" + baseamount
						+ "::0:: " + rows[i].costtype + ":: "
						+ rows[i].costcode);
		...
		for (var i = 0; i < rows.length; i++) {
			var chks = $("#jqxApplyInvoicing").jqxGrid('getcelltext', i,
					'applying');
			var balance = rows[i].balance;
			if (typeof (chks) != "undefined" && typeof (chks) != "NaN"
					&& chks != "") {
				newTextBox = $(document.createElement("input")).attr("type",
						"dil").attr("id", "txtapply" + lengthapply).attr(
						"name", "txtapply" + lengthapply)
						.attr("hidden", "true");
				...
				newTextBox
						.val(rows[i].applying
								+ "::"
								+ (parseFloat(rows[i].out_amount) + parseFloat(rows[i].applying))
								+ "::" + rows[i].currency + "::"
								+ rows[i].tranid + "::" + rows[i].acno);
				newTextBox.appendTo('form');
			}
		}
		if (val2 == 1) {
		...
		for (var i = 0; i < rows.length; i++) {
			...
			if (typeof (chkd) != "undefined" && typeof (chkd) != "NaN"
					&& chkd != "") {
				newTextBox = $(document.createElement("input")).attr("type",
						"dil").attr("id", "txtapplyupdate" + lengthupdate)
						.attr("name", "txtapplyupdate" + lengthupdate).attr(
								"hidden", "true");
				lengthupdate = lengthupdate + 1;

				newTextBox.val(parseFloat(rows[i].out_amount)
						- parseFloat(rows[i].applying) + "::" + rows[i].tranid);
				newTextBox.appendTo('form');
			}
		}
	...
	function setValues() {
		...
		document.getElementById("cmbtotype").value = document
				.getElementById("hidcmbtotype").value;

		if ($('#hidjqxCashPaymentDate').val()) {
			$("#jqxCashPaymentDate").jqxDateTimeInput('val',
					$('#hidjqxCashPaymentDate').val());
		}
		...
		if (indexVal > 0) {
			var check = 1;
			$("#jqxCashPaymentGrid").load(
					"cashPaymentGrid.jsp?txtcashpaydocno2=" + indexVal
							+ "&check=" + check);
		}
		...
		if (indexVal1 > 0) {
			var check = 1;
			$("#jqxApplyInvoicing1").load(
					"applyInvoicingGrid.jsp?txttoaccid1=" + indexVal1
							+ "&txttotrno1=" + indexVal2 + "&check=" + check);
		}
		...
		funRoundAmt($('#txtapplyinvoicebalance').val(),
				"txtapplyinvoicebalance");
		...
		if (!isNaN(toamount)) {
			...
			for (var i = 0; i <= rowlength - 1; i++) {
				...
				if (typeof (baseamount) != "undefined"
						&& typeof (baseamount) != "NaN" && baseamount != "") {
					if (value == true) {
						if (!isNaN(baseamount)) {
							dr = dr + baseamount;
	...
	function getAcc(event) {
		...
		if (x == 114) {
			...
			accountFromSearchContent(
<%=contextPath + "/"%>
	+ "com/finance/accountsDetailsSearch.jsp?date=" + date);
		} else {
		}
	}
	...
	function funPrintBtn() {
		if (($("#mode").val() == "view") && $("#docno").val() != "") {
			...
			$.messager
					.confirm(
							'Confirm',
							'Do you want to have header?',
							function(r) {

								var printUrl = reurl[0]
										+ "printCashPayment?docno="
...
<body onload="setValues();">
	<div id="mainBG" class="hidden-scrollbar homeContent"
		data-type="background">
		<form id="frmCashPayment" action="saveCashPayment" method="post"
			autocomplete="off">
			...
			<div class=' receipt-header'>
				<table width="100%">
					<tr>
						...
						<td width="11%"><div id="jqxCashPaymentDate"
								name="jqxCashPaymentDate" onchange="datechange();"
								onblur="datechange();"
								value='<s:property value="jqxCashPaymentDate"/>'></div> <input
							type="hidden" id="hidjqxCashPaymentDate"
							name="hidjqxCashPaymentDate"
							value='<s:property value="hidjqxCashPaymentDate"/>' /></td>
						<td width="21%" align="left">&nbsp;</td>
						<td width="9%" align="right">Ref. No.</td>
						<td width="29%"><input type="text" id="txtrefno"
							name="txtrefno" 40%;" value='<s:property value="txtrefno"/>' /></td>
						<td width="6%" align="right">Doc No.</td>
						<td width="21%"><input type="text" id="docno"
							name="txtcashpaydocno"
							50%;" value='<s:property value="txtcashpaydocno"/>' tabindex="-1" />
							<button class="myButton" type="button" id="btnvaluechange"
								name="btnvaluechange" onclick="funwarningopen();">Value
								Change</button></td>
					</tr>
					<tr>
						<td colspan="6"></td>
				...
				<div class="section-row">
					...
					<div class="section-block">
						...
						<div class="form-group single-label-dual-input">
							<label for="txtfromaccid">Cash</label> <input type="text"
								id="txtfromaccid" name="txtfromaccid"
								placeholder="Press F3 to Search"
								value='<s:property value="txtfromaccid"/>'
								onkeydown="getAcc(event);" /> <input type="text"
								id="txtfromaccname" name="txtfromaccname"
								value='<s:property value="txtfromaccname"/>' tabindex="-1" /> <input
								type="hidden" id="txtfromdocno" name="txtfromdocno"
								value='<s:property value="txtfromdocno"/>' />
						</div>
						<div class="form-group dual-input">
							<label for="cmbfromcurrency">Currency</label> <select
								id="cmbfromcurrency" name="cmbfromcurrency"
								value='<s:property value="cmbfromcurrency"/>'
								onload="getRate(this.value,$('#jqxCashPaymentDate').val());"
								onchange="getRate(this.value,$('#jqxCashPaymentDate').val());">
								<option></option>
							</select> <input type="hidden" id="hidcmbfromcurrency"
								name="hidcmbfromcurrency"
								value='<s:property value="hidcmbfromcurrency"/>' /> <input
								type="hidden" id="hidfromcurrencytype"
								name="hidfromcurrencytype"
								value='<s:property value="hidfromcurrencytype"/>' /> <label
								for="txtfromrate" style="margin-left: 18px;">Rate</label> <input
								type="text" id="txtfromrate" name="txtfromrate"
								onchange="funvalid()" value='<s:property value="txtfromrate"/>'
								onblur="funRoundRate(this.value,this.id);getBaseAmountFrom();getDrTotal();"
								tabindex="-1" /> <span id="validrate"></span>
						</div>
						<div class="form-group dual-input">
							<label for="txtfromamount">Amount</label> <input type="text"
							...
							<label for="txtfrombaseamount">Base Amount</label> <input
								type="text" id="txtfrombaseamount" name="txtfrombaseamount"
								value='<s:property value="txtfrombaseamount"/>' tabindex="-1" />
						</div>
						<div class="form-group">
							<label for="txtdescription">Description</label> <input
								type="text" id="txtdescription" name="txtdescription"
					...
					<div class="section-block">
						...
						<div class="form-group to-account-row">
							<label for="cmbtotype">Type</label> <select id="cmbtotype"
								name="cmbtotype" onchange="clearClientInfo();"
								value='<s:property value="cmbtotype"/>'>
								...
								value='<s:property value="txttoaccname"/>' tabindex="-1" /> <input
								type="hidden" id="txttodocno" name="txttodocno"
								value='<s:property value="txttodocno"/>' /> <input type="hidden"
								id="txttotranid" name="txttotranid"
								value='<s:property value="txttotranid"/>' /> <input
								type="hidden" id="txttotrno" name="txttotrno"
								value='<s:property value="txttotrno"/>' />
						</div>
						<div class="form-group dual-input">
							<label for="cmbtocurrency">Currency</label> <select
								id="cmbtocurrency" name="cmbtocurrency"
								value='<s:property value="cmbtocurrency"/>'
								onload="getRatevalue(this.value,$('#jqxCashPaymentDate').val());"
								onchange="getRatevalue(this.value,$('#jqxCashPaymentDate').val());">
								<option></option>
							</select> <input type="hidden" id="hidcmbtocurrency"
								name="hidcmbtocurrency"
								value='<s:property value="hidcmbtocurrency"/>' /> <input
								type="hidden" id="hidtocurrencytype" name="hidtocurrencytype"
								value='<s:property value="hidtocurrencytype"/>' /> <label
								for="txttorate" style="margin-left: 18px;">Rate</label> <input
								type="text" id="txttorate" name="txttorate"
								onchange="funvalid1()" value='<s:property value="txttorate"/>'
								onblur="funRoundRate(this.value,this.id);getBaseAmountTo();getDrTotal();"
								tabindex="-1" /> <span id="validrate1"></span>
						</div>
						<div class="form-group dual-input">
							<label for="txttoamount">Amount</label> <input type="text"
							...
							<label for="txttobaseamount" style="margin-left: 18px;">Base
								Amount</label> <input type="text" id="txttobaseamount"
								name="txttobaseamount"
				...
				<%--    <td width="22%"><select id="cmbfromcurrency" name="cmbfromcurrency" 71%;" value='<s:property value="cmbfromcurrency"/>' onload="getRate(this.value,$('#jqxCashPaymentDate').val());" onchange="getRate(this.value,$('#jqxCashPaymentDate').val());">--%>
				...
				<%--    <td width="60%"><input type="text" id="txtfromrate" name="txtfromrate" 32%;text-align: right;" value='<s:property value="txtfromrate"/>' onchange="funvalid()" onblur="funRoundRate(this.value,this.id);getBaseAmountFrom();getCrTotal();" tabindex="-1"/>--%>
				<%--    <span id="validrate"></span>--%>
				<%--    </td>--%>
				<%--  </tr>--%>
				<%--  <tr>--%>
				<%--    <td align="right">Amount</td>--%>
				<%--    <td><input type="text" id="txtfromamount" name="txtfromamount" 70%;text-align: right;" value='<s:property value="txtfromamount"/>' onblur="funRoundAmt(this.value,this.id);getBaseAmountFrom();getCrTotal();" /></td>--%>
				...
				<%--  <tr>--%>
				<%--    <td align="right">Currency</td>--%>
				<%--    <td colspan="2"><select id="cmbtocurrency" name="cmbtocurrency" 50%;" value='<s:property value="cmbtocurrency"/>' onload="getRatevalue(this.value,$('#jqxCashPaymentDate').val());" onchange="getRatevalue(this.value,$('#jqxCashPaymentDate').val());">--%>
				<%--      <option></option></select>--%>
					...
					<div class="table-section">
						...
						<table width="100%">
							<tr>
								...
								<td width="24%"><input type="text" id="txtapplyinvoiceamt"
									name="txtapplyinvoiceamt" 50%;text-align:
									right;" value='<s:property value="txtapplyinvoiceamt"/>' /> <input
									type="hidden" id="txtvalidation" name="txtvalidation"
									value='<s:property value="txtvalidation"/>' /></td>
								<td width="5%" align="right">Applied</td>
								<td width="26%"><input type="text"
									id="txtapplyinvoiceapply" name="txtapplyinvoiceapply"
									50%;text-align:
									right;" value='<s:property value="txtapplyinvoiceapply"/>'
									tabindex="-1" /></td>
								<td width="10%" align="right">Balance</td>
								<td width="27%"><input type="text"
									id="txtapplyinvoicebalance" name="txtapplyinvoicebalance"
									50%;text-align:
									right;" value='<s:property value="txtapplyinvoicebalance"/>'
									tabindex="-1" /></td>
							</tr>
						</table>
				...
				<br />
				<div id="jqxCashPaymentGrid"><jsp:include
						page="cashPaymentGrid.jsp"></jsp:include></div>
				<br />
				<table width="100%">
					<tr>
						<td width="7%" align="right">Dr. Total</td>
						<td width="68%"><input type="text" id="txtdrtotal"
							name="txtdrtotal" value='<s:property value="txtdrtotal"/>' /></td>
						<td width="6%" align="right">Cr. Total</td>
						<td width="19%"><input type="text" id="txtcrtotal"
							name="txtcrtotal" value='<s:property value="txtcrtotal"/>'
							tabindex="-1" /></td>
					</tr>
				</table>
				...
				<input type="hidden" id="msg" name="msg"
					...
					name="txtforsearch" id="txtforsearch" value="0" />
				<div hidden="true" id="maindate" name="maindate"
					value='<s:property value="maindate"/>'></div>
				<input type="hidden" id="hidmaindate" name="hidmaindate"
					value='<s:property value="hidmaindate"/>' /> <input type="hidden"
					id="gridlength" name="gridlength" /> <input type="hidden"
					id="applylength" name="applylength" /> <input type="hidden"
					id="applylengthupdate" name="applylengthupdate" /> <input
					type="hidden" id="hidstatus" name="hidstatus"
