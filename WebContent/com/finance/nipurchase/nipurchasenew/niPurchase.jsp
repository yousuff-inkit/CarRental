	 ...
	 if(x==114){
	  ...
	  function funFocus(){
			   ...
			   newTextBox.appendTo('form');
				...
				if(parseInt(chk)==1)
				...
				else
					{
					 ...
					 document.getElementById("frmNipurchasenew").submit();
					}
		...
		function funSearchLoad(){
			...
			function funReset(){
				//$('#frmNipurchasenew')[0].reset(); 
			}
			function funReadOnly(){
				$('#frmNipurchasenew input').attr('readonly', true );
				$('#frmNipurchasenew select').attr('disabled', true );
			...
			function funRemoveReadOnly(){
				  funinterstate();
				$('#frmNipurchasenew input').attr('readonly', false );
				$('#frmNipurchasenew select').attr('disabled', false );
			...
			function getCurrencyIds(){
				...
				/*    function diserror(){
				    ...
				    function funPrintBtn(){
				        ...
				        var reurl=url.split("saveActionNipurchasenew");
				        
				        $("#docno").prop("disabled", false);                
				        var dtype=$('#formdetailcode').val();
						 var brhid=<%= session.getAttribute("BRANCHID").toString()%>
				  
				var win= window.open(reurl[0]+"printniphsnew?docno="+document.getElementById("masterdoc_no").value+"&dtype="+dtype+"&brhid="+brhid,"_blank","top=250,left=310,Width=800,Height=800,location=no,scrollbars=no,toolbar=yes");
...
<form id="frmNipurchasenew" action="saveActionNipurchasenew" method="post" autocomplete="off">
