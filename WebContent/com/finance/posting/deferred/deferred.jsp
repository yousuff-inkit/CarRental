...
<script type="text/javascript">
    ...
    function funReadOnly(){
		$('#frmdeferred input').attr('readonly', true );
		$('#frmdeferred select').attr('disabled', true);
	...
	function funRemoveReadOnly(){
	    $('#frmdeferred input').attr('readonly', false );
		$('#frmdeferred select').attr('disabled', false);
		 ...
		 //alert(type)
		  if(type!=0){
			  ...
			  $("#overlay, #PleaseWait").show();
		  	  $("#jqxPrePaymentGrid").load('deferredGrid.jsp?txttype='+type+'&accId='+accId+'&fromDate='+fromDate+'&toDate='+toDate+'&check='+check);
		  }
		 }
	...
	function funreloaddistributiongrid(){
	 ...
	 function funPrintSummary() {
			...
			if (($("#mode").val() == "A") && $("#cmbtype").val()=="2") {
				 ...
				 var win= window.open(reurl[0]+"printdeferred?tranid="+document.getElementById("txttranid").value+"&branch="+document.getElementById("brchName").value,"_blank","top=150,left=250,Width=1020,Height=500,location=no,scrollbars=no,toolbar=yes");
			     win.focus();
		     }
		    else {
	 ...
	 function funCalculate(){
		    ...
		    for (var i = 0; i < rows.length; i++) {
					if(selectedrows[j]==i){
						...
						$("#jqxJournalVoucherApplying").jqxGrid('setcellvalue', j+1, "debit", "");
						$("#jqxJournalVoucherApplying").jqxGrid('setcellvalue', j+1, "credit", $('#jqxPrePayment').jqxGrid('getcellvalue', i, "dramount"));
					...
					$("#jqxJournalVoucherApplying").jqxGrid('setcellvalue', 0, "debit", drtotal);
					$("#jqxJournalVoucherApplying").jqxGrid('setcellvalue', 0, "credit","");
					$("#jqxJournalVoucherApplying").jqxGrid('setcellvalue', 0, "description", ("DEFERRED FOR PERIOD "+$('#jqxFromDate').val()+" "+$('#jqxToDate').val()));
...
<form id="frmdeferred" action="savedeferred" method="post" autocomplete="off">
...
<div id="jqxPrePaymentGrid"><jsp:include page="deferredGrid.jsp"></jsp:include></div></fieldset>
...
<table width="100%">
  <tr>
    ...
    <input type="hidden" id="txttranid" name="txttranid" value='<s:property value="txttranid"/>'/>
        <input type="hidden" id="docno" name="docno" value='<s:property value="docno"/>'/></td>
  </tr>
  <tr>
