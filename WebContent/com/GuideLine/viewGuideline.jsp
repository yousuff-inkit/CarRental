...
<script type="text/javascript">
	...
	function changeVideoSource() {
        ...
        var links = document.getElementById('lbllink').value;
       
        var contextPath='<%=contextPath%>';
      if(links){
    	  $("#myVideo").show();

        link=contextPath+'/'+links;
        video.src = link;
        video.load(); 
	...
	function changePDFSource() {
	    ...
	    var links = document.getElementById('lbpdfllink').value;

	    var contextPath = '<%=contextPath%>';
	    
	    if (links) {
	        var link = contextPath + '/' + links +'#toolbar=0';
	        iframe.src = link;
	        iframe.style.display = 'block'; // show PDF viewer
	    } else {
	        iframe.style.display = 'none'; // hide PDF viewer
	    }
	}
...
<span class="subHeading">Guidelines</span><br/>
  ...
  <%int i=0; %>
	...
	<ul>  
     ...
     <video id="myVideo" width="380" height="240" style="margin-left:200px" controls>
        <source id="videoSource" type="video/mp4"></video></td> </tr>
	</table>
	 <input type="hidden" id="lbllink" name="lbllink" value='<s:property value="lbllink"/>'/>
	
</td>
  <td style="width: 50%; vertical-align: top;">
   
     <input type="hidden" id="lbpdfllink" name="lbpdfllink" value='<s:property value="lbpdfllink"/>'/>
<iframe id="pdfViewer" width="100%" height="600px" style="display: none; border: none;" scrolling="yes"></iframe>   
...
</td> <table width="95%" align="center" style="border-collapse: collapse;">
