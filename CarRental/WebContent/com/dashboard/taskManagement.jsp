...
function funDataCardLoad() {
    var userid = '<%= session.getAttribute("USERID").toString()%>';
    $.ajax({
        url: 'getTaskCardDetails.jsp?userid=' + userid,
        method: 'POST',
        dataType: 'text',
        success: function(response) {
        	var resp = response.split('####');
        	var ownobj1, otherobj1; 
        	if (resp.length === 2) {
        		ownobj1 = JSON.parse(resp[0]);
        		otherobj1 = JSON.parse(resp[1]);
        	}  
        	var assignedown = 0, acceptedown = 0, completedown = 0, assignedoth = 0, acceptedoth = 0, completedoth = 0, totalown = 0, totaloth = 0;  
        	
        	if(typeof(ownobj1.Assigned)=="undefined" || typeof(ownobj1.Assigned)=="NaN" || ownobj1.Assigned=="" || ownobj1.Assigned==null){
        		assignedown = 0;
        	}else{
        		assignedown = ownobj1.Assigned;   
        		totalown+= parseInt(assignedown)
        	}
        	
        	if(typeof(ownobj1.Accepted)=="undefined" || typeof(ownobj1.Accepted)=="NaN" || ownobj1.Accepted=="" || ownobj1.Accepted==null){
        		acceptedown = 0;
        	}else{
        		acceptedown = ownobj1.Accepted;   
        		totalown+= parseInt(acceptedown)
        	}
        	
        	if(typeof(ownobj1.Completed)=="undefined" || typeof(ownobj1.Completed)=="NaN" || ownobj1.Completed=="" || ownobj1.Completed==null){
        		completedown = 0;
        	}else{
        		completedown = ownobj1.Completed;    
        		totalown+= parseInt(completedown)
        	}
        	
        	if(typeof(otherobj1.Assigned)=="undefined" || typeof(otherobj1.Assigned)=="NaN" || otherobj1.Assigned=="" || otherobj1.Assigned==null){
        		assignedoth = 0;
        	}else{
        		assignedoth = otherobj1.Assigned;   
        		totaloth+= parseInt(assignedoth)
        	}
        	
        	if(typeof(otherobj1.Accepted)=="undefined" || typeof(otherobj1.Accepted)=="NaN" || otherobj1.Accepted=="" || otherobj1.Accepted==null){
        		acceptedoth = 0;
        	}else{
        		acceptedoth = otherobj1.Accepted;   
        		totaloth+= parseInt(acceptedoth)
        	}
        	
        	if(typeof(otherobj1.Completed)=="undefined" || typeof(otherobj1.Completed)=="NaN" || otherobj1.Completed=="" || otherobj1.Completed==null){
        		completedoth = 0;
        	}else{
        		completedoth = otherobj1.Completed;    
        		totaloth+= parseInt(completedoth)
        	}
        	
            const taskCard1 = document.querySelector('.task-card-1');
            const taskCard2 = document.querySelector('.task-card-2');
           
            taskCard1.setAttribute('data-number', totalown);  
            taskCard1.querySelector('[data-label="New"]').setAttribute('data-count', assignedown);
            taskCard1.querySelector('[data-label="Accepted"]').setAttribute('data-count', acceptedown);
            taskCard1.querySelector('[data-label="Completed"]').setAttribute('data-count', completedown);

            taskCard2.setAttribute('data-number', totaloth);
            taskCard2.querySelector('[data-label="New"]').setAttribute('data-count', assignedoth);
            taskCard2.querySelector('[data-label="Accepted"]').setAttribute('data-count', acceptedoth);
            taskCard2.querySelector('[data-label="Completed"]').setAttribute('data-count', completedoth);       
        },
...
function cardModalOpenOwn(type){
	 var usertype = "own";
	 var userid = '<%= session.getAttribute("USERID").toString()%>';  
	 $("#pnddiv").load("pendingtaskGrid.jsp?userid="+userid+"&type="+type+"&usertype="+usertype+"&id="+2); 
	 funChangeStatus();
	 $("#taskfollowupGrid").jqxGrid('clear');
 	 $("#modalpendingtask").modal('show');
}

function cardModalOpenOth(type){
	 var usertype = "other";  
	 var userid = '<%= session.getAttribute("USERID").toString()%>';  
	 $("#pnddiv").load("pendingtaskGrid.jsp?userid="+userid+"&type="+type+"&usertype="+usertype+"&id="+2); 
	 funChangeStatus();
...
<div id="pendDiv">
	<div class="task-container">
	    <div class="task-card task-card-1" data-number="00">
	        <h4>My Tasks To Do</h4>  
	        <div class="task-details">
	            <div class="task-deadline" data-label="New" data-count="0" onclick="cardModalOpenOwn(this.getAttribute('data-label'))"></div>
	            <div class="task-deadline" data-label="Accepted" data-count="0" onclick="cardModalOpenOwn(this.getAttribute('data-label'))"></div>
	            <div class="task-deadline" data-label="Completed" data-count="0" onclick="cardModalOpenOwn(this.getAttribute('data-label'))"></div>
	        </div>
	    </div>
	    <div class="task-card task-card-2" data-number="00">
	        <h4>Tasks Assigned To Others</h4>    
	        <div class="task-details">
	            <div class="task-deadline" data-label="New" data-count="0" onclick="cardModalOpenOth(this.getAttribute('data-label'))"></div>
	            <div class="task-deadline" data-label="Accepted" data-count="0" onclick="cardModalOpenOth(this.getAttribute('data-label'))"></div>
	            <div class="task-deadline" data-label="Completed" data-count="0" onclick="cardModalOpenOth(this.getAttribute('data-label'))"></div>     
	        </div>
	    </div>
	</div>  
</div>           
