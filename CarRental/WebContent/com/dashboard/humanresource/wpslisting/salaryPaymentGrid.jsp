<%@page import="com.dashboard.humanresource.wpslisting.ClsWpsListingDAO"%>
<%
	ClsWpsListingDAO DAO= new ClsWpsListingDAO();
%> 
<%   String branchval = request.getParameter("branchval")==null?"NA":request.getParameter("branchval").trim();
	 ...
	 String employeebranchchk = request.getParameter("employeebranchchk")==null?"0":request.getParameter("employeebranchchk").trim();
	 %>
...
<script type="text/javascript">
      ...
      //alert(temp);
	    ...
	    if(statusid=="1"){
	  	...
	  	if(temp1!='NA'){  
	  		    data1='<%=DAO.salaryPaymentGridLoading(branchval, year, month, category, agent, statusID, establishmentCode, check , employeebranchchk)%>';  
	  		  
	  		    dataexcel='<%=DAO.salaryPaymentGridLoadingexcel(branchval, year, month, category, agent, statusID, establishmentCode, check , employeebranchchk)%>';
	  		    
	  		    dataSIF='<%=DAO.salaryPaymentGridSifExport(branchval, year, month, category, agent, statusID, establishmentCode, bankName, bankAccount, paymentDate, check , employeebranchchk)%>';
