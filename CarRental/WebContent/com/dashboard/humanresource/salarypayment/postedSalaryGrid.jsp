...
<script type="text/javascript">
	...
	if(temp!='NA') {
		data= '<%=DAO.postedSalaryGridLoading(branchval, year, month, category, agent, establishmentCode, check, employeebranchchk)%>';
	}
	...
	$('#postedSalaryGridID').on('rowdoubleclick', function (event) {
		 ...
		 $("#salaryPaymentDetailsDiv").load("salaryPaymentGrid.jsp?branchval="+branchval+'&year='+year+'&month='+month+'&category='+category+'&agent='+agent+'&statusID='+statusID+'&establishmentCode='+establishmentcode.replace(/ /g, "%20")+'&bankAccount='+bankaccount.replace(/ /g, "%20")+'&bankName='+bankaccountname.replace(/ /g, "%20")+'&paymentdate='+date+'&check=1&employeebranchchk='+employeebranchchk);
