...
String startkm=request.getParameter("startkm")==null||request.getParameter("startkm").equalsIgnoreCase("")||request.getParameter("startkm").equalsIgnoreCase("NaN")?"0":request.getParameter("startkm");
String enddate=request.getParameter("enddate")==null?"":request.getParameter("enddate");
String endtime=request.getParameter("endtime")==null?"":request.getParameter("endtime");
String endkm=request.getParameter("endkm")==null||request.getParameter("endkm").equalsIgnoreCase("")|| request.getParameter("endkm").equalsIgnoreCase("NaN")?"0":request.getParameter("endkm");
...
String modtariff=request.getParameter("tariff")==null || request.getParameter("tariff").equalsIgnoreCase("undefined") || request.getParameter("tariff").equalsIgnoreCase("")?"0.0":request.getParameter("tariff");
String modnighttariff=request.getParameter("nighttarif")==null || request.getParameter("nighttarif").equalsIgnoreCase("undefined") || request.getParameter("nighttarif").equalsIgnoreCase("")?"0.0":request.getParameter("nighttarif");
String modexcesskmchg=request.getParameter("excesskmchg")==null || request.getParameter("excesskmchg").equalsIgnoreCase("undefined") || request.getParameter("excesskmchg").equalsIgnoreCase("")?"0.0":request.getParameter("excesskmchg");
String modexcesshrchg=request.getParameter("excesshrchg")==null || request.getParameter("excesshrchg").equalsIgnoreCase("undefined") || request.getParameter("excesshrchg").equalsIgnoreCase("")?"0.0":request.getParameter("excesshrchg");
String modexcessnighthrchg=request.getParameter("excessnighthrchg")==null || request.getParameter("excessnighthrchg").equalsIgnoreCase("undefined") || request.getParameter("excessnighthrchg").equalsIgnoreCase("")?"0.0":request.getParameter("excessnighthrchg");
String waitingchg=request.getParameter("waitingchg")==null || request.getParameter("waitingchg").equalsIgnoreCase("undefined") || request.getParameter("waitingchg").equalsIgnoreCase("")?"0.0":request.getParameter("waitingchg");

System.out.println(modexcessnighthrchg+" - "+modexcesshrchg+" - "+modexcesskmchg+" - "+modnighttariff+"::"+bookdocno+"::"+jobdocno+"::"+jobname+"::"+tarifdocno+"::"+tarifdetaildocno);
...
try{
	...
	// written to prevent error when external vendor is assigned there will be no end date and time which might throw errors
	String isVendorSQL="";
	int venid=0;
	if(jobtype.equalsIgnoreCase("Transfer")){
		isVendorSQL = "SELECT assignedvendor FROM gl_limobooktransfer WHERE bookdocno="+bookdocno+" AND docname='"+jobname+"'";
	} else if(jobtype.equalsIgnoreCase("Limo")){
		isVendorSQL = "SELECT assignedvendor FROM gl_limobookhours WHERE bookdocno="+bookdocno+" AND docname='"+jobname+"'";
	}
	ResultSet rsven=stmt.executeQuery(isVendorSQL);
	while(rsven.next()){
		venid=rsven.getInt("assignedvendor");
	}
			
	System.out.println(enddate+" - "+endtime);
			
	if(venid>0 && (enddate==null||enddate.equalsIgnoreCase("null")|| enddate.equalsIgnoreCase("")||enddate.equalsIgnoreCase("undefined"))){
		enddate=startdate;
		endtime=starttime;
	}
	System.out.println(enddate+" - "+endtime+" - "+endkm+" - "+startkm);
	// end of edit
	
	int isHideTariffConfig = 0;
	String sqlConfig = "SELECT method FROM gl_config WHERE field_nme='hidetariff'";
	ResultSet rsConfig=stmt.executeQuery(sqlConfig);
	if(rsConfig.next()){
		isHideTariffConfig=rsConfig.getInt("method");
	}
	...
	if(isHideTariffConfig==1) {
		...
		if(jobtype.equalsIgnoreCase("Transfer")){
			exkmrate =  Double.parseDouble(modexcesskmchg);
			extimerate =  Double.parseDouble(modexcesshrchg);
		} else if(jobtype.equalsIgnoreCase("Limo")){
			exhourrate = Double.parseDouble(modexcesshrchg);
			nightexhourrate = Double.parseDouble(modexcessnighthrchg);
		}
	}
	...
	System.out.println("Hide Tariff - "+ isHideTariffConfig);
	if(isHideTariffConfig==1) {
		tarif = Double.parseDouble(modtariff);
		extrahouramt+=Double.parseDouble(waitingchg);
		if(jobtype.equalsIgnoreCase("Limo")) nighttarif = Double.parseDouble(modnighttariff);
	}
