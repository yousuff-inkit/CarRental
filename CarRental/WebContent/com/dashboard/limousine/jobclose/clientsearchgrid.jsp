...
%> 
 ...
 <script type="text/javascript">
 		...
 		$(document).ready(function () { 
 			var urlclient='getclienturl.jsp?clname=<%=clname%>&mob=<%=mob%>&lcno=<%=lcno%>&clientid=<%=clientid%>&driverid=<%=driverid%>&nation=<%=nation%>&dob=<%=dob%>&clientaccount=<%=clientaccount%>&check=<%=check%>';
        	var source = 
            {
                datatype: "json",
                datafields: [
            ...
            $('#jqxclientsearchgrid').on('rowdoubleclick', function (event) {
                var rowindex1=event.args.rowindex;
                var rowindex2 =$('#rowindex').val();
				//funReset();
                document.getElementById("clientid").value= $('#jqxclientsearchgrid').jqxGrid('getcellvalue', rowindex1, "clientid");
                document.getElementById("client").value= $('#jqxclientsearchgrid').jqxGrid('getcellvalue', rowindex1, "refname");
    ...
    <div id="jqxclientsearchgrid"></div>
