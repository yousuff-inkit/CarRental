...
<body >
  ...
  <div class="container-fluid">
    	<div class="row rowgap">
      		<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
        		...
        		<div class="detailpanel custompanel">
          			<button type="button" class="btn btn-default" id="btnpost" ><i class="fa fa-exchange" aria-hidden="true" data-toggle="tooltip" title="Export" data-placement="bottom"></i></button>          			
           		</div>
    	...
    	<div class="row">
      		<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
        		<div id="limoinvoiceexpdiv"><jsp:include page="limoinvoiceexportgrid.jsp"></jsp:include></div>
      		</div>
    	</div>
  </div>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('#btnpost').click(function(){
        	var brhid=$('#cmbbranch').val();
            var selectedRowIndexes = $('#jqxlimoinvoiceexportgrid').jqxGrid('getselectedrowindexes');
            if (selectedRowIndexes.length === 0) {
                Swal.fire({
                    icon: 'warning',
                    title: 'No rows selected',
                    text: 'Please select at least one row before proceeding!',
                    confirmButtonText: 'OK'
                });
            }
            const finalArray = [];
            selectedRowIndexes.forEach(index => {
                const rowData = $('#jqxlimoinvoiceexportgrid').jqxGrid('getrowdata', index);
                const selectedColumns = {
                    docno: rowData.doc_no,
                    trno: rowData.trno
                };
                if(!rowData.invdocno>0){
	                finalArray.push(selectedColumns);
                }
            });
            Swal.fire({
	    		  title: "Are you sure?",
	    		  text: "Do you want to export invoice?",
	    		  icon: "warning",
	    		  ...
	    		  if (result.isConfirmed) {
	    			$('.page-loader').show();
		            exportLimoInvoiceData(finalArray);
	    		  }
	    		});
        });
        ...
        $('#btnexcel').click(function(){
        	$("#jqxlimoinvoiceexportgrid").excelexportjs({
				containerid: "jqxlimoinvoiceexportgrid",
				datatype: 'json',
				dataset: null,
				gridId: "jqxlimoinvoiceexportgrid",
				columns: getColumns("jqxlimoinvoiceexportgrid"),
				worksheetName: "Limousine Invoice Export Data"
			});
        });
    }); 
	...
	function loadGrid(){
		...
		$("#limoinvoiceexpdiv").load("limoinvoiceexportgrid.jsp?brchid="+brchid+"&fromdate="+fromdate+"&todate="+todate+"&chk=1");
	}
	...
	function exportLimoInvoiceData(selectedRowsData){
		var brchid=$('#cmbbranch').val();
		...
		x.open("POST","exportlimodata.jsp?brchid="+brchid,true);
		x.send(JSON.stringify(selectedRowsData));
	}
