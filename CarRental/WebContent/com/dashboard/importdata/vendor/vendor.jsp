...
<body >
    ...
    <div class="row">
      <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
        <div id="vendordiv" class="borderStyle"><jsp:include page="vendorGrid.jsp"></jsp:include></div>
      </div>
    </div>
...
<script type="text/javascript">
   ...
   $("#btnconfirm").attr({'disabled':true});
		...
		$('#btnexcel').click(function(){
			$("#vendordiv").excelexportjs({  
				containerid: "vendordiv",
				datatype: 'json',
				dataset: null,
				gridId: "vendorGrid",
				columns: getColumns("vendorGrid"),
				worksheetName: "Vendor Master "
			});
		});
       ...
       upload();
        ...
        $('#btncategory').click(function(){
        	var type = "Category";
        	populateSelectBox('getVendorData.jsp?type='+type, 'cmbcategory');    
	...
	function funRoundAmt(value,id){
		...
		function ajaxFileUpload(docNo) {  
   		   ...
   		   */
	          ...
	          (  
	              {  
	                  url:'fileAttachAction.action?formCode=VDR&doc_no='+docNo+'&descpt=Excel Import' ,
	                  secureuri:false,  
			...
			Swal.fire({
				...
				confirmButtonText: 'Yes'
					...
					if (result.isConfirmed) {
						saveVendorData("update", category, temptrno, type, modalid);  
					}
			});  
		...
		function funCategoryDataSave(type, modalid){   
		    ...
		    Swal.fire({
				...
				confirmButtonText: 'Yes'
					...
					if (result.isConfirmed) {  
						saveVendorData("save", "", temptrno, type, modalid);    
					}else{
					}
			});     
		}
		
		function saveVendorData(process, docno, temptrno, type, modalid){         
			var x=new XMLHttpRequest();
			x.onreadystatechange=function(){
			if (x.readyState==4 && x.status==200) {   
					var items=x.responseText.trim();
			...
			x.open("GET","saveVendorData.jsp?process="+process+"&type="+type+"&docno="+docno+"&dataarr="+encodeURIComponent(temptrno),true);       
			x.send();
		} 
		...
		function Saveconfirm(process){
			...
			x.open("GET","saveVendorData.jsp?type="+process,true);       
			x.send();
		}
