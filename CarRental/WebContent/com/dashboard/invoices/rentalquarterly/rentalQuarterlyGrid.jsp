 ...
 <%
 ClsRentalQuarterlyInvDAO rentaldao=new ClsRentalQuarterlyInvDAO();%>
...
{ 
	if(mode1=="1"){
		...
		invoicedata='<%=rentaldao.getRentalQuarterlyInvoice(temp,desc1,date1,branch,client,mode)%>';	 
	}
}
$(document).ready(function () {
    ...
    $("#rentalQuarterlyGrid").on("bindingcomplete", function (event) {
    	$("#overlay, #PleaseWait").hide();
    	if(mode1=="1"){
    		$('#btninvoicesave').attr('disabled',false);
    	}
    	getNoSalikConfig();
//    	$('#rentalQuarterlyGrid').jqxGrid({ sortable: true});
    	});
    ...
    $("#rentalQuarterlyGrid").jqxGrid(
    {
       ...
       sortable:false,
        columns: [
						...
						{ text: 'Ac Name', datafield: 'acname', width: '10%'  ,cellclassname: cellclassname},
...
$('#rentalQuarterlyGrid').on('rowselect', function (event) 
    		{
    		    // event arguments.
    		    var args = event.args;
    		    // row's bound index.
    		    ...
    		    var todate=$('#rentalQuarterlyGrid').jqxGrid('getcellvalue',rowBoundIndex, "todate");
    		  	var docdateval=funDateInPeriodNew(todate);
    		    if(docdateval==0){
    		    	$('#rentalQuarterlyGrid').jqxGrid('unselectrow', rowBoundIndex);
    		    }
    		    var duedatecal=$('#rentalQuarterlyGrid').jqxGrid('getcellvalue',rowBoundIndex, "duedatecal");
    		    if(duedatecal==1){
    		    	$('#rentalQuarterlyGrid').jqxGrid('unselectrow', rowBoundIndex);
   ...
   /* if(document.getElementById("btninvoicesave").style.display=="block"){
	   $('#rentalQuarterlyGrid').jqxGrid({ height: 535 });
   } */
});
...
<div id="rentalQuarterlyGrid"></div>