...
<script type="text/javascript">   
    $(document).ready(function(){  
        ...
        $('#btnchangesaleman').click(function(){      
			...
			for(var i=0;i<selectedrows.length;i++){
						var cntno = $('#jqxContractGrid').jqxGrid('getcellvalue',selectedrows[i],'cldocno');
    ...
    function funUpdate(processid){   
		if(processid=="Salesman"){
			...
			for(var i=0;i<selectedrows.length;i++){
						if(i==0){
							docsarray+=$('#jqxContractGrid').jqxGrid('getcellvalue',selectedrows[i],'cldocno');
						}else{
							docsarray+=":"+$('#jqxContractGrid').jqxGrid('getcellvalue',selectedrows[i],'cldocno');
						}
			}  
		...
		if (x.readyState==4 && x.status==200){  
				var items=x.responseText;
					 if(parseInt(items)>0){  	  
								 ...
								 $("#modalsalesmanupdate").modal('hide');         
						}else{
								 swal.fire({  
									    icon: 'warning',
										title: 'Warning',
