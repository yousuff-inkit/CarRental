...
<script type="text/javascript">
	...
	function funValidateUsername() {
		...
		if($("#multicheck").val()<=0){
			...
			var selectedrows = $("#clientPortalGridM").jqxGrid('getselectedrowindexes');
		     		...
		     		if($("#multicheck").val()<=0){
						...
						x.send();	
		     			...
		     			for(var i=selectedrows.length-1; i>=0; i--){
		     			    cldocno = $("#clientPortalGridM").jqxGrid('getcellvalue',selectedrows[i],'cldocno');
		     				cldocnolist+=cldocno+"::"; 
		     			}
		     			funSaveClientDetailsM(clientusername,clientpassword,chksendmail,cldocnolist);
		     		}
	...
	function funreload(event){
		 ...
		 if($("#multicheck").val()<=0){
			...
			$("#clientPortalDiv").load("clientPortalGridM.jsp?id=1");
		 }
	}
	...
	function funSaveClientDetailsM(clientusername,clientpassword,chksendmail,cldocnolist){
		var x=new XMLHttpRequest();
		x.onreadystatechange=function(){
			if (x.readyState==4 && x.status==200){
				var items=x.responseText.trim();
		...
		x.open("GET","saveClientDetailsM.jsp?clientusername="+clientusername+"&clientpassword="+clientpassword+"&chksendmail="+chksendmail+"&cldocnolist="+cldocnolist,true);
		x.send();
	}
	
	
	function funRemoveClientDetails(chksendmail,cldocnolist){
		var x=new XMLHttpRequest();
		x.onreadystatechange=function(){
			if (x.readyState==4 && x.status==200){
				var items=x.responseText.trim();
				...
				if(items=="0"){
					$('#clientusername,#clientpassword').val('');
					document.getElementById("chksendmail").checked=false;
					$.messager.alert('Message','Successfully Deleted','Message');
					funreload("");
				}
				else{
					$.messager.alert('Warning','Not Deleted','Warning');
					return false;
				}
			}
		}
		x.open("GET","removeClientDetails.jsp?chksendmail="+chksendmail+"&cldocnolist="+cldocnolist,true);
		x.send();
	}
	...
	function multiplecheckconfig(){
		...
		if (x.readyState==4 && x.status==200)
	     ...
	     x.open("GET",'multiCheckConfig.jsp',true);
	     x.send();   
	}
	
	function funremoveidp(){
		var chksendmail=0;
		if(document.getElementById("chksendmail").checked==true){
			chksendmail=1;
		}
		 ...
		 $.messager.confirm('Message', 'Do you want to delete the username and password?', function(r){
		     	...
		     	if(r==false)
		     	  {
		     		...
		     		var cldocnolist=""; 
	     			for(var i=selectedrows.length-1; i>=0; i--){
	     				var cldocno = $("#clientPortalGridM").jqxGrid('getcellvalue',selectedrows[i],'cldocno');
	     				cldocnolist+=cldocno+"::"; 
	     			}
	     			funRemoveClientDetails(chksendmail,cldocnolist);
		     	}
		});
	}	
</script>
</head>
<body onload="getBranch();multiplecheckconfig();">
	<form autocomplete="off">
		<div id="mainBG" class="homeContent" data-type="background"> 
			<div class='hidden-scrollbar'>
								...
								<table width="100%" >
									<jsp:include page="../../heading.jsp"></jsp:include>
	     							<tr><td colspan="2"><hr></td></tr>
		 	 						<!-- <tr>
	     							<td colspan="2"><input type="radio" id="upbtn" name="rbutton"><label class="branch" for="upbtn" id="lupbtn">Username & password</label>&nbsp;&nbsp;<input type="radio" id="pbtn" name="rbutton"><label class="branch" for="pbtn" id="lpbtn">password</label>&nbsp;&nbsp;<input type="radio" id="rbtn" name="rbutton"><label class="branch" for="rbtn" id="lrbtn">remove</label></td>
	     							</tr> -->
		 	 						<tr>
		 	 						...
		 	 						<tr><td colspan="2" align="center"><button type="button" name="btnsave" id="btnsave" class="myButton" onClick="funValidateUsername();">Save</button>&nbsp;&nbsp;<button type="button" name="btnremove" id="btnremove" class="myButton" onClick="funremoveidp();">Delete</button></td></tr>
				...
				<input type="hidden" id="multicheck" name="multicheck">
			</div>
		</div>
	</form>
</body>
