...
ClsCommon ClsCommon=new ClsCommon();
	...
	try{
		...
		System.out.println("Expense Account : "+expenseaccount);
		...
		if(amount>0){
			salikarray.add(salikaccount+":: Staff-Allocated Damage is Passed as JVT on "+sqlDate+":: "+currency+":: "+rate+":: "+(amount)*-1+":: "+(amount)*rate*-1+"::"+1+":: -1:: 6:: "+fleetno); //Salik Account
			salikarray.add(expenseaccount+":: Staff-Allocated Damage is Passed as JVT on "+sqlDate+":: "+currencyexp+"::"+rateexp+":: "+(amount)+":: "+(amount)*rateexp+":: "+2+":: 1::0 ::0 "); //Expense Account			
		System.out.println("Amount : "+salikarray);
		}
		if(accfines>0){
			salikarray.add(salikaccount+":: Staff-Allocated Fines is Passed as JVT on "+sqlDate+":: "+currency+":: "+rate+":: "+(accfines)*-1+":: "+(accfines)*rate*-1+"::"+1+":: -1:: 6:: "+fleetno); //Salik Account
			System.out.println("Acc Fine 1: "+salikarray);
			salikarray.add(expenseaccount+":: Staff-Allocated Fines is Passed as JVT on "+sqlDate+":: "+currencyexp+"::"+rateexp+":: "+(accfines)+":: "+(accfines)*rateexp+":: "+2+":: 1::0 ::0 "); //Expense Account
			System.out.println("Acc Fine 2 : "+salikarray);
		...
		docno=jvt.insert(sqlDate, "JVT".concat("-7"), "0"+"-"+fleetno, desc, totalamount, totalamount, salikarray, session, request);
		int jvno=docno;
