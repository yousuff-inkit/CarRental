...
<%	
	...
	try{
		...
		if(btnclick.equalsIgnoreCase("approve")){
			...
			Date baseinvdate=sdf.parse(sqloutDate+"");
			Date baseinvtodate=sdf.parse(sqloutDate+"");
			Calendar calinvtodate=Calendar.getInstance();
			calinvtodate.setTime(baseinvdate);
			
			//System.out.println("Initial:"+calinvtodate.getTime());
			java.sql.Date sqloutdate=null;
			int sqloutday=0;
			...
			if(delcal==0){
				String strdel="select if(delivery=1 and delstatus=1,(select min(din) from gl_vmove where rdocno="+rdocno+" and rdtype='RAG' and "+
						 " trancode='DL' and repno=0  and exchng=0 and custno=0 ),odate) odate,if(delivery=1 and delstatus=1,(select day(min(din)) from gl_vmove where rdocno="+rdocno+" and rdtype='RAG' and "+
						 " trancode='DL' and repno=0 and exchng=0 and custno=0 ),day(odate)) outday from gl_ragmt where doc_no="+rdocno;
				//System.out.println(strdel);
				ResultSet rsdel=stmt.executeQuery(strdel);
				while(rsdel.next()){
					sqloutdate=rsdel.getDate("odate");
					sqloutday=rsdel.getInt("outday");
				}
			}
			else{
				String sqlgetoutdate="select odate,DAY(odate) outday from gl_ragmt where doc_no="+rdocno;
				//System.out.println(sqlgetoutdate);
				ResultSet rsoutdate=stmt.executeQuery(sqlgetoutdate);
				while(rsoutdate.next()){
					sqloutdate=rsoutdate.getDate("odate");
					sqloutday=rsoutdate.getInt("outday");
				}
				
			}
			
			String sqlmonthcal="select method,value from GL_CONFIG WHERE FIELD_NME='monthlycal'";
			ResultSet rsmonthcal=  stmt.executeQuery(sqlmonthcal);
			if(rsmonthcal.next()){
				methodval=rsmonthcal.getString("method"); 
				cofval=rsmonthcal.getInt("value"); 
			}
			
			if(invoicevalue.equalsIgnoreCase("1")){
				//Month End
				//System.out.println("Inside Month End");
				//calinvtodate.add(Calendar.MONTH, 1);
				calinvtodate.set(Calendar.DAY_OF_MONTH, 1);
				calinvtodate.set(Calendar.DAY_OF_MONTH, -1);
			}
			else if(invoicevalue.equalsIgnoreCase("2") || invoicevalue.equalsIgnoreCase("4") || invoicevalue.equalsIgnoreCase("5") || invoicevalue.equalsIgnoreCase("6")){
				//Period
				//System.out.println("Inside Period");
				//System.out.println(calinvtodate.getTime());
				if(Integer.parseInt(methodval)==1){
					calinvtodate.add(Calendar.MONTH, 1);	
				}
				else if(Integer.parseInt(methodval)==0){
					calinvtodate.add(Calendar.DAY_OF_MONTH, cofval);	
				}
				//System.out.println(calinvtodate.getTime());
				int maxDayMonth=calinvtodate.getActualMaximum(Calendar.DAY_OF_MONTH);
				
				//System.out.println(sqloutday+"::"+maxDayMonth);
				if(sqloutday>maxDayMonth){
					calinvtodate.set(Calendar.DAY_OF_MONTH, maxDayMonth);
				}
				else{
					calinvtodate.set(Calendar.DAY_OF_MONTH, sqloutday);
				}
				
				//System.out.println(calinvtodate.getTime());
			}
		
		   	upsql="update  gl_ragmt set invtodate='"+sdf.format(calinvtodate.getTime())+"',advchk='"+advance+"',  invtype='"+invoicevalue+"'   where doc_no='"+rdocno+"'  ";
			...
			//System.out.println("update======="+sqlapprv);
