...
<script type="text/javascript">
       ...
       var type='<%=type%>';
    	...
    	termdata='<%=showDAO.searchTermination(docnos)%>';  
    	console.log(typeof leavedata);
       ...
       var cellclassname = function (row, column, value, data) {
    	  ...
    	  if(parseInt(data.checkclick)==0){
			  ...
			  if(parseInt(data.srno)==parseInt(leaveid) && type==='TER') {
				return "saveClass";
			   }
		  } else {
    	        return "whiteClass"; 
		...
		$(document).ready(function () { 	
            ...
            $('#terminationgrid').on('cellvaluechanged',function (event) {
             ...
             if (rowindex1 == rowlength - 1) {
            	if(columnDataField !=='checkclick')
             	  $("#terminationgrid").jqxGrid('addrow', null, {btnsave:"Apply"});	
    	     }});	
            
            $("#terminationgrid").on('cellclick', function (event) {
                    ...
                    if(datafield=="btnsave"){
                    	...
                    	if($('#terminationgrid').jqxGrid('getcellvalue',rowBoundIndex, "btnsave")=="Apply"){
                 			var rows1 = $("#terminationgrid").jqxGrid('getrows');      
                      	 
                  	   		for(var i=0;i<rows1.length;i++){
                  		   		$("#terminationgrid").jqxGrid('setcellvalue', i , "checkclick", 0);
                  	   		}
                			...
                			var rows = $("#terminationgrid").jqxGrid('getrows');      
                	   		...
                	   		for(var i=0;i<rows.length;i++){
                           		...
                           		if(i==rowBoundIndex){
                        	   		...
                        	   		$("#terminationgrid").jqxGrid('setcellvalue', rowBoundIndex , "checkclick", 1); 
                 	 	...
                 	 	if($('#terminationgrid').jqxGrid('getcellvalue',rowBoundIndex, "btnsave")=="View"){
	                   		 $("#condtiongrid").jqxGrid({ disabled: false});	
	                    	 $('#condtiongrid').jqxGrid('refresh');
                	    	 $('#savebtn').attr('disabled', false);
                	         $('#deltbtn').attr('disabled', false);
                            ...
                            if(i==rowBoundIndex){
                        	   ...
                        	   $("#terminationgrid").jqxGrid('setcellvalue', rowBoundIndex , "checkclick", 1); 
                        	   $("#terminationgrid").jqxGrid('setcellvalue', rowBoundIndex , "btnsave", "Apply");
                        	   var srno=$('#terminationgrid').jqxGrid('getcellvalue',rowBoundIndex, "srno");
            ...
            var noofrows = $('#terminationgrid').jqxGrid('getrows');
