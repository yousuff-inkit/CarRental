...
ClsCommon ClsCommon=new ClsCommon();
	...
	try{
	    ...
	    /* Attendance of Month*/
	    String sqls = "SELECT * FROM hr_timesheet WHERE month=MONTH('"+sqlPayrollDate+"') and year=YEAR('"+sqlPayrollDate+"') "+sqlbrh+"";
		...
		String sql = " SELECT min(salary_paid) lastSalaryPaid,DATE_FORMAT(min(salary_paid),'%d-%m-%Y') lastSalaryPaids, "+
				" DATEDIFF(LAST_DAY(DATE_ADD(min(salary_paid),INTERVAL 1 DAY)),'"+sqlPayrollDate+"') "
				+ " ProcessingDateDiff FROM hr_empm WHERE active=1 and status=3"+sqlbrh;
		...
		System.out.println(days+"==="+lastSalaryPaids+"===="+sqlPayrollDate);
		/*Payroll Process*/
		if(days==0){
			payrollProcess=1;
		}
		...
		if(days>0){
			...
			String sql2 = "select * from hr_timesheet where payroll_processed=1 and payroll_confirmed=1 and month=MONTH('"+sqlPayrollDate+"') and year=YEAR('"+sqlPayrollDate+"') "+sqlbrh ;
			...
			String sql1 = "select * from hr_timesheet where payroll_confirmed=0 and month=MONTH('"+sqlPayrollDate+"') and year=YEAR('"+sqlPayrollDate+"') "+sqlbrh;
