...
<script type="text/javascript">
	...
	function costCodeSearchContent(url) {
	 ...
	 function funRemoveReadOnly(){
		    ...
		    $("#fileexcelimport").attr('disabled', false );
		    $("#btnunpost").attr('disabled', true );
			$("#btnpost").attr('disabled', true );
	 ...
	 function getAttachDocumentNo(){
		...
		x.open("GET","getAttachDocumentNo.jsp?mode=MCPIM",true);      
		x.send();
		}
	function ajaxFileUpload(docNo) {    
	   ...
	   (  
	     {  
	        url:'fileAttachAction.action?formCode=MCPIM&doc_no='+docNo+'&descpt=Excel Import' ,  
	        secureuri:false,  
...
//	                   style:{left:'',right:27,top:document.body.scrollTop+document.documentElement.scrollTop,bottom:''}
		...
		if (x.readyState==4 && x.status==200)
			...
			var items=x.responseText.trim(); 
				if(parseInt(items)==1){   
					...
					$("#overlay, #PleaseWait").hide(); 
					if(parseInt(items)==99){
						$.messager.alert('Message','Account or Quantity or Unit Price might be missing', function(r){});
					}
				}
	...
	function funreload(event){
		// var barchval = document.getElementById("cmbbranch").value; 
		 $("#overlay, #PleaseWait").show();  
		 $("#divMCPGrid").load("multipleCashPurchaseGrid.jsp?excel=1&check="+1);  
	 }    
	     ...
	     var rows = $("#jqxMultipleCashPurchase").jqxGrid('getrows');
			 ...
			 var length=0,val=0;
				 for(var i=0 ; i < rows.length ; i++){
					...
					//alert('total : '+rows[i].total)
					if(typeof(rows[i].vendor)=='undefined' || rows[i].vendor==null || rows[i].vendor=='' || rows[i].vendor==0){
						 
					 }else{
						 if(rows[i].total=="" || typeof(rows[i].total)== "undefined" || rows[i].total==null || rows[i].total==0) {
						 		document.getElementById("errormsg").innerText="Total is Mandatory";
								return 0;
						} 	
							 
					    if(rows[i].qty=="" || typeof(rows[i].qty)=="undefined" || rows[i].qty==null) {	
					    	document.getElementById("errormsg").innerText="Qty is Mandatory";
							return 0;
						}	
					    
					    if(rows[i].unitprice=="" || typeof(rows[i].unitprice)=="undefined" || rows[i].unitprice==null) {
					    	document.getElementById("errormsg").innerText="Unit Price is Mandatory";
							return 0;
						}	
					    
					    if(rows[i].accounts.trim()=="" || typeof(rows[i].accounts)=="undefined" || rows[i].accounts==null) {
...
/* var win= window.open(reurl[0]+"PettyCashPrint?docno="+document.getElementById("docno").value+"&branch="+document.getElementById("brchName").value+"&header=0","_blank","top=150,left=250,Width=1020,Height=500,location=no,scrollbars=no,toolbar=yes");
	...
	//alert(data);
	  ...
	  function funExcelexportsample(){
			JSONToCSVCons(productdata, 'MCP', true);
	}
	
	  function JSONToCSVCons(JSONData, ReportTitle, ShowLabel) {
...
<table width="100%">
  <tr>
    ...
    <td width="10%" align="center"><button class="icon" id="btnexcelsample" name="btnexcelsample" title="Export Sample" type="button" onclick="funExcelexportsample();">
							<img alt="Import Excel" src="<%=contextPath%>/icons/excel-export-sample.png" style="width:25px">
						</button></td>
    <td width="10%" align="right"><input type="file" id="fileexcelimport" name="file"/></td>
    ...
    <td width="19%"><input type="text" id="txtrefno" name="txtrefno" onkeypress="return isNumberKey(event)" style="width:40%;" value='<s:property value="txtrefno"/>'/></td>
...
<table width="100%">
  <tr>
    ...
    <td width="20%"><input type="text" id="txtrate" name="txtrate" onchange="funvalid()" style="width:32%;text-align: right;" value='<s:property value="txtrate"/>' onblur="funRoundRate(this.value,this.id);" tabindex="-1"/>
...
<div id="divex" style="visibility: hidden;"><jsp:include page="samplegrid.jsp"></jsp:include></div>
