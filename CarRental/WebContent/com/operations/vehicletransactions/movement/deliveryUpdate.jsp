...
System.out.println("INSIDE DELIVERY JSP"+doc+fleet+date+time+km+fuel+location+branch+trancode);
	...
	//Checking if Delivery Already Done
	
	String strcheck="select delivery from gl_nrm where status=3 and doc_no="+doc;
	int delstatus=0;
	ResultSet rsdelcheck=stmt.executeQuery(strcheck);
	while(rsdelcheck.next()){
		delstatus=rsdelcheck.getInt("delivery");
	}
