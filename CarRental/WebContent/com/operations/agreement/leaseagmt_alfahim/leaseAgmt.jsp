...
function funViewApp(){
	if($("#masterdoc_no").val()!=""){
		$.get('getMenuPath.jsp',{'docno':$("#masterdoc_no").val()},function(data){
			data=JSON.parse(data);
			console.log(data);
			var leaseappdocno=data.leaseappdocno;
			$.each(data.menudata,function(index,value){
								
				if(value.dtype=='LQT'){
					window.parent.formName.value=value.menuname;
					window.parent.formCode.value=value.dtype;
					var detName=value.menuname;
					var url=document.URL;
	 				var reurl=url.split("com/");
					var path= value.actionpath+"?reqstatus=1&reqmode=view&reqdocno="+leaseappdocno;
...
function funViewCalc(){
	if($("#masterdoc_no").val()!=""){
		$.get('getMenuPath.jsp',{'docno':$("#masterdoc_no").val()},function(data){
			data=JSON.parse(data);
			console.log(data);
			var leasecalcdocno=data.leasecalcdocno;
			$.each(data.menudata,function(index,value){
				if(value.dtype=='LEC'){
					window.parent.formName.value=value.menuname;
					window.parent.formCode.value=value.dtype;
					...
					var url=document.URL;
	 				var reurl=url.split("com/");
					var path= value.actionpath+"?reqstatus=1&reqmode=view&reqdocno="+leasecalcdocno;
					top.addTab( detName,reurl[0]+""+path);
				}
			});
		});
...
function getPervalue(){
				...
				for (var i = 1; i <= 100; i++) {
					optionsloc += '<option value="' + [i] + '">'
							+ [i] + '</option>';
				}
				$("select#per_value").html(optionsloc);
				
				if ($('#hidper_value').val() != null) {
					$('#per_value').val($('#hidper_value').val());
				}
...
<body onload="setValues();getPervalue();">
 ...
 <div id='jqxMenuMore' title="More" align="center" style='visibility: hidden;'>
        <ul>
         <li><a href="#trafficFines" onclick="fine();">Traffic Fines</a></li>
            ...
            <li><a href="#viewapp" onclick="funViewApp();">View Application</a></li>  
            <li><a href="#viewcalc" onclick="funViewCalc();">View Calculator</a></li>               
        </ul>
     </div> 
 </td>
 </tr>
