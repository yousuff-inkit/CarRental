...
public String saveAction() throws ParseException, SQLException{
	...
	if(mode.equalsIgnoreCase("A")){
		int val = 0;
		//System.out.println(getIsclone());
		if(getIsclone().equalsIgnoreCase("1")) {
			 val = tarifdao.clone(sqldate,getCmbtariffor(),getCmbtariftype(),getHidtarifclient(),sqlfromdate,sqltodate,getHidchkdelivery(),getDesc(),mode,session,getBrchName(),getFormdetailcode(),getPrevdocno());
		} else {
			 val=tarifdao.insert(sqldate,getCmbtariffor(),getCmbtariftype(),getHidtarifclient(),sqlfromdate,sqltodate,getHidchkdelivery(),getDesc(),mode,session,getBrchName(),getFormdetailcode());
		}
