...
try {
	...
	cldocno = session.getAttribute("clientcldocno")==null?"0":session.getAttribute("clientcldocno").toString();  
	/*Inserting into emaillog*/
	String sqls=("insert into emaillog (doc_no, brhId, dtype, edate, userId, refid, email, cldocno) values ('"+doc_no+"','"+branch+"','"+formdetailcode+"',now(),'"+userid+"','"+refid+"','"+recipient+"','"+cldocno+"')");
	int datas = stmt10.executeUpdate(sqls);
	/*Inserting into emaillog Ends*/
