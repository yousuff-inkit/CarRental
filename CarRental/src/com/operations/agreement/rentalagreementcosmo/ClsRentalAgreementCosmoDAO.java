package com.operations.agreement.rentalagreementcosmo;
...
public class ClsRentalAgreementCosmoDAO {
	ClsRentalAgreementCosmoBean RentalAgreementBean = new ClsRentalAgreementCosmoBean();
...
CallableStatement stmtrentalagmt = conn.prepareCall("{CALL rentalAgmtDML(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");
   ...
   //  System.out.println("-----------"+stmtrentalagmt);
	...
	{	
         ...
         /*    String selectsql="select t.doc_no,t.account,t.description from my_account a inner JOIN MY_HEAD t on a.acno=t.doc_no where codeno='"+type+"'";
		    ...
		    if(valre<=0)	{
			     ...
			     String sqlrpytlog="INSERT INTO gl_rpytlog(payment,mode,amount,acode,cardno,expdate,card,cardtype,paytype,payid,brhid,rdocno,recieptno)VALUES"
					       + " ('"+(payment[0].equalsIgnoreCase("undefined") || payment[0].isEmpty()?0:payment[0])+"',"
					       + "'"+(payment[1].trim().equalsIgnoreCase("undefined") || payment[1].trim().equalsIgnoreCase("")|| payment[1].trim().equalsIgnoreCase("NaN")|| payment[1].isEmpty()?0:payment[1].trim())+"',"
					       + "'"+(payment[2].trim().equalsIgnoreCase("undefined") || payment[2].trim().equalsIgnoreCase("")|| payment[2].trim().equalsIgnoreCase("NaN")|| payment[2].isEmpty()?0:payment[2].trim())+"',"
					       + "'"+(payment[3].trim().equalsIgnoreCase("undefined") ||payment[3].trim().equalsIgnoreCase("")|| payment[3].trim().equalsIgnoreCase("NaN")|| payment[3].isEmpty()?0:payment[3].trim())+"',"
					       ...
					       + "'"+(payment[10].trim().equalsIgnoreCase("undefined") ||payment[10].trim().equalsIgnoreCase("")|| payment[10].trim().equalsIgnoreCase("NaN")|| payment[10].isEmpty()?0:payment[10].trim())+"',"
					       +"'"+session.getAttribute("BRANCHID").toString()+"','"+docno+"','"+srno+"')";
					     int rpytlog = stmtrentalagmt.executeUpdate (sqlrpytlog);
			     if(rpytlog<=0)
			     {
			    	 conn.close();
			    	 return 0; 
    ...
    public  JSONArray trariffbtnGrid(HttpSession session,String vehgpid,String cldocno,String salesagent) throws SQLException {
...
/*   					String sqltari=("select m.doc_no,m.validto,concat(m.tariftype,' - ',m.notes) tariftype,m.clientid, coalesce(insurexcess,0) insurexcess, coalesce(cdwexcess,0) cdwexcess,  "
				...
				String sqltari=("select m.doc_no,m.validto,concat(m.tariftype,' - ',m.notes) tariftype,m.clientid, coalesce(insurexcess,0) insurexcess, coalesce(cdwexcess,0) cdwexcess,  "
   				...
   				if(!salesagent.equalsIgnoreCase("")){
   					sqltari="select m.doc_no,m.validto,concat(m.tariftype,' - ',m.notes) tariftype,m.clientid, coalesce(insurexcess,0) insurexcess, coalesce(cdwexcess,0) cdwexcess,  "
   							+ "	coalesce(scdwexcess,0) scdwexcess from gl_tarifm  m left join gl_tarifd m1 on m1.doc_no=m.doc_no left join  "
   							+ "	gl_tarifexcess t on t.rdocno=m.doc_no and t.gid='"+vehgpid+"' where    current_date between m.validfrm and m.validto and m.status=3 and "
   							+ "  (m1.gid='"+vehgpid+"' or t.gid='"+vehgpid+"') and m.tariftype='agent' group by m.doc_no";
   				}
   				System.out.println("Tarif Query:"+sqltari);
 ...
 public JSONArray clientDriver(String clientid) throws SQLException {
	 ...
	 Connection conn=null;
			try {
					...
					ResultSet resultSet = stmtVeh.executeQuery("SELECT if(d.visa_exp<=curdate(),1,0) idexpstatus,if(d.pass_exp<=curdate(),1,0) passexpstatus,if(d.led<=curdate(),1,0) licenseexpstatus,coalesce(nat.blockstatus,0) blockstatus,d.dr_id,d.doc_no,d.name,d.dob,d.nation,d.mobno,d.dlno,d.issdate,d.led,d.ltype,d.issfrm,d.passport_no,d.pass_exp,d.visano,d.visa_exp,d.hcdlno,d.hcissdate,d.hcled ,d.cldocno,DATE_FORMAT(FROM_DAYS(DATEDIFF(CURRENT_DATE,d.dob)),'%y ') AS age,DATE_FORMAT(FROM_DAYS(DATEDIFF(CURRENT_DATE,d.issdate)),'%y ') AS expiryear,(select value from gl_config where field_nme='drage') drage ,(select value from gl_config where field_nme='licyr') licyr FROM gl_drdetails d  left join my_natm nat on d.nation=nat.nation WHERE d.cldocno='"+clientid+"' and d.dtype='CRM' ;");
...
/*
	      ...
	      String brid=session.getAttribute("BRANCHID").toString();
			...
			/*		String clsql= ("select distinct a.pcase,d.dob,d.dlno,d.nation,d.name drname,a.contactPerson,a.mail1,a.cldocno,trim(a.RefName) RefName,a.per_mob,concat(coalesce(a.address,''),'  ',coalesce(a.address2,'')) as address ,a.per_tel,a.codeno,a.acno,m.doc_no, "
					...
					String clsql= ("select distinct "+method+" method,c.catid,c.contract_upto tradelicenseexp,if(cat.retailstatus=1,1,0) retailstatus,c.cldocno,c.advance,c.invc_method,c.pcase,d.dob,d.dlno,d.nation,"+
					...
					" left join gl_drdetails d on d.cldocno=c.cldocno and d.dtype=c.dtype left join my_clcatm cat on c.catid=cat.doc_no "+
					" where c.dtype='CRM' and c.status=3 " +sqltest);
	    ...
	    public  ClsRentalAgreementCosmoBean getViewDetails(HttpSession session,int docNo) throws SQLException {
...
*/
	  public  ClsRentalAgreementCosmoBean getPrint(int docno) throws SQLException {
	    	ClsRentalAgreementCosmoBean bean = new ClsRentalAgreementCosmoBean();
	    	 Connection conn =null;
	      try {
	    	   conn = connDAO.getMyConnection();
...
//		   System.out.println("@@@@@@@@@============="+strindigo+"====nation===="+rsindigo.getString("nationality"));
   ...
   Statement stmtinvoice66 = conn.createStatement();
	      ...
	      System.out.println("=======driver"+strcosmodriver);
	       ResultSet rscosmodriver=stmtcosmodriver.executeQuery(strcosmodriver);
		   int drivercount=0;
	       while(rscosmodriver.next()){
	    	   System.out.println("dklfghjj"+drivercount);
	    	   if(drivercount==0){
	    		   System.out.println("drivvevev");
	    	   ...
	    	   if(drivercount==1){
				   bean.setDrivername(rscosmodriver.getString("name"));
				   System.out.println("driverrrr"+rscosmodriver.getString("name"));
		    	   bean.setLblcosmodrivermobile(rscosmodriver.getString("mobno"));
		    	   bean.setLblcosmodriverlicense(rscosmodriver.getString("dlno"));
		    	   bean.setLblcosmodrivervalidupto(rscosmodriver.getString("led"));
		    	   bean.setLblcosmodriverpassport(rscosmodriver.getString("passport_no"));
	       ...
	       while(resultmain.next()){
	    	   if(i==0)
	    	   {
	    		// yessur
					...
					/*bean.setLblcosmodrivermobile("");
			    	bean.setLblcosmodriverlicense("");
			    	bean.setLblcosmodrivervalidupto("");
			    	bean.setLblcosmodriverpassport("");*/
					bean.setAddlicno1("");
			    	bean.setLbladdpassno1("");
			    	bean.setExpdate1(""); 
			    	bean.setLblindigoaddmobile("");
...
ResultSet resulttwo = stmtinvoice2.executeQuery(drtwo); 
	 ...
	 /*  public  ClsRentalAgreementCosmoBean getPrintnoori(int docno, HttpServletRequest request) throws SQLException {
	    	ClsRentalAgreementCosmoBean bean = new ClsRentalAgreementCosmoBean();
	    	 Connection conn =null;
	      try {
	    	   conn = connDAO.getMyConnection();
...
/*						String sql="select j.acno,j.doc_no transno,j.tr_no,j.tranid,j.dtype transtype,j.date trdate,j.rdocno rentaldoc,j.rtype rentaltype, "
	    ...
	    public  ClsRentalAgreementCosmoBean getClosingSummaryPrint(HttpServletRequest request, int docNo) throws SQLException {
			ClsRentalAgreementCosmoBean bean = new ClsRentalAgreementCosmoBean();
			Connection conn=null;
			try {
				 conn = connDAO.getMyConnection();
...
String sql6="";
		...
		public ClsRentalAgreementCosmoBean getKmPrint(HttpServletRequest request,
    ...
    */
	...
	public  JSONArray getPreAuthHistory(String docno,String id) throws SQLException {
		...
		try{
			conn = connDAO.getMyConnection();
			Statement stmt = conn.createStatement ();
            String strhistory=("select rdocno, payment, mode, amount, acode, cardno, expdate, recieptno, brhid, payid, doc_no, card, paytype, cardtype from gl_rpytlog where rdocno='"+docno+"' and payment='Pre Auth'");
			ResultSet rs = stmt.executeQuery(strhistory);
