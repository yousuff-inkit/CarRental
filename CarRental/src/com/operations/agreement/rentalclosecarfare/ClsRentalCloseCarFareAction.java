...
private String hidchkaudit;
private String insurcomptotal,clienttotal;



public String getInsurcomptotal() {
	return insurcomptotal;
}

public void setInsurcomptotal(String insurcomptotal) {
	this.insurcomptotal = insurcomptotal;
}
...
public String getHidchkaudit() {
	return hidchkaudit;
}

public void setHidchkaudit(String hidchkaudit) {
	this.hidchkaudit = hidchkaudit;
}
...
public void setData(int docno,String msg,String vocno,java.sql.Date indate,java.sql.Date collectdate,java.sql.Date closedate,java.sql.Date outdate) {
	setDocno(docno);
	setVoucherno(Integer.parseInt(vocno));
	...
	setMsg(msg);
	setInsurcomptotal(getInsurcomptotal());
	setClienttotal(getClienttotal());
}
...
//		System.out.println("view===="+getAgreementno());
		//ClsNonPoolVehBean bean = new ClsNonPoolVehBean(); 
				...
				setInsurcomptotal(bean.getInsurcomptotal());
				setClienttotal(bean.getClienttotal());
				setHidchkaudit(bean.getHidchkaudit());
...
//			System.out.println(calcarray.get(i));
		...
		ClsRentalCloseCarFareAction masteraction=new ClsRentalCloseCarFareAction();
		masteraction.setHidchkaudit(getHidchkaudit());
		masteraction.setInsurcomptotal(getInsurcomptotal());
		masteraction.setClienttotal(getClienttotal());
		...
		int val=rentalcloseDAO.insert(getAgreementno(),getClientid(),getHidchkcollection(),getCollectkm(),getCmbcollectfuel()==null?"0":getCmbcollectfuel(),collectdate,getCollecttime(),getHidcheckin(),
				...
				getChaufferid(),calcarray,outdate,getClientacno(),getHidfleet(),getCreditnotesum(),request,getCollectchg()==null?"0":getCollectchg(),getBrchName(),getCmbagmtbranch(),getCmbcloseloc(),getDescription(),getHidchkorgregcardcollect(),masteraction);
		if(val>0){
			setData(val,"Successfully Saved",request.getAttribute("VOUCHERNO").toString(),indate,collectdate,closedate,outdate);
			return "success";
		}
		else{
			setData(val,"Not Saved","0",indate,collectdate,closedate,outdate);
			return "fail";
		}
	}
		...
		int val=rentalcloseDAO.delete(getDocno(),getAgreementno(),session,getMode(),getHidfleet(),getBrchName(),request);
		if(val>0){
			setData(getDocno(),"Successfully Deleted",getVoucherno()+"",indate,collectdate,closedate,outdate);
			return "success";
		}
		else if(val==-1){
			setData(getDocno(),"Not Deleted",getVoucherno()+"",indate,collectdate,closedate,outdate);
