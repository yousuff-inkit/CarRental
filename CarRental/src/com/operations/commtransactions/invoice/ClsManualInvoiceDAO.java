...
//			System.out.println("View Sql:"+strSql);
			...
			while (resultSet.next()) {
				...
				bean.setVoucherno(resultSet.getString("voucherno"));
	...
	public int insert(Connection conn,ArrayList<String> invoicearray,String cmbagmttype, Date date, String hidclient, int agmtno,
		...
		try{
			...
			String salikparking="";
			if(dtype.contains("###")) {
				if(dtype.split("###").length>2) {
					if(!dtype.split("###")[2].trim().equalsIgnoreCase("")) {
						salikparking=dtype.split("###")[2].trim();
					}
				}
			}
			if(todate.compareTo(fromdate)<0){
			 ...
			 String salikqty="",trafficqty="",saliktype="";
			int invdescconfig=getInvDescConfig(conn);
			int invsalikcountconfig=getInvSalikCountConfig(conn);
			ClsManualInvoiceBean invoicebean=new ClsManualInvoiceBean();
			String note="";
			note=ledgernote;
			int clientjvconfig=0;//1 for saperate client jv entry for salik,traffic,general.0-single entry
			...
			int forcedateconfig=0;
			String strdateconfig="select method,curdate() basedate from gl_config where field_nme='forceDate'";
			ResultSet rsdateconfig=stmtacperiod.executeQuery(strdateconfig);
			java.sql.Date sqlbasedate=null;
			while(rsdateconfig.next()) {
				forcedateconfig=rsdateconfig.getInt("method");
				sqlbasedate=rsdateconfig.getDate("basedate");
			}
			if(forcedateconfig==1) {
				date=sqlbasedate;
			}
			...
			String strparkconfig="select method from gl_config where field_nme='InvSalikParking'";
			...
			int salikauhacno=0,salikauhsrvcacno=0,salikdxbacno=0,RAGInsurComp=0;
			...
			" (select method from gl_config where field_nme='invClientJV') clientjvconfig, "+
			" (select method from gl_config where field_nme='RAGInsurComp') RAGInsurComp " ;
			...
			while(rssaliksrvcacno.next()){
				...
				clientjvconfig=rssaliksrvcacno.getInt("clientjvconfig");
				RAGInsurComp=rssaliksrvcacno.getInt("RAGInsurComp");
			}
			if(cmbagmttype.equalsIgnoreCase("RAG")){
			...
			String stracperiod="select DATE_ADD(if("+date+" is null,null,'"+date+"'), INTERVAL (select coalesce(period2,0) from my_acbook where cldocno="+hidclient+" and dtype='CRM') DAY) duedate";
			...
			if(!(origin.trim().equalsIgnoreCase("1"))){
			    ...
			    String sqlins=" ";
			    if(RAGInsurComp==1){
			        sqlins=" and coalesce(chkinsurcomp,0)=0 ";
			    }
			    
				if(cmbagmttype.equalsIgnoreCase("RAG")){
					strdelchg="select COALESCE((select if(delivery=1,coalesce(delchg,0),0) from gl_ragmt where doc_no="+agmtno+" and del_invno=0 "+sqlins+" ),0) del,(select acno from gl_invmode where idno=6) delacno ";
 ...
 */
			...
			/*
			 * below if block was commentetd removed comment on 04-07-2025  , there is seperate method also to check the same case
			 * WTI duplicate invocie was generating 
			 * checking change to duplicate invoice during the period and orgin of rental close and lease close also added
			 *  '"+tempfromdate+"' between m.fromdate and m.todate or removed this condition 
			
			this block will not work as while closing if only salik is only there then also restriction was coming 
			if(origin.equalsIgnoreCase("2")  || origin.equalsIgnoreCase("3") || origin.equalsIgnoreCase("4")  || origin.equalsIgnoreCase("5")){
				...
				String strinvduplicatecount="select count(*) rowcount from gl_invm m inner join gl_invd d on m.doc_no=d.rdocno "
				        + "where d.chid=1 and m.ratype='"+cmbagmttype+"' and m.rano="+agmtno+" and (  '"+todate+"'  between m.fromdate and m.todate ) and m.status=3 and m.manual in (2,3,4,5) ";
				...
				if(invduplicatecount>0){
				    System.out.println("********************************** Dupicate Invoice Creation restricted **********************************");
				    return 0;
				}
			}
			 */
			...
			if(invdescconfig==1){
				if(formdetailcode.equalsIgnoreCase("INS")){
					...
					for(int i=0;i< invoicearray.size();i++){
						...
						if(invoice[0].equalsIgnoreCase("8") || invoice[0].equalsIgnoreCase("38") || invoice[0].equalsIgnoreCase("51")){
						    salikqty=(Double.parseDouble(salikqty.equalsIgnoreCase("") || salikqty==null?"0":salikqty)+Double.parseDouble(invoice[3].equalsIgnoreCase("") || invoice[3]==null?"0":invoice[3]))+"";
						    System.out.println("salik qty == "+salikqty);
							if(invoice[0].equalsIgnoreCase("51")) {
								saliktype="Parkings";
							}
							else {
								saliktype="Saliks";
					...
					note=salikqty+" "+saliktype+" of "+temprenttype+" - "+agmtvocno+" from "+strfromdate+" to "+strtodate+" with Reg No "+agmtregno+" and Plate Code "+agmtplate;
				}
			...
			if(invsalikcountconfig==1){
				if(formdetailcode.equalsIgnoreCase("INS")){
					...
					note=salikqty+" "+saliktype;
				}
				
				if(formdetailcode.equalsIgnoreCase("INT")){
					for(int i=0;i< invoicearray.size();i++){
						String[] invoice=invoicearray.get(i).split("::");
						if(invoice[0].equalsIgnoreCase("9") || invoice[0].equalsIgnoreCase("15")){
							trafficqty=invoice[3];
					...
					note=trafficqty+" Traffics ";
				}
			}
			ClsVocno objvoc=new ClsVocno();
            String objvocapp=objvoc.checkvocno(conn,(formdetailcode.trim().equalsIgnoreCase("INS") || formdetailcode.trim().equalsIgnoreCase("INT") || formdetailcode.trim().equalsIgnoreCase("IND")?"INV":formdetailcode),branchid,date);
            
            String docyr="SELECT method config FROM GL_CONFIG where field_nme ='Yeardocno'"; 
			//System.out.println("config chk-----"+docyr);  
			ResultSet rsdocyr=stmtagmtvoc.executeQuery(docyr);
			int yeardocconfig=0;
			while(rsdocyr.next()){
				yeardocconfig=rsdocyr.getInt("config");
			}
            
			stmtManual.registerOutParameter(14, java.sql.Types.INTEGER);
			stmtManual.registerOutParameter(15, java.sql.Types.INTEGER);
			stmtManual.setInt(17, yeardocconfig==5?Integer.parseInt(objvocapp.trim().equalsIgnoreCase("")?"0":objvocapp.trim()):java.sql.Types.INTEGER);
					 ...
					 if(delcal==0){
						 if(cmbagmttype.equalsIgnoreCase("RAG")){
							 strcheckdate="select if(delivery=1 and delstatus=1,(select min(din) from gl_vmove where rdocno="+agmtno+" and rdtype='RAG' and "+
									 " trancode='DL' and repno=0 and exchng=0 and custno=0 ),odate) temptodate from gl_ragmt where doc_no="+agmtno;	
						 }
						 else if(cmbagmttype.equalsIgnoreCase("LAG")){
							 strcheckdate="select if(delivery=1 and delstatus=1,(select min(din) from gl_vmove where rdocno="+agmtno+" and rdtype='LAG' and "+
									 "  trancode='DL' and repno=0 and exchng=0 and custno=0 ),outdate) temptodate from gl_lagmt where doc_no="+agmtno;	
						 }
						 ...
						 if(cmbagmttype.equalsIgnoreCase("RAG") &&  invoice[0].equalsIgnoreCase("1") && Double.parseDouble(invoice[5])>0){
							 ...
							 if(monthcalmethod==1){
								 if(invtype==1){
									 ...
									 //Carfare had a problem that it was updating 1 month extra when changed Period to month end in the middle
									 todatecalc="SELECT LAST_DAY(DATE_ADD('"+todate+"',INTERVAL 1 day))";
								 }
								 if(invtype==2){
...
//										 System.out.println("Inside Lastday");
										 ...
										 //Added startday>28,because when changed cycle of invdate & invtodate,system was adding no:of days of outdate in carfare (04-04-23)
										 if(startday>28) {
											 todatecalc="select date_add("+todatecalc+",interval "+remainday+" day)";
											 System.out.println("remainday todatecalc: "+todatecalc);
										 }
										 ...
										 " SELECT if(date_add('"+temptodate+"',interval -DAY('"+temptodate+"')+1 DAY)='"+temptodate+"',1,0) begindatestatus)a";
											 System.out.println("====remainday onedayextraconfig "+strbegindate);
						...
						if(invtype==1 || invtype==2){
							...
							if(monthcalmethod==1){
								quarterlytodate="date_add(invtodate,interval "+monthno+" month)";
							}
							else{
								quarterlytodate="date_add(invtodate,interval "+monthcalvalue+"*"+monthno+" day)";
							}
							String strupdate="update gl_ragmt set invdate='"+todate+"',invtodate="+quarterlytodate+" where doc_no="+agmtno;
							int agmtupdate=stmtinvoice.executeUpdate(strupdate);
							if(agmtupdate<0){
								System.out.println("Update Ragmt Error");
								return 0;
						 ...
						 if(cmbagmttype.equalsIgnoreCase("LAG")  && invoice[0].equalsIgnoreCase("1")  && Double.parseDouble(invoice[5])>0){
							 ...
							 String tempdate="'"+todate+"'";
							 ResultSet rsgetmonthcal=stmtinvoice.executeQuery("select method,value from GL_CONFIG WHERE FIELD_NME='lesmonthlycal'");
							 while(rsgetmonthcal.next()){
								 monthcalmethod=rsgetmonthcal.getInt("method");
								 monthcalvalue=rsgetmonthcal.getInt("value");
							 }
...
//							System.out.println("check todatecalc: "+todatecalc);
				...
				System.out.println("Checking configs"+invtype+"//"+oneDayExtraConfig+"//"+samedatestatus);
							 ...
							 if(closestatus==0){
								...
								else if(invtype==4 || invtype==5 || invtype==6) {
									int monthno=0;
									if(invtype==4) {
										monthno=3;
									}
								...
								System.out.println("Quarterly Inv Update:"+strupdate);
								int agmtupdate=stmtinvoice.executeUpdate(strupdate);
								if(agmtupdate<0){
					 ...
					 if(invoice[0].equalsIgnoreCase("8") || invoice[0].equalsIgnoreCase("14") || invoice[0].equalsIgnoreCase("38") || invoice[0].equalsIgnoreCase("39") || invoice[0].equalsIgnoreCase("51") || invoice[0].equalsIgnoreCase("52")){
						 salikamt+=Double.parseDouble((String) (invoice[5].equalsIgnoreCase("undefined") || invoice[5]==null?0:invoice[5]));
						 salikcount++;
						 salikqty=invoice[3];
						 if(invoice[0].equalsIgnoreCase("8")  || invoice[0].equalsIgnoreCase("38")  || invoice[0].equalsIgnoreCase("51")){
							 salikqty=(Double.parseDouble(salikqty.equalsIgnoreCase("") || salikqty==null?"0":salikqty)+Double.parseDouble(invoice[3].equalsIgnoreCase("") || invoice[3]==null?"0":invoice[3]))+"";
						 ...
						 if(invoice[0].equalsIgnoreCase("14")  || invoice[0].equalsIgnoreCase("39")  || invoice[0].equalsIgnoreCase("52")){
							 ...
							 if(invoice[0].equalsIgnoreCase("52")){
								 salikparksrvc=objcommon.Round(Double.parseDouble((String) (invoice[5].equalsIgnoreCase("undefined")?0:invoice[5])),2);
							 }
						 }
					 }
...
//						 System.out.println("Check General Amount:"+generalamt);
					...
					costmovarray.add(tempamt+""+"::"+rscostmov.getString("fleet_no"));
							 ...
							 for(int j=0;j<costmovarray.size();j++){
								 ...
								 if(counter==1){
									 ...
									 if(invoice[1].equalsIgnoreCase(saliksrvcacno+"") || invoice[1].equalsIgnoreCase(trafficsrvcacno+"") || invoice[1].equalsIgnoreCase(salikauhsrvcacno+"") || invoice[1].equalsIgnoreCase(parksrvcacno+"")){
										 ...
										 //			System.out.println("Insert Costtran Sql:"+strcostinsert);
										 */
										 if(invoice[1].equalsIgnoreCase(saliksrvcacno+"") || invoice[1].equalsIgnoreCase(salikauhsrvcacno+"")  || invoice[1].equalsIgnoreCase(parksrvcacno+"")){
											 salikcostentry++;
			...
			if(salikamt>0){
				...
				if(!(origin.trim().equalsIgnoreCase("1"))){
					...
					if(invoicetypeno==8){
						String salikfilters="";
						if(!salikparking.trim().equalsIgnoreCase("")) {
							salikfilters+=" and type='"+salikparking+"'";
						}
						strupdatesalik="update gl_salik set inv_no="+aaa+",inv_type='INV',status=1 where ra_no="+agmtno+" and isallocated=1 and rtype "+rtype+" and "+
								" sal_date>='"+fromdate+"' and sal_date<='"+todate+"' and inv_no=0 "+salikfilters;	
					}
					else{
						strupdatesalik="update gl_salik set inv_no="+aaa+",inv_type='INV',status=1 where ra_no="+agmtno+" and  isallocated=1 and rtype "+rtype+" and "+
...
//						 sendSMS(aaa+"", branchid, dtype, salikamt+"", hidclient,"",conn);
				...
				String saliknote=""+salikqty+" "+saliktype;
				...
				if(invdescconfig==1){
					...
					salikqty="0";
					for(int i=0;i<invoicearray.size();i++){
						if(invoicearray.get(i).split("::")[0].equalsIgnoreCase("8")  || invoicearray.get(i).split("::")[0].equalsIgnoreCase("38")  || invoicearray.get(i).split("::")[0].equalsIgnoreCase("51")){
							 salikqty=(Double.parseDouble(salikqty.equalsIgnoreCase("") || salikqty==null?"0":salikqty)+Double.parseDouble(invoicearray.get(i).split("::")[3].equalsIgnoreCase("") || invoicearray.get(i).split("::")[3]==null?"0":invoicearray.get(i).split("::")[3]))+"";
						}
						if(invoicearray.get(i).split("::")[0].equalsIgnoreCase("51")) {
							saliktype="Parkings";
						}
						else {
							saliktype="Saliks";
					...
					saliknote=salikqty+" "+saliktype+" of "+temprenttype+" - "+agmtvocno+" from "+strfromdate+" to "+strtodate+""+" with Reg No "+agmtregno+" and Plate Code "+agmtplate;
					if(invsalikdatedescconfig==1){
						saliknote=salikqty+" "+saliktype+" of "+temprenttype+" - "+agmtvocno+" from "+salikmindate+" to "+salikmaxdate+""+" with Reg No "+agmtregno+" and Plate Code "+agmtplate;
					}
				}
				...
				saliksrvcvat+=objcommon.sqlRound(getSalikTrafficVAT(conn,"Salik#DXB",salikdxbsrvc+salikauhsrvc,hidclient,date,branchid,aaa),2);
				...
				System.out.println("Before General Amount: "+generalamt);
				if(clientjvconfig==0 || formdetailcode.equalsIgnoreCase("INS")){
					generalamt+=salikamt;
				}
				else{
...
//					System.out.println("strupdatetraffic :"+strupdatetraffic);
					...
					if(trafficcostentry>0){
						...
						System.out.println(trafficcostentryarray);
...
//							sendSMS(aaa+"", branchid, dtype, trafficamt+"", hidclient,traffic_date,conn);
				...
				if(invdescconfig==1){
					...
					trafficnote=trafficqty+" Traffics of "+temprenttype+" - "+agmtvocno+" from "+strfromdate+" to "+strtodate+" with Reg No "+agmtregno+" and Plate Code "+agmtplate;
				}
				// System.out.println("description ==== "+trafficnote);
				trafficsrvcvat=objcommon.sqlRound(getSalikTrafficVAT(conn,"Traffic",trafficsrvc,hidclient,date,branchid,aaa),2);
				trafficamt+=trafficsrvcvat;
				trafficldr=trafficamt*testcurrate;
				if(clientjvconfig==0 || formdetailcode.equalsIgnoreCase("INT")){
...
//						System.out.println("Inside TaxDetail");
					...
					String strtaxapplied="select total from gl_invd inv left join gl_invmode ac on inv.chid=ac.idno where ac.tax=1 and ac.idno<>20 and inv.rdocno="+aaa;
...
//						 System.out.println("Test Sum Jvtran"+testjv1);
			...
			if (aaa > 0) {
				...
				String strcolumns="";
				if(cmbagmttype.trim().equalsIgnoreCase("RAG")) {
					strcolumns="desc gl_ragmt";
				}
				if(cmbagmttype.trim().equalsIgnoreCase("LAG")) {
					strcolumns="desc gl_lagmt";
				}
				ResultSet rscolumns=stmtinvoice.executeQuery(strcolumns);
				while(rscolumns.next()) {
					columnarray.add(rscolumns.getString("Field"));
				}
				boolean firstInvDocnoPresent=false;
				if(columnarray.contains("firstinvdocno")) {
					firstInvDocnoPresent=true;
				}
				
				if(firstInvDocnoPresent==true) {
					//Updating First Invoice Number
					String strupdateagmt="";
					if(cmbagmttype.trim().equalsIgnoreCase("RAG")) {
						strupdateagmt="UPDATE gl_ragmt agmt SET firstinvdocno=(SELECT m.doc_no FROM gl_invm m INNER JOIN gl_invd d ON m.doc_no=d.rdocno AND d.chid=1 WHERE m.doc_no="+aaa+") WHERE agmt.status=3 AND agmt.firstinvdocno=0 and agmt.doc_no="+agmtno;
					}
					if(cmbagmttype.trim().equalsIgnoreCase("LAG")) {
						strupdateagmt="UPDATE gl_lagmt agmt SET firstinvdocno=(SELECT m.doc_no FROM gl_invm m INNER JOIN gl_invd d ON m.doc_no=d.rdocno AND d.chid=1 WHERE m.doc_no="+aaa+") WHERE agmt.status=3 AND agmt.firstinvdocno=0 and agmt.doc_no="+agmtno;
					}
					int updateagmt=stmtinvoice.executeUpdate(strupdateagmt);
				}
				//			System.out.println("InvNo > 0");
				int total=0;
				JSONObject validobj = new JSONObject();
				validobj = objcommon.JVTotalCheck(conn,testtrno);
...
private int getInvSalikCountConfig(Connection conn) throws SQLException {
		// TODO Auto-generated method stub
	int config=0;
	try{
		Statement stmt=conn.createStatement();
		String strconfig="select method from gl_config where field_nme='invSalikTrafficCount'";
...
public double getSalikTrafficVAT(Connection conn, String type,
			double trafficsrvc, String hidclient, Date date,String branchid,int invdocno) {
		// TODO Auto-generated method stub
		double srvcvat=0.0;
		try{
			Statement stmt=conn.createStatement();
			...
			if(!saliktype.equalsIgnoreCase("")){
				...
				else if(saliktype.equalsIgnoreCase("PAR")){
					strgetsaliktaxstatus="select coalesce(tax,0) tax from gl_invmode where idno=52";
				}
				...
				//Check Salik is Tax Applicable
				String strchecksalik="select total,idno from gl_invd inv left join gl_invmode ac on inv.chid=ac.idno where ac.tax=1 and inv.rdocno="+invdocno+" and inv.chid in (8,38)";
				ResultSet rschecksalik=stmt.executeQuery(strchecksalik);
				while(rschecksalik.next()){
					trafficsrvc+=rschecksalik.getDouble("total");
				}
			}
...
public int getInvDescConfig(Connection conn) {
	...
	public   ClsManualInvoiceBean getPrint(String fromno,String tono,String branch,String printdocno,String allbranch,String hidheader,String chkdeletedinv,String trafic,String bankdocno) throws SQLException {
		//ClsManualInvoiceBean bean = new ClsManualInvoiceBean();
		Connection  conn =null;
		HttpServletRequest request=ServletActionContext.getRequest();
		...
		try {
			...
			Statement stmtinvoice = conn.createStatement ();
			Statement stmtmanual = conn.createStatement ();
			Statement stmtmanuals = conn.createStatement ();
		...
		//	System.out.println("-banknnno--"+bankdocno);
	...
	System.out.println("branchhhkkkkk--"+bankinfosql);
			...
			while (resultsetbank.next()) {
				...
				bean.setLblbankbeneficiary1(resultsetbank.getString("beneficiary"));
				...
				bean.setLblbankibanno1(resultsetbank.getString("ibanno"));
			...
			if(bankdocno!=null && !bankdocno.trim().equalsIgnoreCase("") && !bankdocno.isEmpty() && !bankdocno.trim().equalsIgnoreCase("undefined") && !bankdocno.trim().equalsIgnoreCase("0")){
			    String bankinfosql = "select name,beneficiary,account,ibanno,swiftcode,logo,address branchaddress from cm_bankdetails where status=3 and doc_no='"+bankdocno+"'";
	            ResultSet resultsetbank = stmtinvoice.executeQuery(bankinfosql);
	    //System.out.println("branchhhkkkkk--"+bankinfosql);
			...
			String projectname=" select proj.project_name from gl_invm inv left join gl_ragmt agmt on inv.rano=agmt.doc_no left join gl_project proj on (proj.status=3 and agmt.rentalproject=proj.doc_no) where 1=1 "+sqltest+" group by inv.doc_no ";
			//System.out.println("projectname----------"+projectname);
			ResultSet resultSetprojectname = stmtinvoice.executeQuery (projectname);
			   while (resultSetprojectname.next()) {
				   
				   bean.setLblprojectname(resultSetprojectname.getString("project_name"));
				//   System.out.println("pro in dao-----"+resultSetprojectname.getString("project_name"));
			   }

			String printname="";
				/*	strSql="select br.tinno brtinno,inv.manual,inv.brhid,coalesce(veh.CH_NO,'') chasisno,inv.status,DATE_FORMAT(coalesce(if(lagmt.per_name=1,DATE_ADD(lagmt.outdate,INTERVAL lagmt.per_value YEAR),DATE_ADD(lagmt.outdate,INTERVAL lagmt.per_value MONTH)),agmt.ddate),'%d/%m/%y')enddate,"
					...
					+" if(inv.ratype='RAG',agmt.pono,lagmt.po) pono, DATE_FORMAT(CURDATE(),'%d/%m/%Y') finaldate,cur.code,cmp.company,cmp.address ,cmp.tel,cmp.fax,cmp.pbno cmppbno,cmp.email,"
					+" cmp.web website,br.branchname,br.address branchaddress,br.tel branchtel,br.fax branchfax,CASE WHEN LOCATE(ac.salutation, ac.refname) = 1 THEN CONCAT(SUBSTRING(ac.refname, LENGTH(ac.salutation) + 1)) ELSE ac.refname END AS refname,ac.com_mob,ac.per_tel,ac.per_mob,ac.fax1,dr.name, inv.doc_no, DATE_FORMAT(inv.date,'%d/%m/%Y') date,"
...
*/
			...
			int printdata=0;
			Statement stmtprintconfig=conn.createStatement();
			String strsqlprintconfig="select method from gl_config where field_nme='UnitedInvoicePrint'";
		//	System.out.println("config========="+strsqlprintconfig);
			ResultSet rsprint=stmtprintconfig.executeQuery(strsqlprintconfig);
			while(rsprint.next()){
				printdata=rsprint.getInt("method");
			}
			String ragquery="";
			if(printdata==1) {
				
		         ragquery=" left join (select if(r.clstatus=0,concat(rff.rentaltype,' - ',round(rff1.rate,2)),if(d.rentaltype!=rtf.rentaltype,concat(r1.rentaltype,' - ',round(r1.rate,2)),concat(rtf.rentaltype,' - ',round(rtf1.rate,2))))  rentaltype,r.doc_no from gl_ragmt r left join gl_rtarif rff  on rff.rdocno=r.doc_no and rff.rstatus=5  left join gl_rtarif rff1  on rff1.rdocno=r.doc_no and rff1.rstatus=7  "
	                        +" left join  gl_ragmtclosed d on d.rdocno=r.doc_no left join gl_rtarif rtf  on rtf.rdocno=d.rdocno and rtf.rstatus=5 left join gl_rtarif rtf1  on rtf1.rdocno=d.rdocno and rtf1.rstatus=7 left join gl_rtarif r1 on r1.rdocno=d.rdocno  and r1.rentaltype=d.rentaltype and d.rentaltype in('Daily','Weekly','Monthly')) rt "
	                        +" on ( rt.doc_no=inv.rano and inv.ratype='RAG' )";
	        }else {
				ragquery="left join (select concat(t.rentaltype,' - ',round(t1.rate,2)) rentaltype,r.doc_no from gl_ragmt r inner join gl_rtarif t " 
						+" on r.doc_no=t.rdocno and t.rstatus=5 inner join gl_rtarif t1 on r.doc_no=t1.rdocno and t1.rstatus=7) rt " 
						+" on ( rt.doc_no=inv.rano and inv.ratype='RAG' ) ";
			}
			//added type and rate for rag using ragmtclosed rate on 02-04-2025 using config
			strSql="select br.tinno brtinno,inv.manual,inv.brhid,coalesce(veh.CH_NO,'') chasisno,inv.status,DATE_FORMAT(coalesce(if(lagmt.per_name=1,DATE_ADD(lagmt.outdate,INTERVAL lagmt.per_value YEAR),DATE_ADD(lagmt.outdate,INTERVAL lagmt.per_value MONTH)),agmt.ddate),'%d/%m/%y')enddate,"
					+ " DATE_FORMAT(if((( select g.method FROM gl_config g where g.field_nme='delcal') =0) and (coalesce(if(inv.ratype='RAG',agmt.delivery,lagmt.delivery),'') =1), din , coalesce(agmt.odate,lagmt.outdate) ),'%d/%m/%Y') agmtdate,ac.trnnumber clienttrn,coalesce(br.tinno,'') comptrn,coalesce(inv.ldgrnote,'') ledgernote,datediff(inv.todate,inv.fromdate) creditdiff,"
			+" insp.doc_no inspno,inspveh.reg_no inspregno,inv.dtype,coalesce(if(inv.ratype='RAG',agmt.refno,lagmt.refno),'') refno,"
			+" case when inv.ratype='RAG' then agmt.otime when inv.ratype='LAG' then lagmt.outtime else '' end invfromtime, "
			...
			+" from gl_invm inv left join gl_ragmt agmt on (inv.rano=agmt.doc_no and inv.ratype='RAG')"
			+" left join gl_lagmt lagmt on(inv.rano=lagmt.doc_no and inv.ratype='LAG')  "
		    /*this line rag we done using ragmtclosed on 02-04-2025*/ + " "+ragquery+"  "   
			+" left join ( select concat('Lease' ,' - ' ,round(t.rate,2)) rentaltype,l.doc_no "
			+" from gl_lagmt l inner join gl_ltarif t on l.doc_no=t.rdocno ) lt on (lt.doc_no=inv.rano and inv.ratype='LAG' ) "
			+" left join gl_ragmtclosem agmtcl on (agmt.doc_no=agmtcl.agmtno) left join gl_lagmtclosem lagmtcl on (lagmt.doc_no=lagmtcl.agmtno)"
			+" left join my_acbook ac on (inv.cldocno=ac.cldocno and ac.dtype='CRM')"
			+" left join my_salm sal on (ac.sal_id=sal.doc_no and ac.dtype='CRM')"
			...
			+" inner join (select min(lo.loc) loc,lo.loc_name,lo.brhid from my_locm lo group by brhid) as lc"
			+" on(lc.loc=l.loc and lc.brhid=br.doc_no) left join my_user u on u.doc_no=inv.userid"
			+" left join gl_vmove v on v.rdocno=inv.rano and (v.rdtype=inv.ratype) and trancode='dl' and repno=0  and exchng=0 and custno=0 "
			+" where 1=1 "+sqltest+" group by inv.doc_no";
			...
			while(resultSet.next()){
				...
				fleet2=fleet2+" Fleet :"+resultSet.getString("flname");   
				...
				System.out.println("Current===="+currentfleet2);
...
//				System.out.println("Salik Size: "+salikdet.size());
				...
				for(int k=0;k<trafficdetelse.size();k++){
				   ...
				   String strSqls = "select method from gl_config where field_nme='invLogo'";
					ResultSet rs3 = stmtmanual.executeQuery(strSqls);
					int method=0;
					while(rs3.next()) {
						method=rs3.getInt("method");
					} 
					String bchhh="",cmpy="",addr="";
					if(method==1){
					   String sqlbrch="select brcode,imgpath from my_brch where doc_no="+branch;
						ResultSet rs1 = stmtmanuals.executeQuery(sqlbrch);
						String brcode="",imgpath="";
						while(rs1.next()) {
							brcode=rs1.getString("brcode");
							imgpath=rs1.getString("imgpath");
							
						}
						bchhh=brcode;
						addr=brcode;
						cmpy=resultSet.getString("branchname").toUpperCase();
						bean.setLblbranch(brcode);
						bean.setLblcomplogo(imgpath);
						bean.setLblcompname(cmpy);
						bean.setLblcompaddress(addr);
						
					}else {
						
						bchhh=resultSet.getString("branchname");
						cmpy=resultSet.getString("company");
						addr=resultSet.getString("address");
						bean.setLblbranch(resultSet.getString("branchname"));
						bean.setLblcomplogo("/icons/epic.jpg");
				...
				employees.add(new ClsManualInvoiceAction(resultSet.getString("voc_no"),resultSet.getString("refname"),resultSet.getString("codeno"),resultSet.getString("date"),
						...
						resultSet.getString("fromdate"),resultSet.getString("todate"),fleet,cmpy,addr,
						resultSet.getString("tel"),resultSet.getString("fax"),printname,bchhh,resultSet.getString("branchaddress"),
						...
						easycmp,salikauharray.size(),salikdxbarray.size(),resultSet.getString("manual"),trafic,resultSet.getString("cmppbno"),bean.getLblcompbranchaddress(),bean.getLblbankdetails(),bean.getLblbankbeneficiary1(),bean.getLblbankaccountno(),bean.getLblbeneficiarybank(),bean.getLblbankibanno1(),bean.getLblcomplogo(),bean.getLblprojectname(),resultSet.getString("brtinno")));  
				...
				bean.setLblcomppbno(resultSet.getString("cmppbno"));
				bean.setLblcomptrn(resultSet.getString("comptrn"));      
				...
				bean.setLblbrtinno(resultSet.getString("brtinno"));
				   System.out.println("pro in dao-----"+resultSet.getString("brtinno"));
...
//			System.out.println("Check Agmt Details Details: "+printagmtdetailmaster);
	...
	public ArrayList<String> getPrintTaxSummary(Connection conn, String docno) {
		...
		try{
			...
			String strtaxconfig="select (select if(saliktaxdate='0000-00-00',null,saliktaxdate) from my_comp where status=3) saliktaxdate,(select method from gl_config where field_nme='invPrintTax') printtaxconfig,(select cldocno from gl_invm where doc_no="+docno+" and status=3) cldocno,(select date from gl_invm where doc_no="+docno+" and status=3) date";
...
//			System.out.println(strsql);
	...
	public   String getCurrentFleet2(String lblrano,String lblratype,java.sql.Date sqlfromdate,java.sql.Date sqltodate) throws SQLException {
		...
		try {
			...
			while(rscurrent.next()){
				currentfleet="Reg No:"+rscurrent.getString("reg_no")+"  "+rscurrent.getString("code_name")+"  "+rscurrent.getString("authname")+" Fleet:"+rscurrent.getString("flname");
			}
		}
		catch(Exception e) {
			e.printStackTrace();
	...
	public   ArrayList<String> getPrintdetails(String docno) throws SQLException {
		...
		strSqldetail="select r.chid,r.units,round(r.amount,2) amount,round(r.total,2) total,m.description from gl_invmode m left join (select r.chid,"+
			...
			String strsaliktaxdate="select if(saliktaxdate='0000-00-00',null,saliktaxdate) saliktaxdate from my_comp where status=3";
			java.sql.Date saliktaxdate=null;
...
//				System.out.println(strchecktax);
				...
				if(taxstatus==1){
					...
					if(printsrvc==0){
						...
						//Commented by Rahis with discussion with Nitin.
						//Above Query wasnt getting the print format correctly as Sr.No,Description,Qty,Rate,Amount,Tax Percent,Tax Value,Net Amount.
						//Includes Normal Charges,Salik & Salik Srvc as single line, Salik AUH & Salik AUH Srvc as single line,Traffic & Traffic Srvc as single line.
						
						// config added for maximum company on 11-10-2024 to show unit as invd.units data if config 1 only this work otherwise old query will work
					
						int compmethod=0;String strsql="";
						Statement stmtconfig=conn.createStatement();
						String strsqlconfig="select method from gl_config where field_nme='InvPrintUnitConfig'";
					//	System.out.println("config========="+strsqlconfig);
						ResultSet rss=stmtconfig.executeQuery(strsqlconfig);
						while(rss.next()){
							compmethod=rss.getInt("method");
						}
						//System.out.println("compmethod------------"+compmethod);
						if(compmethod==1) {
						strsql="select cast(invd.units as UNSIGNED) qty,format(round(invd.amount,2),2) rate,if(mo.idno=1,concat(mo.description,' (',invd.units,')'),mo.description) description,format(round(invd.total,2),2) amount,"+
								" format(round(case when mo.tax=1 and mo.idno in (8,38) and inv.date>='"+saliktaxdate+"' then "+vatpercent+" when mo.tax=1 and mo.idno not in (8,38) then "+vatpercent+" else 0 end,2),2) taxpercent,"+
								" format(round(case when mo.tax=1 and mo.idno in (8,38) and inv.date>='"+saliktaxdate+"' then coalesce(invd.total*"+vatval+",0) when mo.tax=1 and mo.idno not in (8,38) then coalesce(invd.total*"+vatval+",0) else 0 end,2),2) taxamount,"+
								...
								" on (taxdet.rdocno=invd.rdocno) where invd.rdocno="+docno+" and invd.chid not in (8,9,14,15,38,39,20,19) union all "+
								" select cast(invd.units as UNSIGNED) qty,format(round(sum(invd.amount),2),2) rate,if(mo.idno=1,concat(mo.description,' (',invd.units,')'),mo.description) description,format(round(sum(invd.total),2),2) amount,"+
								" format(round(case when mo.tax=1 and mo.idno in (8,38) and inv.date>='"+saliktaxdate+"' then "+vatpercent+" when mo.tax=1 and mo.idno not in (8,38) then "+vatpercent+" else 0 end,2),2) taxpercent,"+
								" format(round(case when mo.tax=1 and mo.idno in (8,38) and inv.date>='"+saliktaxdate+"' then coalesce(sum(invd.total)*"+vatval+",0) when mo.tax=1 and mo.idno not in (8,38) then coalesce(sum(invd.total)*"+vatval+",0) else 0 end,2),2) taxamount,"+
								" format(round(sum(invd.total),2)+round(case when mo.tax=1 and mo.idno in (8,38) and inv.date>='"+saliktaxdate+"' then coalesce(sum(invd.total)*"+vatval+",0) when mo.tax=1 and mo.idno not in (8,38) then coalesce(sum(invd.total)*"+vatval+",0) else 0 end,2),2) total from gl_invd invd left join gl_invm inv on inv.doc_no=invd.rdocno left join gl_invmode mo on"+
								" invd.chid=mo.idno  left join (select total,rdocno from gl_invd where chid=20 group by rdocno,chid) taxdet "+
								" on (taxdet.rdocno=invd.rdocno) where invd.rdocno="+docno+" and invd.chid in (8,14) group by invd.rdocno,chid union all"+
								" select cast(invd.units as UNSIGNED) qty,format(round(sum(invd.amount),2),2) rate,if(mo.idno=1,concat(mo.description,' (',invd.units,')'),mo.description) description,format(round(sum(invd.total),2),2) amount,"+
								" format(round(case when mo.tax=1 and mo.idno in (8,38) and inv.date>='"+saliktaxdate+"' then "+vatpercent+" when mo.tax=1 and mo.idno not in (8,38) then "+vatpercent+" else 0 end,2),2) taxpercent,"+
								" format(round(case when mo.tax=1 and mo.idno in (8,38) and inv.date>='"+saliktaxdate+"' then coalesce(sum(invd.total)*"+vatval+",0) when mo.tax=1 and mo.idno not in (8,38) then coalesce(sum(invd.total)*"+vatval+",0) else 0 end,2),2) taxamount,"+
								" format(round(sum(invd.total),2)+round(case when mo.tax=1 and mo.idno in (8,38) and inv.date>='"+saliktaxdate+"' then coalesce(sum(invd.total)*"+vatval+",0) when mo.tax=1 and mo.idno not in (8,38) then coalesce(sum(invd.total)*"+vatval+",0) else 0 end,2),2) total from gl_invd invd left join gl_invm inv on inv.doc_no=invd.rdocno left join gl_invmode mo on"+
								" invd.chid=mo.idno  left join (select total,rdocno from gl_invd where chid=20 group by rdocno,chid) taxdet "+
								" on (taxdet.rdocno=invd.rdocno) where invd.rdocno="+docno+" and invd.chid in (38,39) group by invd.rdocno,chid union all"+
								" select cast(invd.units as UNSIGNED) qty,format(round(sum(invd.amount),2),2) rate,if(mo.idno=1,concat(mo.description,' (',invd.units,')'),mo.description) description,format(round(sum(invd.total),2),2) amount,"+
								" format(round(case when mo.tax=1 and mo.idno in (8,38) and inv.date>='"+saliktaxdate+"' then "+vatpercent+" when mo.tax=1 and mo.idno not in (8,38) then "+vatpercent+" else 0 end,2),2) taxpercent,"+
								" format(round(case when mo.tax=1 and mo.idno in (8,38) and inv.date>='"+saliktaxdate+"' then coalesce(sum(invd.total)*"+vatval+",0) when mo.tax=1 and mo.idno not in (8,38) then coalesce(sum(invd.total)*"+vatval+",0) else 0 end,2),2) taxamount,"+
								" format(round(sum(invd.total),2)+round(case when mo.tax=1 and mo.idno in (8,38) and inv.date>='"+saliktaxdate+"' then coalesce(sum(invd.total)*"+vatval+",0) when mo.tax=1 and mo.idno not in (8,38) then coalesce(sum(invd.total)*"+vatval+",0) else 0 end,2),2) total from gl_invd invd left join gl_invm inv on inv.doc_no=invd.rdocno left join gl_invmode mo on"+
								" invd.chid=mo.idno  left join (select total,rdocno from gl_invd where chid=20 group by rdocno,chid) taxdet "+
								" on (taxdet.rdocno=invd.rdocno) where invd.rdocno="+docno+" and invd.chid in (9,15) group by invd.rdocno,chid";
						
						System.out.println("remarkssss if con"+strsql);
						}
						else{
						...
						//	System.out.println("config========="+strsqlprintconfig);
							...
							if(printdata==1) {
							  ...
							  //round(case when invd.chid=1 and inv.ratype='RAG' then rtrf.rate when invd.chid=1 and inv.ratype='LAG' then ltrf.rate else invd.amount end,2) rate,
							    strsql="select cast(invd.units as UNSIGNED) qty,FORMAT(ROUND(CASE WHEN ROUND(CASE WHEN  inv.ratype = 'RAG' THEN rtrf.rate WHEN  inv.ratype = 'LAG' THEN ltrf.rate ELSE invd.total END, 2) > ROUND(invd.total, 2) THEN invd.total / invd.units ELSE coalesce(rtrf.rate,ltrf.rate) END, 2 ), 2 ) rate,"+
							    		" if(mo.idno=1,concat(mo.description,' (',invd.units,')'),mo.description) description, format(round(invd.total,2),2) amount,"+
                                        " format(round(case when mo.tax=1 and mo.idno in (8,38) and inv.date>='"+saliktaxdate+"' then "+vatpercent+" when mo.tax=1 and mo.idno not in (8,38) then "+vatpercent+" else 0 end,2),2) taxpercent,"+
                                        " format(round(case when mo.tax=1 and mo.idno in (8,38) and inv.date>='"+saliktaxdate+"' then coalesce(invd.total*"+vatval+",0) when mo.tax=1 and mo.idno not in (8,38) then coalesce(invd.total*"+vatval+",0) else 0 end,2),2) taxamount,"+
                                        " format(round(invd.total,2)+round(case when mo.tax=1 and mo.idno in (8,38) and inv.date>='"+saliktaxdate+"' then coalesce(invd.total*"+vatval+",0) when mo.tax=1 and mo.idno not in (8,38) then coalesce(invd.total*"+vatval+",0) else 0 end,2),2) total from gl_invd invd left join gl_invm inv on inv.doc_no=invd.rdocno left join gl_invmode mo on"+
                                        " invd.chid=mo.idno  left join (select total,rdocno from gl_invd where chid=20 group by rdocno,chid) taxdet "+
                                        " on (taxdet.rdocno=invd.rdocno) left join gl_rtarif rtrf on (inv.rano=rtrf.rdocno and inv.ratype='RAG' and rtrf.rstatus=7) left join gl_ltarif ltrf on (inv.rano=ltrf.rdocno and inv.ratype='LAG') where invd.rdocno="+docno+" and invd.chid not in (8,9,14,15,38,39,20,19) union all "+
                                        " select cast(invd.units as UNSIGNED) qty,round(case when invd.chid=1 and inv.ratype='RAG' then rtrf.rate when invd.chid=1 and inv.ratype='LAG' then ltrf.rate else invd.amount end,2) rate,if(mo.idno=1,concat(mo.description,' (',invd.units,')'),mo.description) description,format(round(sum(invd.total),2),2) amount,"+
                                        " format(round(case when mo.tax=1 and mo.idno in (8,38) and inv.date>='"+saliktaxdate+"' then "+vatpercent+" when mo.tax=1 and mo.idno not in (8,38) then "+vatpercent+" else 0 end,2),2) taxpercent,"+
                                        " format(round(case when mo.tax=1 and mo.idno in (8,38) and inv.date>='"+saliktaxdate+"' then coalesce(sum(invd.total)*"+vatval+",0) when mo.tax=1 and mo.idno not in (8,38) then coalesce(sum(invd.total)*"+vatval+",0) else 0 end,2),2) taxamount,"+
                                        " format(round(sum(invd.total),2)+round(case when mo.tax=1 and mo.idno in (8,38) and inv.date>='"+saliktaxdate+"' then coalesce(sum(invd.total)*"+vatval+",0) when mo.tax=1 and mo.idno not in (8,38) then coalesce(sum(invd.total)*"+vatval+",0) else 0 end,2),2) total from gl_invd invd left join gl_invm inv on inv.doc_no=invd.rdocno left join gl_invmode mo on"+
                                        " invd.chid=mo.idno  left join (select total,rdocno from gl_invd where chid=20 group by rdocno,chid) taxdet "+
                                        " on (taxdet.rdocno=invd.rdocno) left join gl_rtarif rtrf on (inv.rano=rtrf.rdocno and inv.ratype='RAG' and rtrf.rstatus=7) left join gl_ltarif ltrf on (inv.rano=ltrf.rdocno and inv.ratype='LAG') where invd.rdocno="+docno+" and invd.chid in (8,14) group by invd.rdocno,chid union all"+
                                        " select cast(invd.units as UNSIGNED) qty,round(case when invd.chid=1 and inv.ratype='RAG' then rtrf.rate when invd.chid=1 and inv.ratype='LAG' then ltrf.rate else invd.amount end,2) rate,if(mo.idno=1,concat(mo.description,' (',invd.units,')'),mo.description) description,format(round(sum(invd.total),2),2) amount,"+
                                        " format(round(case when mo.tax=1 and mo.idno in (8,38) and inv.date>='"+saliktaxdate+"' then "+vatpercent+" when mo.tax=1 and mo.idno not in (8,38) then "+vatpercent+" else 0 end,2),2) taxpercent,"+
                                        " format(round(case when mo.tax=1 and mo.idno in (8,38) and inv.date>='"+saliktaxdate+"' then coalesce(sum(invd.total)*"+vatval+",0) when mo.tax=1 and mo.idno not in (8,38) then coalesce(sum(invd.total)*"+vatval+",0) else 0 end,2),2) taxamount,"+
                                        " format(round(sum(invd.total),2)+round(case when mo.tax=1 and mo.idno in (8,38) and inv.date>='"+saliktaxdate+"' then coalesce(sum(invd.total)*"+vatval+",0) when mo.tax=1 and mo.idno not in (8,38) then coalesce(sum(invd.total)*"+vatval+",0) else 0 end,2),2) total from gl_invd invd left join gl_invm inv on inv.doc_no=invd.rdocno left join gl_invmode mo on"+
                                        " invd.chid=mo.idno  left join (select total,rdocno from gl_invd where chid=20 group by rdocno,chid) taxdet "+
                                        " on (taxdet.rdocno=invd.rdocno) left join gl_rtarif rtrf on (inv.rano=rtrf.rdocno and inv.ratype='RAG' and rtrf.rstatus=7) left join gl_ltarif ltrf on (inv.rano=ltrf.rdocno and inv.ratype='LAG') where invd.rdocno="+docno+" and invd.chid in (38,39) group by invd.rdocno,chid union all"+
                                        " select cast(invd.units as UNSIGNED) qty,round(case when invd.chid=1 and inv.ratype='RAG' then rtrf.rate when invd.chid=1 and inv.ratype='LAG' then ltrf.rate else invd.amount end,2) rate,if(mo.idno=1,concat(mo.description,' (',invd.units,')'),mo.description) description,format(round(sum(invd.total),2),2) amount,"+
                                        " format(round(case when mo.tax=1 and mo.idno in (8,38) and inv.date>='"+saliktaxdate+"' then "+vatpercent+" when mo.tax=1 and mo.idno not in (8,38) then "+vatpercent+" else 0 end,2),2) taxpercent,"+
                                        " format(round(case when mo.tax=1 and mo.idno in (8,38) and inv.date>='"+saliktaxdate+"' then coalesce(sum(invd.total)*"+vatval+",0) when mo.tax=1 and mo.idno not in (8,38) then coalesce(sum(invd.total)*"+vatval+",0) else 0 end,2),2) taxamount,"+
                                        " format(round(sum(invd.total),2)+round(case when mo.tax=1 and mo.idno in (8,38) and inv.date>='"+saliktaxdate+"' then coalesce(sum(invd.total)*"+vatval+",0) when mo.tax=1 and mo.idno not in (8,38) then coalesce(sum(invd.total)*"+vatval+",0) else 0 end,2),2) total from gl_invd invd left join gl_invm inv on inv.doc_no=invd.rdocno left join gl_invmode mo on"+
                                        " invd.chid=mo.idno  left join (select total,rdocno from gl_invd where chid=20 group by rdocno,chid) taxdet "+
                                        " on (taxdet.rdocno=invd.rdocno) left join gl_rtarif rtrf on (inv.rano=rtrf.rdocno and inv.ratype='RAG' and rtrf.rstatus=7) left join gl_ltarif ltrf on (inv.rano=ltrf.rdocno and inv.ratype='LAG') where invd.rdocno="+docno+" and invd.chid in (9,15) group by invd.rdocno,chid";
                            
							}else {
								 strsql="select cast(if(mo.idno=1,1,invd.units) as char(200)) qty,format(round(invd.amount,2),2) rate,if(mo.idno=1,concat(mo.description,' (',invd.units,')'),mo.description) description,format(round(invd.total,2),2) amount,"+
								" format(round(case when mo.tax=1 and mo.idno in (8,38) and inv.date>='"+saliktaxdate+"' then "+vatpercent+" when mo.tax=1 and mo.idno not in (8,38) then "+vatpercent+" else 0 end,2),2) taxpercent,"+
								" format(round(case when mo.tax=1 and mo.idno in (8,38) and inv.date>='"+saliktaxdate+"' then coalesce(invd.total*"+vatval+",0) when mo.tax=1 and mo.idno not in (8,38) then coalesce(invd.total*"+vatval+",0) else 0 end,2),2) taxamount,"+
								" format(round(invd.total,2)+round(case when mo.tax=1 and mo.idno in (8,38) and inv.date>='"+saliktaxdate+"' then coalesce(invd.total*"+vatval+",0) when mo.tax=1 and mo.idno not in (8,38) then coalesce(invd.total*"+vatval+",0) else 0 end,2),2) total from gl_invd invd left join gl_invm inv on inv.doc_no=invd.rdocno left join gl_invmode mo on"+
								" invd.chid=mo.idno  left join (select total,rdocno from gl_invd where chid=20 group by rdocno,chid) taxdet "+
								" on (taxdet.rdocno=invd.rdocno) where invd.rdocno="+docno+" and invd.chid not in (8,9,14,15,38,39,20,19) union all "+
								" select cast(if(mo.idno=1,1,invd.units) as char(200)) qty,format(round(sum(invd.amount),2),2) rate,if(mo.idno=1,concat(mo.description,' (',invd.units,')'),mo.description) description,format(round(sum(invd.total),2),2) amount,"+
								" format(round(case when mo.tax=1 and mo.idno in (8,38) and inv.date>='"+saliktaxdate+"' then "+vatpercent+" when mo.tax=1 and mo.idno not in (8,38) then "+vatpercent+" else 0 end,2),2) taxpercent,"+
								" format(round(case when mo.tax=1 and mo.idno in (8,38) and inv.date>='"+saliktaxdate+"' then coalesce(sum(invd.total)*"+vatval+",0) when mo.tax=1 and mo.idno not in (8,38) then coalesce(sum(invd.total)*"+vatval+",0) else 0 end,2),2) taxamount,"+
								" format(round(sum(invd.total),2)+round(case when mo.tax=1 and mo.idno in (8,38) and inv.date>='"+saliktaxdate+"' then coalesce(sum(invd.total)*"+vatval+",0) when mo.tax=1 and mo.idno not in (8,38) then coalesce(sum(invd.total)*"+vatval+",0) else 0 end,2),2) total from gl_invd invd left join gl_invm inv on inv.doc_no=invd.rdocno left join gl_invmode mo on"+
								" invd.chid=mo.idno  left join (select total,rdocno from gl_invd where chid=20 group by rdocno,chid) taxdet "+
							...
							System.out.println("remarkssss else"+strsql);
						}
						...
						while(rs.next()){
							...
							/*  salik with differnt amount should be shown differently in print 
						 * 
						...
						*/
						...
						String strsql="select mo.idno,cast(if(mo.idno=1,1,invd.units) as char(200)) qty,format(round(invd.amount,2),2) rate,if(mo.idno=1,concat(mo.description,' (',invd.units,')'),mo.description) description,format(round(invd.total,2),2) amount,"+
								" format(round(case when mo.tax=1 and mo.idno in (8,38) and inv.date>='"+saliktaxdate+"' then "+vatpercent+" when mo.tax=1 and mo.idno not in (8,38) then "+vatpercent+" else 0 end,2),2) taxpercent,"+
								" format(round(case when mo.tax=1 and mo.idno in (8,38) and inv.date>='"+saliktaxdate+"' then coalesce(invd.total*"+vatval+",0) when mo.tax=1 and mo.idno not in (8,38) then coalesce(invd.total*"+vatval+",0) else 0 end,2),2) taxamount,"+
								" format(round(invd.total,2)+round(case when mo.tax=1 and mo.idno in (8,38) and inv.date>='"+saliktaxdate+"' then coalesce(invd.total*"+vatval+",0) when mo.tax=1 and mo.idno not in (8,38) then coalesce(invd.total*"+vatval+",0) else 0 end,2),2) total from gl_invd invd left join gl_invm inv on inv.doc_no=invd.rdocno left join gl_invmode mo on"+
								" invd.chid=mo.idno  left join (select total,rdocno from gl_invd where chid=20 group by rdocno,chid) taxdet "+
								" on (taxdet.rdocno=invd.rdocno) where invd.rdocno="+docno+" and idno not in (20,19,8,38,51) "
								+ "  union all  select mo.idno,invd.qty,rate,mo.description,invd.amount, format(round(case when mo.tax=1 and mo.idno in (8,38) and invd.date>='"+saliktaxdate+"' then "+vatpercent+"  when mo.tax=1 and mo.idno not in (8,38) then "+vatpercent+" else 0 end,2),2) taxpercent,format(round(case when mo.tax=1 and mo.idno in (8,38) and invd.date>='"+saliktaxdate+"'  then coalesce(invd.total*"+vatval+",0) when mo.tax=1 and mo.idno not in (8,38) then coalesce(invd.total*"+vatval+",0) else 0 end,2),2) taxamount, format(round(invd.total,2)+round(case when mo.tax=1 and mo.idno in (8,38) and invd.date>='"+saliktaxdate+"'  then coalesce(invd.total*"+vatval+",0) when mo.tax=1 and mo.idno not in (8,38) then coalesce(invd.total*"+vatval+",0) else 0 end,2),2) total  from ( select count(*) qty, amount rate,if(source='auh', 38, if(type='PAR', 51, 8)) chid,  sum(amount) amount, sum(amount) total,m.date  from gl_salik s inner join gl_invm m on m.doc_no=s.inv_no   where inv_no="+docno+"  group by s.amount,chid ) invd left join gl_invmode mo on invd.chid=mo.idno order by idno; ";
...
//				System.out.println("Invoice Data:"+invoicearray.get(i));
			...
			int clientjvconfig=0;//1 for saperate client jv entry for salik,traffic,general.0-single entry
			 ...
			 String saliktype="";
			 double saliksrvcvat=0.0,trafficsrvcvat=0.0;
			 double saliksrvc=0.0,trafficsrvc=0.0,salikparksrvc=0.0;
			...
			String strsaliksrvcacno="select (select acno from gl_invmode where idno=14)salikdxbsrvcacno,(select acno from gl_invmode where idno=39)salikauhsrvcacno,(select method from gl_config where field_nme='invClientJV') clientjvconfig";
...
//						System.out.println("Currency Error");
					...
					String stracperiod="select DATE_ADD('"+date+"', INTERVAL (select coalesce(period2,0) from my_acbook where cldocno="+hidclient+" and dtype='CRM') DAY) duedate";
					//				System.out.println(stracperiod);
					ResultSet rsacperiod=stmtacperiod.executeQuery(stracperiod);
					while(rsacperiod.next()){
						duedate=rsacperiod.getDate("duedate");
					...
					for(int i=0;i< invoicearray.size();i++){
						String[] invoice=invoicearray.get(i).split("::");
						if(invoice[0].equalsIgnoreCase("8") || invoice[0].equalsIgnoreCase("14") || invoice[0].equalsIgnoreCase("51") || invoice[0].equalsIgnoreCase("52")){
...
//									System.out.println("Rcalc Error");
							...
							if(invoice[0].equalsIgnoreCase("8") || invoice[0].equalsIgnoreCase("14") || invoice[0].equalsIgnoreCase("38") || invoice[0].equalsIgnoreCase("39") || invoice[0].equalsIgnoreCase("51") || invoice[0].equalsIgnoreCase("52")){
								...
								if(invoice[0].equalsIgnoreCase("14") || invoice[0].equalsIgnoreCase("39") || invoice[0].equalsIgnoreCase("52")){
									...
									if(invoice[0].equalsIgnoreCase("52")){
										salikparksrvc=Double.parseDouble((String) (invoice[5].equalsIgnoreCase("undefined")?0:invoice[5]));
									}
								}
...
//							System.out.println("==== "+salikcostentryarray.get(i).split("::")[1]);
						...
						if(invdescconfig==1){
							...
							saliknote=salikqty+" "+saliktype+" of "+temprenttype+" - "+agmtvocno+" from "+strfromdate+" to "+strtodate;
						}
						/*if(salikauhsrvc>0.0){
						...
						saliksrvcvat+=objcommon.sqlRound(getSalikTrafficVAT(conn,"Salik#DXB",salikdxbsrvc+salikauhsrvc,hidclient,date,branchid,docno),2);
						System.out.println("Salik Srvc VAT: "+saliksrvcvat);
						salikamt+=saliksrvcvat;
						salikldr=salikamt*testcurrate;
						if(salikamt!=0){
					...
					if(trafficamt>0){
						...
						trafficsrvcvat=objcommon.sqlRound(getSalikTrafficVAT(conn,"Traffic",trafficsrvc,hidclient,date,branchid,docno),2);
						trafficamt+=trafficsrvcvat;
						trafficldr=trafficamt*testcurrate;
						if(clientjvconfig==0){
							generalamt+=trafficamt;
...
//							System.out.println("Inside TaxDetail");
						...
						// Getting amount in which tax is applicable
						Statement stmtgettax=conn.createStatement();
						String strtaxapplied="select round(total,2) total from gl_invd inv left join gl_invmode ac on inv.chid=ac.idno where ac.tax=1 and ac.idno<>20 and inv.rdocno="+docno;
...
//						System.out.println("General Jv sql:"+sqljv);
					...
					if (docno > 0) {
						...
						jsobj = objcommon.JVTotalCheck(conn,trno);  
	...
	public boolean delete(Connection conn,int docno,Date date,Date fromdate,Date todate,String cmbagmttype,int agmtno,String branchid,String userid,String formdetailcode) throws SQLException {
		...
		try{
			...
			String strclearsalik="update gl_salik set status=0,inv_no=0,inv_type='' where inv_no="+docno;
			Statement stmtclearsalik=conn.createStatement();
			int clearsalikval=stmtclearsalik.executeUpdate(strclearsalik);
			if(clearsalikval<0){
				return false;
	...
	public String getVoucherno(String val) throws SQLException {
		// TODO Auto-generated method stub
		Connection conn=null;
		String voucherno="";
		try{
			...
			while(rsvoucher.next()){
				voucherno=rsvoucher.getString("voc_no");
			}
			stmtvoucher.close();
			conn.close();
		}
	...
	public   ClsManualInvoiceBean sendMail (String fromno,String branch) throws SQLException {
		...
		try {
			...
			strSql="select  veh.CH_NO chassisno,coalesce(br.tinno,'') as tinno,coalesce(br.pbno,'') as pbno,coalesce(br.stcno,'') stcno,coalesce(br.cstno,'') cstno,coalesce(dr.name,dr1.name) driven,inv.fromdate sqlfromdate,inv.todate sqltodate,br.branch,year(inv.date) dateyear,agmt.pono,DATE_FORMAT(CURDATE(),'%d/%m/%Y')"+
					" finaldate,cur.code,cmp.company,cmp.address ,cmp.tel,cmp.fax,cmp.pbno cmppbno,br.branchname,ac.refname,ac.com_mob,ac.per_mob,ac.fax1,dr.name,"+
...
public int getOneDayExtraConfig(Connection conn) {
	...
	public int getDuplicateInvCount(Connection conn,String agmttype,String agmtno,java.sql.Date sqlfromdate,java.sql.Date sqltodate,String manual){
		...
		try{
			...
			String strinvduplicatecount="select count(*) rowcount from gl_invm inv left join gl_invd invd on inv.doc_no=invd.rdocno where invd.chid=1 and ratype='"+agmttype+"' and rano="+agmtno+" and ( '"+tempfromdate+"' between inv.fromdate and inv.todate or '"+sqltodate+"'  between inv.fromdate and inv.todate )  and status=3 and manual="+manual;
