...
//				System.out.println("==nettax==="+(purorderarray[8].trim().equalsIgnoreCase("undefined") || purorderarray[8].trim().equalsIgnoreCase("NaN")||purorderarray[8].trim().equalsIgnoreCase("")|| purorderarray[8].isEmpty()?0:purorderarray[8].trim()));
			    ...
			    if(!(purorderarray[1].trim().equalsIgnoreCase("undefined")|| purorderarray[1].trim().equalsIgnoreCase("NaN")||purorderarray[1].trim().equalsIgnoreCase("")|| purorderarray[1].isEmpty()))
			     {
			    	...
			    	String jobid=""+(purorderarray[12].trim().equalsIgnoreCase("undefined") || purorderarray[12].trim().equalsIgnoreCase("NaN")||purorderarray[12].trim().equalsIgnoreCase("")|| purorderarray[12].isEmpty()?0:purorderarray[12].trim())+"";    
	 				...
	 				+ "'"+(purorderarray[2].trim().equalsIgnoreCase("undefined") || purorderarray[2].trim().equalsIgnoreCase("NaN")|| purorderarray[2].trim().equalsIgnoreCase("")|| purorderarray[2].isEmpty()?0:purorderarray[2].trim())+"', "+ "'"+cmbcurrency+"','"+rate+"',"+fdramt*-1+","+tdramt*-1+",0,"+fid+",5,"+(i+1)+",0,0,0,'SRS', "
		...
		if (docno > 0) {
			...
			if(masteraction.getSrsrip().equalsIgnoreCase("1")) {
				String updateRip = "update my_rip set issrs="+tranno+" where month="+masteraction.getSrsmonth()+" and year="+masteraction.getSrsyear();
				int executeUpdate = stmtnipurchase.executeUpdate(updateRip);
				...
				if(!(purorderarray[1].trim().equalsIgnoreCase("undefined")|| purorderarray[1].trim().equalsIgnoreCase("NaN")||purorderarray[1].trim().equalsIgnoreCase("")|| purorderarray[1].isEmpty()))
			     {
				  ...
				  String acno1=""+(purorderarray[14].trim().equalsIgnoreCase("undefined") || purorderarray[14].trim().equalsIgnoreCase("NaN")||purorderarray[14].trim().equalsIgnoreCase("")|| purorderarray[14].isEmpty()?0:purorderarray[14].trim())+"";
						...
						+ "'"+cmbcurrency+"','"+rate+"',"+(fdramt*-1)+","+tdramt*-1+",0,"+((fdramt*-1)<0?-1:1)+",5,"+(i+1)+",0,0,0,'SRS', "
...
//			System.out.println("result"+result);
		    ...
		    String setStatusjv = "update my_jvtran set status=7 where out_amount=0 and tr_no="+tr_no;
            int rs = stmtnipurchase.executeUpdate(setStatusjv);
            if(rs==0)   
            {
                conn.close();
                return false;
...
public   JSONArray getGridDataSRS(HttpSession session,String data,String isSrs) throws SQLException {
	...
	try {
		...
		// System.out.println("Array Data - "+jsonArray);
			 conn = connDAO.getMyConnection();
			Statement stmtVeh1 = conn.createStatement ();   
			/*
			 * String pySql=
			 * ("select case when d.costtype=1 then cc.doc_no when d.costtype in(3,4) then cm.doc_no when d.costtype=11 then sc.voc_no when d.costtype=13 then ac.voc_no else 0 end as costcodeid,d.srno,d.desc1 description,d.unitprice,d.qty,d.qty qutval,d.total,d.discount,d.nettotal,d.nuprice,d.acno headdoc,h.gr_type grtype ,"
			 * +
			 ...
			 * + "  " );
			 */	
			String  pySql = "select 0 discount,1 qty,sm.description,h.gr_type grtype,h.account account,h.description accname,h.atype type,sm.srno  from gl_sumac sm left join my_head h on h.doc_no=sm.acno where status=1";
			
			//System.out.println("=====++==="+pySql);
			ResultSet resultSet = stmtVeh1.executeQuery(pySql);
			JSONObject obj = new JSONObject();
			while(resultSet.next()) {
				obj.put("qty", resultSet.getString("qty"));
				obj.put("description", resultSet.getString("description"));
				obj.put("account", resultSet.getString("account"));
				obj.put("grtype", resultSet.getString("grtype"));
				obj.put("headdoc", resultSet.getString("account"));
				obj.put("accname", resultSet.getString("accname"));
				obj.put("type", resultSet.getString("type"));
				obj.put("discount", resultSet.getString("discount"));
				obj.put("srno", resultSet.getString("srno"));
				
				
				for(int i=0;i<jsonArray.size();i++) {
					if(resultSet.getString("description").equalsIgnoreCase(jsonArray.getJSONObject(i).getString("description"))) {
						System.out.println(jsonArray.getJSONObject(i).optDouble("grossamount",0)*jsonArray.getJSONObject(i).optDouble("id",1));
						obj.put("taxper", jsonArray.getJSONObject(i).optDouble("vatper",0));
						obj.put("taxperamt", jsonArray.getJSONObject(i).optDouble("vat",0));
						obj.put("taxamount", jsonArray.getJSONObject(i).optDouble("netamount",0));
						obj.put("unitprice", jsonArray.getJSONObject(i).optDouble("grossamount",0));
						obj.put("qutval", jsonArray.getJSONObject(i).optDouble("grossamount",0));
						obj.put("total", jsonArray.getJSONObject(i).optDouble("grossamount",0));
						obj.put("qutval", jsonArray.getJSONObject(i).optDouble("grossamount",0));
						obj.put("nuprice", jsonArray.getJSONObject(i).optDouble("grossamount",0));
						obj.put("nettotal", jsonArray.getJSONObject(i).optDouble("grossamount",0));
					}
				}
				resultArray.add(obj);
			}
