package com.dashboard.humanresource.timesheetreviewaccess;
...
//					 System.out.println("hottime = "+hotTime+"== hottimehours = "+hotTimeHours[1]);
	...
	public JSONArray employeeDetailsSearch(String empid,String employeename,String contact,String category) throws SQLException {
	    ...
	    try {
				...
				//System.out.println("vvvvvvv"+sql);
				ResultSet resultSet = stmtBTSR.executeQuery(sql);
...
//					 System.out.println(sql6s);
					...
					// System.out.println("working hrs "+actualWhrs);  
...
//					 System.out.println("second--"+normalTime+"---"+otTime+"---"+hotTime+"--"+normalHourInHour+"--"+otInHour+"--"+hotInHour+"--");
	...
	public int setval(String date,String empdocno,String inrowno,String normalhrs,String intime,String outtime) throws SQLException {
		...
		//	System.out.println("values===========date,emp,row,normal,in,out"+date+":"+empdocno+":"+inrowno+":"+normalhrs+":"+intime+":"+outtime);
			conn=ClsConnection.getMyConnection();
	//		conn.setAutoCommit(false);
		...
		Statement stmtBHTS1=conn.createStatement();
		 ...
		 SimpleDateFormat sdf1 = new SimpleDateFormat("yyyy-MM-dd");
			...
			Date sqlStartDate = new java.sql.Date(javadate.getTime());
		 ...
		 while (resultSets.next()) {
			 sqlHolidayDate=resultSets.getDate("holidaysofmonth");
			 if(sqlHolidayDate.compareTo(sqlStartDate) == 0){
				 holidayWorked=1;
				 break;
			 }
