...
public class ClsSeverancePay  { 
				...
				Statement stmtBTSP = conn.createStatement();
			     ...
			     if(!(reportType.equalsIgnoreCase(""))){
		            	if(reportType.equalsIgnoreCase("1")) {
		            		strSql=strSql+" and ((m.active=1 and m.status=3) or (m.terminated_date>'"+sqlUpToDate+"'))";  
		            	} else if(reportType.equalsIgnoreCase("0")) {
		            		strSql=strSql+" and ((m.active=0 and m.status=7) and (m.terminated_date<='"+sqlUpToDate+"'))";  
		            	}
		            }
	...
	public JSONArray employeeDetailsSearch(String empid,String employeename,String contact, String partyStatus,String employeebranchchk,String brhid) throws SQLException {
	    Connection conn=null;
