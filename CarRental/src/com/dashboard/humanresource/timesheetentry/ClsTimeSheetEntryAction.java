...
public class ClsTimeSheetEntryAction {
	
	ClsCommon ClsCommon=new ClsCommon();
	ClsTimeSheetEntryDAO timeSheetDAO=new ClsTimeSheetEntryDAO();
	...
	public String saveAction() throws ParseException, SQLException{
		...
		if(mode.equalsIgnoreCase("A")){
			...
			int val=timeSheetDAO.insert(getCmbbranch(),timesheetarray,session,request,mode);
			if(val>0.0){
				...
				setDetail("Human Resource");
				setDetailname("Time Sheet Entry");
				//setHidcmbyear(getCmbyear());
				//setHidcmbmonth(getCmbmonth());
				setMsg("Not Saved");
				return "fail";
