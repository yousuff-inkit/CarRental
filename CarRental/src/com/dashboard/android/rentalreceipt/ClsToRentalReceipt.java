...
*/            	String sqldata= "SELECT rr.branch_no brno,ac.cldocno,mm.acno txtdoc,ac.acno txtacno,coalesce(m.doc_no,0) cardtype,coalesce(mm.doc_no,0) type,"
            	              ...
            	              + " left join my_brch br on rr.branch_no=br.doc_no left join my_cardm m on m.doc_no=rr.cardtype and m.dtype='card'"
            	              + " left join my_acbook ac on ac.cldocno=rr.cldocno and ac.dtype='CRM'"
            	              + " left join my_cardm mm on mm.doc_no=rr.paytype and mm.dtype='mode' "
            	              + "where date(doc_date) between '"+sqlfromdate+"' and '"+sqltodate+"' and rr.rrv_trno='0' "+sqltest+" ";
