...
public class ClslaDueDateDAO {
	...
	public JSONArray detailsgrid(String brnchval,String uptodate,String cldocno,String fleet) throws SQLException {
		...
		try {
			...
			/*String sql ="select * from (select ms.sal_name,l.voc_no,l.brhid,d.name drname,coalesce(manualra,'') mrno,l.doc_no, "
					...
					+ " j.rtype='lag' where l.status=3 group by rdocno) jv on jv.docno=l.doc_no where l.status=3 and l.clstatus=0 and l.dispute=0 "+sqltest+" group by l.doc_no) a "
					+ "where a.duedate<='"+sqluptodate+"' ";*/
			
			String sql ="select bv.fdate,ms.sal_name,l.voc_no,l.brhid,d.name drname,coalesce(manualra,'') mrno,l.doc_no, "
					+ "if(l.perfleet=0,l.tmpfleet,l.perfleet) fleet_no,l.cldocno,l.outdate,l.outtime,l.outkm,v.reg_no,v.FLNAME vehdetails,"
					+ "a.refname,a.per_mob,a.contactperson,l.duedate,l.per_value,coalesce(round(rt.rate,2),0) rate,coalesce(round(jv.sum,2),0) sum from gl_lagmt l"
					+ " left join gl_vehmaster v on if(l.perfleet=0,l.tmpfleet,l.perfleet)=v.fleet_no"
					+ " left join my_acbook a on a.cldocno=l.cldocno and a.dtype='CRM' left join gl_vehgroup g on g.doc_no=v.vgrpid "
					+ "left join gl_vehbrand br on br.doc_no=v.brdid  left join gl_vehmodel mo on  mo.doc_no=v.vmodid "
					+ "left join gl_ldriver dr on dr.rdocno=l.doc_no left join  gl_drdetails d on (d.dr_id=dr.drid )"
					+ " left join my_salm ms on ms.doc_no=l.salid"
					+ " left join gl_ltarif rt on l.doc_no=rt.rdocno  left join (select max(b.doc_no) doc_no,rdocno from gl_bldr b group by  rdocno) b on(b.rdocno=l.doc_no) "
					+ " left join gl_bldr bv on b.doc_no=bv.doc_no  left join( select l.doc_no docno,sum(coalesce(j.dramount,0)) sum from gl_lagmt l inner join my_jvtran j  on j.acno=l.acno and l.doc_no=j.rdocno and"
					+ " j.rtype='lag' group by rdocno) jv on jv.docno=l.doc_no where  l.clstatus=0 and l.dispute=0 and l.duedate<='"+sqluptodate+"'group by l.doc_no order by duedate";
	...
	public JSONArray exceldetailsgrid(String brnchval,String uptodate,String cldocno,String fleet) throws SQLException {
		...
		try {
			...
			String sql ="select bv.fdate,l.voc_no 'LA NO',if(l.perfleet=0,l.tmpfleet,l.perfleet) 'Fleet',"
					+ "v.FLNAME 'Fleet Name',v.reg_no 'Reg No',a.refname 'Client Name',a.contactperson 'Contact Person',"
					+ "d.name 'Driver',coalesce(a.per_mob,'') 'Mob NO',date_format(l.outdate,'%d.%m.%Y') 'Out Date',l.outtime 'Out Time',coalesce(manualra,'') 'Manual LA', "
					+ "date_format(l.duedate,'%d.%m.%Y') 'Due Date',"
					+ "ms.sal_name 'Salesman',coalesce(round(rt.rate,2),0) Rate,coalesce(jv.sum,0) 'Credit_Balance',l.duedate from gl_lagmt l"
					+ " left join gl_vehmaster v on if(l.perfleet=0,l.tmpfleet,l.perfleet)=v.fleet_no"
					+ " left join my_acbook a on a.cldocno=l.cldocno and a.dtype='CRM' left join gl_vehgroup g on g.doc_no=v.vgrpid "
					+ "left join gl_vehbrand br on br.doc_no=v.brdid  left join gl_vehmodel mo on  mo.doc_no=v.vmodid "
					+ "left join gl_ldriver dr on dr.rdocno=l.doc_no left join  gl_drdetails d on (d.dr_id=dr.drid )"
					+ " left join my_salm ms on ms.doc_no=l.salid   left join (select max(b.doc_no) doc_no,rdocno from gl_bldr b group by  rdocno) b on(b.rdocno=l.doc_no) " 
					+ " left join gl_bldr bv on b.doc_no=bv.doc_no "
					+ " left join gl_ltarif rt on l.doc_no=rt.rdocno  left join"
					+ "( select l.doc_no docno,sum(coalesce(j.dramount,0)) sum from gl_lagmt l inner join my_jvtran j  on j.acno=l.acno and l.doc_no=j.rdocno and"
					+ " j.rtype='lag' where l.status=3 group by rdocno) jv on jv.docno=l.doc_no  where  l.clstatus=0 and l.dispute=0 and l.duedate<='"+sqluptodate+"'  group by l.doc_no order by duedate";
	...
	public JSONArray duedateDetails(String rdocno) throws SQLException {
		...
		try {
				...
				Statement stmtVeh = conn.createStatement ();
            		...
            		String sql=" select Date_FORMAT(m.date, '%Y-%m-%d') as  detdate,m.remarks remk,m.fdate,u.user_name user from gl_bldr m "
