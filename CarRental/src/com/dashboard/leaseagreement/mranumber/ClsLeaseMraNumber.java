package com.dashboard.leaseagreement.mranumber;
...
public class ClsLeaseMraNumber 
{
...
//System.out.println(salesman);
   ...
   //	System.out.println("hjbf"+salesmandoc);
 	...
 	if(!((brnchval.equalsIgnoreCase("a")) || (brnchval.equalsIgnoreCase("NA") || brnchval.equalsIgnoreCase("NULL")))){
 	...
 	try {
				...
				Statement stmtVeh = conn.createStatement ();
         		...
         		String sql =" select @i:=@i+1 slno, a.* from(select  @s:=@s+1 SL,r.doc_no Docno,r.voc_no 'LA NO',r.manualra 'PO NO',if(r.perfleet=0,r.tmpfleet,r.perfleet) Fleet,v.FLNAME 'Fleet Name',"
         				...
         				+ "(SELECT @s:= 0) AS s  where r.status=3     " +sqltest+ " group by r.doc_no)a,(select @i:=0)c";
