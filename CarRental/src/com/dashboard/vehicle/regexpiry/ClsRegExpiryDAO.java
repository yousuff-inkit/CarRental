...
public class ClsRegExpiryDAO {
	...
	public JSONArray getRegExpiryData(String branch,String expdate,String fromdate,String id,String cldocno) throws SQLException {
		...
		try {
			    ...
			    String sqlexp="select ROUND(@i:=@i+1 ,0) slno,a.* from(select case when m.tran_code in ('RA','RD','RW','RM') then 'Rental' when m.tran_code in ('LA','LC') then 'Lease' else '' end agmttype,"+
				...
				" gl_lagmt lag on ((if(lag.tmpfleet=0,lag.perfleet,lag.tmpfleet)=m.fleet_no) and lag.clstatus=0) left join my_acbook rac on"+
				" (rag.cldocno=rac.cldocno and rac.dtype='CRM') left join my_acbook lac on (lag.cldocno=lac.cldocno and lac.dtype='CRM') "+
				" where m.statu=3 and fstatus in ('L','N') "+sqltest+")a,(select @i:=0)b" ;
			    System.out.println("sqlexp 1 "+sqlexp);
