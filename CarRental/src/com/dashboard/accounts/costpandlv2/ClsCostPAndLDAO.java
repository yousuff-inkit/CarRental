package com.dashboard.accounts.costpandlv2;
 ...
 */
        		...
        		sql=" select e.costtype,CONVERT(CASE WHEN e.costtype=1 THEN CONCAT(e.costcode,' - ',cs.description) WHEN e.costtype=6 THEN CONCAT('Fleet No. :',e.costcode,' - Reg No. :',v.reg_no,' - ',v.flname)"+casestatement+" WHEN e.costtype in(3,4) THEN CONCAT(e.costcode,' - ',ac.refname) ELSE e.costcode END,CHAR(1000)) AS 'costcode',"
        				...
        				+ "select brhid,a.costtype,a.costcode,a.acno,a.tr_no,round(sum(a.income),2) income,round(sum(a.expenditure),2) expenditure,round((sum(a.income)+sum(a.expenditure)),2) netamount,a.gr_type from "
        				+ "(select t.brhid,c.costtype,c.jobid costcode,t.acno,t.tr_no, c.amount income,0 expenditure,h.gr_type from my_costtran c inner join my_jvtran t on c.tranid=t.tranid inner join my_head h on h.doc_no=t.acno "
        				...
        				+ " select t.brhid,c.costtype,c.jobid costcode,t.acno,t.tr_no, 0 income,c.amount expenditure,h.gr_type from my_costtran c inner join my_jvtran t on c.tranid=t.tranid inner join my_head h on h.doc_no=t.acno and h.gr_type=4 "
        				...
        				+ " select d.brhid,d.costtype,d.costcode,d.acno,d.tr_no,round(sum(d.income),2) income,round(sum(d.expenditure),2) expenditure,round((sum(d.income)+sum(d.expenditure)),2) netamount,d.gr_type "
        				+ " from ( select t.brhid,c.costtype,c.jobid costcode,t.acno,t.tr_no,if(h.gr_type=5,c.amount,0) income,if(h.gr_type=4,c.amount,0) expenditure,h.gr_type from my_costtran c inner join my_jvtran t "
        				...
        				+ " and t.date<='"+sqlToDate+"' and t.date>='"+sqlFromDate+"' "+sql1+" ) d UNION ALL select * from (select brhid,'UNALLOCATED' costtype, 'UNALLOCATED'  costcode,d.acno,d.tr_no,round(sum(d.income),2) income, "
        				+ " round(sum(d.expenditure),2) expenditure,round((sum(d.income)+sum(d.expenditure)),2) netamount,d.gr_type  from (select t.brhid,t.costtype,t.costcode,t.acno,t.tr_no,if(h.gr_type=5,t.dramount-COALESCE(C.AMOUNT,0),0) income,  "
        				...
        				+ "   where h.gr_type in (4,5) and t.status=3 and t.yrid=0 and t.date between '"+sqlFromDate+"' and  '"+sqlToDate+"'  )d ) d group by d.costcode) e left join my_costunit u "
        				+ " on e.costtype=u.costtype left join my_ccentre cs on cs.doc_no=e.costcode and e.costtype=1 left join gl_vehmaster v on v.fleet_no=e.costcode and e.costtype=6  left join cm_srvcontrm cm on cm.doc_no=e.costcode and cm.brhid=e.brhid and cm.costtype=e.costtype left join my_acbook ac on ac.cldocno=cm.cldocno and ac.dtype='crm'"+joins+"";
