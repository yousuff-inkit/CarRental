...
public class ClsCurrentAgeingStatementMonthlyDAO  { 
	...
	public JSONArray currentAgeingStatementMonthlyGridLoading(String branch,String sqlUpToDate,String atype,String accdocno, String salesperson,String category,String loadInBaseCurr,String check) throws SQLException {
		...
		try {
				...
				if(atype.equalsIgnoreCase("AR")){
				    ...
				    select = "select ag.currency,ag.category,ag.name,ag.account, ag.acno, ag.brhid, ag.doc_no, ag.date,CONVERT(if(sum(ag.u6<0),round((sum(ag.u6)),2),''),CHAR(50)) 'unapplied',CONVERT(if((if(sum(ag.t7+ag.u6)>0,round((sum(ag.t7+ag.u6)),2),0))-(if(sum(ag.t7+ag.u6)<0,round((sum(ag.t7+ag.u6)*-1),2),0))=0,'',"
				    	   ...
				    	   + "select cr.code currency,a.name,a.account, a.acno, a.brhid, a.doc_no, a.date, a.bal, a.u6, a.t7,round(if((MONTH('"+sqlUpToDate+"')=MONTH(a.date) and  YEAR('"+sqlUpToDate+"')=YEAR(a.date)) and a.bal>0 ,round((a.bal),2),0),2) currentmonth,"
					...
					select = "select ag.currency,ag.category,ag.name,ag.account, ag.acno, ag.brhid, ag.doc_no, ag.date,CONVERT(if(sum(ag.u6>0),round((sum(ag.u6)),2),''),CHAR(50)) 'unapplied',CONVERT(if((if(sum(ag.t7+ag.u6)<0,round((sum(ag.t7+ag.u6)*-1),2),0))-(if(sum(ag.t7+ag.u6)>0,round((sum(ag.t7+ag.u6)),2),0))=0,''," 
				...
				if(!(category.equalsIgnoreCase(""))){
				  ...
				  if((loadInBaseCurr.equalsIgnoreCase("1"))){
					  sql =  select+"  ( "  
							  + "select j.acno,j.brhid,h.account,h.description name,sum(dramount-coalesce(out_amount,0))*j.rate bal, j.tranid,j.doc_no,coalesce(j.duedate,j.date) date from my_jvtran j inner join my_head h on j.acno=h.doc_no where j.status=3 and h.atype='"+atype+"' "
							  + " "+sql+" group by j.tranid having bal<>0) d ) a left join my_acbook bk on (a.acno=bk.acno and bk.status=3) left join my_clcatm cat on bk.catid = cat.doc_no left join my_salm s on bk.sal_id=s.doc_no"
							  + " left join my_curr cr on cr.doc_no=bk.curId where 1=1"+sql1+") ag group by ag.acno";
