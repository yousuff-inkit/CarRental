...
public class ClsAgeingStatementDAO  { 
	...
	public JSONArray ageingStatement(String branch,String uptodate,String atype,String accdocno, String salesperson,String category,String clientStatus,String level1from, String level1to, String level2from, String level2to,
		...
		try {
				...
				if(atype.equalsIgnoreCase("AR")){
					...
					select ="select rostatus+lostatus activecont,coalesce(cat.cat_name,'') catname,a.*,bk.period2 creditperiod,bk.credit creditlimit,bk.mail1 email,if(bk.per_mob is null,bk.com_mob,if(bk.per_mob='NA',bk.com_mob,if(bk.per_mob=' ',bk.com_mob,bk.per_mob))) mobile_no,bk.contactPerson contact_person,s.sal_name from (select ag.code,ag.accno,name 'account_name',CONVERT(if(sum(t7+u6)<0,round((sum(t7+u6)*-1),2),''),CHAR(50)) 'advance',\r\n" + 
							"CONVERT(if(sum(t7+u6)>0,round((sum(t7+u6)),2),''),CHAR(50)) 'balance',CONVERT(if(sum(u6<0),round((sum(u6*-1)),2),''),CHAR(50)) 'unapplied',\r\n" + 
							"CONVERT(if(sum(t7)>0,round((sum(t7)),2),''),CHAR(50)) 'total',CONVERT(if(sum(l1)>0,round((sum(l1)),2),''),CHAR(50)) 'level_1',\r\n" + 
							"CONVERT(if(sum(l2)>0,round((sum(l2)),2),''),CHAR(50)) 'level_2',CONVERT(if(sum(l3)>0,round((sum(l3)),2),''),CHAR(50)) 'level_3',\r\n" + 
							"CONVERT(if(sum(l4)>0,round((sum(l4)),2),''),CHAR(50)) 'level_4',CONVERT(if(sum(l5)>0,round((sum(l5)),2),''),CHAR(50)) 'level_5',\r\n" + 
							"ag.acno 'account',ag.brhid 'branch_id' from (select d.code,d.accno,d.name,d.acno,d.brhid,d.doc_no,\r\n" + 
					...
					/*select ="select name 'account_name',ag.contact 'contact_person',ag.pmob 'mobile_no',CONVERT(if(sum(t7+u6)>0,round((sum(t7+u6)),2),''),CHAR(50)) 'advance',"
							...
							+ "and "+level3to+"  and d.bal<0,round((d.bal),2),0) l3,if(d.duedys between "+level4from+" and "+level4to+"  and d.bal<0,round((d.bal),2),0) l4,"  
							+ "if(d.duedys >="+level5from+"  and d.bal<0,round((d.bal),2),0) l5,CONVERT(if(d.bal>0,round((d.bal),2),''),CHAR(10)) U6,if(d.bal<0,d.bal,0) t7 from ";*/
					select ="select coalesce(cat.cat_name,'') catname,a.*,bk.period2 creditperiod,bk.credit creditlimit,bk.mail1 email,if(bk.per_mob is null,bk.com_mob,if(bk.per_mob='NA',bk.com_mob,if(bk.per_mob=' ',bk.com_mob,bk.per_mob))) mobile_no,bk.contactPerson contact_person,s.sal_name from (select ag.code,ag.accno,name 'account_name',CONVERT(if(sum(t7+u6)>0,round((sum(t7+u6)),2),''),CHAR(50)) 'advance',\r\n" + 
							"CONVERT(if(sum(t7+u6)<0,round((sum(t7+u6)*-1),2),''),CHAR(50)) 'balance',CONVERT(if(sum(u6>0),round((sum(u6)),2),''),CHAR(50)) 'unapplied',\r\n" + 
							"CONVERT(if(sum(t7)<0,round((sum(t7)*-1),2),''),CHAR(50)) 'total',CONVERT(if(sum(l1)<0,round((sum(l1)*-1),2),''),CHAR(50)) 'level_1',\r\n" + 
							"CONVERT(if(sum(l2)<0,round((sum(l2)*-1),2),''),CHAR(50)) 'level_2',CONVERT(if(sum(l3)<0,round((sum(l3)*-1),2),''),CHAR(50)) 'level_3',\r\n" + 
				...
				sql = select+" (select c.code,j.acno,j.brhid,h.description name,h.account accno,sum(dramount) - coalesce(o.amount,0)*id bal, j.tranid, j.doc_no,TIMESTAMPDIFF(Day,cast(j.date as datetime),cast('"+sqlUpToDate+"' as datetime)) duedys\r\n" + 
						"from my_jvtran j inner join my_head h on j.acno=h.doc_no left join my_curr c on c.doc_no=h.curid left join (select ap_trid,o.tranid,sum(coalesce(amount,0)) amount from my_outd o inner join my_jvtran j on j.tranid=o.tranid where\r\n" + 
						...
						" union all select c.code,j.acno,j.brhid,h.description name,h.account accno,sum(dramount)- coalesce(o.amount,0)*id bal, j.tranid, j.doc_no,TIMESTAMPDIFF(Day,cast(j.date as datetime),cast('"+sqlUpToDate+"' as datetime)) duedys\r\n" + 
						" from my_jvtran j inner join my_head h on j.acno=h.doc_no left join my_curr c on c.doc_no=h.curid left join (select ap_trid,o.tranid,sum(coalesce(amount,0)) amount from my_outd o inner join my_jvtran j on j.tranid=o.ap_trid where j.date<='"+sqlUpToDate+"'\r\n" + 
	...
	try {
		...
		String branchfilter="";
		if(branch.equalsIgnoreCase("a") || branch.equalsIgnoreCase("") || branch.equalsIgnoreCase("NA")) {
			branchfilter=" and b.mainbranch=1";
		}
		else {
			branchfilter=" and b.doc_no="+branch;
		}
/*
 ...
 * 		String headersql="select b.doc_no brhid,'Outstanding Statement' vouchername,(DATE_FORMAT('"+sqlUpToDate+"','%D %M  %Y ')) vouchername1,c.company,c.address,c.tel,c.fax,lc.loc_name "
		...
		String headersql="select b.doc_no brhid,'Outstanding Statement' vouchername,(DATE_FORMAT('"+sqlUpToDate+"','%D %M  %Y ')) vouchername1,c.company,c.address,c.tel,c.fax,lc.loc_name location,c.email"
				+ ",b.branchname,b.pbno,b.stcno,b.cstno from  my_brch b inner join "
				...
				+ "brhid) as lc on(lc.loc=l.loc and lc.brhid=b.doc_no) where 1=1 "+branchfilter;
...
//					System.out.println("======="+bean.getLblcompname());
					...
					bean.setLblcompemail(resultSetHead.getString("email"));
