...
public class ClsIndividualAgeing  { 
	...
	public JSONArray individualAgeingOldDate(String branch,String uptodate,String atype,String accdocno, String salesperson,String category,String check) throws SQLException {
		...
		try {
            	...
            	int agmtconfig=0;
            	ResultSet rsagmtconfig=stmtIndividualAgeing.executeQuery(stragmtconfig);
            	while(rsagmtconfig.next()) {
            		agmtconfig=rsagmtconfig.getInt("method");
            	}
            	String agmtselect="", agmtselect2="";
            	if(agmtconfig==1 && atype.trim().equalsIgnoreCase("AR")) {
            		...
            		agmtselect=",CONCAT(auth.AUTHNAME,' ',veh.reg_no,' ',plt.CODE_NAME) regno,a.rtype agmttype,convert(CASE WHEN a.rtype='RAG' THEN rag.voc_no WHEN a.rtype='LAG' THEN lag.voc_no ELSE '' END,char(15)) agmtvocno,\r\n" + 
            				"CASE WHEN a.rtype='RAG' THEN COALESCE(rag.pono,'') WHEN a.rtype='LAG' THEN COALESCE(lag.po,'') ELSE '' END lpono";
            		agmtselect2=", regno, agmttype, agmtvocno, lpono";
            	}
            	
				if(atype.equalsIgnoreCase("AR")){
					condition=" and bk.dtype='CRM'";
					
					sql = "select runbal, agmtconfig, name, contact, salname, pmob, ADVANCE, BALANCE, UNAPPLIED, TOTAL, date, transtype, convert(transno, char(45)) transno, acno, brhid, age, description, code, famount , fdate , fcount, fbalance"+agmtselect2+" from(  select @tot:=@tot+balance runbal,"+agmtconfig+" agmtconfig,a.name,a.contact,coalesce(sm.sal_name,'') salname,a.pmob,CONVERT(if(if(balance>0,balance,0)+if(balance<0,round((balance),2),0)<0,round((if(balance>0,balance,0)+if(balance<0,round((balance),2),0)*-1),2),''),CHAR(50)) 'ADVANCE',"
	                        ...
	                        + "a.date,a.transtype,"+casestatement+"a.acno,a.brhid,a.age,a.description,a.code, famount , fdate , fcount, CONVERT(IF(IF(balance>0,balance,0)+IF(balance<0,ROUND((balance),2),0)>0,ROUND((IF(balance>0,balance,0)+IF(balance<0,ROUND((balance),2),0)),2)-coalesce(famount,0.0),''),CHAR(50)) fbalance "+agmtselect+" from ("
	                        + "select cc.code,bk.sal_id,j.date,j.dtype transtype,convert(j.doc_no,char(50)) transno,j.acno,h.description name,bk.per_mob pmob,bk.contactPerson contact,if(j.description='0','',j.description) description,sum(dramount) amount,CONVERT(coalesce(o.amount,''),CHAR(50)) applied,sum(dramount) - "
	                        + "coalesce(o.amount,0) balance, j.tranid, j.brhid,TIMESTAMPDIFF(Day,cast(j.date as datetime),cast('"+sqlUpToDate+"' as datetime)) age,j.rdocno,j.rtype,j.tr_no, famount , fdate , fcount from my_jvtran j inner join my_brch b on j.brhId=b.doc_no "
	                        + "inner join my_curr bc on b.curId=bc.doc_no inner join my_head h on j.acno=h.doc_no inner join my_curr c on j.curId=c.doc_no left join my_acbook bk on h.cldocno=bk.cldocno "+condition+" left join my_curr cc on cc.doc_no=h.curid "
	                        + "left join (select ap_trid,o.tranid,SUM(IF(j.date<='"+sqlUpToDate+"',COALESCE(amount,0),0)) amount ,  SUM(IF(j.date>'"+sqlUpToDate+"',COALESCE(amount,0),0)) famount ,  \r\n" + 
	                        "GROUP_CONCAT(IF(j.date>'"+sqlUpToDate+"',DATE_FORMAT(j.date,'%d.%m.%Y'),NULL)) fdate ,\r\n" + 
	                        "SUM(IF(j.date>'"+sqlUpToDate+"',1,0)) fcount  from my_outd o inner join my_jvtran j on j.tranid=o.tranid where 1=1 group by ap_trid ) o on j.tranid=o.ap_trid "
	                        + "where j.status=3 and h.atype='"+atype+"' "+sql+" and j.id>0 group by j.tranid having balance<>0 union all "
	                        + "select cc.code,bk.sal_id,j.date,j.dtype transtype,convert(j.doc_no,char(50)) transno,j.acno,h.description name,bk.per_mob pmob,bk.contactPerson contact,if(j.description='0','',j.description) description,sum(dramount) amount,CONVERT(coalesce(o.amount,''),CHAR(50)) applied,sum(dramount)+coalesce(o.amount,0) "
	                        + "balance, j.tranid, j.brhid,TIMESTAMPDIFF(Day,cast(j.date as datetime),cast('"+sqlUpToDate+"' as datetime)) age,j.rdocno,j.rtype,j.tr_no, famount , fdate , fcount from my_jvtran j inner join my_brch b on j.brhId=b.doc_no inner join my_curr bc on b.curId=bc.doc_no "
	                        + "inner join my_head h on j.acno=h.doc_no inner join my_curr c on j.curId=c.doc_no left join my_acbook bk on h.cldocno=bk.cldocno "+condition+" left join my_curr cc on cc.doc_no=h.curid left join (select ap_trid,o.tranid,SUM(IF(j.date<='"+sqlUpToDate+"',COALESCE(amount,0),0)) amount ,"+ 
	                        " SUM(IF(j.date>'"+sqlUpToDate+"',COALESCE(amount,0),0)) famount ,  \r\n" + 
	                        " GROUP_CONCAT(IF(j.date>'"+sqlUpToDate+"',DATE_FORMAT(j.date,'%d.%m.%Y'),NULL)) fdate ,\r\n" + 
	                        " SUM(IF(j.date>'"+sqlUpToDate+"',1,0)) fcount from my_outd o inner join my_jvtran j on j.tranid=o.ap_trid where 1=1 group by tranid) o on j.tranid=o.tranid where j.status=3 and h.atype='"+atype+"'  "+sql+" and j.id<0 "
	                        + "group by j.tranid having balance<>0) a left join my_salm sm on sm.doc_no=a.sal_id "+joins+"  , (select @tot:=0) b order by acno,date)b";
					//System.out.println("ia load AR==== "+sql);
				} else if(atype.equalsIgnoreCase("AP")){
					condition=" and bk.dtype='VND'";
					
					sql = "select runbal, agmtconfig, name, contact, salname, pmob, ADVANCE, BALANCE, UNAPPLIED, TOTAL, date, transtype, convert(transno, char(45)) transno, acno, brhid, age, description, code, famount , fdate , fcount, fbalance"+agmtselect2+" from(select  @tot:=@tot+balance runbal,"+agmtconfig+" agmtconfig,a.name,a.contact,coalesce(sm.sal_name,'') salname,a.pmob,CONVERT(if(if(balance>0,balance,0)+if(balance<0,round((balance),2),0)>0,round((if(balance>0,balance,0)+if(balance<0,round((balance),2),0)),2),''),CHAR(50)) 'ADVANCE',"  
                            ...
                            + "a.date,a.transtype,"+casestatement+"a.acno,a.brhid,a.age,a.description,a.code, famount , fdate , fcount,CONVERT(if(if(balance>0,balance,0)+if(balance<0,round((balance),2),0)<0,round((if(balance>0,balance,0)+if(balance<0,round((balance),2),0)*-1),2)-coalesce(famount,0.0),''),CHAR(50)) fbalance from ("
                            + "select cc.code,bk.sal_id,j.date,j.dtype transtype,convert(j.doc_no,char(50)) transno,j.acno,h.description name,bk.per_mob pmob,bk.contactPerson contact,if(j.description='0','',j.description) description,sum(dramount) amount,CONVERT(coalesce(o.amount,''),CHAR(50)) applied,sum(dramount)+ coalesce(o.amount,0)"
                            + " balance, j.tranid, j.brhid,TIMESTAMPDIFF(Day,cast(j.date as datetime),cast('"+sqlUpToDate+"' as datetime)) age,famount , fdate , fcount from my_jvtran j inner join my_brch b on j.brhId=b.doc_no "
                            + "inner join my_curr bc on b.curId=bc.doc_no inner join my_head h on j.acno=h.doc_no inner join my_curr c on j.curId=c.doc_no left join my_acbook bk on h.cldocno=bk.cldocno "+condition+" left join my_curr cc on cc.doc_no=h.curid "
                            + "left join (select ap_trid,o.tranid,SUM(IF(j.date<='"+sqlUpToDate+"',COALESCE(amount,0),0)) amount ,  SUM(IF(j.date>'"+sqlUpToDate+"',COALESCE(amount,0),0)) famount ,"+ 
                            " GROUP_CONCAT(IF(j.date>'"+sqlUpToDate+"',DATE_FORMAT(j.date,'%d.%m.%Y'),NULL)) fdate ,"+ 
                            " SUM(IF(j.date>'"+sqlUpToDate+"',1,0)) fcount from my_outd o inner join my_jvtran j on j.tranid=o.tranid where 1=1 group by ap_trid ) o on j.tranid=o.ap_trid "
                            + "where j.status=3 and h.atype='"+atype+"' "+sql+" and j.id<0 group by j.tranid having balance<>0 union all "
                            + "select cc.code,bk.sal_id,j.date,j.dtype transtype,convert(j.doc_no,char(50)) transno,j.acno,h.description name,bk.per_mob pmob,bk.contactPerson contact,if(j.description='0','',j.description) description,sum(dramount) amount,CONVERT(coalesce(o.amount,''),CHAR(50)) applied,sum(dramount)- coalesce(o.amount,0)"
                            + "balance, j.tranid, j.brhid,TIMESTAMPDIFF(Day,cast(j.date as datetime),cast('"+sqlUpToDate+"' as datetime)) age, famount , fdate , fcount from my_jvtran j inner join my_brch b on j.brhId=b.doc_no inner join my_curr bc on b.curId=bc.doc_no "
                            + "inner join my_head h on j.acno=h.doc_no inner join my_curr c on j.curId=c.doc_no left join my_acbook bk on h.cldocno=bk.cldocno "+condition+" left join my_curr cc on cc.doc_no=h.curid left join (select ap_trid,o.tranid,SUM(IF(j.date<='"+sqlUpToDate+"',COALESCE(amount,0),0)) amount ,"+ 
                            " SUM(IF(j.date>'"+sqlUpToDate+"',COALESCE(amount,0),0)) famount ,GROUP_CONCAT(IF(j.date>'"+sqlUpToDate+"',DATE_FORMAT(j.date,'%d.%m.%Y'),NULL)) fdate ,"+
                            " SUM(IF(j.date>'"+sqlUpToDate+"',1,0)) fcount from my_outd o inner join my_jvtran j on j.tranid=o.ap_trid where 1=1 group by tranid) o on j.tranid=o.tranid where j.status=3 and h.atype='"+atype+"' "+sql+" and j.id>0 "
                            +" group by j.tranid having balance<>0) a left join my_salm sm on sm.doc_no=a.sal_id "+joins+"  , (select @tot:=0) b order by acno,date)b";
					//System.out.println("ia load AP==== "+sql);
				}  
	...
	public JSONArray individualAgeing(String branch,String uptodate,String atype,String accdocno, String salesperson,String category,String check) throws SQLException {
		...
		try {
				...
				sql = "select  runbal, agmtconfig, name, contact, salname,pmob, ADVANCE , BALANCE, UNAPPLIED,TOTAL,date,transtype,convert(transno, char(45)) transno,acno,brhid,age,description,code"+agmtselect2+" FROM (select  @tot:=@tot+balance runbal,"+agmtconfig+" agmtconfig,a.name,a.contact,coalesce(sm.sal_name,'') salname,a.pmob,CONVERT(if(if(balance>0,balance,0)+if(balance<0,round((balance),2),0)<0,round((if(balance>0,balance,0)+if(balance<0,round((balance),2),0)*-1),2),''),CHAR(50)) 'ADVANCE',"
						+ "CONVERT(if(if(balance>0,balance,0)+if(balance<0,round((balance),2),0)>0,round((if(balance>0,balance,0)+if(balance<0,round((balance),2),0)),2),''),CHAR(50)) 'BALANCE',CONVERT(if(if(balance<0,"
						+ "round((balance),2),0)<0,round((if(balance<0,round((balance),2),0)*-1),2),''),CHAR(50)) 'UNAPPLIED',CONVERT(if(if(balance>0,balance,0)>0,round((if(balance>0,balance,0)),2),''),CHAR(50)) 'TOTAL',"
						+ "a.date,a.transtype,"+casestatement+"a.acno,a.brhid,a.age,a.description,a.code"+agmtselect+" from ("
						+ "select cc.code,bk.sal_id,j.date,j.dtype transtype,convert(j.doc_no,char(50)) transno,j.acno,h.description name,bk.per_mob pmob,bk.contactPerson contact,if(j.description='0','',j.description) description,sum(dramount) amount,CONVERT(coalesce(o.amount,''),CHAR(50)) applied,sum(dramount) - "
						+ "coalesce(o.amount,0) balance, j.tranid, j.brhid,TIMESTAMPDIFF(Day,cast(j.date as datetime),cast('"+sqlUpToDate+"' as datetime)) age,j.rdocno,j.rtype,j.tr_no from my_jvtran j inner join my_brch b on j.brhId=b.doc_no "
						+ "inner join my_curr bc on b.curId=bc.doc_no inner join my_head h on j.acno=h.doc_no inner join my_curr c on j.curId=c.doc_no left join my_acbook bk on h.cldocno=bk.cldocno "+condition+" left join my_curr cc on cc.doc_no=h.curid "
						...
						+ "select cc.code,bk.sal_id,j.date,j.dtype transtype,convert(j.doc_no,char(50)) transno,j.acno,h.description name,bk.per_mob pmob,bk.contactPerson contact,if(j.description='0','',j.description) description,sum(dramount) amount,CONVERT(coalesce(o.amount,''),CHAR(50)) applied,sum(dramount)+coalesce(o.amount,0) "
						+ "balance, j.tranid, j.brhid,TIMESTAMPDIFF(Day,cast(j.date as datetime),cast('"+sqlUpToDate+"' as datetime)) age,j.rdocno,j.rtype,j.tr_no from my_jvtran j inner join my_brch b on j.brhId=b.doc_no inner join my_curr bc on b.curId=bc.doc_no "
						+ "inner join my_head h on j.acno=h.doc_no inner join my_curr c on j.curId=c.doc_no left join my_acbook bk on h.cldocno=bk.cldocno "+condition+" left join my_curr cc on cc.doc_no=h.curid left join (select ap_trid,o.tranid,sum(coalesce(amount,0)) "
					...
					sql = "select  runbal, agmtconfig, name, contact, salname,pmob, ADVANCE , BALANCE, UNAPPLIED,TOTAL,date,transtype,convert(transno, char(45)) transno,acno,brhid,age,description,code"+agmtselect2+" FROM (select  @tot:=@tot+balance runbal,"+agmtconfig+" agmtconfig,a.name,a.contact,coalesce(sm.sal_name,'') salname,a.pmob,CONVERT(if(if(balance>0,balance,0)+if(balance<0,round((balance),2),0)>0,round((if(balance>0,balance,0)+if(balance<0,round((balance),2),0)),2),''),CHAR(50)) 'ADVANCE',"  
							+ "CONVERT(if(if(balance>0,balance,0)+if(balance<0,round((balance),2),0)<0,round((if(balance>0,balance,0)+if(balance<0,round((balance),2),0)*-1),2),''),CHAR(50)) 'BALANCE',CONVERT(if(if(balance>0,"
							+ "round((balance),2),0)>0,round((if(balance>0,round((balance),2),0)),2),''),CHAR(50)) 'UNAPPLIED',CONVERT(if(if(balance<0,balance,0)<0,round((if(balance<0,balance,0)),2),''),CHAR(50)) 'TOTAL',"
							+ "a.date,a.transtype,"+casestatement+"a.acno,a.brhid,a.age,a.description,a.code from ("
							+ "select cc.code,bk.sal_id,j.date,j.dtype transtype,convert(j.doc_no,char(50)) transno,j.acno,h.description name,bk.per_mob pmob,bk.contactPerson contact,if(j.description='0','',j.description) description,sum(dramount) amount,CONVERT(coalesce(o.amount,''),CHAR(50)) applied,sum(dramount)+ coalesce(o.amount,0)"
		...
		try {
		        ...
		        if(!(sqlUpToDate==null)){
				 ...
				 if(atype.equalsIgnoreCase("AR")){
					...
					sql = "select ag.code,ag.name 'ACCOUNT_NAME',ag.contact 'CONTACT_PERSON',coalesce(sm.sal_name,'') 'SALES_PERSON',ag.pmob 'MOBILE_NO',CONVERT(if(sum(if(balance>0,balance,0)+if(balance<0,round((balance),2),0))<0,"
						...
						+ "round((sum(if(balance>0,balance,0))),2),''),CHAR(50)) 'TOTAL',ag.acno,ag.brhid 'BRANCH_ID' from (select cc.code,bk.sal_id,j.date,j.dtype,j.doc_no,j.acno,h.description name,"
						...
						+ "left join my_acbook bk on h.cldocno=bk.cldocno "+condition+" left join my_curr cc on cc.doc_no=h.curid left join (select ap_trid,o.tranid,sum(coalesce(amount,0)) amount from my_outd o inner join my_jvtran j on "
						...
						+ "and j.id>0 group by j.tranid having balance<>0 union all select cc.code,bk.sal_id,j.date,j.dtype,j.doc_no,j.acno,h.description name,bk.per_mob pmob,bk.contactPerson contact,"
						...
						+ "b.curId=bc.doc_no inner join my_head h on j.acno=h.doc_no inner join my_curr c on j.curId=c.doc_no left join my_acbook bk on h.cldocno=bk.cldocno "+condition+" left join my_curr cc on cc.doc_no=h.curid left join "
		        	...
		        	sql = "select ag.code,ag.name 'ACCOUNT_NAME',ag.contact 'CONTACT_PERSON',coalesce(sm.sal_name,'') 'SALES_PERSON',ag.pmob 'MOBILE_NO',CONVERT(if(sum(if(balance>0,balance,0)+if(balance<0,round((balance),2),0))>0,round((sum(if(balance>0,"
		        			...
		        			+ "'UNAPPLIED',CONVERT(if(sum(if(balance<0,balance,0))<0,round((sum(if(balance<0,balance,0))),2)*-1,''),CHAR(50)) 'TOTAL',ag.acno,ag.brhid 'BRANCH_ID' from (select cc.code,bk.sal_id,j.date,j.dtype,j.doc_no,j.acno,"
							...
							+ "inner join my_curr bc on b.curId=bc.doc_no inner join my_head h on j.acno=h.doc_no inner join my_curr c on j.curId=c.doc_no left join my_acbook bk on h.cldocno=bk.cldocno "+condition+" left join my_curr cc on cc.doc_no=h.curid "
							+ "left join (select ap_trid,o.tranid,sum(coalesce(amount,0)) amount from my_outd o inner join my_jvtran j on j.tranid=o.tranid where j.date<='"+sqlUpToDate+"' group by ap_trid ) o on j.tranid=o.ap_trid "
							+ "where j.status=3 and h.atype='"+atype+"' and j.date<='"+sqlUpToDate+"'  "+sql+" and j.id<0 group by j.tranid having balance<>0 union all select cc.code,bk.sal_id,j.date,j.dtype,j.doc_no,j.acno,h.description name,"
							...
							+ "b.curId=bc.doc_no inner join my_head h on j.acno=h.doc_no inner join my_curr c on j.curId=c.doc_no left join my_acbook bk on h.cldocno=bk.cldocno "+condition+" left join my_curr cc on cc.doc_no=h.curid left join (select ap_trid,o.tranid,"
	...
	public ClsIndividualAgeingBean getPrint(String branch,String uptodate,String atype,String accdocno, String check) throws SQLException {
		...
		try {
				...
				if(atype.equalsIgnoreCase("AR")){
					...
					printqry = "select @i:=@i+1 as srno,b.* from(select "+agmtconfig+" agmtconfig,a.name,a.contact,coalesce(sm.sal_name,'') salname,a.pmob,CONVERT(if(if(balance>0,balance,0)+if(balance<0,round((balance),2),0)<0,round((if(balance>0,balance,0)+if(balance<0,round((balance),2),0)*-1),2),''),CHAR(50)) 'ADVANCE',"
							+ "CONVERT(if(if(balance>0,balance,0)+if(balance<0,round((balance),2),0)>0,round((if(balance>0,balance,0)+if(balance<0,round((balance),2),0)),2),''),CHAR(50)) 'BALANCE',CONVERT(if(if(balance<0,"
							+ "round((balance),2),0)<0,round((if(balance<0,round((balance),2),0)*-1),2),''),CHAR(50)) 'UNAPPLIED',CONVERT(if(if(balance>0,balance,0)>0,round((if(balance>0,balance,0)),2),''),CHAR(50)) 'TOTAL',"
							+ "a.date,a.transtype,"+casestatement+"a.acno,a.brhid,a.age,a.description"+agmtselect+" from ("
