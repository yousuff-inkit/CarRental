...
public class ClsCostLedgerDAO  { 
	...
	public JSONArray costCodeDetailsSearch(String type, String costcode, String regno, String costcodename, String check, String brhid) throws SQLException {
	    JSONArray RESULTDATA=new JSONArray();
	    if(!check.equalsIgnoreCase("1")) {
	    	return RESULTDATA;
	    }
		...
		try {
	        	...
	        	if(type.equalsIgnoreCase("1")) {
	        		...
	        		if(!brhid.equalsIgnoreCase("a") && !brhid.equals("0")) {
				...
				/* Call Register */
	        		...
	        		String sql="select m.doc_no,m.doc_no code,a.refname,cs.site name from cm_cuscallm m left join cm_srvcsited cs on cs.rowno=m.siteid left join my_acbook a on m.cldocno=a.doc_no and a.dtype='CRM' where m.status=3 and a.status=3 and m.dtype='CREG'"+sql1+""+sqltest+"";
	        		//System.out.println("+++++++++++++++++++++"+sql);
	        		ResultSet resultSet = stmtCostLedger.executeQuery (sql);
	        		RESULTDATA=ClsCommon.convertToJSON(resultSet);
	        		stmtCostLedger.close();
			    ...
			    /* Fleet */
	        		...
	        		String sql="select fleet_no doc_no,fleet_no code,flname name,reg_no from gl_vehmaster where cost=0"+sql1+""+sqltest+"";
	        		ResultSet resultSet = stmtCostLedger.executeQuery (sql);
	        		RESULTDATA=ClsCommon.convertToJSON(resultSet);
	        		stmtCostLedger.close();
					conn.close();
				...
				/* Job Card */
		        	...
		        	if(!(costcode.equalsIgnoreCase("0")) && !(costcode.equalsIgnoreCase(""))){
			            sql1=sql1+" and m.code like '%"+costcode+"%'";
			        }
