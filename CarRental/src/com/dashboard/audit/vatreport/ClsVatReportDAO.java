...
public class ClsVatReportDAO {
	...
	public JSONArray getVatOutputData(String id,String branch,String fromdate,String todate) throws SQLException
	{
		...
		try{
			...
			String isLIN = "SELECT * FROM my_menu WHERE menu_name='Limousine Invoice' AND gate='E'";
			ResultSet rsisLIN = stmt.executeQuery(isLIN);
			if(rsisLIN.first()) {
				unionLIN = "UNION ALL SELECT sale.date,br.branchname,ac.refname,ac.trnnumber,'LIN' dtype,CONVERT(sale.voc_no,CHAR(25)) vocno,sale.doc_no, (ROUND((VS.dramount),2)) totalvalue, vs.vat5per vatapplied, vs.vat0per vatnotapplied, COALESCE(vs.vatvalue,0) vatcollected FROM gl_limoinvm sale  LEFT JOIN my_acbook ac ON  (sale.cldocno=ac.cldocno AND ac.dtype='CRM')  LEFT JOIN my_brch br ON SALE.brhid=br.doc_no LEFT JOIN (SELECT SUM(total)+SUM(vatvalue) dramount, SUM(total) vatapplied,SUM(vatvalue) vatvalue ,SUM(IF(vatvalue>0,total,0)) vat5per,SUM(IF(vatvalue=0,total,0)) vat0per ,RDOCNO FROM gl_limoinvd GROUP BY RDOCNO )  VS ON VS.RDOCNO=SALE.DOC_NO   WHERE sale.status=3 and sale.date>='"+sqlfromdate+"' and sale.date<='"+sqltodate+"' "+sqltest2;
			}
			...
			"  union all select m.date,br.branchname,h.description,'' trnnumber,m.dtype,convert(m.doc_no,char(25)) vocno,h.doc_no,sum(if(d.nettot<0,d.nettot*-1,d.nettot)*-1)    totalvalue,"
			...
			+ "left join my_head h on h.doc_no=m.acno left join my_brch br on m.brhid=br.doc_no where m.dtype='TCN' and m.status=3 and h.atype='AR' and  m.date between '"+sqlfromdate+"' and '"+sqltodate+"' "+sqltest1+" group by m.tr_no "
			
			+"  union all select m.date,br.branchname,h.description,'' trnnumber,m.dtype,convert(m.doc_no,char(25)) vocno,h.doc_no,sum(if(d.nettot<0,d.nettot*-1,d.nettot)) totalvalue,"
			+ " sum(if(taxamount!=0,if(d.amount<0,d.amount*-1,d.amount),0)) vatapplied, sum(if(taxamount=0,if(d.amount<0,d.amount*-1,d.amount),0))  vatnotapplied,"
			...
			+ "left join my_head h on h.doc_no=m.acno left join my_brch br on m.brhid=br.doc_no where m.dtype='TDN' and m.status=3 and h.atype='AR' and  m.date between '"+sqlfromdate+"' and '"+sqltodate+"' "+sqltest1+" group by m.tr_no "
           /* " select c.date,branchname,refname,trnnumber,jv.dtype,convert(jv.doc_no,char(25)) vocno,jv.doc_no,"+  
			" round(sum(jv.dramount),2)*-1 totalvalue,round(sum(if(ac.tax=1 and invhead.idno<>20 and invhead.tax=1 and (c.date between tax.fromdate"+
			" and tax.todate) and coalesce(taxjv.dramount,0)>0.00, jv.dramount,0)),2)*-1 vatapplied,0 vatnotapplied,round(coalesce(taxjv.dramount*-1,0),2)  vatcollected"+
			" from my_jvtran jv inner join my_cnot c on jv.tr_no=c.tr_no left join gl_invmode invhead on (jv.acno=invhead.acno) left join"+
			...
			+" "+ unionLIN +" union all select m.date,br.branchname,ac.refname,ac.trnnumber,m.dtype,convert(m.voc_no,char(100))  vocno,m.doc_no ,"+
...
+ "left join my_head h on h.doc_no=m.acno left join my_brch br on m.brhid=br.doc_no where m.dtype='TCN' and m.status=3 and  m.date between '"+sqlfromdate+"' and '"+sqltodate+"' "+sqltest1+" group by m.tr_no "
	...
	public JSONArray getVatInputData(String id,String branch,String fromdate,String todate) throws SQLException
	{
		...
		try{
			...
			" if(a.dtype='PIV',a.vatapplied,if(a.vatcollected<>0,(a.totalvalue-a.vatcollected),0)) vatapplied,"
			+ "if(a.dtype='PIV',a.vatnotapplied,(a.totalvalue-if(a.vatcollected<>0,(a.totalvalue-a.vatcollected),0)-a.vatcollected)) vatnotapplied, "
			+ "if(dtype in ('pir','CPR'),vatcollected*-1,a.vatcollected) vatcollected from ("+
			...
			"  union all select jv.refno invno,jv.date,branchname,refname,trnnumber,jv.dtype,convert(jv.doc_no,char(25)) vocno,jv.doc_no, "
			...
			" union all  select convert(m.refno,char(150)) invno,m.date,br.branchname,h.description,'' trnnumber,m.dtype,convert(m.doc_no,char(25)) vocno,h.doc_no,"
			+ " sum(if(d.nettot<0,d.nettot*-1,d.nettot))*-1   totalvalue, sum(if(taxamount!=0,if(d.amount<0,d.amount*-1,d.amount)*-1,0)) vatapplied, sum(if(taxamount=0,if(d.amount<0,d.amount*-1,d.amount)*-1,0))  vatnotapplied,"
			+ " sum(coalesce(if(d.taxamount<0,d.taxamount*-1,d.taxamount)*-1,0))  vatcollected from my_cnot m left join my_cnotd d on m.tr_no=d.tr_no left join my_head h on h.doc_no=m.acno "
			+ " left join my_brch br on m.brhid=br.doc_no where m.dtype='TDN' and h.atype='AP' and m.status=3 and  m.date between '"+sqlfromdate+"' and '"+sqltodate+"'  "+sqltest1+" group by m.tr_no "+
			
			" union all  select convert(m.refno,char(150)) invno,m.date,br.branchname,h.description,'' trnnumber,m.dtype,convert(m.doc_no,char(25)) vocno,h.doc_no,"
			+ " sum(if(d.nettot<0,d.nettot*-1,d.nettot)) totalvalue, sum(if(taxamount!=0,if(d.amount<0,d.amount*-1,d.amount),0)) vatapplied, sum(if(taxamount=0,if(d.amount<0,d.amount*-1,d.amount),0))  vatnotapplied,"
			+ " sum(coalesce(if(d.taxamount<0,d.taxamount*-1,d.taxamount),0))  vatcollected from my_cnot m left join my_cnotd d on m.tr_no=d.tr_no left join my_head h on h.doc_no=m.acno "
			+ " left join my_brch br on m.brhid=br.doc_no where m.dtype='TCN' and h.atype in ('AP','GL') and m.status=3 and  m.date between '"+sqlfromdate+"' and '"+sqltodate+"'  "+sqltest1+" group by m.tr_no "+
			  
			" union all select m.refinvno invno,m.date,br.branchname,ac.refname,ac.trnnumber,m.dtype,convert(m.voc_no,char(100)) vocno ,m.doc_no,"
			+ "coalesce(totalvalue,0)+coalesce(srtotalvalue,0) totalvalue,coalesce(vatapplied,0)+coalesce(srvatapplied,0)vatapplied," 
				+ "coalesce(vatnotapplied,0)+coalesce(srvatnotapplied,0)vatnotapplied,coalesce(vatcollected,0)+coalesce(srvatcollected,0)vatcollected"
				...
				+ " on d3.rdocno=m.doc_no left join my_acbook ac on ac.cldocno=m.cldocno and ac.dtype='VND' left join my_brch br on br.doc_no=m.brhid  "
				+ "left join (select tr_no,sum(nettotal+srvtaxamount) srtotalvalue,sum(srvtaxamount) srvatcollected," 
				+ "sum(if(srvtaxamount>0,nettotal,0)) srvatapplied,sum(if(srvtaxamount=0,nettotal,0)) srvatnotapplied "
				+ "from my_srvser group by tr_no) sr on m.tr_no=sr.tr_no "
...
/*					+ " union all select convert(m.invno,char(25))  invno,m.date,br.branchname,ac.refname,ac.trnnumber,'CPU' dtype,"
				   ...
				   + " union all select invno, date, branchname, refname, trnnumber, dtype, vocno, doc_no, if(totalvalue<0,totalvalue,totalvalue*-1) totalvalue,if(vatapplied<0,vatapplied,vatapplied*-1) vatapplied,if(vatnotapplied<0,vatnotapplied,vatnotapplied*-1) vatnotapplied,if(vatcollected<0,vatcollected,vatcollected*-1) vatcollected from(select convert(m.invno,char(25))  invno,m.date,br.branchname,ac.refname,ac.trnnumber,'CPR' dtype, convert(m.voc_no,char(25)) vocno,m.doc_no,sum(d.nettotal+d.taxamount)  totalvalue,aa.app vatapplied, (sum(d.nettotal+d.taxamount))-aa.app-coalesce(aa.collect,0) vatnotapplied, coalesce(aa.collect,0)  vatcollected from my_srvpurretm m left join (select   sum(nettotal) app,sum(taxamount) collect,rdocno from my_srvpurretd where taxamount>0 group by rdocno) aa on aa.rdocno=m.doc_no left join (select   sum(nettotal) notapp,rdocno from my_srvpurretd where taxamount=0 group by rdocno) bb on bb.rdocno=m.doc_no left join my_srvpurretd d on d.rdocno=m.doc_no left join my_acbook ac on (m.acno=ac.acno and ac.dtype='VND') left join my_brch br on m.brhid=br.doc_no "
				   + " where  m.status=3 and m.date>='"+sqlfromdate+"' and   m.date<='"+sqltodate+"' "+sqltest1+" group by m.tr_no)a)g order by g.date asc";  
