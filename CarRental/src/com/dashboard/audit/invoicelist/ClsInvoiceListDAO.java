package com.dashboard.audit.invoicelist;
	...
	String sqltest="";
		...
		try {
            	...
            	if(branchval.equalsIgnoreCase("a"))
            	{
            		...
            		String sql ="select br.branchname branch,m.dtype,if(m.ratype='RAG',r.voc_no,l.voc_no) refvocno,'Audit' btnaudit,'Print' btnprint,m.doc_no,m.voc_no,h.description acname,h.account acno,m.doc_no,m.ratype,m.rano,m.cldocno,a.refname,m.fromdate,m.todate,convert(coalesce(sum(d.total),''),char(50)) amount , "
            				...
            				+ " left join gl_ragmt r on r.doc_no=m.rano left join my_head h on h.doc_no=m.acno left join my_brch br on m.brhid=br.doc_no  where m.status=3 and m.audit=0"
...
//        System.out.println("============"+sql);
            		...
            		String sql ="select br.branchname branch, m.dtype,if(m.ratype='RAG',r.voc_no,l.voc_no) refvocno,'Audit' btnaudit,'Print' btnprint, m.doc_no,m.voc_no,h.description acname,h.account acno,m.doc_no,m.ratype,m.rano,m.cldocno,a.refname,m.fromdate,m.todate,convert(coalesce(sum(d.total),''),char(50)) amount , "
            				...
            				+ " from gl_invm m left join gl_invd d on m.doc_no=d.rdocno "
            				+ " left join my_acbook a on a.doc_no=m.cldocno and a.dtype='CRM' left join gl_lagmt l on l.doc_no=m.rano"
            				+ " left join gl_ragmt r on r.doc_no=m.rano left join my_head h on h.doc_no=m.acno  left join my_brch br on m.brhid=br.doc_no  where m.status=3 and m.audit=1"
