			...
			java.sql.Date sqlfromdate=null,sqltodate=null;
	           try {
				...
				if(!hiduser.equalsIgnoreCase("")){
					...
					Statement stmtmanualss = conn.createStatement ();
					...
					//System.out.println("sqltypechk----------"+sqltypechk);
					
					
					String strvoctable="select voc_joins from datalogref where vocno_status=1 and type="+sqltypechk+" ";
					//System.out.println("strvoctable----------"+strvoctable);
					ResultSet rsvocjoins=stmtmanual.executeQuery(strvoctable);
					String vocJoins="";
					while(rsvocjoins.next()){
						vocJoins+=" "+rsvocjoins.getString("voc_joins");
					}	
					//System.out.println("vocJoins----------"+vocJoins);
					
					joins=vocJoins;
					
					
					
					String strvoctables="select alias_name,dtype from datalogref where vocno_status=1 and type="+sqltypechk+"";
					//System.out.println("alias0--------------------"+strvoctables);
					ResultSet rsvocjoinss=stmtmanuals.executeQuery(strvoctables);
					
					String aliasName="";String vocDtypeName="";
					String vocCase="CASE";
			    	
					while(rsvocjoinss.next()){
						aliasName=rsvocjoinss.getString("alias_name");
						vocDtypeName=rsvocjoinss.getString("dtype");
						
						vocCase+=" WHEN a.transType in ("+vocDtypeName+") THEN "+aliasName+" ";
					}
					vocCase+=" ELSE a.transno END AS 'transno',";
	           
	           casestatement=vocCase;
					...
					if(value.equalsIgnoreCase("formbtn")){
						...
						*/
						...
						strSql="select "+casestatement+" a.dtype,a.branchname, a.transtype,a.formname , a.user, a.action, a.date from ( select t.doc_no transNo,br.branchname,"
								+ "t.dtype,t.dtype transType,t.brhid,case when t.dtype='INT' then 'Invoice Traffic' when t.dtype='INS' then 'Invoice Salik' else menu.menu_name end formname,"
								+ "usr.user_name user,CONVERT(case when t.entry='A' then 'Add' when t.entry='E' then 'Edit' when t.entry='D' then 'Delete' when t.entry='R' then 'Release'  when t.entry='X' then 'Exchange' when t.entry='C' then 'Cancel' "
								+ "else t.entry end,CHAR(25)) action,t.edate date from datalog t left join my_menu menu on t.dtype=menu.doc_type  left join my_user usr on t.userid=usr.doc_no "
								+ "left join  my_brch br on t.brhid=br.doc_no where 1=1 "+sqltest+") a"+joins+"";
					 }
						...
						/* strSql="select t.doc_no transno,br.branchname, t.dtype transtype,case when t.dtype='INT' then 'Invoice Traffic' "+
						 ...
						 strSql="select "+casestatement+" a.dtype,a.branchname, a.transType,a.formname ,a.user, a.action, a.date  from(select t.dtype,t.doc_no transno,t.brhid brhid,br.branchname, t.dtype transtype,case when t.dtype='INT' then 'Invoice Traffic' "+
									"  when t.dtype='INS' then 'Invoice Salik' else menu.description end formname, "+
									" usr.user_name user,CONVERT(case when t.entry='A' then 'Add' when t.entry='E' "+
									" then 'Edit' when t.entry='D' then 'Delete' when t.entry='R' then 'Release' "+
									" when t.entry='X' then 'Exchange' when t.entry='C' then 'Cancel' "+
									 ...
									 " left join my_user usr on t.userid=usr.doc_no "+
									 " left join my_brch br on t.brhid=br.doc_no "+
									  " where 1=1   "+sqltest+") a "+joins+" ";
					 }
					System.out.println("griddat-----------"+strSql);
					ResultSet resultSet = stmtmanualss.executeQuery (strSql);
			...
			java.sql.Date sqlfromdate=null,sqltodate=null;
	           ...
	           casestatement=vocCase;
					...
					*/
						...
						strSql="select "+casestatement+" a.count,a.dtype,a.branchname,a.transtype,a.formname , a.user, a.action, a.date from ( select count(*) count ,t.doc_no transNo,br.branchname,"
												+ "t.dtype,t.dtype transType,t.brhid,case when t.dtype='INT' then 'Invoice Traffic' when t.dtype='INS' then 'Invoice Salik' else menu.menu_name end formname,"
												+ "usr.user_name user,CONVERT(case when t.entry='A' then 'Add' when t.entry='E' then 'Edit' when t.entry='D' then 'Delete' when t.entry='R' then 'Release'  when t.entry='X' then 'Exchange' when t.entry='C' then 'Cancel' "
												+ "else t.entry end,CHAR(25)) action,t.edate date from datalog t left join my_menu menu on t.dtype=menu.doc_type  left join my_user usr on t.userid=usr.doc_no "
												+ "left join  my_brch br on t.brhid=br.doc_no where 1=1 "+sqltest+" group by usr.doc_no) a"+joins+"  ";
						...
						/* strSql="select t.doc_no transno,br.branchname, t.dtype transtype,case when t.dtype='INT' then 'Invoice Traffic' "+
						 ...
						 strSql="select "+casestatement+" a.count,a.dtype,a.branchname, a.transType,a.formname ,a.user, a.action, a.date  from(select t.doc_no transno,br.branchname,t.brhid brhid,t.dtype,t.dtype transtype,case when t.dtype='INT' then 'Invoice Traffic' "+
									"  when t.dtype='INS' then 'Invoice Salik' else menu.description end formname, "+
									"count(*) count, usr.user_name user,CONVERT(case when t.entry='A' then 'Add' when t.entry='E' "+
									" then 'Edit' when t.entry='D' then 'Delete' when t.entry='R' then 'Release' "+
									 " else t.entry end,CHAR(25)) action,t.edate date "+
									 ...
									 " left join my_user usr on t.userid=usr.doc_no "+
									 " left join my_brch br on t.brhid=br.doc_no "+
									  " where 1=1   "+sqltest+" group by usr.doc_no) a "+joins+"   "; 
					 }
