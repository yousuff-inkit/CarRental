...
private String isinvbranch;
private String invbranch;
...
public String getIsinvbranch() {
	return isinvbranch;
}
public void setIsinvbranch(String isinvbranch) {
	this.isinvbranch = isinvbranch;
}
public String getInvbranch() {
	return invbranch;
}
public void setInvbranch(String invbranch) {
...
public String saveAction() throws ParseException, SQLException{
	...
	try{
		...
		if(!getInvoicedate().equalsIgnoreCase("") && getInvoicedate()!=null){
//			sqldate=objcommon.changeStringtoSqlDate(getCurrentdate()); as per topzeed they wanted the invoice to be generated date instead of current date 
			sqldate=objcommon.changeStringtoSqlDate(getInvoicedate());
		}
		
		String branch = cmbbranch;
		if(isinvbranch.equalsIgnoreCase("1")) {
			branch = invbranch;
		}
		...
		if(getLimoinvtype().equalsIgnoreCase("clientwise")){
			for(int i=0;i<processarray.size();i++){
				...
				if(clientno.equalsIgnoreCase(processarray.get(i).split("::")[0])){
					...
					if(i==processarray.size()-1){
						...
						int val=objinvoice.insert(sqldate, sqlfromdate, sqltodate, clientno, note, note, invoicearray2, mode, "LIN", request, branch, session, "2", 0.0,getLpono(),getEventno());
						if(val<=0){
							status=1;
							break;
						}
					...
					if(i==processarray.size()-1){
						...
						note=getFromdate()+" to "+getTodate()+" of Client "+clientname;
						System.out.println("Before Insertion");
						int newval=objinvoice.insert(sqldate, sqlfromdate, sqltodate, clientno, note, note, invoicearray2, mode, "LIN", request, branch, session, "2", 0.0,getLpono(),getEventno());
						if(newval<=0){
							status=1;
							break;
						}
			...
			/*	//Process Array
				...
				int val=objinvoice.insert(sqldate, sqlfromdate, sqltodate, clientno, note, note, invoicearray, mode, "LIN", request, branch, session, "2", 0.0,getLpono(),getEventno());
				if(val<=0){
					status=1;
					break;
				}
