...
public class ClsToAssign {
	...
	public JSONArray searchDriver() throws SQLException {
		...
		try {
				conn=ClsConnection.getMyConnection();
				Statement stmtVeh1 = conn.createStatement ();
            	String sqldata="select s.sal_name name,d.dob,d.nation,d.mobno,d.dlno,d.issdate,d.led,d.issfrm,d.passport_no,s.doc_no from my_salesman s left join gl_drdetails d on s.sal_type=d.dtype and s.doc_no=d.doc_no\r\n" + 
            			"WHERE s.status=3 and s.sal_type='DRV' ";
