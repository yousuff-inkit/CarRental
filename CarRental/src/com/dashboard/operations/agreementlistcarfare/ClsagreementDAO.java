package com.dashboard.operations.agreementlistcarfare;
...
public class ClsagreementDAO 
{
    ...
    public JSONArray detailsgrid(String brnchval,String fromdate,String todate,String cldocno,String status,String fleet, 
    	...
    	try {
				...
				String sql ="(select ROUND(@i:=@i+1 ,0) slno,a.* from(select coalesce(insur.refname,'') insurrefname,contractgrp.gname contractgroup,contractyom.yom contractyom,hd.account account,CASE WHEN r.clstatus=0 THEN 'Open' WHEN r.clstatus=1 THEN 'Closed' WHEN r.clstatus=2 THEN 'Cancelled' ELSE '' END AS agmtstatus,coalesce(a.mail1,'') clientmail, cl.cat_name category,concat(au.authid,v.reg_no,pl.code_name) vehiclenum,tc.kmrest,tc.exkmrte,r.okm,r.insex,r.pono,r.rtaremarks,a.cldocno,r.date agmtdate,auth.authname authority,contractauth.authname contractauthority,plate.code_name platecode,contractplate.code_name contractplatecode,'Rent' as rltype,r.voc_no doc,r.fleet_no,v.FLNAME vehdetails,v.reg_no,r.ofleet_no ofno,v1.reg_no oreg_no,a.refname,a.contactperson,\r\n" + 
        				...
        				" ,'' AS project,v1.eng_no originalfleet,v1.ch_no originalchase,'' replacefleet,'' replacechase,ur.user_name,clur.user_name closeuser,ms.sal_name,a.period2 crdp,mbr.branchname brch,ml.loc_name locname,case when r.invtype=1 then 'Month End' when r.invtype=2 then  'Period' when r.invtype=3 then  'Complimentory'  when r.invtype=4 then  'Quarterly' when r.invtype=5 then  'Half Yearly' when r.invtype=6 then  'Yearly' end invr,gy.yom,round(coalesce(r.racrate,0.00),2) racrate,if(r.advchk=1,'Yes','No') advance\r\n" + 
        				" \r\n" + 
        				"from gl_ragmt r \r\n" + 
        				" left join gl_vehmaster v on r.fleet_no=v.fleet_no left join gl_vehauth au on au.doc_no=v.authid  left join gl_vehplate pl on pl.doc_no=v.pltid left join my_acbook a on a.cldocno=r.cldocno and a.dtype='CRM' left join my_clcatm cl on cl.doc_no=a.catid \r\n" + 
        			...
        			" left join my_user ur on ur.doc_no=r.userid "+
        				...
        				+ "left join my_brch mbr on mbr.doc_no=r.brhid left join my_locm ml on r.locid=ml.doc_no left join gl_yom gy on v.yom=gy.doc_no "+
        				" left join my_acbook insur on (insur.cldocno=r.insurcldocno and insur.dtype='CRM')"+
        				" left join gl_vehgroup contractgrp on contractgrp.doc_no=v1.vgrpid "+
        				" left join gl_yom contractyom on v1.yom=contractyom.doc_no"+
        				" left join my_head hd on a.acno=hd.doc_no"+
        				" where r.status=3\r\n" + 
        				"        	"+rsqltest+" )a,(select @i:=0)b)\r\n" + 
        				" UNION all \r\n" + 
        				"(select ROUND(@i:=@i+1 ,0) slno,a.* from(select '' insurrefname,contractgrp.gname contractgroup,contractyom.yom contractyom,hd.account account,CASE WHEN l.clstatus=0 THEN 'Open' WHEN l.clstatus=1 THEN 'Closed' WHEN l.clstatus=2 THEN 'Cancelled' ELSE '' END AS agmtstatus,coalesce(a.mail1,'') clientmail,cl.cat_name category,concat(au.authid,v.reg_no,pl.code_name) vehiclenum, tr.kmrest,tr.exkmrte,l.outkm,l.insurexcess,l.po,l.rtaremarks,a.cldocno,l.date agmtdate,auth.authname authority,contractauth.authname contractauthority,plate.code_name platecode,contractplate.code_name contractplatecode,'Lease' as rltype, l.voc_no doc, l.tmpfleet fleet_no,v.FLNAME vehdetails,v.reg_no,l.perfleet ofno,orgveh.reg_no oreg_no,a.refname,a.contactperson,\r\n" + 
        				...
        				"convert(coalesce(repveh.fleet_no,''),char(25)) replacefleet,coalesce(repveh.ch_no,'') replacechase,ur.user_name,clur.user_name closeuser,ms.sal_name, a.period2 crdp ,br1.branchname brch ,ml.loc_name locname, case when l.inv_type=1 then 'Month End' when l.inv_type=2 then  'Period' when l.inv_type=3 then  'Complimentory'  when l.inv_type=4 then  'Quarterly' when l.inv_type=5 then  'Half Yearly' when l.inv_type=6 then  'Yearly' end   invr,gy.yom,0.00 racrate,if(l.adv_chk=1,'Yes','No') advance\r\n" + 
        				...
        				"  left join gl_vehmaster orgveh on l.perfleet=orgveh.fleet_no left join gl_vehauth au on au.doc_no=v.authid  left join gl_vehplate pl on pl.doc_no=v.pltid left join gl_vehmaster repveh on l.tmpfleet=repveh.fleet_no\r\n" + 
        				"  left join my_acbook a on a.cldocno=l.cldocno and a.dtype='CRM' left join my_clcatm cl on cl.doc_no=a.catid left join gl_vehgroup g on g.doc_no=v.vgrpid\r\n" + 
        				"   left join gl_vehbrand br on br.doc_no=v.brdid  left join gl_vehmodel mo on  mo.doc_no=v.vmodid\r\n" + 
        				...
        				+ " left join gl_lagmtclosem cs on cs.agmtno=l.doc_no\r\n" + 
        				 " left join my_user clur on clur.doc_no=cs.userid "+
        				"    left join my_salm ms on ms.doc_no=l.salid left join my_brch br1 on br1.doc_no=l.brhid  left join (select brhid,loc_name from my_locm group by brhid) ml on ml.brhid=br1.doc_no  left join gl_yom gy on orgveh.yom=gy.doc_no "+
        				" left join gl_vehgroup contractgrp on contractgrp.doc_no=orgveh.vgrpid "+
        				" left join gl_yom contractyom on orgveh.yom=contractyom.doc_no "+
        				" left join my_head hd on a.acno=hd.doc_no "+
        				" where l.status=3 "+sqltest+")a,(select @i:=0)b)";
