package com.dashboard.invoices.rentalV2;
...
public class ClsRentalInvoiceV2Action {
	ClsConnection ClsConnection=new ClsConnection();

	ClsCommon ClsCommon=new ClsCommon();

	
ClsRentalInvoiceV2DAO rentalinvoiceDAO=new ClsRentalInvoiceV2DAO();
...
//	System.out.println("chk=========== "+requestParams);
	...
	try{
		...
		"(select acno from gl_invmode where idno=17) cdw,"+
		"(select acno from gl_invmode where idno=24) pai,"+
		"(select acno from gl_invmode where idno=25) cdw1,"+
		"(select acno from gl_invmode where idno=26) pai1,"+
		"(select acno from gl_invmode where idno=29) gps,"+
		"(select acno from gl_invmode where idno=27) babyseater,"+
		"(select acno from gl_invmode where idno=28) cooler,"+
		"(select acno from gl_invmode where idno=30) exkmrte,"+
		"(select acno from gl_invmode where idno=31) oinschg,"+
		"(select acno from gl_invmode where idno=5) exhrchg,"+
		"(select acno from gl_invmode where idno=32) chaufexchg";
		...
		int cdwacno=0,paiacno=0,cdw1acno=0,pai1acno=0,gpsacno=0,babyseateracno=0,cooleracno=0;
		int exkmrteacno=0,oinschgacno=0,exhracno=0,chaufexacno=0;
		while(rsacno.next()){
			...
			cdwacno=rsacno.getInt("cdw");//17
			paiacno=rsacno.getInt("pai");//24
			cdw1acno=rsacno.getInt("cdw1");//25
			pai1acno=rsacno.getInt("pai1");//26
			gpsacno=rsacno.getInt("gps");//29
			babyseateracno=rsacno.getInt("babyseater");//27
			cooleracno=rsacno.getInt("cooler");//28
			exkmrteacno=rsacno.getInt("exkmrte");
			oinschgacno=rsacno.getInt("oinschg");
			exhracno=rsacno.getInt("exhrchg");
			chaufexacno=rsacno.getInt("chaufexchg");
		}
	...
	//		System.out.println("inside tarifsave");
			...
			for(int j=0;j <getInvgridlength();j++){
				...
				int invcount=0;
				String agmttype="RAG";
				String strgetinvtype="",rentaltype="";
				java.sql.Date tempfromdate=null;
				if(agmttype.equalsIgnoreCase("RAG")){
					strgetinvtype="select (select invtype from gl_ragmt where doc_no="+agmtno+") invtype,(select count(*) from gl_invm where rano="+agmtno+" and ratype='RAG') invcount , (select rentaltype from gl_rtarif where rdocno="+agmtno+" and rstatus=5 ) rentaltype";
					
				}
				else{
					strgetinvtype="select (select inv_type from gl_lagmt where doc_no="+agmtno+") invtype,(select count(*) from gl_invm where rano="+agmtno+" and ratype='LAG') invcount, 'Monthly' rentaltype";
				}
				...
				while(rsgetinvtype.next()){
					...
					rentaltype=rsgetinvtype.getString("rentaltype");
				}
				...
				// double chaufchg=rentalinvoiceDAO.getChaufferCharge(agmtno, fromdate, todate, "RAG");
				double chaufchg=rentalinvoiceDAO.getChauffercharge(agmtno,rentaltype, fromdate, todate);
				...
				double cdw=Double.parseDouble(invoice[32]);
				double pai=Double.parseDouble(invoice[33]);
				double cdw1=Double.parseDouble(invoice[34]);
				double pai1=Double.parseDouble(invoice[35]);
				double gps=Double.parseDouble(invoice[36]);
				double babyseater=Double.parseDouble(invoice[37]);
				double cooler=Double.parseDouble(invoice[38]);
				
				double exkmrte=Double.parseDouble(invoice[39]);
				double oinschg=Double.parseDouble(invoice[40]);
				double exhrchg=Double.parseDouble(invoice[41]);
				double chaufexchg=Double.parseDouble(invoice[42]);
				
				if(exkmrte>0){
					newarray.add("30"+"::"+exkmrteacno+"::"+note+"::"+qty+"::"+exkmrte+"::"+exkmrte);
				}
				if(oinschg>0){
					newarray.add("31"+"::"+oinschgacno+"::"+note+"::"+qty+"::"+oinschg+"::"+oinschg);
				}
				/*if(exhrchg>0){
					newarray.add("5"+"::"+exhracno+"::"+note+"::"+qty+"::"+exhrchg+"::"+exhrchg);
				}*/
				if(chaufexchg>0){
					newarray.add("32"+"::"+chaufexacno+"::"+note+"::"+qty+"::"+chaufexchg+"::"+chaufexchg);
				}
				if(chaufchg>0){
				...
				if(cdw>0){
					newarray.add("17"+"::"+cdwacno+"::"+note+"::"+qty+"::"+cdw+"::"+cdw);
				}
				if(pai>0){
					newarray.add("24"+"::"+paiacno+"::"+note+"::"+qty+"::"+pai+"::"+pai);
				}
				if(cdw1>0){
					newarray.add("25"+"::"+cdw1acno+"::"+note+"::"+qty+"::"+cdw1+"::"+cdw1);
				}
				if(pai1>0){
					newarray.add("26"+"::"+pai1acno+"::"+note+"::"+qty+"::"+pai1+"::"+pai1);
				}
				if(gps>0){
					newarray.add("29"+"::"+gpsacno+"::"+note+"::"+qty+"::"+gps+"::"+gps);
				}
				if(babyseater>0){
					newarray.add("27"+"::"+babyseateracno+"::"+note+"::"+qty+"::"+babyseater+"::"+babyseater);
				}
				if(cooler>0){
					newarray.add("28"+"::"+cooleracno+"::"+note+"::"+qty+"::"+cooler+"::"+cooler);
				}
				/*
				 * if(Double.parseDouble(invoice[9])>0){
				 * newarray.add("2"+"::"+accacno+"::"+note+"::"+qty+"::"+invoice[9]+"::"+invoice
				 * [9]); } if(Double.parseDouble(invoice[17])>0){
				 * newarray.add("17"+"::"+insuracno+"::"+note+"::"+qty+"::"+invoice[17]+"::"+
				 * invoice[17]); }
				 */
