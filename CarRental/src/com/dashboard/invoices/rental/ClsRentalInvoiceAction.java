...
private String hidchkfuturedate;



public String getHidchkfuturedate() {
	return hidchkfuturedate;
}
public void setHidchkfuturedate(String hidchkfuturedate) {
	this.hidchkfuturedate = hidchkfuturedate;
}
...
//	System.out.println("chk=========== "+requestParams);
	...
	try{
		...
		if(parkconfig==1) {
			...
			ResultSet rsparkacno=stmt.executeQuery(strparkacno);
		...
		String sqlacno="select CURDATE() sqlcurdate,(select acno from gl_invmode where idno=1) rental,(select acno from gl_invmode where idno=2) acc,"+
		...
		while(rsacno.next()){
			sqlcurdate=rsacno.getDate("sqlcurdate");
		...
		//Checking Future Date Config
		
		int futuredateconfig=0;
		String strfuturedate="select method from gl_config where field_nme='showInvFutureDate'";
		ResultSet rsfuturedate=stmt.executeQuery(strfuturedate);
		while(rsfuturedate.next()) {
			futuredateconfig=rsfuturedate.getInt("method");
		}
	...
	//		System.out.println("inside tarifsave");
			...
			for(int j=0;j <getInvgridlength();j++){
				...
				double salikparkamt=Double.parseDouble(invoice[32]);
				double salikparksrvc=Double.parseDouble(invoice[33]);
				int salikparkcount=Integer.parseInt(invoice[34]);
				double salikparkrate=Double.parseDouble(invoice[35]);
				double salikparksrvcrate=Double.parseDouble(invoice[36]);
				...
				if(getHidchkexsalik().equalsIgnoreCase("0") && getHidchksalik().equalsIgnoreCase("0")){
					...
					if(salikparkamt>0.0){
						newarray.add(51+"::"+parkacno+"::"+note+"::"+salikparkcount+"::"+salikparkrate+"::"+salikparkamt);
						newarray.add(52+"::"+parksrvcacno+"::"+note+"::"+salikparkcount+"::"+salikparksrvcrate+"::"+salikparksrvc);
					}
				}
				...
				if(getHidchksalik().equalsIgnoreCase("1")){
					...
					if(salikparkamt>0.0){
						salikarray.add(51+"::"+parkacno+"::"+note+"::"+salikparkcount+"::"+salikparkrate+"::"+salikparkamt);
						salikarray.add(52+"::"+parksrvcacno+"::"+note+"::"+salikparkcount+"::"+salikparksrvcrate+"::"+salikparksrvc);
					}
				}
				...
				//For Fasttrack-If user selected futuredate, Invoice Date should go as current date
				if(futuredateconfig==2 && getHidchkfuturedate().trim().equalsIgnoreCase("1")) {
					sqldate=sqlcurdate;
				}
