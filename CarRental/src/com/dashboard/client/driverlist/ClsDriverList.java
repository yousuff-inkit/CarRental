...
public class ClsDriverList {
	...
	public JSONArray driverListGridLoading(String branch,String cldocno,String check) throws SQLException {
		...
		try {
				...
				String clsql = "(select ROUND(@i:=@i+1 ,0) slno,a.* from(SELECT d.doc_no,d.dr_id,d.sr_no srno,d.name,d.dob,d.nation as nation1,d.mobno,d.passport_no,d.pass_exp,"
						...
						+ "left join my_acbook a on d.doc_no=a.doc_no and a.dtype=d.dtype where  d.dtype='CRM' and a.status=3"+sql+" order by d.doc_no)a,(select @i:=0)b)";
				//System.out.println("clsql1"+clsql);
				
				ResultSet resultSet = stmtDriverList.executeQuery (clsql);       
