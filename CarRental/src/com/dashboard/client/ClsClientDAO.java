...
public class ClsClientDAO  {
	...
	Connection conn = null;
	  ...
	  try {
		    ...
		    String clsql="SELECT cl.per_salikrate salcharge,cl.per_trafficharge trafcharge,h.account,cat.acc_group,tx.desc1 taxcat,cl.credit,cl.period,cl.period2,cat.category,refname,cl.cldocno,per_mob,sal_name,"
		    		...
		    		+" left join my_head h on cl.acno=h.doc_no left join my_cltax tx on cl.taxid=tx.doc_no  left join (select min(sr_no) srno,rdocno,type,cardno from my_clbankdet where 1=1 group by rdocno order by sr_no) clb  on clb.rdocno=cl.cldocno  left join my_clbankdet mclb  on clb.srno=mclb.sr_no left join (select min(dr_id) dr_id,cldocno,name from gl_drdetails where dtype='CRM' group by cldocno order by dr_id)drd on drd.cldocno=cl.cldocno left join gl_drdetails dr on dr.dr_id=drd.dr_id where cl.dtype='CRM' and cl.status=3";
      ...
      Connection conn = null;
		...
		try {
				...
				if(chk.equalsIgnoreCase("yes")) {	
					...
					String sqlexp="select * from (select if(a.pcase=1,'Litigation',if(a.pcase=2,'Dispute',if(a.pcase=3,'Over Due',if(a.pcase=4,'Frequent Defaulter','')))) status,'RA' choice,convert(r.voc_no,char(10)) voc_no,bv.fdate,convert(r.doc_no,char(10)) rdocno, "
							...
							+ " r.dispute=2 and m.status=3 "+sqls+") aa	union all  	select if(a.pcase=1,'Litigation',if(a.pcase=2,'Dispute',if(a.pcase=3,'Over Due',if(a.pcase=4,'Frequent Defaulter','')))) status,'DI' choice,'' voc_no,bv.fdate,'' rdocno, "
	...
	public JSONArray underlitigationExcel(String branch,String chk) throws SQLException {
	  ...
	  public JSONArray clentdetails() throws SQLException {
	        ...
	        try {
				 ...
				 String sql="";
					String configchk="select method from gl_config where field_nme='documentexpclient'";
	        	 ...
	        	 sql="select cldocno,refname from my_acbook where status=3 and dtype='CRM' and pcase=0 ";
	        }
	        	//System.out.println("clentdetails---------"+sql);
 ...
 +" left join gl_lagmt la on ac.cldocno=la.cldocno and la.clstatus=0  left join gl_ragmt ra on ac.cldocno=ra.cldocno and ra.clstatus=0 left join gl_rdriver rd on (ra.doc_no=rd.rdocno and  aa.dr_id=rd.drid)  and rd.status=3  left join gl_ldriver ld on (la.doc_no=ld.rdocno and aa.dr_id=ld.drid and ld.status=3 ) where 1=1 "+sqlnew+"  "+sqltest2+"  group by refname,name,document ";*/
				 sql="select sm.sal_name,aa.document,aa.expirydate,aa.dr_id sr_no,aa.cldocno,aa.name,ac.refname,ac.per_mob,ac.brhid,aa.visa_exp,CONVERT(aa.fdate,CHAR(100)) followupdate,CONVERT(concat(' Client : ',coalesce(ac.refname,' '), ' * ','Ref. Detail  : ',coalesce(aa.name,' '),' * ','Mobile No. : ' ,coalesce(ac.per_mob,' '),' * ','Document : ', coalesce(aa.document,' '),' * ','Expiry Date : ', DATE_FORMAT(coalesce(aa.expirydate,' '),'%d-%m-%Y')),CHAR(1000)) clientinfo from "
							+" (select  coalesce(bb.fdate,'') fdate,'Licence Expired' document,d.led expirydate,d.dr_id,d.cldocno,d.name,d.led visa_exp  from gl_drdetails d    left join my_acbook b on(d.cldocno=b.doc_no and b.dtype='CRM') left join (select max(doc_no) doc,dr_id from gl_bcde group by dr_id) a on a.dr_id=d.dr_id left join gl_bcde bb on a.doc=bb.doc_no where b.status=3 and d.led<='"+sqlUpToDate+"'   union "
			+" select  coalesce(bb.fdate,'') fdate,'Passport Expired' document,d.pass_exp expirydate,d.dr_id,d.cldocno,d.name,d.pass_exp  from gl_drdetails d    left join my_acbook b on(d.cldocno=b.doc_no and b.dtype='CRM') left join (select max(doc_no) doc,dr_id from gl_bcde group by dr_id) a on a.dr_id=d.dr_id left join gl_bcde bb on a.doc=bb.doc_no where b.status=3 and d.pass_exp<='"+sqlUpToDate+"'  "
			+" union select  coalesce(bb.fdate,'') fdate,'ID Expired' document,d.visa_exp expirydate,d.dr_id ,d.cldocno,d.name,d.visa_exp  from gl_drdetails d  left join my_acbook b on(d.cldocno=b.doc_no and b.dtype='CRM') left join (select max(doc_no) doc,dr_id from gl_bcde group by dr_id) a on a.dr_id=d.dr_id left join gl_bcde bb on a.doc=bb.doc_no where b.status=3 and d.visa_exp<='"+sqlUpToDate+"'     union "
			+"  select coalesce(bb.fdate,'') fdate,'Trade Licence Expired' document,b.contract_upto expirydate,d.dr_id ,b.cldocno,coalesce(b.contractno,'') name,b.contract_upto  from my_acbook b left join gl_drdetails d on(d.cldocno=b.doc_no and b.dtype='CRM') left join (select max(doc_no) doc,cldocno from gl_bcde group by cldocno) a on a.cldocno=b.cldocno left join gl_bcde bb on a.doc=bb.doc_no where b.status=3 and b.dtype='crm'  and b.contract_upto<='"+sqlUpToDate+"') aa  left join my_acbook ac on ac.cldocno=aa.cldocno and ac.dtype='crm' left join my_salm sm on sm.doc_no=ac.sal_id "
	 +" left join gl_lagmt la on ac.cldocno=la.cldocno and la.clstatus=0  left join gl_ragmt ra on ac.cldocno=ra.cldocno and ra.clstatus=0 left join gl_rdriver rd on (ra.doc_no=rd.rdocno and  aa.dr_id=rd.drid)  and rd.status=3  left join gl_ldriver ld on (la.doc_no=ld.rdocno and aa.dr_id=ld.drid and ld.status=3 ) where 1=1 "+sqlnew+"  "+sqltest2+"  group by refname,name,document ";
