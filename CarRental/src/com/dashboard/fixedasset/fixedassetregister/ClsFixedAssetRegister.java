	...
	public JSONArray fixedAsssetGridLoading(String branch,String fromdate,String todate,String assetNo,String group,String reportType,String check) throws SQLException {
	    ...
	    try {
				...
				/*sql = "select d.asset_no,d.date,CONVERT(if(sum(d.asset_opn)>0,round(sum(d.asset_opn),2),''),CHAR(100)) assetopn,CONVERT(if(sum(d.asset_add)>0,round(sum(d.asset_add),2),''),CHAR(100)) assetadd,CONVERT(if(sum(d.asset_del)>0,"
				 ...
				 sql = "select ROUND(@i:=@i+1 ,0) slno,a.* from(SELECT m.branchname,m.loc_name,m.asset_no,m.assetname,m.assetid,m.age,m.deprper,m.purdate,m.brhid,m.assetgp,m.date,m.assetgpid,\r\n" + 
				     		...
				     		"SELECT l.branchname,l.loc_name,l.asset_no,l.assetid,l.assetname,l.assetgp,l.age,l.purdate,l.asset_opn,l.asset_add,l.asset_del,\r\n" + 
				     		...
				     		"select d.branchname,d.loc_name,d.asset_no,d.date,round(SUM(d.asset_opn),2) asset_opn,round(SUM(d.asset_add),2) asset_add,round(SUM(d.asset_del),2)\r\n" + 
				     		...
				     		"select  b.branchname,f.loc_name,a.asset_no,a.date,if(a.date<'"+sqlFromDate+"',a.dramount,0.00) asset_opn,\r\n" + 
				     		...
				     		"my_fatran a left join my_fixm v on a.asset_no=v.sr_no left join my_fagrp g on v.assetgp=g.doc_no  left join my_brch b on b.doc_no=v.brhid left join my_faloc f on v.loc=f.doc_no where a.acno=v.fixastacno\r\n" + 
				     		"UNION ALL\r\n" + 
				     		"select b.branchname,f.loc_name,a.asset_no,a.date,0.00 asset_opn,0.00 asset_add,0.00 asset_del,if(a.date<'"+sqlFromDate+"',a.dramount,0.00) depr_opn,\r\n" + 
				     		...
				     		"left join my_fixm v on a.asset_no=v.sr_no left join my_fagrp g on v.assetgp=g.doc_no left join my_brch b on b.doc_no=v.brhid left join my_faloc f on v.loc=f.doc_no  where a.acno=v.accdepacno\r\n" + 
				     		") d\r\n" + 
				     		"left join my_fasaled sd on sd.assetid=d.asset_no left join my_fasalem sm on sm.doc_no=sd.rdocno where coalesce(sm.date,\r\n" + 
				     		"'"+sqlFromDate+"')>='"+sqlFromDate+"' and d.purdate<='"+sqlToDate+"'"+sql+" group by d.asset_no"+strSql+" ) l ) m)a,(select @i:=0)b";
