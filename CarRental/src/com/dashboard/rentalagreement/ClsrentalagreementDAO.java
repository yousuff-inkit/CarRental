...
//..
    ...
    public JSONArray detailsgrid(String brnchval,String fromdate,String todate,String cldocno,String status,String fleet, 
    	...
    	try {
				...
				ResultSet rsinsurcompconfig=stmtVeh.executeQuery(strinsurcompconfig);
				...
				if(insurcompconfig==1) {
					...
					sqlselect+=" "+insurcompconfig+" insurcompconfig,coalesce(insurac.refname,'') insurcompname,r.insurcompdays,round(coalesce(r.racrate,0),2) racrate,";
				}
            		
            		String sql ="select @i:=@i+1 slno, a.* from(select "+sqlselect+" concat(au.authid,v.reg_no,pl.code_name) vehiclenum,brch.branchname,CASE WHEN r.clstatus=0 THEN 'Open' WHEN r.clstatus=1 THEN 'Close' WHEN r.clstatus=2 THEN 'Cancel' ELSE '' END agmtstatus,coalesce(proj.project_name,'') project_name,'attach' as attach,if(coalesce(cs.indate,'')='',DATEDIFF(curdate(),r.odate),DATEDIFF(cs.indate,r.odate)) used_days,outloc.loc_name outlocation,r.tdocno,r.ofleet_no,v1.reg_no oreg_no,convert(coalesce(cs.indate,''),char(30)) indate,coalesce(cs.intime,'') intime,ur.user_name,clur.user_name closeuser, ms.sal_name ,"
            				...
            				+ " r.ddate,r.dtime,vm.din deldate,vm.tin deltime,cs.coldate,cs.coltime,v.reg_no,v.FLNAME vehdetails,a.refname,a.acno,a.per_mob,a.contactperson,t.rentaltype,tc.rate rent,tc.cdw, "
            				...
            				+ "coalesce((select invoiced from gl_rcalc where rdocno=r.doc_no and invno >0 and idno=17 and rowno=(select max(rowno) from gl_rcalc where rdocno=r.doc_no and invno >0 and idno=17)),0) invcdw,cm.txtname enqtype "
            				+ " from gl_ragmt r left join gl_vmove vm on vm.rdocno=r.doc_no and vm.rdtype='rag' and vm.trancode='dl' and vm.repno=0  and vm.exchng=0 and vm.custno=0 "
            				+ "	left join gl_vehmaster v on r.fleet_no=v.fleet_no left join gl_vehauth au on au.doc_no=v.authid  left join gl_vehplate pl on pl.doc_no=v.pltid left join my_acbook a on a.cldocno=r.cldocno   and a.dtype='CRM'"
            				...
            				+ " left join my_brch brch on r.brhid=brch.doc_no "+sqljoins+" left join cm_enqsource cm on cm.doc_no=r.enqtype"
            				+ " where r.status=3   "+sqltest+ " group by r.doc_no)a,(select @i:=0)c";	
		...
		try {
            	...
            	/*	String sql ="select r.brhid, r.doc_no,r.fleet_no,r.cldocno,r.odate,r.otime,r.okm,r.ddate,r.dtime,t.gid,t.brhid branchname,r.tdocno, "
            		...
            		String sql ="select r.voc_no,r.advchk radvc,r.invtype rinvtype,  a.advance clientadvc,a.invc_method clientinvtype,\r\n" 
            				...
            				+ " left join gl_vmove mm on mm.rdocno=r.doc_no and mm.rdtype='RAG' and mm.status='IN' and mm.trancode='DL' and mm.repno=0 and mm.custno=0 and mm.exchng=0  "
          ...
          //  	System.out.println(sql);
            ...
            /*		String sql ="select r.brhid, r.doc_no,r.fleet_no,r.cldocno,r.odate,r.otime,r.okm,r.ddate,r.dtime,t.gid,t.brhid branchname,r.tdocno, "
            		...
            		String sql ="select r.voc_no,r.advchk radvc,r.invtype rinvtype,  a.advance clientadvc,a.invc_method clientinvtype,\r\n" 
            				...
            				+ "left join gl_vehbrand br on br.doc_no=v.brdid left join gl_vehmodel mo on  mo.doc_no=v.vmodid "
            				+ " left join gl_tarifexcess insu on insu.rdocno=r.tdocno and insu.gid=t.gid "
            				+ "  left join gl_vmove mm on mm.rdocno=r.doc_no and mm.rdtype='RAG' and mm.status='IN' and mm.trancode='DL' and mm.repno=0 and mm.custno=0  and exchng=0  "
     ...
     public JSONArray tariffchangedetailsgrid(String brnchval,String fromdate,String todate,String cldocno,String status,String fleet,String group,String brand,String model,String type) throws SQLException {
		...
		try {
            	...
            	/*	String sql ="select r.brhid, r.doc_no,r.fleet_no,r.cldocno,r.odate,r.otime,r.okm,r.ddate,r.dtime,t.gid,t.brhid branchname,r.tdocno, "
            		...
            		String sql ="select r.doc_no,coalesce(app.itemcount,0) itemcount,r.voc_no,r.advchk radvc,r.invtype rinvtype,DATE_FORMAT(r.invdate,'%d-%m-%Y') rinvdate, a.advance clientadvc,a.invc_method clientinvtype,\r\n" 
           				 +	"( select method from gl_config where field_nme='Clientinvchk') method, "
           				 + "(select 0) AS caludays, "
           				+ " r.insex,coalesce(insurexcess,0) insurexcess, coalesce(cdwexcess,0) cdwexcess,coalesce(scdwexcess,0) scdwexcess, "
           				+ "r.brhid, r.doc_no,r.fleet_no,r.cldocno,r.odate,r.otime,r.okm,r.ddate,r.dtime,t.gid,t.brhid branchname,r.tdocno, "
           				+ "v.reg_no,v.FLNAME vehdetails,a.refname,a.per_mob,a.contactperson,t.rentaltype from gl_ragmt r left join (select count(*) itemcount,rano rdocno from gl_brex where approvestatus=0 group by rano) app on r.doc_no=app.rdocno "
           				+ "left join gl_vehmaster v on r.fleet_no=v.fleet_no left join my_acbook a on a.cldocno=r.cldocno and a.dtype='CRM' "
           				+ " left join gl_vehgroup g on g.doc_no=v.vgrpid left join gl_rtarif t on t.rdocno=r.doc_no and t.rstatus=5  "
           				+ "left join gl_vehbrand br on br.doc_no=v.brdid left join gl_vehmodel mo on  mo.doc_no=v.vmodid  "
           				+ "left join gl_tarifexcess insu on insu.rdocno=r.tdocno and insu.gid=t.gid "
   ...
   public JSONArray ratariffchangegrid(String rdocno) throws SQLException {
        ...
        Connection conn=null;
  		try {
				 ...
				 * 
				 * String
				 * sqls="SELECT MAX(idgl_brex) idgl_brex from gl_brex where rano='"+rdocno+"'";
				 * 
				 * ResultSet rsbrex=stmtVeh5.executeQuery(sqls);
				 * 
				 * if(rsbrex.next()) {
				 * 
				 ...
				 * 
				 * String tarifsql=
				 * ("select rentaltype, rate, cdw, pai, cdw1, pai1, gps, babyseater, cooler,exkmrte,chaufchg, chaufexchg from gl_brexd  where rdocno='"
				 * +rdocno+"' and refno='"+id_brex+"' order by rstatus;");
				 ...
				 */				 
				    if(RESULTDATA.isEmpty() || RESULTDATA==null) {  	
				  	...
				  	String  tarifsql1=("select  rentaltype, rate, cdw, pai, cdw1, pai1, gps, babyseater, cooler, kmrest, exkmrte, oinschg, exhrchg, chaufchg,"
						+ " chaufexchg from gl_rtarif where rdocno='"+rdocno+"' order by rstatus;");
    ...
    public JSONArray ratariffapprovegrid(String rdocno,String brexid) throws SQLException {
        ...
        Connection conn=null;
  		try {
  				...
  				System.out.println("approvegrid==========="+tarifsql);
  				ResultSet resultSet = stmtVeh5.executeQuery (tarifsql);
  				RESULTDATA=ClsCommon.convertToJSON(resultSet);
  				stmtVeh5.close();
  				conn.close();
    	...
    	try {
				...
				Statement stmtVeh = conn.createStatement (); 
            		...
            		String sql ="select r.ofleet_no,v1.reg_no oreg_no,convert(coalesce(cs.indate,''),char(30)) indate,coalesce(cs.intime,'') intime,ur.user_name,clur.user_name closeuser, ms.sal_name ,"
            				...
            				+ " coalesce((select invoiced from gl_rcalc where rdocno=r.doc_no and invno >0 and idno=1 and rowno=(select max(rowno) from gl_rcalc where rdocno=r.doc_no and invno >0 and idno=1)),0) invRental, "
            				+ " coalesce((select invoiced from gl_rcalc where rdocno=r.doc_no and invno >0 and idno=17 and rowno=(select max(rowno) from gl_rcalc where rdocno=r.doc_no and invno >0 and idno=17)),0) invcdw "
            				+ " from gl_ragmt r left join gl_vmove vm on vm.rdocno=r.doc_no and vm.rdtype='rag' and vm.trancode='dl' and vm.repno=0  and exchng=0 and custno=0 "
            				+ "	left join gl_vehmaster v on r.fleet_no=v.fleet_no left join my_acbook a on a.cldocno=r.cldocno   and a.dtype='CRM' "
            				+ " left join gl_vehgroup g on g.doc_no=v.vgrpid    left join gl_vehmaster v1 on r.ofleet_no=v1.fleet_no  left join  gl_rtarif t on t.rdocno=r.doc_no and t.rstatus=5 "
