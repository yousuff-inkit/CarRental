		...
		try {
			...
			if(!hidfleet.equalsIgnoreCase("")){
				...
				if(gridtype.equalsIgnoreCase("detail")){
					...
					strSql="select sum(inv) as income,sum(exp) as expenses,(sum(inv)-sum(exp)) netamount,"+sqlselect+",coalesce(veh.tval,0) purcost,((sum(inv) - sum(exp))  / tval)* 100 returnprice, veh.reg_no, au.authname, pl.code_name, DATEDIFF(CURDATE(),veh.PRCH_DTE) age "+
						...
						" left join gl_vehbrand brd on veh.brdid=brd.doc_no left join gl_vehmodel model on veh.vmodid=model.doc_no left join gl_vehgroup"+
						" grp on veh.vgrpid=grp.doc_no left join gl_yom yom on veh.yom=yom.doc_no LEFT JOIN gl_vehauth au ON au.doc_no=veh.authid LEFT JOIN gl_vehplate pl ON pl.doc_no=veh.pltid where 1=1 "+sqlcommon+""+sqlgroup;
					}  
