...
public class ClsRevenueReportDAO {
	...
	public JSONArray categorySearch() throws SQLException {
	    ...
	    Connection conn = null;
	      ...
	      try {
	        ...
	        sqls = "select doc_no doc_no,category category from my_clcatm where status=3 and dtype='CRM' ";
	        
	        ResultSet resultSet = stmtSailk.executeQuery(sqls);
	...
	public JSONArray agreementSearch(String branch, String sclname,String smob,String rno,String flno,String sregno,String rentaltype) throws SQLException {
	 ...
	 public JSONArray getRevenuev2(String branchval,String cldocno,String rentaltype,String agmtNo,String clstatuss,String cmbtariftype,String invstatuss,String hidtxtcategory,String check,String uptodate) throws SQLException {
      ...
      * if(!(fromdate.equalsIgnoreCase("undefined"))&&!(fromdate.equalsIgnoreCase(""))&&!(fromdate.equalsIgnoreCase("0")))
         ...
         if(!(hidtxtcategory.equalsIgnoreCase("")|| hidtxtcategory.equalsIgnoreCase("NA"))){
             sqltest=sqltest+" and ac.catid='"+hidtxtcategory+"'";
             cnotefilters+=" and ac.catid="+hidtxtcategory;
...
// old edited query
             ...
             /*String sql="select "+showfleet+" showfleet,cl.category clcategory,inv.voc_no docnoo,convert(h.account,char(50)) acno,inv.cldocno cldocno,ac.refname clname,"+showfleetcolumn+"if(inv.status=3,round(convert(coalesce(coalesce(sum(rent),0.0)+coalesce(sum(inschg),0.0)+coalesce(sum(accchg),0.0)+coalesce(sum(salik), 0.0)+coalesce(sum(saliksrv),0.0)+coalesce(sum(traffic),0.0)+coalesce(sum(trafficsrv),0.0)+coalesce(sum(others),0.0)+ coalesce(sum(vat),0.0),0),char(50)),0.0),2) revtotal , "
                     + "if(inv.status=7,round(convert(coalesce(coalesce(sum(rent),0.0)+coalesce(sum(inschg),0.0)+coalesce(sum(accchg),0.0)+coalesce(sum(salik), 0.0)+coalesce(sum(saliksrv),0.0)+coalesce(sum(traffic),0.0)+coalesce(sum(trafficsrv),0.0)+coalesce(sum(others),0.0)+ coalesce(sum(vat),0.0),0),char(50)),0.0),2) delchrgs "
                     + " ,round(coalesce(sum(rent),0.0),2) rental,round(coalesce(sum(inschg),0.0),2) insur,"+
                     " round(coalesce(sum(accchg),0.0),2) acctot,round((coalesce(sum(salik),0.0)+coalesce(sum(saliksrv),0.0)) ,2)salik,round((coalesce(sum(traffic),0.0)+"+
                     " coalesce(sum(trafficsrv),0.0)),2) traffic,round(coalesce(sum(others),0.0),2) other, round(coalesce(sum(vat),0.0),2) vatamt"+
                     " from invoice_view inv left join my_acbook ac on"+
                     " ac.doc_no=inv.cldocno and  ac.dtype='CRM' left join my_clcatm cl on cl.doc_no=ac.catid left join gl_lagmt l on l.doc_no=inv.rano and inv.ratype='LAG' left join"+
                     " gl_ragmt r on (r.doc_no=inv.rano and inv.ratype='RAG') left join gl_rtarif rtarif on (r.doc_no=rtarif.rdocno and rtarif.rstatus=5) left join  my_head h on h.doc_no=inv.acno left join my_brch br"+
                     " on inv.brhid=br.doc_no left join my_salm ms on ms.doc_no=r.salid or ms.doc_no=l.salid where inv.date<='"+sqluptodate+"' "+sqltest +" "+
                     " group by inv.doc_no union all"+
                     " SELECT "+showfleet+" showfleet,cl.category clcategory,m.doc_no docnoo,convert(hd.account,char(50)) acno,ac.cldocno cldocno,ac.refname clname,round(SUM(if(m.status=3,if(m.dtype in ('CNO','TCN'),if(d.amount>0, d.amount*-1, d.amount),if(d.amount>0,d.amount,d.amount*-1)),0)),2) revtotal,\r\n" +   
                     " round(SUM(IF(m.status=7,if(m.dtype in ('CNO','TCN'),if(d.amount>0, d.amount*-1, d.amount),if(d.amount>0,d.amount,d.amount*-1)),0)),2) delchrgs,round(SUM(IF(invac.idno=1,if(m.dtype in ('CNO','TCN'),if(d.amount>0,d.amount*-1,d.amount),if(d.amount>0,d.amount,d.amount*-1)),0)),2) rental,round(SUM(IF(invac.idno IN (17),if(m.dtype in ('CNO','TCN'),if(d.amount>0, d.amount*-1, d.amount),if(d.amount>0,d.amount,d.amount*-1)),0)),2) insur,round(SUM(IF(invac.idno=2,if(m.dtype in ('CNO','TCN'),if(d.amount>0, d.amount*-1, d.amount),if(d.amount>0,d.amount,d.amount*-1)),0)),2) acctot,\r\n" + 
                     ...
                     " round(SUM(IF(invac.idno NOT IN (1,6,2,8,14,38,39,9,15,17,20),if(m.dtype in ('CNO','TCN'),if(d.amount>0, d.amount*-1, d.amount),if(d.amount>0,d.amount,d.amount*-1)),0)),2) other,round(SUM(case when m.dtype='TCN' then d.taxamount*-1 when m.dtype='TDN' then d.taxamount when invac.idno in (20) and m.dtype='CNO' then d.amount*-1 when invac.idno in (20) and m.dtype<>'CNO' then d.amount else 0.0 end ),2) vatamt FROM (select dtype, tr_no from my_jvtran where status=3 group by tr_no) j left join my_cnot m on m.dtype=j.dtype and m.tr_no=j.tr_no  LEFT JOIN my_cnotd d ON m.TR_NO=d.TR_NO \r\n" + 
                     " LEFT JOIN gl_invmode invac ON d.acno=invac.acno\r\n" + 
                     " LEFT JOIN my_brch br ON m.brhid=br.DOC_NO\r\n" + 
                     " LEFT JOIN my_head hd ON m.acno=hd.DOC_NO left join my_acbook ac on hd.doc_no=ac.acno left join my_clcatm cl on cl.doc_no=ac.catid LEFT JOIN my_salm ms ON ac.sal_id=ms.doc_no \r\n" + 
                     "\r\n" + 
                     " WHERE m.date<='"+sqluptodate+"' "+cnotefilters+" AND hd.atype='AR' GROUP BY m.tr_no";       */
             
             
             
             String sql="select coalesce(coalesce(l.lcount,0)+coalesce(r.rcount,0),0) noofveh , a.clcategory,a.acno,a.cldocno,a.clname,sum(a.revtotal) revtotal ,sum(a.rental) rental,sum(a.insur) insur,sum(a.other) other,sum(a.acctot) acctot,sum(a.vatamt) vatamt,sum(a.salik) salik, sum(a.traffic) traffic from(select inv.date,"+showfleet+" showfleet,cl.category clcategory,inv.voc_no docnoo,convert(h.account,char(50)) acno,inv.cldocno cldocno,ac.refname clname,"+showfleetcolumn+"if(inv.status=3,round(convert(coalesce(coalesce(sum(rent),0.0)+coalesce(sum(inschg),0.0)+coalesce(sum(accchg),0.0)+coalesce(sum(salik), 0.0)+coalesce(sum(saliksrv),0.0)+coalesce(sum(traffic),0.0)+coalesce(sum(trafficsrv),0.0)+coalesce(sum(others),0.0)+ coalesce(sum(vat),0.0),0),char(50)),2),0.0) revtotal , "
                     + "round(coalesce(sum(rent),0.0),2) rental,round(coalesce(sum(inschg),0.0),2) insur,"+
                     " round(coalesce(sum(accchg),0.0),2) acctot,round((coalesce(sum(salik),0.0)+coalesce(sum(saliksrv),0.0)) ,2)salik,round((coalesce(sum(traffic),0.0)+"+
                     " coalesce(sum(trafficsrv),0.0)),2) traffic,round(coalesce(sum(others),0.0),2) other, round(coalesce(sum(vat),0.0),2) vatamt"+
                     " from invoice_view inv left join my_acbook ac on"+
                     " ac.acno=inv.acno left join my_clcatm cl on cl.doc_no=ac.catid"+
                     " left join  my_head h on h.doc_no=inv.acno "+
                     "  where (inv.date between  DATE_FORMAT('"+sqluptodate+"' ,'%Y-%m-01') AND '"+sqluptodate+"' ) "+sqltest +" "+
                     " group by inv.acno union all"+
                     " SELECT m.date,"+showfleet+" showfleet,cl.category clcategory,m.doc_no docnoo,convert(hd.account,char(50)) acno,ac.cldocno cldocno,ac.refname clname,round(SUM(if(m.status=3,if(m.dtype in ('CNO','TCN'),if(d.amount>0, d.amount*-1, d.amount),if(d.amount>0,d.amount,d.amount*-1)),0)),2) revtotal,\r\n" +   
                     " round(SUM(IF(invac.idno=1,if(m.dtype in ('CNO','TCN'),if(d.amount>0,d.amount*-1,d.amount),if(d.amount>0,d.amount,d.amount*-1)),0)),2) rental,round(SUM(IF(invac.idno IN (17),if(m.dtype in ('CNO','TCN'),if(d.amount>0, d.amount*-1, d.amount),if(d.amount>0,d.amount,d.amount*-1)),0)),2) insur,round(SUM(IF(invac.idno=2,if(m.dtype in ('CNO','TCN'),if(d.amount>0, d.amount*-1, d.amount),if(d.amount>0,d.amount,d.amount*-1)),0)),2) acctot,\r\n" + 
                     " round(SUM(IF(invac.idno IN (8,14,38,39),if(m.dtype in ('CNO','TCN'),if(d.amount>0, d.amount*-1, d.amount),if(d.amount>0,d.amount,d.amount*-1)),0)),2) salik,round(SUM(IF(invac.idno IN (9,15),if(m.dtype in ('CNO','TCN'),if(d.amount>0, d.amount*-1, d.amount),if(d.amount>0,d.amount,d.amount*-1)),0)),2) traffic,\r\n" + 
                     " round(SUM(IF(invac.idno NOT IN (1,6,2,8,14,38,39,9,15,17,20),if(m.dtype in ('CNO','TCN'),if(d.amount>0, d.amount*-1, d.amount),if(d.amount>0,d.amount,d.amount*-1)),0)),2) other,round(SUM(case when m.dtype='TCN' then d.taxamount*-1 when m.dtype='TDN' then d.taxamount when invac.idno in (20) and m.dtype='CNO' then d.amount*-1 when invac.idno in (20) and m.dtype<>'CNO' then d.amount else 0.0 end ),2) vatamt FROM (select dtype, tr_no from my_jvtran where status=3 group by tr_no) j left join my_cnot m on m.dtype=j.dtype and m.tr_no=j.tr_no  LEFT JOIN my_cnotd d ON m.TR_NO=d.TR_NO \r\n" + 
                     " LEFT JOIN gl_invmode invac ON d.acno=invac.acno\r\n" + 
                     " LEFT JOIN my_head hd ON m.acno=hd.DOC_NO left join my_acbook ac on hd.doc_no=ac.acno left join my_clcatm cl on cl.doc_no=ac.catid " + 
                     " WHERE (m.date between  DATE_FORMAT('"+sqluptodate+"' ,'%Y-%m-01') AND '"+sqluptodate+"' ) "+cnotefilters+" AND hd.atype='AR' GROUP BY m.acno)a"
                     +" left join (select doc_no,acno, count(*) lcount from gl_lagmt  where status=3 and  clstatus=0  group by acno) l on (l.acno=a.acno )"
                     +" left join (select doc_no,acno, count(*) rcount from gl_ragmt  where status=3 and  clstatus=0  group by acno) r on (r.acno=a.acno ) group by a.acno ";       
