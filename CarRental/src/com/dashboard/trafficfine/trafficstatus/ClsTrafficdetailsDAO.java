...

{
	...
	public JSONArray masterdetails(String code,String fromdate,String todate,String ticketno,String regno) throws SQLException {
		...
		try {
			...
			{
				String sql="select t.blackpoints,if(ac.ser_default=0,ac.per_trafficharge,0) trafficsrvc,veh.ch_no,t.tcno,t.regno, t.fleetno ,t.source, t.ticket_no ,"
				+ " t.traffic_date ,t.time,t.location, convert(coalesce(if((fine_source like '%DUBAI%' "+strgovfeesparking+" ) && "+govfees+">0"
				+",amount+"+govfees+",amount),''),char(100)) amount,t.fine_source,vh.code_name,t.inv_desc description,coalesce(t.desc1,'') trafficdesc,t.autodate from gl_traffic t left join gl_vehplate vh on "+
		...
		else if(code.equalsIgnoreCase("ANI"))
		{
     				...
     				*/
     				...
     				String sql=" select  t.blackpoints,if(ac.ser_default=0,ac.per_trafficharge,0) trafficsrvc,veh.ch_no,t.tcno, case when t.rtype in  ('RA','RD','RW','RF','RM') then 'RENTAL AGRREMENT' when  "
     						...
     						+ " t.fleetno ,t.source, t.ticket_no ,t.traffic_date ,time,t.location, convert(coalesce(if((t.fine_source like '%DUBAI%' "+strgovfeesparking+" ) && "+govfees+">0,t.amount+"+govfees+",t.amount),''),char(100)) amount,fine_source,vh.code_name,t.inv_desc description,coalesce(t.desc1,'') trafficdesc,t.autodate"
		...
		/*	String sql="select regno, fleetno ,source, ticket_no ,traffic_date ,location, convert(coalesce(amount,''),char(100)) amount  from gl_traffic   "
			...
			String sql="select t.blackpoints,invd.srvcamt trafficsrvc,veh.ch_no,t.tcno, convert(coalesce(m.voc_no,t.inv_no),char(50)) invno,coalesce(t.inv_type,'') invtype,case when t.rtype in  ('RA','RD','RW','RF','RM') then 'RENTAL AGRREMENT' when  "
					+ " t.rtype in ('LA','LC') then 'LEASE AGRREMENT' else  st.st_desc end as 'dtypedesc', "
					+ " case when t.rtype in ('RM','RA', 'RD','RW','RF') then ragmt.voc_no when  "
					+ " t.rtype in ('LA','LC')  then lagmt.voc_no else t.ra_no end as 'rano',t.regno,  coalesce(ac.refname,sa.sal_name) refname, "
					+ " t.fleetno ,t.source, t.ticket_no ,t.traffic_date ,time,t.location, convert(coalesce(if((t.fine_source like '%DUBAI%' "+strgovfeesparking+" ) && "+govfees+">0,t.amount+"+govfees+",t.amount),''),char(100)) amount,fine_source,vh.code_name,t.inv_desc description,coalesce(t.desc1,'') trafficdesc,t.autodate  "
		...
		else if(code.equalsIgnoreCase("RES"))
		{
			...
			String sql="select * from (select  t.blackpoints,invd.srvcamt trafficsrvc,veh.ch_no,t.tcno,vh.code_name,t.postdocno,o.doc_no receiptno,o.date receiptdate,"
					+ " if(o.out_amount>t.amount+invd.srvcamt,t.amount+invd.srvcamt,o.out_amount) out_amount,convert(coalesce(m.voc_no,t.inv_no),char(50)) invno,coalesce(t.inv_type,'') invtype,case when t.rtype in  ('RA','RD','RW','RF','RM') then 'RENTAL AGRREMENT' when  "
					+ " t.rtype in ('LA','LC') then 'LEASE AGRREMENT' else  st.st_desc end as 'dtypedesc', "
					+ " case when t.rtype in ('RM','RA', 'RD','RW','RF') then ragmt.voc_no when  "
					+ " t.rtype in ('LA','LC')  then lagmt.voc_no else t.ra_no end as 'rano',t.regno, coalesce(ac.refname,sa.sal_name) refname, "
					...
					+ " coalesce(if(t.fine_source like '%DUBAI%' && "+govfees+">0,t.amount+"+govfees+",t.amount),0)+coalesce(invd.srvcamt,0)-coalesce(if(coalesce(o.out_amount,0)>t.amount+coalesce(invd.srvcamt,0),t.amount+coalesce(invd.srvcamt,0),o.out_amount),0) outstanding,t.autodate  "
					+ " from gl_traffic t   left join gl_ragmt ragmt on (t.ra_no=ragmt.doc_no and t.rtype in ('RA','RD','RW','RF','RM'))  "
					+ " left join gl_lagmt  lagmt on (t.ra_no=lagmt.doc_no and t.rtype in ('LA','LC')) left join gl_status st on t.rtype=st.Status  "
		...
		/*	String sql="select regno, fleetno ,source, ticket_no ,traffic_date ,location, convert(coalesce(amount,''),char(100)) amount  from gl_traffic   "
			...
			String sql="select  t.blackpoints,if(ac.ser_default=0,ac.per_trafficharge,0) trafficsrvc,postjv.date postdate,veh.ch_no,t.tcno, vh.code_name,t.postdocno,coalesce(m.voc_no,t.inv_no) invno,coalesce(t.inv_type,'') invtype,case when t.rtype in  ('RA','RD','RW','RF','RM') then 'RENTAL AGRREMENT' when  "
					+ " t.rtype in ('LA','LC') then 'LEASE AGRREMENT' else  st.st_desc end as 'dtypedesc', "
					+ " case when t.rtype in ('RM','RA', 'RD','RW','RF') then ragmt.voc_no when  "
					+ " t.rtype in ('LA','LC')  then lagmt.voc_no else t.ra_no end as 'rano',t.regno, coalesce(ac.refname,sa.sal_name) refname, "
					+ " t.fleetno ,t.source, t.ticket_no ,t.traffic_date ,time,t.location, convert(coalesce(if((t.fine_source like '%DUBAI%' "+strgovfeesparking+") && "+govfees+">0,t.amount+"+govfees+",t.amount),''),char(100)) amount,fine_source,t.inv_desc description,coalesce(t.desc1,'') trafficdesc,t.autodate  "
					+ " from gl_traffic t   left join gl_ragmt ragmt on (t.ra_no=ragmt.doc_no and t.rtype in ('RA','RD','RW','RF','RM'))  "
		...
		{
			String sql="select t.blackpoints,convert(coalesce(ac.cldocno,sa.doc_no),char(45)) cldocno,coalesce(ac.refname,sa.sal_name) refname,coalesce(j2.dramount*-1,0) trafficsrvc,veh.ch_no,t.tcno,j1.date postdate,convert(coalesce(m.voc_no,t.inv_no),char(45)) invno,t.ra_no  as 'rano',st1.st_desc as 'dtypedesc',coalesce(t.inv_type,'') invtype,t.postdocno,coalesce(ac.refname,sa.sal_name) refname,vh.code_name,o.doc_no receiptno,o.date receiptdate,if(coalesce(o.out_amount,0)>t.amount+coalesce(j2.dramount*-1,0),t.amount+coalesce(j2.dramount*-1,0),o.out_amount)  out_amount,regno, fleetno ,source, ticket_no ,traffic_date ,time,location,convert(coalesce(if((fine_source like '%DUBAI%' "+strgovfeesparking+" ) && "+govfees+">0,amount+"+govfees+",amount),''),char(100)) amount,fine_source,t.inv_desc description, "
			+ " coalesce(amount,0)+coalesce(j2.dramount*-1,0)-coalesce(if(coalesce(o.out_amount,0)>t.amount+coalesce(j2.dramount*-1,0),t.amount+coalesce(j2.dramount*-1,0),o.out_amount),0) outstanding,coalesce(t.desc1,'') trafficdesc,t.autodate from gl_traffic t"
			+ " left join gl_vehmaster veh on t.fleetno=veh.fleet_no left join gl_invm m on m.doc_no=t.inv_no and t.inv_type='INV'  "
...
*/				
	...
	public JSONArray getRegno(String id,String fromdate,String todate) throws SQLException{
		...
		try{
			 ...
			 */
			String strsql=" SELECT fleet_no,reg_no,flname,auth.AUTHNAME authority,plt.CODE_NAME platecode,veh.salik_tag saliktag FROM gl_vehmaster veh \r\n" + 
					" LEFT JOIN gl_vehauth auth ON veh.authid=auth.doc_no\r\n" + 
					" LEFT JOIN gl_vehplate plt ON veh.pltid=plt.doc_no WHERE statu=3 ";
