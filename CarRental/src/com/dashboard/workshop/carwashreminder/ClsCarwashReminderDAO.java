package com.dashboard.workshop.carwashreminder;
...
public class ClsCarwashReminderDAO {
	...
	public JSONArray vehicleSearchData(String flname,String regno,String flno,String id) throws SQLException{
		...
		try {
			...
			String strqry="select doc_no,reg_no regno,fleet_no fleetno,flname name from gl_vehmaster where 1=1"+sqltest;	
...
public JSONArray getSpareData(String fromdate,String todate, String vehdocno,String cldocno,String id) throws SQLException{
		...
		try {
			...
			String strsql="select gate.date,gate.doc_no gatedocno,br.branchname branch,veh.doc_no vehdocno,m.psrno partdocno,m.part_no partno,m.productname,m.doc_no,u.unit,m.munit as unitdocno,m.psrno,parts.qty,"+
					...
					" left join gl_workgateinpassm gate on parts.gateinpassdocno=gate.doc_no left join my_brch br on gate.brhid=br.doc_no left join gl_vehmaster veh on gate.fleet_no=veh.fleet_no "+
...
public JSONArray getMaitenanceData(String todate, String id) throws SQLException{
	...
	try {
		...
		String strsql="select convert(concat(gate.indate,' / ',gate.intime),char(100)) indatetime,convert(concat(gate.outdate,' / ',gate.outtime),char(100)) outdatetime, "
				+ "veh.fleet_no,flname,veh.reg_no,gate.inkm,gate.date,gate.doc_no gatedocno,br.branchname branch,veh.doc_no vehdocno,maint.docno maintenancedocno, "
				+ "maint.mtype  maintenances, maint.name descriptions from gl_vehmaster veh "
				+ "left join (select max(srno) srno ,fleet_no from gl_workgateinpassm where status=3 group by fleet_no) gate1 on gate1.fleet_no=veh.fleet_no "
				+ "left join gl_workgateinpassm gate on gate.srno=gate1.srno "
				+ "left join gl_worksrvcadvisormaint m on m.gateinpassdocno=gate.doc_no "
				+ "left join gl_vrepm maint on m.maintenancedocno=maint.docno "
				+ "left join my_brch br on gate.brhid=br.doc_no "
				+ "where veh.statu=3 and (( maint.wash=1 and coalesce(datediff('"+sqltodate+"',gate.outdate),0)>15) or gate.doc_no is null or coalesce(maint.wash,0)=0) group by fleet_no;";	
...
public JSONArray getMaitenanceExcelData(String fromdate,String todate, String vehdocno,String cldocno,String id) throws SQLException{
	...
	try {
		...
		+" left join my_brch br on gate.brhid=br.doc_no"
		+" left join gl_vehmaster veh on gate.fleet_no=veh.fleet_no"
