...
public class ClsCommon {
	...
	public  JSONArray convertToJSON(ResultSet resultSet)
			...
			//	  String resultString = inputString.replaceAll("[^a-zA-Z0-9\\s!@#$%^&*()_+-={}[]|;:'\",.<>?/`~\\\\]", "");
	...
	public  Date changeStringtoSqlDate4(String startDate){
		java.sql.Date sqlStartDate=null;
		try{
			SimpleDateFormat sdf1 = new SimpleDateFormat("dd/MM/yyyy");
			java.util.Date date = sdf1.parse(startDate);
...
//		System.out.println("inside get salik");
		...
		Statement	stmt = conn.createStatement();
			...
			ResultSet rsparkconfig=stmt.executeQuery(strparkconfig);
		...
		double salikamt=0.0;
		double salikauhamt=0.0,salikdxbamt=0.0,saliksrvcchg=0.0;
...
//		System.out.println("Acbbok"+stracbook);
		...
		if(multiplesrvcconfig==1){
			...
			double trafficfees=getTrafficFees(Integer.parseInt(agmtno),fromdate,todate,agmttype,"1");
			...
			double temptrafficamt=0.0;
			while(rstrafficvalues.next()) {
				temptrafficamt=rstrafficvalues.getDouble("trafficamt");
			}
			...
			String strdefault="select "+strsalikauhclient+"ser_default,per_salikrate,per_trafficharge from my_acbook where cldocno='"+clientid+"' and dtype='CRM' and status<>7";
			//System.out.println("Acbbok"+stracbook);
			ResultSet rsdefault=stmt.executeQuery(strdefault);
			while(rsdefault.next()){
				srvcdefault=rsdefault.getInt("ser_default");
				if(srvcdefault==0){
					salikrate=rsdefault.getDouble("per_salikrate");
					trafficrate=rsdefault.getDouble("per_trafficharge");
					if(!strsalikauhclient.equalsIgnoreCase("")){
						salikauhsrvcrate=rsdefault.getDouble("salikauhrate");
					}
			...
			if(chkauhsalikpercent>0 && salikauhamt>0.0) {
				salikauhsrvcrate=Round((salikauhamt*(salikauhpercent/100)/salikauhcount),2);
				saliksrvcchg=Round((salikauhamt*(salikauhpercent/100)),2);
			}
			if(chkdxbsalikpercent>0 && salikdxbamt>0.0) {
				salikrate=Round((salikdxbamt*(salikpercent/100)/salikdxbcount),2);
				saliksrvcchg=Round((salikdxbamt*(salikpercent/100)),2);
			}
			if(chktrafficpercent>0 && temptrafficamt>0.0) {
				trafficrate=Round(temptrafficamt*(trafficpercent/100),2);
			}
			if(chkparkingpercent>0 && salikparkamt>0.0) {
				salikparksrvc=Round(salikparkamt*(parkingpercent/100),2);
				saliksrvcchg=Round((salikparkamt*(parkingpercent/100)),2);
			}
			
			
			/*if(chksalikpercent==1){
				if(salikdxbamt>0.0) {
					salikrate=Round((salikdxbamt*(salikpercent/100)/salikdxbcount),2);
				}
				if(salikauhamt>0.0) {
				...
				if(temptrafficamt>0.0) {
					trafficrate=Round(temptrafficamt*(trafficpercent/100),2);
				}
				System.out.println("Percent Srvc:"+salikrate+"::"+salikauhsrvcrate+"::"+trafficrate);
			}
			else{
			...
			stracbook="select "+strsalikauhclient+"ser_default,per_salikrate,per_trafficharge,del_charges,parkingamt from my_acbook where cldocno='"+clientid+"' and dtype='CRM' and status<>7";
		//	System.out.println("Acbbok"+stracbook);
...
//			System.out.println(strtraffic);
		...
		if(srvcdefault==1 && chksalikpercent==0){
			...
			salikparksrvc=Round(salikparkamt*(parksrvcpercent/100),2);
...
//				System.out.println("Traffic Method Check:"+trafficmethod);
	...
	/*		if(srvcdefault==1){
		...
		if(saliksrvcchg==0){
		    saliksrvcchg=(salikauhcount*salikauhsrvcrate)+(salikdxbcount*salikrate);
		}
		...
		/*if(salikparkamt>0.0 && parksrvcpercent>0.0) {
			salikparksrvc=Round((salikparkamt*(parksrvcpercent/100)), 2);
		}*/
		salikarray.add(salikparkamt);
		salikarray.add(salikparksrvc);
		salikarray.add(salikparkamt>0.0?(salikparkamt/salikparkcount):0);
		salikarray.add((double)salikparkcount);
...
//		System.out.println(salikarray);
    ...
    public  double Round(double value,int deci) throws SQLException
    {
    	...
    	try{
    	    if (Double.isNaN(value) || Double.isInfinite(value)) {
                System.out.println("Invalid value passed to Round: " + value + ". Defaulting to 0.0");
                value=0.0;
            }
    		conn=ClsConnection.getMyConnection();
    		Statement stmt=conn.createStatement();
    ...
    public  JSONObject JVTotalCheck(Connection conn, int trno) throws SQLException {  
    	...
    	try{
			...
			while (rs0.next()) {
			    System.out.println(rs0.getDouble("dramount")+" = JV LINE AMOUNT = "+rs0.getDouble("jvtotal")+" For A/c"+rs0.getDouble("acno"));
			} 
...
public String format(String value,Connection conn){  
    ...
    public String parseSafeString(String value) {
        if (value == null || value.trim().equalsIgnoreCase("undefined") 
                || value.trim().equalsIgnoreCase("NaN") || value.trim().isEmpty()) {
                return "";
        }
        return value.trim();
    }
    ...
    public double parseSafeDouble(String value) {  
          if (value == null || value.trim().equalsIgnoreCase("undefined") 
                    || value.trim().equalsIgnoreCase("NaN") || value.trim().isEmpty()) {
                    return 0.0;
          }
