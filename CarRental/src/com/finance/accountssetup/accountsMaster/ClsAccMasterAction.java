...
public class ClsAccMasterAction {
 ...
 // for inter branch tick
	...
	public int getRadiosaveval() {
		return radiosaveval;
	}

	public void setRadiosaveval(int radiosaveval) {
		this.radiosaveval = radiosaveval;
	}
	...
	public String saveAction() throws ParseException, SQLException{
		...
		if(mode.equalsIgnoreCase("A")){
			...
			if(val>0){
				...
				setDatehidden(sqlStartDate.toString());
				setRadiotick(getRadiotick());
				setRadiosaveval(getRadiosaveval());
				setDocno(val);
			...
			boolean Status=accMasterDAO.delete(getDocno(),getMaindel(),session,getFormdetailcode(),accountcode,request);
		if(Status){
			setRadiotick(getRadiotick());
			if(getOtherdis()==1)
			{
